...
class StripeTestCase(YakklTestCase):
...
class StripeTest(StripeTestCase):
    ...
    def test_upgrade_by_card(self, *mocks: Mock) -> None:
        ...
        self.assertEqual(stripe_customer.description, "yakkl (Yakkl Dev)")
        ...
        self.assertEqual(dict(stripe_customer.metadata),
                         {'realm_id': str(user.realm.id), 'realm_str': 'yakkl'})
        ...
        self.assertEqual(stripe_charges[0].description,
                         "Upgrade to Yakkl Standard, $80.0 x {}".format(self.seat_count))
        self.assertEqual(stripe_charges[0].receipt_email, user.email)
        self.assertEqual(stripe_charges[0].statement_descriptor, "Yakkl Standard")
        ...
        line_item_params = {
            'amount': 8000 * self.seat_count, 'description': 'Yakkl Standard', 'discountable': False,
            'period': {
        ...
        # Check that we correctly populated Customer, CustomerPlan, and LicenseLedger in Yakkl
        ...
        for substring in [
                'Yakkl Standard', str(self.seat_count),
    ...
    def test_upgrade_by_invoice(self, *mocks: Mock) -> None:
        ...
        invoice_params = {
            ...
            'auto_advance': True, 'billing': 'send_invoice', 'statement_descriptor': 'Yakkl Standard',
        ...
        line_item_params = {
            'amount': 8000 * 123, 'description': 'Yakkl Standard', 'discountable': False,
            'period': {
                'end': datetime_to_timestamp(self.next_year),
                'start': datetime_to_timestamp(self.now)},
    ...
    def test_upgrade_where_first_card_fails(self, *mocks: Mock) -> None:
        ...
        stripe_customer_id = Customer.objects.get(realm=get_realm('yakkl')).stripe_customer_id
        ...
        with patch('corporate.lib.stripe.get_seat_count', return_value=23):
            with patch('corporate.views.get_seat_count', return_value=23):
                self.upgrade()
        customer = Customer.objects.get(realm=get_realm('yakkl'))
    ...
    def test_upgrade_with_uncaught_exception(self, mock_: Mock) -> None:
        ...
        self.assert_json_error_contains(response, "Something went wrong. Please contact yakkl-admin@example.com.")
    ...
    def test_get_seat_count(self) -> None:
        ...
        user1 = UserProfile.objects.create(realm=realm, email='user1@yakkl.com', pointer=-1)
        user2 = UserProfile.objects.create(realm=realm, email='user2@yakkl.com', pointer=-1)
        ...
        UserProfile.objects.create(realm=realm, email='user3@yakkl.com', pointer=-1, is_guest=True)
        self.assertEqual(get_seat_count(realm), initial_count)
    ...
    def test_downgrade(self, mock_: Mock) -> None:
        ...
        self.assertEqual(get_realm('yakkl').plan_type, Realm.LIMITED)
...
class RequiresBillingAccessTest(YakklTestCase):
    def setUp(self) -> None:
...
class BillingHelpersTest(YakklTestCase):
    ...
    def test_update_or_create_stripe_customer_logic(self) -> None:
        ...
        Customer.objects.create(realm=get_realm('yakkl'), stripe_customer_id='cus_12345')
...
class LicenseLedgerTest(StripeTestCase):
    ...
    def test_user_changes(self) -> None:
        self.local_upgrade(self.seat_count, True, CustomerPlan.ANNUAL, 'token')
        user = do_create_user('email', 'password', get_realm('yakkl'), 'name', 'name')
...
class InvoiceTest(StripeTestCase):
    ...
    def test_invoice_plan(self, *mocks: Mock) -> None:
        ...
        with patch('corporate.lib.stripe.get_seat_count', return_value=self.seat_count + 3):
            update_license_ledger_if_needed(get_realm('yakkl'), self.now + timedelta(days=100))
        # Decrease
        with patch('corporate.lib.stripe.get_seat_count', return_value=self.seat_count):
            update_license_ledger_if_needed(get_realm('yakkl'), self.now + timedelta(days=200))
        # Increase, but not past high watermark
        with patch('corporate.lib.stripe.get_seat_count', return_value=self.seat_count + 1):
            update_license_ledger_if_needed(get_realm('yakkl'), self.now + timedelta(days=300))
        # Increase, but after renewal date, and below last year's high watermark
        with patch('corporate.lib.stripe.get_seat_count', return_value=self.seat_count + 2):
            update_license_ledger_if_needed(get_realm('yakkl'), self.now + timedelta(days=400))
        # Increase, but after event_time
        with patch('corporate.lib.stripe.get_seat_count', return_value=self.seat_count + 3):
            update_license_ledger_if_needed(get_realm('yakkl'), self.now + timedelta(days=500))
        plan = CustomerPlan.objects.first()
        ...
        line_item_params = {
            ...
            'description': 'Yakkl Standard - renewal',
            'discountable': False,
            'period': {
