...
from zerver.decorator import yakkl_login_required, require_billing_access
...
def payment_method_string(stripe_customer: stripe.Customer) -> str:
    ...
    return _("Unknown payment method. Please contact %s.") % (settings.YAKKL_ADMINISTRATOR,)  # nocoverage
...
def billing_home(request: HttpRequest) -> HttpResponse:
    ...
    if plan is not None:
        plan_name = {
            CustomerPlan.STANDARD: 'Yakkl Standard',
            CustomerPlan.PLUS: 'Yakkl Plus',
        }[plan.tier]
        now = timezone_now()
