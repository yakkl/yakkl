...
BILLING_LOG_PATH = os.path.join('/var/log/yakkl'
...
class BillingError(Exception):
    # error messages
    CONTACT_SUPPORT = _("Something went wrong. Please contact %s.") % (settings.YAKKL_ADMINISTRATOR,)
        ...
        if settings.DEVELOPMENT and not settings.TEST_SUITE:  # nocoverage
            if STRIPE_PUBLISHABLE_KEY is None:
                raise BillingError('missing stripe config', "Missing Stripe config. "
                                   "See https://yakkl.readthedocs.io/en/latest/subsystems/billing.html.")
        try:
...
def compute_plan_parameters(
    ...
    if billing_schedule == CustomerPlan.ANNUAL:
        # TODO use variables to account for Yakkl Plus
...
def process_initial_upgrade(user: UserProfile, licenses: int, automanage_licenses: bool,
    ...
    if charge_automatically:
        stripe_charge = stripe.Charge.create(
            ...
            description="Upgrade to Yakkl Standard, ${} x {}".format(price_per_license/100, licenses),
            receipt_email=user.email,
            statement_descriptor='Yakkl Standard')
