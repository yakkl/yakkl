...
import type { Preview2Account, Preview2Token, Preview2Chain } from '../types';
...
export class MigrationUtils {
  /**
   * Convert legacy YakklAccount to Preview2Account format
   */
  ...
  /**
   * Convert legacy Token to Preview2Token format
   */
  static convertToken(token: Token & { totalValue?: string; totalQuantity?: string; price?: string }): Preview2Token {
    return {
      symbol: token.symbol,
      name: token.name,
   ...
   */
  static convertTokens(tokens: (Token & { totalValue?: string; totalQuantity?: string; price?: string })[]): Preview2Token[] {
    return tokens.map(token => this.convertToken(token));
  }

  /**
   * Convert legacy chain data to Preview2Chain format
   */
  static convertChain(chainData: any): Preview2Chain {
    return {
