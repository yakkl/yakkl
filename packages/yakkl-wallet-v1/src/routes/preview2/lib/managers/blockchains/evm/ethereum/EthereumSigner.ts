...
export class EthereumSigner extends Signer {
	...
	constructor(privateKey: string | null, provider: Provider | null) {
		...
		this.wallet = new ethersv6.Wallet(
			privateKey,
			providerNative ? providerNative : (provider as any)
		);
	}
	 ...
	 */
	async signTypedData(
		domain: ethersv6.TypedDataDomain,
		types: Record<string, ethersv6.TypedDataField[]>,
	...
	async verifySigner(
		...
		try {
			const value =
				ethersv6.recoverAddress(ethersv6.hashMessage(messageToVerify), signature) === signerAddress;
			return value;
		} catch (e) {
			log.error(e);
			return false;
	...
	private transactionToEthersTransaction(
	...
	private async ethersTransactionResponseToTransactionResponse(
	...
	private async ethersTransactionReceiptToTransactionReceipt(
