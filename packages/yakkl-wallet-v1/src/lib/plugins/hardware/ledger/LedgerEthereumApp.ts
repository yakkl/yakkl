...
export class LedgerEthereumApp {
	...
	private handleError(error: any): never {
		...
		if (error.message?.includes('Ledger device locked')) {
			throw new HardwareWalletError('Please unlock your Ledger device', 'DEVICE_LOCKED', 'ledger');
		}

		if (error.message?.includes('DisconnectedDevice')) {
			throw new HardwareWalletError('Ledger device disconnected', 'DEVICE_DISCONNECTED', 'ledger');
		}
