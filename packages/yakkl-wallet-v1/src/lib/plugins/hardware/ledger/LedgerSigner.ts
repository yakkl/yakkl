...
export class LedgerSigner {
	...
	async signMessage(deviceId: string, derivationPath: string, message: string): Promise<string> {
		if (!message || message.length === 0) {
			throw new HardwareWalletError('Message cannot be empty', 'INVALID_MESSAGE', 'ledger');
		}
	...
	async signTypedData(
		...
		if (!typedData.domain || !typedData.types || !typedData.message) {
			throw new HardwareWalletError('Invalid typed data structure', 'INVALID_TYPED_DATA', 'ledger');
		}
	...
	private validateTransaction(tx: TransactionData): void {
		...
		if (!tx.chainId) {
			throw new HardwareWalletError('Chain ID is required', 'MISSING_CHAIN_ID', 'ledger');
		}

		// Validate address format
		if (tx.to && !this.isValidAddress(tx.to)) {
			throw new HardwareWalletError('Invalid recipient address', 'INVALID_ADDRESS', 'ledger');
		}
		...
		for (const field of numericFields) {
			...
			if (value && !this.isValidHexNumber(value.toString())) {
				throw new HardwareWalletError(`Invalid ${field} value`, 'INVALID_VALUE', 'ledger');
			}
		}
	}
