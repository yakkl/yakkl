...
export class SessionPortManager {
	 ...
	 */
	registerSessionPort(message: any, port: Runtime.Port, existingConnectionId?: string): string {
		// Use existing connection ID if provided, otherwise generate new one
		const connectionId =
			existingConnectionId || `connection-${Date.now()}-${Math.random().toString(36).slice(2)}`;
