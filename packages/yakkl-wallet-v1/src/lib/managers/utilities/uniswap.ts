...
export function tokenInNetAmount(
	tokenOut: SwapToken,
	tokenOutAmount: BigNumberish,
	exchangeRate: bigint,
	feeBasisPoint: number = YAKKL_FEE_BASIS_POINTS
): BigNumberish {
	try {
		...
		// Constants
		const tokenOutAmt: bigint =
			(BigNumber.toBigInt(tokenOutAmount) as bigint) * BigInt(10 ** tokenOut.decimals); // 30 WETH with 18 decimals (using bigint for WETH)
		...
		const tokenOutAmountBeforeFee: bigint =
			(tokenOutAmt * feeDivisor) / (feeDivisor - feeBasisPoints);
