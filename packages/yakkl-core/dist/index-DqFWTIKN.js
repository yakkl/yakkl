...
const legacy = require("./legacy-Idj-Xe9n.js");
...
function _abytes2(value, length, title = "") {
  const bytes = utils.isBytes(value);
...
function numberToBytesBE(n, len) {
  return utils.hexToBytes(n.toString(16).padStart(len * 2, "0"));
}
...
function ensureBytes(title, hex, expectedLength) {
  ...
  if (typeof hex === "string") {
    try {
      res = utils.hexToBytes(hex);
    } catch (e) {
      throw new Error(title + " must be hex string or Uint8Array, cause: " + e);
    }
  } else if (utils.isBytes(hex)) {
...
function weierstrassN(params, extraOpts = {}) {
  ...
  function pointToBytes(_c, point, isCompressed) {
    ...
    if (isCompressed) {
      ...
      return utils.concatBytes(pprefix(hasEvenY), bx);
    } else {
      return utils.concatBytes(Uint8Array.of(4), bx, Fp.toBytes(y));
    }
  }
  ...
  class Point2 {
    ...
    toHex(isCompressed = true) {
      return utils.bytesToHex(this.toBytes(isCompressed));
    }
    toString() {
...
function ecdh(Point2, ecdhOpts = {}) {
  const { Fn } = Point2;
  const randomBytes_ = ecdhOpts.randomBytes || utils.randomBytes;
  ...
  return Object.freeze({ getPublicKey, getSharedSecret, keygen, Point: Point2, utils: utils$1, lengths });
}
function ecdsa(Point2, hash, ecdsaOpts = {}) {
  ...
  const randomBytes = ecdsaOpts.randomBytes || utils.randomBytes;
  const hmac$1 = ecdsaOpts.hmac || ((key, ...msgs) => hmac.hmac(hash, key, utils.concatBytes(...msgs)));
  const { Fp, Fn } = Point2;
  const { ORDER: CURVE_ORDER, BITS: fnBits } = Fn;
  const { keygen, getPublicKey, getSharedSecret, utils: utils$1, lengths } = ecdh(Point2, ecdsaOpts);
  ...
  class Signature {
    ...
    static fromHex(hex, format) {
      return this.fromBytes(utils.hexToBytes(hex), format);
    }
    ...
    recoverPublicKey(messageHash) {
      ...
      const R = Point2.fromBytes(utils.concatBytes(pprefix((rec & 1) === 0), x));
    ...
    toBytes(format = defaultSigOpts_format) {
      validateSigFormat(format);
      if (format === "der")
        return utils.hexToBytes(DER.hexFromSig(this));
      const r = Fn.toBytes(this.r);
      const s = Fn.toBytes(this.s);
      if (format === "recovered") {
        if (this.recovery == null)
          throw new Error("recovery bit must be present");
        return utils.concatBytes(Uint8Array.of(this.recovery), r, s);
      }
      return utils.concatBytes(r, s);
    }
    toHex(format) {
      return utils.bytesToHex(this.toBytes(format));
    }
    // TODO: remove
    assertValidity() {
    }
    ...
    toDERHex() {
      return utils.bytesToHex(this.toBytes("der"));
    }
    toCompactRawBytes() {
      return this.toBytes("compact");
    }
    toCompactHex() {
      return utils.bytesToHex(this.toBytes("compact"));
    }
  }
  ...
  function prepSig(message, privateKey, opts) {
    ...
    const seed = utils.concatBytes(...seedArgs);
    const m = h1int;
  ...
  function tryParsingSig(sg) {
    let sig = void 0;
    const isHex = typeof sg === "string" || utils.isBytes(sg);
...
const secp256k1 = createCurve({ ...secp256k1_CURVE, Fp: Fpk1, lowS: true, endo: secp256k1_ENDO }, sha2.sha256);
/*! scure-bip32 - MIT License (c) 2022 Patricio Palladino, Paul Miller (paulmillr.com) */
const Point = secp256k1.ProjectivePoint;
const base58check = index.createBase58check(sha2.sha256);
...
const hash160 = (data) => legacy.ripemd160(sha2.sha256(data));
...
class HDKey {
  ...
  get privateExtendedKey() {
    ...
    return base58check.encode(this.serialize(this.versions.private, utils.concatBytes(new Uint8Array([0]), priv)));
  }
  ...
  static fromMasterSeed(seed, versions = BITCOIN_VERSIONS) {
    ...
    const I = hmac.hmac(sha2.sha512, MASTER_SECRET, seed);
    return new HDKey({
      versions,
  ...
  deriveChild(index2) {
    ...
    const I = hmac.hmac(sha2.sha512, this.chainCode, data);
...
exports.HDKey = HDKey;
//# sourceMappingURL=index-DqFWTIKN.js.map
