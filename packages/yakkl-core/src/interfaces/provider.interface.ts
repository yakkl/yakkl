...
export interface ProviderCostMetrics {
  ...
  methodCosts?: Record<string, {
    cu?: number;      // Compute units per call
  ...
  billingPeriod?: {
    ...
    costSoFar?: number; // In USD or credits
  };
}
...
export interface ProviderMetadata {
  ...
  features?: {
    archive?: boolean;      // Archive node access
    trace?: boolean;        // Trace API support
    debug?: boolean;        // Debug API support
    websocket?: boolean;    // WebSocket support
    batchRequests?: boolean; // Batch request support
  };
...
export interface ProviderInterface {
  ...
  // ======== Optional Chain-Specific Features ========

  // ENS support (Ethereum-specific)
  ...
  // Signing (if provider supports it)
