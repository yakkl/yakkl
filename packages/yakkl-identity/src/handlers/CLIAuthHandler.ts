...
export class CLIAuthHandler {
  private pendingAuths: Map<string, any> = new Map();
   ...
   */
  async browserFlow(credentials: any): Promise<Identity | null> {
   ...
   */
  async deviceCodeFlow(credentials: any): Promise<Identity | null> {
    // Request device code
   ...
   */
  async interactiveFlow(credentials: any): Promise<Identity | null> {
   ...
   */
  async serviceAccountFlow(credentials: any): Promise<Identity | null> {
    // Check environment first
    if (process.env.YAKKL_SERVICE_ACCOUNT_KEY) {
      return this.authenticateServiceAccount(
        process.env.YAKKL_SERVICE_ACCOUNT_ID!,
        process.env.YAKKL_SERVICE_ACCOUNT_KEY
      );
    }
  ...
  private async exchangeCode(code: string, provider: string): Promise<Identity | null> {
  ...
  private async requestDeviceCode(provider: string): Promise<any> {
  ...
  private async pollForCompletion(deviceCode: string, provider: string): Promise<Identity | null> {
  ...
  private async checkDeviceAuth(deviceCode: string): Promise<any> {
  ...
  private async verifyMagicLink(email: string, code: string): Promise<Identity | null> {
  ...
  private async loadServiceAccount(keyPath: string): Promise<Identity | null> {
  ...
  private async authenticateServiceAccount(id: string, key: string): Promise<Identity | null> {
