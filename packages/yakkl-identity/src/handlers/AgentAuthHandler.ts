...
export class AgentAuthHandler {
   ...
   */
  private async authenticateExistingAgent(credentials: any): Promise<Identity | null> {
    ...
    // Verify signature
    const valid = await this.verifyAgentSignature(
      agent,
   ...
   */
  private async createDelegatedAgent(credentials: any): Promise<Identity | null> {
    // Verify parent token
   ...
   */
  private async authenticateMCPServer(credentials: any): Promise<Identity | null> {
  ...
  private async verifyAgentSignature(
    ...
    try {
      ...
      const key = await crypto.subtle.importKey(
        'raw',
        new TextEncoder().encode(agent.secretKey),
   ...
   */
  private getToolPermissions(tools: string[]): string[] {
   ...
   */
  async listAgents(parentIdentityId: string): Promise<Identity[]> {
