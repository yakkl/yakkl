		...
		class u {
			...
			filterRawSettings(t) {
				const e = {},
					...
					r = Object.keys(i),
					n = new Set(this.propertiesToWorkWith);
				return (
					r.forEach((t) => {
						n.has(t) && (e[t] = i[t]);
					}),
			...
			_replaceScript(i) {
				const { scriptURL: r, scriptElement: n, rawSettings: o, id: a, overrideHost: l } = i,
					c =
						l ||
						(function (t) {
							if ('BATTLE' === g) {
				...
				const u = n.nonce || n.getAttribute('nonce'),
					m = (function (t) {
				...
				u && T.setAttribute('nonce', u), this.iframeContainer.appendChild(T);
				const C = this.hasCopyright() ? `calc(100% - ${w})` : '100%',
					_ = location.hostname,
					A = p ? 'widget_new' : 'widget',
					k = this.widgetUtmName();
				(this.settings.utm_source = _),
					(this.settings.utm_medium = A),
					(this.settings.utm_campaign = k),
					this._updateCopyrightHrefParams(_, A, k);
				const L =
						this.settings.iframeTitle || `${this.widgetId.replace('-', ' ')} TradingView widget`,
					P = this.settings.iframeLang || 'en';
				(this.iframe = this._createIframe(C, c, a, L, P)),
					this._addCSPErrorListener(c),
					u && this.iframe.setAttribute('nonce', u);
				const E = this.iframeContainer.querySelector('.tradingview-widget-container__widget');
				if (
					(E
						? ((0, t.ensureNotNull)(E.parentElement).replaceChild(this.iframe, E), n?.remove())
						: f
							? (this.iframeContainer.appendChild(this.iframe), n?.remove())
							: (this.iframeContainer.appendChild(this.iframe),
								h.replaceChild(this.iframeContainer, (0, t.ensureNotNull)(n))),
			...
			_validateSettings() {
				...
				return (
					null !== e && null !== i && ((this.settings.width = e), (this.settings.height = i), !0)
				);
			}
