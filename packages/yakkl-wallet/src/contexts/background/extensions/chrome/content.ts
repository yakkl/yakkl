...
class ContentScriptManager {
	...
	constructor() {
		...
		this.initialize().catch(error => {
			log.warn('Failed to initialize content script connection:', false, error);
	...
	private setupMessageHandlers() {
		...
		try {
			...
			log.warn('Error setting up message handlers', false, error);
			// throw error;
		}
	}
				...
				if (!isExtensionContextValid()) {
					...
					setTimeout(() => {
						if (isExtensionContextValid() && !this.isConnected) {
							...
							this.initialize().catch(error => {
								log.warn('Failed to reconnect after context restoration:', false, error);
							});
						}
					}, 2000);
		...
		try {
			...
			switch (this.securityLevel) {
				...
				case SecurityLevelEnum.MEDIUM:
					...
					const isTrustedDomain = WALLET_SECURITY_CONFIG.TRUSTED_DOMAINS.some((domain) =>
						frameOrigin.includes(domain)
					);
