...
export async function onSigningRequestListener(
	...
	try {
		if (message && typeof message === 'object' && 'type' in message) {
			...
			if (
				msgType === 'REQUEST_SESSION_PORT' ||
				msgType === 'REGISTER_SESSION_PORT' ||
				msgType === 'UNREGISTER_SESSION_PORT' ||
				msgType === 'STORE_SESSION_HASH' ||
		...
		if (!request || (request.type !== 'personal_sign' && request.type !== 'eth_signTypedData_v4')) {
			log.info('onSigningRequestListener - Method not supported for signing request', false, {
				request
			});
			return undefined;
		}
