// Minimal security helpers for permissions
// Uses the new security-bridge for actual security operations
...
export async function getSecurityLevel(): Promise<SecurityLevel> {
  try {
    const level = await storage.get(SECURITY_LEVEL_KEY);
...
export async function setSecurityLevel(level: SecurityLevel): Promise<void> {
  try {
    await storage.set(SECURITY_LEVEL_KEY, level);
