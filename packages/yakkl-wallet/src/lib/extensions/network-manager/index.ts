/**
 * Network Manager Extension - Enhanced network switching and management
 */
...
export class NetworkManagerExtension implements Extension {
  ...
  async initialize(engine: WalletEngine): Promise<void> {
    ...
    console.log('🌐 Network Manager extension initialized');
  ...
  async destroy(): Promise<void> {
    ...
    console.log('🌐 Network Manager extension destroyed');
  }
  ...
  private setupEventListeners(): void {
    ...
    this.engine.on('network:changed', (network) => {
      ...
      this.emit('extension:network:changed', { network, source: 'network-manager' });
    });

    this.engine.on('network:error', (error) => {
      console.warn('🌐 Network error detected:', error);
      this.emit('extension:network:error', { error, suggestions: this.getErrorSuggestions(error) });
    });
  }
  ...
  private async switchNetwork(chainId: number): Promise<any> {
    ...
    try {
      ...
      this.emit('extension:network:switched', { 
        chainId, 
        network,
        timestamp: Date.now(),
        source: 'network-manager-extension'
      });

      return { success: true, network };
    } catch (error) {
      this.emit('extension:network:switch-failed', { chainId, error: error.message });
      throw error;
    }
  }
  ...
  private async addCustomNetwork(networkData: any): Promise<any> {
    try {
      ...
      this.emit('extension:network:added', { network: validated });
      
      return { success: true, network: validated };
    } catch (error) {
      this.emit('extension:network:add-failed', { networkData, error: error.message });
      throw error;
    }
  }
...
export default NetworkManagerExtension;