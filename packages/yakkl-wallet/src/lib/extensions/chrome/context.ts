...
async function clearLockdownState(): Promise<void> {
  ...
  try {
    pendingCleanupPromise = (async () => {
      ...
      const shouldSendClearMessage = now - (clearLockdownState as any).lastClearTime > CLEANUP_THROTTLE;
    ...
    if (!isLockdownInitiated) {
      ...
      if (currentState !== 'active') {
        ...
        if (idleLockDelay <= 0) {
          ...
          // Delayed lockdown
          log.info(`[ContextTracker] ${state} detected, lockdown will occur in ${idleLockDelay/1000} seconds`);
...
async function startLockdownSequence(): Promise<void> {
  try {
    ...
    log.info(`[ContextTracker] Starting lockdown sequence for ${protectedContextsLocal.length} protected context(s)`);
      ...
      if (context.tabId && context.id) {
        messagePromises.push(
          browser.tabs.sendMessage(context.tabId, {
            ...
            log.warn(`[ContextTracker] Failed to send lockdownImminent to context ${context.id}:`, false, err);
          })
        );
      }
    }
...
async function executeLockdown(): Promise<void> {
  try {
    ...
    log.info(`[ContextTracker] Executing lockdown for ${protectedContextsLocal.length} protected context(s)`);
      ...
      if (context.tabId) {
        lockdownPromises.push(
          browser.tabs.sendMessage(context.tabId, {
            ...
            log.warn(`[ContextTracker] Failed to send lockdown to context ${context.id}:`, false, err);
          })
        );
      }
    }
...
function broadcastIdleStatus(state: IdleState): void {
  try {
    ...
    log.info(`[ContextTracker] Broadcasting idle status '${state}' to ${protectedContextsLocal.length} protected context(s)`);
