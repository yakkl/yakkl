...
import { getYakklTokenCache } from '$lib/common/stores';
  ...
  private async calculateETHValueUSD(ethBalance: string): Promise<number> {
    try {
      ...
      if (ethPrice === 0) {
        ...
        ethPrice = BigNumberishUtils.toNumber(priceData.price);
      }
      ...
      return DecimalMath.of(balanceETH).mul(BigNumberishUtils.toNumber(ethPrice)).toNumber();
    } catch (error) {
