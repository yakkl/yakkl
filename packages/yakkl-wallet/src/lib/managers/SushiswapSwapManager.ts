...
export class SushiSwapManager<T extends BaseTransaction> extends SwapManager {
	...
	protected async prepareTokensForSwap(tokenIn: Token, tokenOut: Token): Promise<[Token, Token]> {
	...
	async getQuote(
		...
		const priceImpactRatio =
			((Number(amountOutBigInt) - Number(amountOutWithFee)) / Number(amountOutBigInt)) * 100;
	...
	async getPoolInfo(
		...
		const [reserves, token0] = await Promise.all([pair.call('getReserves'), pair.call('token0')]);
	...
	private async getPairAddress(tokenA: string, tokenB: string): Promise<string> {
		...
		const factory = this.blockchain.createContract(ADDRESSES.SUSHISWAP_FACTORY, [
	...
	async executeSwap(params: SwapParams): Promise<TransactionResponse> {
		const { tokenIn, tokenOut, amount, slippage, deadline, recipient } = params;
