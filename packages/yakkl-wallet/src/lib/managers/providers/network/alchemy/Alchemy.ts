...
import {
	type BlockTag as CustomBlockTag,
	type BigNumberish,
	type Deferrable,
...
import {
	...
	type Filter as AlchemyFilter,
	type Log as AlchemyLog,
	type BlockTag as AlchemyBlockTag
} from 'alchemy-sdk';
...
export class Alchemy extends AbstractProvider {
	...
	async call(
		...
		blockTag: CustomBlockTag | Promise<CustomBlockTag> = 'latest'
	): Promise<string> {
		try {
			...
			const result = await this.alchemy.core.call(
				ethersTransaction as any,
				resolvedBlockTag as any
			);

			eventManager.emit('call', {
	...
	async estimateGas(transaction: Deferrable<TransactionRequest>): Promise<bigint> {
		try {
			...
			if (isBackgroundContext) {
				...
				const rpcAlchemy = new RPCAlchemy(config.apiKey);

				// Convert transaction to RPC format
				const rpcTransaction = {
					from: resolvedTransaction.from,
				...
				const result = await rpcAlchemy.estimateGas(rpcTransaction);
				const gasEstimate = BigInt(result);
				eventManager.emit('estimateGas', { transaction: resolvedTransaction, gasEstimate });
				return gasEstimate;
			}

			// Use full Alchemy API in non-background context
			await this.getAlchemy(this.chainId);
			if (!this.alchemy) {
				throw new Error('No Alchemy set');
			}
	...
	private async resolveDeferredTransaction(
		transaction: Deferrable<TransactionRequest>
	...
	async getBalance(
		...
		try {
			...
			if (isBackgroundContext) {
				...
				const rpcAlchemy = new RPCAlchemy(config.apiKey);
				const result = await rpcAlchemy.getBalance(addressOrName, blockTagStr);
			...
			const result = await this.alchemy.core.getBalance(
				addressOrName,
				resolvedBlockTag as AlchemyBlockTag
			);
	...
	async getCode(
		...
		try {
			...
			if (isBackgroundContext) {
				...
				const rpcAlchemy = new RPCAlchemy(config.apiKey);
				return await rpcAlchemy.getCode(addressOrName, blockTagStr);
			}
			...
			return await this.alchemy.core.getCode(addressOrName, resolvedBlockTag as AlchemyBlockTag);
		} catch (error) {
	...
	async getStorageAt(
		...
		try {
			...
			eventManager.emit('getStorageAt', {
				addressOrName,
				position,
				blockTag: resolvedBlockTag,
				storage
	 ...
	 */
	async getBlock(
		blockHashOrBlockTag: CustomBlockTag | Promise<CustomBlockTag> = 'latest'
	): Promise<Block> {
		try {
			log.info('getBlock', false, { blockHashOrBlockTag });
