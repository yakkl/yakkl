...
export class DAppPopupManager {
   ...
   */
  private requestsMap = new Map<string, {
    method: string;
    createdAt: number;
  ...
  async showDappPopup(
    ...
    try {
      ...
      const sameDappPendingRequest = this.findSameDappPendingRequest(method, originTabId, originWindowId);
   ...
   */
  private findAnySamePendingMethod(
    method: string
  ): { requestId: string } | null {
    // Check the queue first
    for (const item of this.requestQueue) {
  ...
  private showDuplicateRequestWarning(method: string): void {
    try {
      ...
      sendNotificationMessage('Request in Progress', `A ${method} request from this DApp is already being processed. Please complete that request first.`);
      return;
  ...
  private showMaxQueueWarning(): void {
    try {
      ...
      sendNotificationMessage('Too Many Requests', 'There are too many pending DApp requests. Please complete some existing requests first.');
      return;
    } catch (error) {
