...
/**
  ...
  private static startBadgeCountdownEnhanced(seconds: number): void {
    ...
    if (typeof window !== 'undefined') {
      ...
      log.info('[NotificationService] Badge countdown started:', false, { seconds, intervalId: this._badgeCountdownInterval });
    }
  }
  ...
  static async sendLockdownWarningEnhanced(
    ...
    try {
      ...
      log.info('[NotificationService] sendLockdownWarningEnhanced called with full attention features', false, { delayMs, delaySeconds, options });
      ...
      if (browser_ext?.notifications) {
        try {
          ...
          log.warn('[NotificationService] Failed to create enhanced browser notification:', false, err);
        }
      }
    ...
    try {
      ...
      if (browser_ext?.notifications) {
        try {
          ...
          log.info(`[NotificationService] Found ${Object.keys(notifications).length} notifications to clear`);
      ...
      log.info('[NotificationService] All enhanced alerts cleared successfully (no UI messages sent)');
    } catch (error) {
