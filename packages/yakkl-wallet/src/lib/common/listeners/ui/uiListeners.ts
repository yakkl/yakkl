...
function currentContextNeedsIdleProtection(): boolean {
  ...
  if (contextType === 'unknown' && typeof window !== 'undefined') {
    const href = window.location.href;
    if (href.includes('chrome-extension://') && (href.includes('index.html') || href.endsWith('/'))) {
...
function isMessageTargeted(message: any): boolean {
  ...
  const effectiveContextType = currentContextType === 'unknown' ? 'popup-wallet' : currentContextType;
    ...
    switch(message.type) {
      case 'lockdown': {
        ...
        if (!currentContextNeedsIdleProtection()) {
          log.info('[uiListeners] Ignoring lockdown - context does not need idle protection:', false, getCurrentContextType());
          return false; // Let other listeners handle it if any
        }

        if (!isMessageTargeted(message)) {
          log.info('[uiListeners] Ignoring lockdown - not targeted to this context type:', false, getCurrentContextType());
          return false;
        }

        log.info('[uiListeners] EXECUTING lockdown for protected context:', false, getCurrentContextType());
      ...
      case 'lockdownImminent': {
        ...
        if (!currentContextNeedsIdleProtection()) {
          log.info('[uiListeners] Ignoring lockdownImminent - context does not need idle protection:', false, getCurrentContextType());
          return false; // Let other listeners handle it if any
        }

        if (!isMessageTargeted(message)) {
          log.info('[uiListeners] Ignoring lockdownImminent - not targeted to this context type:', false, getCurrentContextType());
          return false;
        }

        log.info('[uiListeners] EXECUTING lockdownImminent for protected context:', false, getCurrentContextType());
        ...
        try {
          ...
          if (message.enhanced) {
            ...
            await NotificationService.sendLockdownWarningEnhanced(
              message?.delayMs || 30000,
            ...
            await NotificationService.sendLockdownWarning(
              message?.delayMs || 30000,
              {contextMessage: 'Use YAKKL before timeout to stop lockdown'}
            );
          }
      ...
      case 'LOCKDOWN_WARNING_ENHANCED': {
        ...
        if (!currentContextNeedsIdleProtection()) {
          log.info('[uiListeners] Ignoring enhanced lockdown warning - context does not need protection');
          return false;
        }
        ...
        try {
          ...
          if (typeof window !== 'undefined') {
            ...
            const flashInterval = setInterval(() => {
              document.title = flashCount % 2 === 0 ? '🚨 URGENT - YAKKL Security Alert!' : originalTitle;
              flashCount++;
      ...
      case 'IDLE_STATUS_CHANGED': {
        ...
        if (!currentContextNeedsIdleProtection()) {
          log.debug('[uiListeners] Ignoring idle status change - context does not need idle protection:', false, getCurrentContextType());
          return false;
        }

        if (!isMessageTargeted(message)) {
          log.debug('[uiListeners] Ignoring idle status change - not targeted to this context type:', false, getCurrentContextType());
          return false;
        }

        // Handle idle status change for protected contexts
        log.info(`[uiListeners] PROCESSING idle status changed to ${message.state} for protected context ${getCurrentContextType()}`);
