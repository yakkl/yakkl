 ...
 * @param email - Email address to get gravatar for
...
export function getGravatarUrl(
  ...
  const params = new URLSearchParams({
    s: size.toString(),
    d: defaultImage,
    r: 'pg' // Rating: suitable for all audiences
  });
...
/**
 * Check if an email has a custom Gravatar (not default)
 * This is done by requesting with 404 as default
 */
export async function hasCustomGravatar(email: string): Promise<boolean> {
  try {
