...
function createWalletCacheStore() {
	...
	const saveToStorage = debounce(async (state: WalletCacheController) => {
		...
		try {
			// Validate before saving - save if we have meaningful data
			...
			if (state.chainAccountCache) {
				Object.values(state.chainAccountCache).forEach((chainData) => {
					Object.values(chainData).forEach((accountData: any) => {
						// Check for various types of valid data
						...
						// - Portfolio has been calculated with value > 0
						// - Any token has a balance > 0 (even if portfolio not calculated yet)
						// - Any token has a price > 0 (price data is valuable)
						// - Native token exists (always preserve native token data)
						// - Any token has calculated value > 0
						// - At least some token data exists (prevents completely empty saves)
						// - We have token metadata (even with zero balances, metadata is valuable)
						if (
							hasPortfolioValue ||
			...
			if (hasValidData) {
				...
				await setObjectInExtensionStorage(CACHE_KEY, serializedState);
	...
	return {
		...
		updateTokens(chainId: number, address: string, tokens: TokenCache[]) {
			update((cache) => {
				...
				const hasAnyMeaningfulData = tokens.some(
					(token) =>
						(token.balance && BigNumber.toBigInt(token.balance) > 0n) ||
						(token.price && (token.price || 0) > 0) ||
				...
				const existingHasValues = accountCache.tokens.some(
					(token) =>
						(token.balance && BigNumber.toBigInt(token.balance) > 0n) ||
						(token.value && BigNumber.toBigInt(token.value) || 0n > 0n) ||
						token.isNative
				);

				const allNewTokensZero = tokens.every(
					(token) =>
						(!token.balance || BigNumber.toBigInt(token.balance) === 0n) &&
						(!token.value || BigNumber.toBigInt(token.value) === 0n) &&
						(!token.price || token.price === 0)
				);

				// Allow saving if:
				// 1. We have the native token (always important to cache)
				// 2. We have price data (even with zero balance)
				// 3. This is the first load (no existing tokens)
				// 4. We have token metadata (symbol, name, etc)
				const hasNativeToken = tokens.some((t) => t.isNative);
				const hasPriceData = tokens.some(
					(t) => t.price && t.price > 0
				);
				const isFirstLoad = accountCache.tokens.length === 0;
				const hasTokenMetadata = tokens.some((t) => t.symbol && t.name && t.decimals);
				...
				if (
					...
					log.warn(
						'[WalletCache] Rejecting token update - all new values are zero and no meaningful data',
						false
					);
					return cache;
				}
				...
				const validatedTokens = tokens.map((token) => {
					// Parse balance as a float (already in ETH/token units, not wei)
					const balance = BigNumber.toBigInt(token.balance || '0');
