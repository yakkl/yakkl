<script lang="ts">
  ...
  import { calculateSubmenuPositionFixed } from './utils-fixed';
  ...
  let submenuId = `submenu-${Math.random().toString(36).substring(2, 9)}`;
  ...
  function positionSubmenu() {
    ...
    submenuElement.classList.remove("menu-left", "menu-right");
    submenuElement.classList.add(`menu-${position.direction}`);
  }
