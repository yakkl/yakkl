<script lang="ts">
  ...
  import type { HardwareDevice, HardwareAccount } from '$lib/plugins/hardware/common/HardwareWalletTypes';
  import { 
  ...
  async function connectLedger() {
    ...
    try {
      ...
      if (error instanceof UserRejectedError) {
        errorMessage = 'Connection cancelled. Please try again and approve the connection on your device.';
        ...
        errorMessage = 'Your browser does not support hardware wallets. Please use Chrome, Edge, or another Chromium-based browser.';
...
<div class="ledger-connector">
  {#if !isSupported}
    <div class="alert alert-warning">
      ...
      <span>Hardware wallets are not supported in this browser. Please use Chrome, Edge, or another Chromium-based browser.</span>
    </div>
              ...
              {#each discoveredAccounts as account, index}
                <div class="flex items-center justify-between p-3 bg-base-200 rounded-lg">
                  <div>
                    ...
                    <div class="text-xs text-base-content/70">Account {index + 1} • {account.derivationPath}</div>
                  </div>
