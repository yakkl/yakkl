...
<script lang="ts">
  ...
  import { canUpgrade } from '$lib/common/utils';
	...
	import { checkRegistrationStatus, isUserLoggedIn, type RegistrationStatus } from '$lib/common/auth-utils';
  ...
  let {
    show = $bindable(false),
    onComplete = $bindable(() => {}),
    onClose = $bindable(() => {safeLogout()}),
    onCancel = $bindable(() => {show = false, showConfirmation = false, showNotification = false, showError = false}),
    openWallet,
    useAuthStore = false
  }: Props = $props();
  ...
  const isSidepanel = $derived(contextType === 'sidepanel' || (contextType?.includes?.('sidepanel') ?? false));
  ...
  function handleLoginSuccess(profile: any, digest: string, _isMinimal: boolean, jwtToken?: string) {
    isLoggedIn = true;
    ...
    if (profile) {
      ...
      if (profile.data?.email) {
        ...
        if (isEncryptedData(profile.data)) {
          ...
          decryptData(profile.data, digest).then((profileData) => {
...
<ErrorNoAction bind:show={showError} value="You are already using a {planLevelAvailable.toUpperCase()} level plan" title="Congratulations!" onClose={isSidepanel ? handleSidepanelCancel : onCancel} />
<Confirmation bind:show={showConfirmation} title="Upgrading to {planLevelAvailable.toUpperCase()}!" message="Are you sure you want to upgrade to {planLevelAvailable.toUpperCase()}?" onConfirm={onConfirm} onCancel={isSidepanel ? handleSidepanelCancel : onCancel}  />
<Notification bind:show={showNotification} title="Upgraded to {planLevelAvailable.toUpperCase()}!" message="You are now using the {planLevelAvailable.toUpperCase()} plan. You can now access all the features of the {planLevelAvailable.toUpperCase()} plan." onClose={isSidepanel ? handleNotificationClose : onClose} />

<Modal bind:show title="Upgrade to {planLevelAvailable.toUpperCase()}" onClose={isSidepanel ? handleSidepanelClose : onClose}>
  <div class="space-y-6 p-6">
    ...
    <AuthError
      ...
      onRetry={retryCount < maxRetries && !isUpgrading && !isInitializing ? handleInitRetry : undefined}
