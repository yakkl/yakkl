<script lang="ts">
  ...
  import { PATH_LEGAL_PRIVACY, PATH_LEGAL_TOS, PATH_LEGAL_SUBSCRIPTION, type ProfileData } from '$lib/common';
  ...
  $effect(() => {
    if (menuOpen && !menuButtonElement) {
      log.warn('Menu is open but button element is not set!');
    }
  });
  ...
  onMount(async () => {
    try {
      ...
      log.error('Failed to load user profile:', false, error);
    }
  });
  ...
  let name = $derived.by(() => {
    ...
    return acc.ens || acc.name || shortAddr(acc.address) || 'Wallet';
  });
  ...
  let avatarInitial = $derived.by(() => {
    ...
    const acc = effectiveAccount;
    if (!acc) return 'W';
    if (acc.name) return acc.name[0].toUpperCase();
        ...
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              icon: { svg: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>' },
              action: () => { menuOpen = false; goto(PATH_LEGAL_SUBSCRIPTION); }
            },
            {
              type: 'item',
              label: 'Compliance',
...
>
  <div class="relative">
    <button
      ...
      aria-label={`Manage account for ${name}`}
