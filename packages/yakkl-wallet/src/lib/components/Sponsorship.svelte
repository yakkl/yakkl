...
<script lang="ts">
  ...
  function getNextSponsor(): Sponsor {
    const validSponsors = sponsors.filter(sponsor => {
      const now = new Date();
      const hasValidDates = (!sponsor.startDate || now >= sponsor.startDate) &&
                           (!sponsor.endDate || now <= sponsor.endDate);
      const hasValidImpressions = !sponsor.maxImpressions ||
                                 (sponsor.analytics?.impressions || 0) < sponsor.maxImpressions;
  ...
  function trackImpression(sponsor: Sponsor) {
    ...
    if (timeSinceLastImpression >= 30000) { // 30 seconds
