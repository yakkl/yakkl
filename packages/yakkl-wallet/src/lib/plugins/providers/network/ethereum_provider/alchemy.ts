...
class AlchemyProvider extends EventEmitter {
  ...
  public async getBlockByNumber(blockNumber: string, includeTransactions: boolean): Promise<any> {
    try {
      return await makeAlchemyRequest('eth_getBlockByNumber', [blockNumber, includeTransactions], this.chainId);
    } catch (error) {
      log.error('Error getting block by number', false, { blockNumber, includeTransactions, error });
      throw error;
    }
  }
  ...
  public async getBlockByHash(blockHash: string, includeTransactions: boolean): Promise<any> {
    try {
      return await makeAlchemyRequest('eth_getBlockByHash', [blockHash, includeTransactions], this.chainId);
    } catch (error) {
  ...
  public async getStorageAt(address: string, position: string, blockTag: string = 'latest'): Promise<string> {
    try {
      return await makeAlchemyRequest('eth_getStorageAt', [address, position, blockTag], this.chainId);
    } catch (error) {
