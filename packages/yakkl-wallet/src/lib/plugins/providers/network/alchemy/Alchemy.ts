...
import { RPCAlchemy } from '$plugins/providers/network/alchemy/RPCAlchemy';
...
export class Alchemy extends AbstractProvider {
  ...
  async getBalance(addressOrName: string, blockTag: CustomBlockTag | Promise<CustomBlockTag> = 'latest'): Promise<bigint> {
    try {
      ...
      const result = await this.alchemy.core.getBalance(addressOrName, resolvedBlockTag as AlchemyBlockTag);
