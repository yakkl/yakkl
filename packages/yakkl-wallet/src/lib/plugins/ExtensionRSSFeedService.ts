...
export class ExtensionRSSFeedService {
  ...
  private async fetchArticleContent(url: string): Promise<{ content: string; quality: 'complete' | 'partial' | 'minimal' }> {
    try {
  ...
  private extractArticleContent(html: string): { content: string; quality: 'complete' | 'partial' | 'minimal' } {
  ...
  private assessContentQuality(content: string, wordCount: number): 'complete' | 'partial' | 'minimal' {
  ...
  private detectContentType(item: Element, description: string): 'article' | 'video' | 'podcast' | 'unknown' {
    ...
    const processItems = async () => {
      ...
      for (const chunk of chunks) {
        await Promise.all(chunk.map(async (item) => {
          const itemTitle = item.querySelector('title')?.textContent?.replace(/<!\[CDATA\[|\]\]>/g, '') || '';
          const itemLink = item.querySelector('link')?.textContent || '';
          const itemDescription = item.querySelector('description')?.textContent?.replace(/<!\[CDATA\[|\]\]>/g, '') || '';
          ...
          if (contentType === 'article') {
            try {
              ...
              const encodedContent = contentEncodedElement?.textContent?.replace(/<!\[CDATA\[|\]\]>/g, '') || '';
          ...
          const rssItem: RSSItem = {
            ...
            guid,
            categories,
            author: item.querySelector('author')?.textContent?.replace(/<!\[CDATA\[|\]\]>/g, '') || '',
            contentType,
            contentQuality
          };
