...
export class StandardNewsManager implements INewsManager {
	...
	async updateBookmark(
		bookmarkId: string,
		updates: Partial<BookmarkedArticle>
	): Promise<BookmarkedArticle> {
		if (!this.initialized) {
			throw new Error('News manager not initialized');
		}
	...
	async exportNewsData(): Promise<NewsExport> {
		throw new UpgradeRequiredError('News data export requires a PRO plan', 'News Export', 'PRO');
	}

	async importNewsData(data: NewsExport): Promise<boolean> {
		throw new UpgradeRequiredError('News data import requires a PRO plan', 'News Import', 'PRO');
	}
