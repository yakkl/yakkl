...
export interface IHardwareWalletManager {
  ...
  // Signing Operations
  signTransaction(deviceId: string, derivationPath: string, transaction: TransactionData): Promise<SignedTransaction>;
  ...
  // Pro Features
  bulkSignTransactions(deviceId: string, transactions: TransactionBatch[]): Promise<SignedTransaction[]>;
