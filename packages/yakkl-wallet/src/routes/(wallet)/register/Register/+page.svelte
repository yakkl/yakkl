<script lang="ts">
	...
	import {
		profile as profileDefaults,
		yakklPreferences as yakklPreferencesDefaults,
		yakklCurrentlySelected as yakklCurrentlySelectedDefaults,
		yakklSettings as yakklSettingsDefaults
	...
	$effect(() => {
		strength = zxcvbn($form.password).score ?? 0;
	});
		...
		try {
			if (browserSvelte && browser_ext) {
				...
				if (isEncryptedData(currentlySelected?.data)) {
					currentlySelected.data = (await decryptData(
				...
				const profileData: ProfileData =
					typeof yakklProfile.data === 'object' && yakklProfile.data !== null
						? (yakklProfile.data as ProfileData)
						: (profileDefaults.data as ProfileData);
				...
				await browser_ext.runtime.setUninstallURL(
					encodeURI(
						`https://yakkl.com/cta/bye?userName=${userName}&utm_source=yakkl&utm_medium=extension&utm_campaign=uninstall&utm_content=${VERSION}&utm_term=extension`
					)
				);
				...
				sendNotificationMessage(
					'Welcome to YAKKL!',
					'Your account is set up. Thank you for choosing YAKKL Smart Wallet. \nStart exploring swaps, low fees, and more. 🚀'
				);
	...
	const { form, errors, handleChange, handleSubmit } = createForm({
		...
		validationSchema: yup.object().shape({
			userName: yup
				...
				.matches(
					/^[a-z0-9]{6,}$/,
					"Must be lowercase and at least 6 characters and not contain '.nfs.id'"
				),
			password: yup
				.string()
			...
			email: yup
				.string()
				.email('Please enter a valid email address. Required for additional security')
				.max(255)
			...
			accountName: yup
				...
				.matches(
					/^[A-Za-z0-9#@!*&_.() ]{3,}$/,
					'Can contain uppercase, lowercase, numeric, and a few special characters, and at least 3 characters'
				)
		}),
		onSubmit: async (data) => {
			try {
	...
	<ul>
		...
		<li class="flex items-center mb-1">
			<svg
				...
				stroke-width="1.5"
				stroke="currentColor"
				class="mr-2 w-4 h-4 text-green-400 dark:text-gray-400"
				><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg
			>
			A symbol (#$&amp;!)
		</li>
...
<Popover class="text-sm z-50" triggeredBy="#nam-help" placement="top">
	<h3 class="font-semibold text-gray-900 dark:text-white">
		Must have at least 6 lowercase alphanumeric characters
	</h3>
	<div class="grid grid-cols-4 gap-2">
		<div class="h-1 bg-orange-300 dark:bg-orange-400"></div>
		<div class="h-1 bg-orange-300 dark:bg-orange-400"></div>
...
<Popover class="text-sm z-50" triggeredBy="#act-help" placement="top">
	<h3 class="font-semibold text-gray-900 dark:text-white">
		Must be alphanumeric. Some special characters are allowed.
	</h3>
	<div class="grid grid-cols-4 gap-2">
		<div class="h-1 bg-orange-300 dark:bg-orange-400"></div>
		<div class="h-1 bg-orange-300 dark:bg-orange-400"></div>
...
<div class="relative m-2 h-[95%] bg-base-100 text-base-content rounded-xl">
	<main class="mt-1 mx-auto w-full text-center p-4">
		...
		<p class="mt-1 mx-auto text-base">A smart wallet that works as you would expect</p>
		<p class="mx-auto text-base">Intuitive, Secure, Fast, and Powerful</p>
		<hr class="my-2" />
		<div class="w-full">
			{#if init}
				...
				<div
					...
					class="font-bold text-md uppercase my-5"
				>
					NOTE: It appears you have already registered! Click here to go to login instead. If you
					continue with registering below then it will RESET your account to empty and start over
					(BE CAREFUL)!
				</div>
			{/if}
			<span class="mb-2 text-md uppercase font-bold text-center"
				>[Registration data is used for securing your authenticity! Never leaves YAKKL, fully
				encrypted, and is Required.]</span
			>
			<form class="w-full" onsubmit={handleSubmit}>
				<div class="my-1">
					<div class="flex flex-row mt-2">
						...
						<svg
							...
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 20 20"
							class="w-6 h-6 ml-1 mt-4 fill-gray-300"
						>
							<path
								fill-rule="evenodd"
					...
					<div class="flex flex-row mt-2">
						<div class="form-control w-full">
							...
							<svg
								...
								viewBox="0 0 24 24"
								fill="currentColor"
								class="w-6 h-6 ml-1 fill-gray-200 absolute right-11 z-10 mt-5 cursor-pointer"
							>
								<path
					...
					<div class="w-[93%] bg-gray-200 rounded-full my-1 mt-1">
						{#if strength < 3}
							<div
								class="bg-red-600 h-1.75 mt-1 text-2xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-l-full"
								style="width: 33%"
							>
								Weak
							</div>
						...
						{#if strength === 3}
							<div
								class="bg-yellow-400 h-1.75 mt-1 text-2xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-l-full"
								style="width: 66%"
							>
								Average
							</div>
						...
						{#if strength > 3}
							<div
								class="bg-green-500 h-1.75 mt-1 text-2xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full"
								style="width: 100%"
							>
								Strong
							</div>
					...
					<div class="w-full text-center mt-1">
						<span class="text-sm font-bold"
							>Weak password or anything less than 8 characters will not be allowed!</span
						>
					</div>
					<div class="mt-2">
						<div class="flex flex-row">
							...
							<svg
								...
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 20 20"
								class="w-6 h-6 relative ml-1 mt-2 fill-gray-200"
							>
								<path
									fill-rule="evenodd"
									d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z"
					...
					<div class="flex flex-row mt-2">
						...
						<svg
							...
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 20 20"
							class="w-6 h-6 relative ml-1 mt-4 fill-gray-200"
						>
							<path
								fill-rule="evenodd"
								d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z"
				...
				<div class="mt-2 inline-block text-center">
					<button type="submit" class="btn btn-primary w-64 rounded-full mt-2">
