<script lang="ts">
	...
	onMount(async () => {
		try {
			if (browserSvelte) {
				...
				if (settings !== null) {
					planType = settings.plan.type ? settings.plan.type : PlanType.BASIC_MEMBER;
				} else {
	...
	async function createAccount() {
		try {
			...
			await createSubportfolioAccount(yakklMiscStore, currentlySelected, profile)
				.then(async (result) => {
					try {
						...
						let yakklAccount: YakklAccount = (currentlySelected.data as CurrentlySelectedData)
						...
						let yakklPrimaryAccount: YakklPrimaryAccount = (
							currentlySelected.data as CurrentlySelectedData
						).primaryAccount as YakklPrimaryAccount;
...
<Modal bind:open={successDialog} autoclose>
	<div class="text-center">
		...
		<h3 class="mb-2 text-lg font-normal text-green-700 dark:text-gray-400">
			<span class="font-bold">SUCCESS - </span> Additional Wallet Account Created!
		</h3>
		<div class="text-left">
		...
		<div class="text-center">
			<p class="mb-5 text-md font-bold text-green-600">
				...
				University. You can find it on the Home screen.
			</p>
...
{:then _}
	<div class="print:hidden min-h-[40rem] mx-2">
        ...
        {/if} -->
				...
				>
					...
					<h4 class="text-center font-extrabold text-lg text-gray-300 mb-4">
						PRINT THIS PAGE and COPY YOUR SECRET PHRASE SOMEWHERE SAFE!
					</h4>

					<div class="ml-2 mr-2 text-center">
						<h3 class="text-lg font-medium leading-6 text-gray-300">
							{blockchain} - Subportfolio Account Secrets Emergency Kit
						</h3>
						<br />
						...
						<h4 class="block text-sm text-gray-300">
					...
					<div class="mt-5 border-t mx-1 border-gray-400 break-words overflow-x-scroll">
						<dl class="divide-y divide-gray-400">
							<div class="py-4 grid grid-cols-5 bg-gray-100">
								...
								<dd
									class="text-sm font-extrabold font-mono text-gray-700 col-span-4 ml-5 mt-0 break-all"
								>
									{userName}
								</dd>
							</div>
					...
					<span class="block mt-5 text-base text-gray-300 justify-center relative"
						><span class="font-bold">VERY IMPORTANT:</span> NEVER EVER GIVE ANYONE THE INFORMATION ON
						THIS PAGE (except for CRYPTO ADDRESS since it is public)! NO ONE FROM YAKKL WILL EVER ASK
						FOR YOUR PRIVATE KEY, PASSWORD, OR SECRET PHRASE! If anyone, any site, or any project ever
						asks for anything other than your public crypto address then THEY MOST LIKELY ARE SCAMMERS!
						A scammer's website will look good, they may show a good number of social media followers,
						or an "influencer" may recommend them (maybe not even knowing). But, if they ask for anything
	...
	<div class="hidden print:block">
		<div
			class="ml-1 mr-1 mb-[10rem] print:ml-0 print:mr-0 w-[1000px] bg-white overscroll-none overflow-scroll"
		>
			<h4 class="text-center font-extrabold text-lg text-gray-900 m-4 print:hidden">
				VERY IMPORTANT! PRINT THIS PAGE AND STORE SOMEWHERE SAFE!
			</h4>
			<div id="printPDF">
				...
				<div class="w-full text-center">
					<h3 class="text-lg print:text-2xl font-medium leading-6 text-gray-900">
						{blockchain} - Subportfolio Account Emergency Kit
					</h3>
					<br />
				...
				<span class="block mt-5 text-base text-gray-900 justify-center relative"
					><span class="font-bold">VERY IMPORTANT:</span> NEVER EVER GIVE ANYONE THE INFORMATION ON
					THIS PAGE (except for CRYPTO ADDRESS since it is public)!
					...
					a good number of social media followers, or an "influencer" may recommend them (maybe not even
					knowing). But, if they ask for anything except your crypto address then treat them like a scammer
					to stay safe!</span
				>
				<br />
				...
				>
				<br />
				<span class="block mt-3 text-base text-gray-900 justify-center relative"
					><span class="font-bold">IMPORTANT:</span> To safely destroy this document, redact and shred
					or burn it!</span
				>
				<br />
