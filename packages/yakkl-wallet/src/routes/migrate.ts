...
/**
 * Main migration script to move from old system to v2
 */
...
export class V2Migration {
  ...
  /**
   * Apply the migration to v2 stores
   */
  private async applyMigration(convertedData: any): Promise<void> {
    this.log('Applying migration to v2 stores...');
...
export async function quickMigrate(options: MigrationConfig = {}): Promise<void> {
  const migration = new V2Migration({
    verbose: true,
    ...options
  });
...
/**
 * Enable v2 features without full migration (for development)
 */
export async function enableV2(): Promise<void> {
  ...
  console.log('V2 enabled in development mode');
}
...
export async function isMigrationNeeded(): Promise<boolean> {
  const settings = await getSettings();
  return !(settings as any).v2Enabled && !(settings as any).migrationCompleted;
}
