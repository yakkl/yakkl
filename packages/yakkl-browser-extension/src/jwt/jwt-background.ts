...
export interface JWTPayload {
  ...
  iss: string;           // Issuer
  aud: string;           // Audience
  secureHash?: string;   // Optional secure hash for backend API
}
...
class BackgroundJWTManager {
  ...
  async refreshTokenIfNeeded(
    ...
    try {
      ...
      return token; // Token doesn't need refresh
    } catch (error) {
  ...
  async clearBlacklist(): Promise<void> {
    try {
      if (this.isExtensionContext()) {
        await browser.storage.local.remove([this.BLACKLIST_KEY]);
      } else if (typeof localStorage !== 'undefined') {
  ...
  private async addToBlacklist(token: string): Promise<void> {
    try {
      ...
      if (this.isExtensionContext()) {
        await browser.storage.local.set({ [this.BLACKLIST_KEY]: cleanedBlacklist });
      } else if (typeof localStorage !== 'undefined') {
  ...
  private async getBlacklist(): Promise<Record<string, number>> {
    try {
      if (this.isExtensionContext()) {
        const result = await browser.storage.local.get([this.BLACKLIST_KEY]);
        return result[this.BLACKLIST_KEY] || {};
      } else if (typeof localStorage !== 'undefined') {
...
// Type is already exported above in the interface definition
