...
var MemoryCache = class {
  constructor(options = {}) {
    ...
    this.cache = new (0, _lrucache.LRUCache)({
  ...
  async set(key, value, options) {
    const ttl = _optionalChain([options, 'optionalAccess', _2 => _2.ttl]) || this.defaultTTL;
    const now = Date.now();
    const entry = {
      value,
      createdAt: now,
      ...
      metadata: {
        source: "memory",
        version: 1,
        immutable: _optionalChain([options, 'optionalAccess', _3 => _3.strategy]) === "blockchain" && !_optionalChain([options, 'optionalAccess', _4 => _4.ttl])
      }
    };
...
var IndexedDBCache = class {
  ...
  async initializeDB() {
    try {
      this.db = await _idb.openDB.call(void 0, this.dbName, 1, {
        upgrade(db) {
  ...
  async set(key, value, options) {
    try {
      const db = await this.ensureDB();
      const ttl = _optionalChain([options, 'optionalAccess', _5 => _5.ttl]) || this.defaultTTL;
      const now = Date.now();
      const entry = {
        value,
        createdAt: now,
        ...
        metadata: {
          source: "indexeddb",
          version: 1,
          immutable: _optionalChain([options, 'optionalAccess', _6 => _6.strategy]) === "blockchain" && ttl > 24 * 60 * 60 * 1e3
        }
      };
  ...
  async setMany(entries, options) {
    try {
      ...
      const tx = db.transaction("cache", "readwrite");
      const store = tx.objectStore("cache");
      const ttl = _optionalChain([options, 'optionalAccess', _7 => _7.ttl]) || this.defaultTTL;
      const now = Date.now();
      for (const [key, value] of entries) {
        const entry = {
          value,
...
var PersistentCache = class {
  constructor(options = {}) {
    ...
    this.compress = _nullishCoalesce(options.compress, () => ( true));
  ...
  async set(key, value, options) {
    const fullKey = this.prefix + key;
    const ttl = _optionalChain([options, 'optionalAccess', _8 => _8.ttl]) || this.defaultTTL;
    const now = Date.now();
    try {
      const entry = {
        value,
...
var Deduplicator = class {
   ...
   */
  wrap(fn, options) {
    const ttl = _optionalChain([options, 'optionalAccess', _9 => _9.ttl]) || this.ttl;
    const keyGen = _optionalChain([options, 'optionalAccess', _10 => _10.keyGenerator]) || this.keyGenerator;
    return async (...args) => {
      const key = keyGen(...args);
      return this.execute(key, () => fn(...args), ttl, _optionalChain([options, 'optionalAccess', _11 => _11.onDeduplicated]));
    };
  }
  /**
...
var BatchProcessor = class {
  constructor(options) {
    ...
    this.pQueue = new (0, _pqueue2.default)({ concurrency: this.options.concurrency });
  }
  /**
...
var CacheManager = class {
  ...
  setupSynchronization() {
    if (typeof window !== "undefined" && window.addEventListener) {
      window.addEventListener("storage", (event) => {
        if (_optionalChain([event, 'access', _12 => _12.key, 'optionalAccess', _13 => _13.startsWith, 'call', _14 => _14("yakkl-cache:")])) {
  ...
  async get(key, options) {
    ...
    if (this.deduplicator && _optionalChain([options, 'optionalAccess', _15 => _15.deduplicate])) {
      const cached = null;
      if (cached) {
   ...
   */
  async set(key, value, options) {
    const tier = _optionalChain([options, 'optionalAccess', _16 => _16.tier]) || this.selectTier(value, options);
    const cache = this.tiers.get(tier);
    if (!cache) {
      throw new CacheError(`Invalid cache tier: ${tier}`, "INVALID_KEY");
    }
    if (this.deduplicator && _optionalChain([options, 'optionalAccess', _17 => _17.deduplicate])) {
      await this.deduplicator.deduplicate(`set:${key}`, async () => {
        return value;
      });
    }
    if (this.batchProcessor && _optionalChain([options, 'optionalAccess', _18 => _18.batch])) {
    ...
    if (this.costTracker && _optionalChain([options, 'optionalAccess', _19 => _19.strategy]) === "blockchain") {
  ...
  selectTier(value, options) {
    if (_optionalChain([options, 'optionalAccess', _20 => _20.tier])) {
      return options.tier;
    }
    if (_optionalChain([options, 'optionalAccess', _21 => _21.ttl])) {
...
var BlockchainCache = class {
  ...
  getDataMutability(query) {
    ...
    if (query.method === "eth_getLogs" && _optionalChain([query, 'access', _22 => _22.params, 'access', _23 => _23[0], 'optionalAccess', _24 => _24.toBlock]) && query.params[0].toBlock !== "latest") {
      return { mutable: false };
    }
    return { mutable: true };
  }
...
var SemanticCache = class {
  ...
  async findSimilar(question, options = {}) {
    ...
    const similar = results.filter((r) => r.score >= threshold).map((r) => {
      const query = {
        id: r.id,
        question: _optionalChain([r, 'access', _25 => _25.metadata, 'optionalAccess', _26 => _26.question]) || "",
        answer: r.text || "",
        confidence: r.score,
        category: _optionalChain([r, 'access', _27 => _27.metadata, 'optionalAccess', _28 => _28.category]),
        metadata: r.metadata
      };
      if (options.boostRecent && _optionalChain([query, 'access', _29 => _29.metadata, 'optionalAccess', _30 => _30.timestamp])) {
      ...
      if (options.boostHelpful && _optionalChain([query, 'access', _31 => _31.metadata, 'optionalAccess', _32 => _32.helpful])) {
  ...
  async getFAQs(category, limit = 10) {
    ...
    for (const key of keys.slice(0, limit * 2)) {
      ...
      if (query && _optionalChain([query, 'access', _33 => _33.metadata, 'optionalAccess', _34 => _34.source]) === "faq") {
        faqs.push(query);
      }
    }
    faqs.sort((a, b) => {
      const aScore = (_optionalChain([a, 'access', _35 => _35.metadata, 'optionalAccess', _36 => _36.votes]) || 0) * (_optionalChain([a, 'access', _37 => _37.metadata, 'optionalAccess', _38 => _38.helpful]) ? 2 : 1);
      const bScore = (_optionalChain([b, 'access', _39 => _39.metadata, 'optionalAccess', _40 => _40.votes]) || 0) * (_optionalChain([b, 'access', _41 => _41.metadata, 'optionalAccess', _42 => _42.helpful]) ? 2 : 1);
  ...
  async searchByMetadata(filters, limit = 10) {
    ...
    return results.map((r) => ({
      id: r.id,
      question: _optionalChain([r, 'access', _43 => _43.metadata, 'optionalAccess', _44 => _44.question]) || "",
      answer: r.text || "",
      confidence: r.score,
      category: _optionalChain([r, 'access', _45 => _45.metadata, 'optionalAccess', _46 => _46.category]),
      metadata: r.metadata
    })).slice(0, limit);
  }
  /**
...
var VectorDBManager = class {
  ...
  async initialize() {
    switch (this.config.provider) {
      case "cloudflare":
        const { CloudflareVectorProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./CloudflareVectorProvider-QDPCD5SN.cjs")));
        this.provider = new CloudflareVectorProvider(this.config);
        break;
      case "postgres":
        const { PostgresVectorProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./PostgresVectorProvider-FAAAWK6L.cjs")));
        this.provider = new PostgresVectorProvider(this.config);
        break;
      case "pinecone":
        const { PineconeProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./PineconeProvider-YDWPM2IX.cjs")));
        this.provider = new PineconeProvider(this.config);
        break;
      case "weaviate":
        const { WeaviateProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./WeaviateProvider-Z3XH43AI.cjs")));
        this.provider = new WeaviateProvider(this.config);
        break;
      case "qdrant":
        const { QdrantProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./QdrantProvider-KN5MS2OM.cjs")));
        this.provider = new QdrantProvider(this.config);
        break;
      case "chroma":
        const { ChromaProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./ChromaProvider-FCY7SP4H.cjs")));
  ...
  async createCollection(name, schema) {
    this.ensureConnected();
    const dimension = _optionalChain([schema, 'optionalAccess', _47 => _47.dimension]) || this.config.dimension || 384;
    const metric = _optionalChain([schema, 'optionalAccess', _48 => _48.metric]) || this.config.metric || "cosine";
...
var SQLManager = class {
  ...
  async initialize() {
    switch (this.config.provider) {
      case "postgres":
        const { PostgreSQLProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./PostgreSQLProvider-5AWKJIOM.cjs")));
        this.provider = new PostgreSQLProvider(this.config);
        break;
      case "cloudflare-d1":
        const { CloudflareD1Provider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./CloudflareD1Provider-5V4RF6YR.cjs")));
        this.provider = new CloudflareD1Provider(this.config);
        break;
      case "sqlite":
        const { SQLiteProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./SQLiteProvider-LVHKALGU.cjs")));
        this.provider = new SQLiteProvider(this.config);
        break;
      case "mysql":
        const { MySQLProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./MySQLProvider-LCLXM6AK.cjs")));
        this.provider = new MySQLProvider(this.config);
        break;
      case "cockroachdb":
        const { CockroachDBProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./CockroachDBProvider-I6AHSXIX.cjs")));
  ...
  async select(options) {
    this.ensureConnected();
    const columns = _optionalChain([options, 'access', _49 => _49.columns, 'optionalAccess', _50 => _50.join, 'call', _51 => _51(", ")]) || "*";
...
var ObjectStorageManager = class {
  ...
  async initialize() {
    switch (this.config.provider) {
      case "s3":
        const { S3Provider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./S3Provider-UOUOSPBM.cjs")));
        this.provider = new S3Provider(this.config);
        break;
      case "gcs":
        const { GCSProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./GCSProvider-ZE2XZD3H.cjs")));
        this.provider = new GCSProvider(this.config);
        break;
      case "azure":
        const { AzureBlobProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./AzureBlobProvider-XAEALBUN.cjs")));
        this.provider = new AzureBlobProvider(this.config);
        break;
      case "cloudflare-r2":
        const { CloudflareR2Provider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./CloudflareR2Provider-6Q2CQLGE.cjs")));
        this.provider = new CloudflareR2Provider(this.config);
        break;
      case "backblaze-b2":
        const { BackblazeB2Provider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./BackblazeB2Provider-LYUDWXE2.cjs")));
        this.provider = new BackblazeB2Provider(this.config);
        break;
      case "minio":
        const { MinioProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./MinioProvider-PR53BTY5.cjs")));
  ...
  async upload(key, data, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _52 => _52.bucket]) || this.defaultBucket;
    if (!bucket) {
  ...
  async download(key, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _53 => _53.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
  ...
  async delete(key, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _54 => _54.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
    await this.provider.deleteObject(bucket, key, _optionalChain([options, 'optionalAccess', _55 => _55.versionId]));
  }
  /**
  ...
  async deleteMany(keys, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _56 => _56.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
  ...
  async copy(sourceKey, destKey, options) {
    this.ensureConnected();
    const sourceBucket = _optionalChain([options, 'optionalAccess', _57 => _57.sourceBucket]) || this.defaultBucket;
    const destBucket = _optionalChain([options, 'optionalAccess', _58 => _58.destBucket]) || this.defaultBucket;
  ...
  async getMetadata(key, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _59 => _59.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
    return this.provider.headObject(bucket, key, _optionalChain([options, 'optionalAccess', _60 => _60.versionId]));
  }
  /**
  ...
  async exists(key, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _61 => _61.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
  ...
  async list(options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _62 => _62.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
  ...
  async getSignedUrl(key, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _63 => _63.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
  ...
  async getUrl(key, options) {
    const bucket = _optionalChain([options, 'optionalAccess', _64 => _64.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
    switch (this.config.provider) {
  ...
  async uploadLarge(key, data, options) {
    this.ensureConnected();
    const bucket = _optionalChain([options, 'optionalAccess', _65 => _65.bucket]) || this.defaultBucket;
    if (!bucket) {
      throw new Error("No bucket specified");
    }
    const partSize = _optionalChain([options, 'optionalAccess', _66 => _66.partSize]) || 5 * 1024 * 1024;
    const uploadId = await this.provider.createMultipartUpload(
      bucket,
      key,
      _optionalChain([options, 'optionalAccess', _67 => _67.metadata])
    );
    try {
      const parts = [];
      let partNumber = 1;
      if (_buffer.Buffer.isBuffer(data)) {
        for (let i = 0; i < data.length; i += partSize) {
          ...
          partNumber++;
          if (_optionalChain([options, 'optionalAccess', _68 => _68.progress])) {
...
var KVStoreManager = class {
  ...
  async initialize() {
    switch (this.config.provider) {
      case "cloudflare-kv":
        const { CloudflareKVProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./CloudflareKVProvider-OHABQS4R.cjs")));
        this.provider = new CloudflareKVProvider(this.config);
        break;
      case "redis":
        const { RedisProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./RedisProvider-AUYHQBA2.cjs")));
        this.provider = new RedisProvider(this.config);
        break;
      case "upstash":
        const { UpstashProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./UpstashProvider-TDWJY5XD.cjs")));
        this.provider = new UpstashProvider(this.config);
        break;
      case "memcached":
        const { MemcachedProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./MemcachedProvider-MEM44NAX.cjs")));
        this.provider = new MemcachedProvider(this.config);
        break;
      case "dynamodb":
        const { DynamoDBProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./DynamoDBProvider-MMZ36O2G.cjs")));
        this.provider = new DynamoDBProvider(this.config);
        break;
      case "etcd":
        const { EtcdProvider } = await Promise.resolve().then(() => _interopRequireWildcard(require("./EtcdProvider-6U7IBFR7.cjs")));
  ...
  async set(key, value, options) {
    this.ensureConnected();
    const metadata = {
      expirationTtl: _optionalChain([options, 'optionalAccess', _69 => _69.ttl]) || this.defaultTTL,
      metadata: _optionalChain([options, 'optionalAccess', _70 => _70.metadata])
    };
  ...
  async setIfNotExists(key, value, options) {
    this.ensureConnected();
    const metadata = {
      expirationTtl: _optionalChain([options, 'optionalAccess', _71 => _71.ttl]) || this.defaultTTL,
      metadata: _optionalChain([options, 'optionalAccess', _72 => _72.metadata])
    };
...
exports.AutoBatchProcessor = AutoBatchProcessor; exports.BatchProcessor = BatchProcessor; exports.BlockchainCache = BlockchainCache; exports.CacheError = CacheError; exports.CacheManager = CacheManager; exports.CachePresets = CachePresets; exports.CostTracker = CostTracker; exports.Deduplicator = Deduplicator; exports.DeduplicatorGroup = DeduplicatorGroup; exports.IndexedDBCache = IndexedDBCache; exports.KVStoreManager = KVStoreManager; exports.MemoryCache = MemoryCache; exports.MockEmbeddingProvider = MockEmbeddingProvider; exports.ObjectStorageManager = ObjectStorageManager; exports.PersistentCache = PersistentCache; exports.SQLManager = SQLManager; exports.SemanticCache = SemanticCache; exports.VectorDBManager = VectorDBManager; exports.createCache = createCache;
