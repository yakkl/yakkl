# Django settings for yakkl project.
########################################################################
# Here's how settings for the Yakkl project work:
#
# * settings.py contains non-site-specific and settings configuration
# for the Yakkl Django app.
...
#
# See https://yakkl.readthedocs.io/en/latest/subsystems/settings.html for more information
#
########################################################################
...
# Whether this instance of Yakkl is running in a production environment.
PRODUCTION = config_file.has_option('machine', 'deploy_type')
DEVELOPMENT = not PRODUCTION

secrets_file = configparser.RawConfigParser()
if PRODUCTION:
    secrets_file.read("/etc/yakkl/yakkl-secrets.conf")
else:
...
# that the owner of an email account has uploaded an avatar to Yakkl, which isn't
...
os.environ['BOTO_CONFIG'] = '/etc/yakkl/boto.cfg'
...
# (/etc/yakkl/settings.py in production, or dev_settings.py or test_settings.py
...
# prod_settings_template.py, and in the initial /etc/yakkl/settings.py on a new
# install of the Yakkl server.
DEFAULT_SETTINGS = {
    ...
    # settings in yakkl-secrets.conf instead of settings.py in development.
    ...
    # Not documented for in /etc/yakkl/settings.py, since it's rarely needed.
    ...
    'DEFAULT_LOGO_URI': '/static/images/logo/yakkl-org-logo.png',
    'S3_AVATAR_BUCKET': '',
...
#    like yakkl.com or to work around a problem on another server.
DEFAULT_SETTINGS.update({
    ...
    'FEEDBACK_BOT': 'feedback@yakkl.com',
    'FEEDBACK_BOT_NAME': 'Yakkl Feedback Bot',
    'FEEDBACK_STREAM': None,
    # SYSTEM_BOT_REALM would be a constant always set to 'yakkl',
    # except that it isn't that on yakkl.com.  We will likely do a
    # migration and eliminate this parameter in the future.
    'SYSTEM_BOT_REALM': 'yakklinternal',
    ...
    # Used to construct URLs to point to the Yakkl server.  Since we
    # only support HTTPS in production, this is just for development.
    'EXTERNAL_URI_SCHEME': "https://",

    # Whether anyone can create a new organization on the Yakkl server.
    ...
    # Settings for APNS.  Only needed on push.yakkl.com.
    'APNS_CERT_FILE': None,
    'APNS_SANDBOX': True,
    ...
    # By default, Yakkl uses websockets to send messages.  In some
    # networks, websockets don't work.  One can configure Yakkl to
    ...
    # set this to YAKKL_ADMINISTRATOR by default.
    'SERVER_EMAIL': None,
    # Django setting for who receives error emails.  We set to
    # YAKKL_ADMINISTRATOR by default.
    'ADMINS': '',
    ...
    # Used to change the Yakkl logo in portico pages.
    ...
    'STAGING': False,
    # Configuration option for our email/Yakkl error reporting.
...
REQUIRED_SETTINGS = [("EXTERNAL_HOST", "yakkl.example.com"),
...
if ADMINS == "":
    ADMINS = (("Yakkl Administrator", YAKKL_ADMINISTRATOR),)
MANAGERS = ADMINS
...
if DEVELOPMENT:
    ...
    # Yakkl was originally on PBKDF2 so we need it for compatibility
...
# https://cloud.google.com/console/project/apps~yakkl-android/apiui/credential
...
# These are the bots that Yakkl sends automated messages as.
...
# YakklStorage is a modified version of ManifestStaticFilesStorage,
...
# YakklStorage when not DEBUG.

if not DEBUG:
    STATICFILES_STORAGE = 'zerver.lib.storage.YakklStorage'
    if PRODUCTION:
        STATIC_ROOT = '/home/yakkl/prod-static'
    else:
...
base_template_engine_settings = {
    ...
    'OPTIONS': {
        ...
        'context_processors': [
            'zerver.context_processors.yakkl_default_context',
...
default_template_engine_settings.update({
    ...
    'DIRS': [
        ...
        # The python-yakkl-api:yakkl_bots package templates
...
YAKKL_PATHS = [
    ("SERVER_LOG_PATH", "/var/log/yakkl/server.log"),
    ("ERROR_FILE_LOG_PATH", "/var/log/yakkl/errors.log"),
    ("MANAGEMENT_LOG_PATH", "/var/log/yakkl/manage.log"),
    ("WORKER_LOG_PATH", "/var/log/yakkl/workers.log"),
    ("JSON_PERSISTENT_QUEUE_FILENAME_PATTERN", "/home/yakkl/tornado/event_queues%s.json"),
    ("EMAIL_LOG_PATH", "/var/log/yakkl/send_email.log"),
    ("EMAIL_MIRROR_LOG_PATH", "/var/log/yakkl/email_mirror.log"),
    ("EMAIL_DELIVERER_LOG_PATH", "/var/log/yakkl/email-deliverer.log"),
    ("EMAIL_CONTENT_LOG_PATH", "/var/log/yakkl/email_content.log"),
    ("LDAP_SYNC_LOG_PATH", "/var/log/yakkl/sync_ldap_user_data.log"),
    ("QUEUE_ERROR_DIR", "/var/log/yakkl/queue_error"),
    ("DIGEST_LOG_PATH", "/var/log/yakkl/digest.log"),
    ("ANALYTICS_LOG_PATH", "/var/log/yakkl/analytics.log"),
    ("ANALYTICS_LOCK_DIR", "/home/yakkl/deployments/analytics-lock-dir"),
    ("API_KEY_ONLY_WEBHOOK_LOG_PATH", "/var/log/yakkl/webhooks_errors.log"),
    ("WEBHOOK_UNEXPECTED_EVENTS_LOG_PATH", "/var/log/yakkl/webhooks_unexpected_events.log"),
    ("SOFT_DEACTIVATION_LOG_PATH", "/var/log/yakkl/soft_deactivation.log"),
    ("TRACEMALLOC_DUMP_DIR", "/var/log/yakkl/tracemalloc"),
    ("SCHEDULED_MESSAGE_DELIVERER_LOG_PATH",
     "/var/log/yakkl/scheduled_message_deliverer.log"),
    ("RETENTION_LOG_PATH", "/var/log/yakkl/message_retention.log"),
]

# The Event log basically logs most significant database changes,
# which can be useful for debugging.
if EVENT_LOGS_ENABLED:
    YAKKL_PATHS.append(("EVENT_LOG_DIR", "/home/yakkl/logs/event_log"))
else:
    EVENT_LOG_DIR = None

for (var, path) in YAKKL_PATHS:
    if DEVELOPMENT:
        # if DEVELOPMENT, store these files in the Yakkl checkout
...
YAKKL_WORKER_TEST_FILE = '/tmp/yakkl-worker-test-file'
...
DEFAULT_YAKKL_HANDLERS = (
    (['yakkl_admins'] if ERROR_REPORTING else []) +
...
LOGGING = {
    ...
    'formatters': {
        'default': {
            '()': 'zerver.lib.logging_util.YakklFormatter',
        }
    },
    'filters': {
        'YakklLimiter': {
            '()': 'zerver.lib.logging_util.YakklLimiter',
        },
        'EmailLimiter': {
    ...
    'handlers': {
        'yakkl_admins': {
            ...
            'filters': (['YakklLimiter', 'require_debug_false', 'require_really_deployed']
    ...
    'loggers': {
        ...
        '': {
            ...
            'handlers': DEFAULT_YAKKL_HANDLERS,
        },
        ...
        'pika.adapters': {
            ...
            # write to the log for use in debugging, but no error emails/Yakkls.
            'handlers': ['console', 'file', 'errors_file'],
            'propagate': False,
        },
        'pika.connection': {
            # Leave `yakkl_admins` out of the handlers.  See pika.adapters above.
            'handlers': ['console', 'file', 'errors_file'],
            'propagate': False,
        },
        'requests': {
        ...
        'tornado.general': {
            ...
            # logger.  These should not result in error emails/Yakkls.
            #
        ...
        'yakkl.soft_deactivation': {
            'handlers': ['file', 'errors_file'],
            'propagate': False,
        },
        'yakkl.zerver.lib.webhooks.common': {
            'level': 'DEBUG',
            'handlers': ['file', 'errors_file'],
            'propagate': False,
        },
        'yakkl.zerver.webhooks': {
            'level': 'DEBUG',
            'handlers': ['file', 'errors_file'],
            'propagate': False,
        },
...
# iOS App IDs
YAKKL_IOS_APP_ID = 'com.yakkl'

########################################################################
# SSO AND LDAP SETTINGS
########################################################################

USING_APACHE_SSO = ('zproject.backends.YakklRemoteUserBackend' in AUTHENTICATION_BACKENDS)
...
if len(AUTHENTICATION_BACKENDS) == 1 and (AUTHENTICATION_BACKENDS[0] ==
                                          "zproject.backends.YakklRemoteUserBackend"):
    HOME_NOT_LOGGED_IN = "/accounts/login/sso/"
    ONLY_SSO = True
else:
    HOME_NOT_LOGGED_IN = '/login/'
    ONLY_SSO = False
AUTHENTICATION_BACKENDS += ('zproject.backends.YakklDummyBackend',)
...
if POPULATE_PROFILE_VIA_LDAP and \
   'zproject.backends.YakklLDAPAuthBackend' not in AUTHENTICATION_BACKENDS:
    AUTHENTICATION_BACKENDS += ('zproject.backends.YakklLDAPUserPopulator',)
else:
    POPULATE_PROFILE_VIA_LDAP = (
        'zproject.backends.YakklLDAPAuthBackend' in AUTHENTICATION_BACKENDS or
        POPULATE_PROFILE_VIA_LDAP)
...
if REGISTER_LINK_DISABLED is None:
    ...
    if (len(AUTHENTICATION_BACKENDS) == 2 and
            ('zproject.backends.YakklLDAPAuthBackend' in AUTHENTICATION_BACKENDS)):
...
# Yakkl versions where /etc/yakkl/settings.py has not been migrated yet.
...
# Django setting. Not used in the Yakkl codebase.
DEFAULT_FROM_EMAIL = YAKKL_ADMINISTRATOR
...
if SERVER_EMAIL is None:
    SERVER_EMAIL = YAKKL_ADMINISTRATOR
...
if PRODUCTION:
    ...
    DEFAULT_EXCEPTION_REPORTER_FILTER = 'zerver.filters.YakklExceptionReporterFilter'
...
CROSS_REALM_BOT_EMAILS = {
    'feedback@yakkl.com',
    'notification-bot@yakkl.com',
    'welcome-bot@yakkl.com',
    'emailgateway@yakkl.com',
}
