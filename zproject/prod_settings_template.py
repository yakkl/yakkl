...
################################################################
# Yakkl Server settings.
#
# This file controls settings that affect the whole Yakkl server.
# See our documentation at:
#   https://yakkl.readthedocs.io/en/latest/production/settings.html
#
# For developer documentation on the Yakkl settings system, see:
#   https://yakkl.readthedocs.io/en/latest/subsystems/settings.html
#
...
# The email address for the person or team who maintains the Yakkl
...
# 'Yakkl Support <support@example.com>'.
YAKKL_ADMINISTRATOR = 'yakkl-admin@example.com'

# The user-accessible Yakkl hostname for this installation, e.g.
# yakkl.example.com.  This should match what users will put in their
...
# EXTERNAL_HOST to e.g. yakkl.example.com:1234 here.
EXTERNAL_HOST = 'yakkl.example.com'

# Alternative hostnames.  A comma-separated list of strings
# representing the host/domain names that your users can enter in
# their browsers to access Yakkl.  This is a security measure; for
# details, see the Django documentation:
# https://docs.djangoproject.com/en/1.11/ref/settings/#allowed-hosts
#
# Yakkl automatically adds to this list 'localhost', '127.0.0.1', and
...
#ALLOWED_HOSTS = ['yakkl-alias.example.com', '192.0.2.1']


################
# Outgoing email (SMTP) settings.
#
# Yakkl needs to be able to send email (that is, use SMTP) so it can
...
# advice for troubleshooting, see the Yakkl documentation:
...
# for user EMAIL_HOST_USER goes in `/etc/yakkl/yakkl-secrets.conf`.
...
# e.g. noreply-{random_token}@yakkl.example.com (if EXTERNAL_HOST is
# yakkl.example.com).  There are potential security issues if you set
# ADD_TOKENS_TO_NOREPLY_ADDRESS=False to remove the token; see
# https://yakkl.readthedocs.io/en/latest/production/email.html for details.
...
# See https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html
...
AUTHENTICATION_BACKENDS = (
    ...
    # 'zproject.backends.YakklLDAPAuthBackend',  # LDAP, setup below
    # 'zproject.backends.YakklRemoteUserBackend',  # Local SSO, setup docs on readthedocs
)  # type: Tuple[str, ...]
...
# correspond to your Yakkl instance.
#
...
#   https://yakkl.example.com/accounts/login/google/done/
...
# client secret in yakkl-secrets.conf as `social_auth_google_secret`.
...
#   https://yakkl.example.com/complete/github/ as
...
# yakkl-secrets.conf as `social_auth_github_secret`.
...
#   https://auth.yakkl.example.com/complete/github/
#
...
#   https://yakkl.example.com/complete/azuread-oauth2/
...
# (3) Put the application password in yakkl-secrets.conf as 'azure_oauth2_secret'.
#SOCIAL_AUTH_AZUREAD_OAUTH2_KEY = ''

########
# SSO via REMOTE_USER.
#
# If you are using the YakklRemoteUserBackend authentication backend,
...
# push notifications will be forwarded through a Yakkl push
# notification bouncer server to the mobile apps.  See
# https://yakkl.readthedocs.io/en/latest/production/mobile-push-notifications.html
# for information on how to sign up for and configure this.
#PUSH_NOTIFICATION_BOUNCER_URL = 'https://push.yakkl.com'
...
# Whether to submit basic usage statistics to help the Yakkl core team.  Details at
#
#   https://yakkl.readthedocs.io/en/latest/production/mobile-push-notifications.html
#
...
# https://yakkl.readthedocs.io/en/latest/production/security-model.html.
# PASSWORD_MIN_LENGTH = 6
# PASSWORD_MIN_GUESSES = 10000

# Controls whether Yakkl sends "new login" email notifications.
...
FEEDBACK_EMAIL = YAKKL_ADMINISTRATOR
...
# Controls whether or not Yakkl will provide inline image preview when
...
# Controls whether or not Yakkl will provide inline previews of
# websites that are referenced in links in messages.  Note: this feature
# can also be disabled in a realm's organization settings.
#INLINE_URL_EMBED_PREVIEW = True

# Controls whether or not Yakkl will parse links starting with
...
# directly on the Yakkl server.  You can configure files being instead
# stored in Amazon S3 or another scalable data store here.  See docs at:
#
#   https://yakkl.readthedocs.io/en/latest/production/upload-backends.html
#
# If you change LOCAL_UPLOADS_DIR to a different path, you will also
# need to manually edit Yakkl's nginx configuration to use the new
# path.  For that reason, we recommend replacing /home/yakkl/uploads
# with a symlink instead of changing LOCAL_UPLOADS_DIR.
LOCAL_UPLOADS_DIR = "/home/yakkl/uploads"
...
# custom default avatar image at /home/yakkl/local-static/default-avatar.png
...
#TERMS_OF_SERVICE = '/etc/yakkl/terms.md'

# Similarly if you want to set a Privacy Policy.
#PRIVACY_POLICY = '/etc/yakkl/privacy.md'


################
# Twitter integration.

# Yakkl supports showing inline Tweet previews when a tweet is linked
# to in a message.  To support this, Yakkl must have access to the
...
#    and twitter_access_token_secret in /etc/yakkl/yakkl-secrets.conf.


################
# Email gateway integration.
#
# The Email gateway integration supports sending messages into Yakkl
# by sending an email.
# For details, see the documentation:
#   https://yakkl.readthedocs.io/en/latest/production/settings.html#email-gateway
...
#
# For detailed instructions, see the Yakkl documentation:
#   https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html#ldap

# The LDAP server to connect to.  Setting this enables Yakkl
...
# query LDAP.  If unset, Yakkl does an anonymous bind.
AUTH_LDAP_BIND_DN = ""

# Passwords and secrets are not stored in this file.  The password
# corresponding to AUTH_LDAP_BIND_DN goes in `/etc/yakkl/yakkl-secrets.conf`.
# In that file, set `auth_ldap_bind_password`.  For example:
#   auth_ldap_bind_password = abcd1234


########
# LDAP integration, part 2: Mapping user info from LDAP to Yakkl.
#
# For detailed instructions, see the Yakkl documentation:
#   https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html#ldap
...
# filter, the string `%(user)s` is a Python placeholder.  The Yakkl
# server will replace this with the user's Yakkl username, i.e. the
# name they type into the Yakkl login form.
#
...
# This map defines how to populate attributes of a Yakkl user from LDAP.
#
# The format is `yakkl_name: ldap_name`; each entry maps a Yakkl
...
AUTH_LDAP_USER_ATTR_MAP = {
    ...
    # Yakkl will combine those to construct a full_name automatically.
    "full_name": "cn",
    # "first_name": "fn",
    # "last_name": "ln",
    ...
    # This line is for having Yakkl to automatically deactivate users
...
#
# By default, Yakkl connects to rabbitmq running locally on the machine,
# but Yakkl also supports connecting to RabbitMQ over the network;
# to use a remote RabbitMQ instance, set RABBITMQ_HOST to the hostname here.
# RABBITMQ_HOST = "127.0.0.1"
# To use another rabbitmq user than the default 'yakkl', set RABBITMQ_USERNAME here.
# RABBITMQ_USERNAME = 'yakkl'

# Memcached configuration
#
# By default, Yakkl connects to memcached running locally on the machine,
# but Yakkl also supports connecting to memcached over the network;
...
# Redis configuration
#
# By default, Yakkl connects to redis running locally on the machine,
# but Yakkl also supports connecting to redis over the network;
# to use a remote Redis instance, set REDIS_HOST here.
# REDIS_HOST = '127.0.0.1'
# For a different redis port set the REDIS_PORT here.
# REDIS_PORT = 6379
# If you set redis_password in yakkl-secrets.conf, Yakkl will use that password
# to connect to the redis server.

# Controls whether Yakkl will rate-limit user requests.
# RATE_LIMITING = True

# By default, Yakkl connects to the thumbor (the thumbnailing software
...
# THUMBOR_URL here.  Setting THUMBOR_URL='' will let Yakkl server know that
# thumbor is not running or configured.
#THUMBOR_URL = 'http://127.0.0.1:9995'
#
# This setting controls whether images shown in Yakkl's inline image
