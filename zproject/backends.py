# Documentation for Yakkl's authentication backends is split across a few places:
#
# * https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html and
#   zproject/prod_settings_template.py have user-level configuration documentation.
# * https://yakkl.readthedocs.io/en/latest/subsystems/auth.html has developer-level
#   documentation, especially on testing authentication backends in the Yakkl
...
# This first batch of methods is used by other code in Yakkl to check
...
class YakklDummyBackend(YakklAuthMixin):
...
class EmailAuthBackend(YakklAuthMixin):
    """
...
class YakklRemoteUserBackend(RemoteUserBackend):
    ...
    that has an Apache REMOTE_USER integration.  For manual testing, see
      ...
      https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html
...
class YakklLDAPException(_LDAPUser.AuthenticationFailed):
    """Since this inherits from _LDAPUser.AuthenticationFailed, these will
    be caught and logged at debug level inside django-auth-ldap's authenticate()"""
    pass

class YakklLDAPExceptionOutsideDomain(YakklLDAPException):
    pass

class YakklLDAPConfigurationError(Exception):
    pass

LDAP_USER_ACCOUNT_CONTROL_DISABLED_MASK = 2

class YakklLDAPAuthBackendBase(YakklAuthMixin, LDAPBackend):
    """Common code between LDAP authentication (YakklLDAPAuthBackend) and
    using LDAP just to sync user data (YakklLDAPUserPopulator).
        ...
        if settings.LDAP_APPEND_DOMAIN:
            if is_valid_email(username):
                if not username.endswith("@" + settings.LDAP_APPEND_DOMAIN):
                    raise YakklLDAPExceptionOutsideDomain("Email %s does not match LDAP domain %s." % (
    ...
    def get_mapped_name(cls, ldap_user: _LDAPUser) -> Tuple[str, str]:
        """Constructs the user's Yakkl full_name and short_name fields from
        the LDAP data"""
        if "full_name" in settings.AUTH_LDAP_USER_ATTR_MAP:
            ...
            raise YakklLDAPException("Missing required mapping for user's full name")
    ...
    def sync_full_name_from_ldap(self, user_profile: UserProfile,
        ...
        if full_name != user_profile.full_name:
            try:
                ...
                raise YakklLDAPException(e.msg)
    ...
    def sync_custom_profile_fields_from_ldap(self, user_profile: UserProfile,
        ...
        for var_name, value in values_by_var_name.items():
            try:
                ...
                raise YakklLDAPException('Custom profile field with name %s not found.' % (var_name,))
            if existing_values.get(var_name) == value:
                continue
            result = validate_user_custom_profile_field(user_profile.realm.id, field, value)
            if result is not None:
                raise YakklLDAPException('Invalid data for %s field: %s' % (var_name, result))
            profile_data.append({
                'id': field.id,
                'value': value,
            })
    ...
    def get_or_build_user(self, username: str,
        ...
        """This is used only in non-authentication contexts such as:
           ...
           In authentication contexts, this is overriden in YakklLDAPAuthBackend.
        """
...
class YakklLDAPAuthBackend(YakklLDAPAuthBackendBase):
    ...
    def authenticate(self, *, username: str, password: str, realm: Realm,
        ...
        except YakklLDAPExceptionOutsideDomain:
            if return_data is not None:
        ...
        return YakklLDAPAuthBackendBase.authenticate(self,
                                                     request=None,
                                                     username=username,
                                                     password=password)
    ...
    def get_or_build_user(self, username: str, ldap_user: _LDAPUser) -> Tuple[UserProfile, bool]:
        ...
        account in this Yakkl realm.  If so, it will log them in.

        Otherwise, to provide a seamless Single Sign-On experience
        with LDAP, this function can automatically create a new Yakkl
        ...
        if settings.LDAP_EMAIL_ATTR is not None:
            ...
            if settings.LDAP_EMAIL_ATTR not in ldap_user.attrs:
                ...
                raise YakklLDAPException("LDAP user doesn't have the needed %s attribute" % (
        ...
        if 'userAccountControl' in settings.AUTH_LDAP_USER_ATTR_MAP:  # nocoverage
            ...
            if ldap_disabled:
                # Treat disabled users as deactivated in Yakkl.
                return_data["inactive_user"] = True
                raise YakklLDAPException("User has been deactivated")
        ...
        if return_data.get("inactive_realm"):
            ...
            raise YakklLDAPException("Realm has been deactivated")
        ...
        try:
            ...
            raise YakklLDAPException("This email domain isn't allowed in this organization.")
        except (DisposableEmailError, EmailContainsPlusError):
            raise YakklLDAPException("Email validation failed.")
...
class YakklLDAPUserPopulator(YakklLDAPAuthBackendBase):
    """Just like YakklLDAPAuthBackend, but doesn't let you log in.  Used
...
class DevAuthBackend(YakklAuthMixin):
    """Allow logging in as any user without a password.  This is used for
    convenience when developing Yakkl, and is disabled in production."""
...
def social_associate_user_helper(backend: BaseAuth, return_data: Dict[str, Any],
    ...
    """Responsible for doing the Yakkl-account lookup and validation parts
    of the Yakkl Social auth pipeline (similar to the authenticate()
...
def social_auth_finish(backend: Any,
    ...
    if mobile_flow_otp is not None:
        ...
        # will redirect to a special yakkl:// URL that is handled by
    ...
    # subdomain.yakkl.example.com, we would just call
    ...
    # auth.yakkl.example.com (depending on
    # settings.SOCIAL_AUTH_SUBDOMAIN), which cannot store cookies on
    # the subdomain.yakkl.example.com domain.  So instead we serve a
    # redirect (encoding the authentication result data in a
    # cryptographically signed token) to a route on
    # subdomain.yakkl.example.com that will verify the signature and
...
class SocialAuthMixin(YakklAuthMixin):
...
class GitHubAuthBackend(SocialAuthMixin, GithubOAuth2):
    ...
    def user_data(self, access_token: str, *args: Any, **kwargs: Any) -> Dict[str, str]:
        ...
        social auth backends into a single Yakkl backend for GitHub Oauth2"""
...
AUTH_BACKEND_NAME_MAP = {
    ...
    'LDAP': YakklLDAPAuthBackend,
    'RemoteUser': YakklRemoteUserBackend,
}  # type: Dict[str, Any]
