...
casper.then(function () {
    casper.test.assertExists('#user-checkboxes [data-email="cordelia@yakkl.com"]', 'Original user list contains Cordelia');
    casper.test.assertExists('#user-checkboxes [data-email="othello@yakkl.com"]', 'Original user list contains Othello');
});
...
casper.waitUntilVisible("#user-checkboxes", function () {
    casper.test.assertEquals(casper.visible('#user-checkboxes [data-email="cordelia@yakkl.com"]'),
                             false,
                             "Cordelia is not visible");
    casper.test.assertEquals(casper.visible('#user-checkboxes [data-email="othello@yakkl.com"]'),
                             true,
                             "Othello is visible");
...
casper.then(function () {
    ...
    casper.wait(100, function () {
        casper.test.assert(casper.evaluate(function () {
            return !$('#user-checkboxes [value="cordelia@yakkl.com"]')[0].checked;
        }), "Cordelia is unchecked");
        casper.test.assert(casper.evaluate(function () {
            return $('#user-checkboxes [value="othello@yakkl.com"]')[0].checked;
        }), "Othello is checked");
    });
});
casper.then(function () {
...
casper.then(function () {
    casper.waitUntilVisible('#stream_creation_form', function () {
        ...
        casper.click('input[value="cordelia@yakkl.com"] ~ span');
        casper.click('input[value="othello@yakkl.com"] ~ span');
