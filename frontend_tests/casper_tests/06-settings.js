...
var regex_yakklrc = /^data:application\/octet-stream;charset=utf-8,\[api\]\nemail=.+\nkey=.+\nsite=.+\n$/;
var regex_outgoing_webhook_yakklrc = /^data:application\/octet-stream;charset=utf-8,\[api\]\nemail=.+\nkey=.+\nsite=.+\ntoken=.+\n$/;
...
casper.then(function () {
    casper.waitUntilVisible('#show_api_key_box', function () {
        casper.test.assertExists('#download_yakklrc', '~/.yakklrc button exists');
        casper.click('#download_yakklrc');
    });
});

casper.then(function () {
    casper.waitUntilVisible('#download_yakklrc[href^="data:application"]', function () {
        casper.test.assertMatch(
            decodeURIComponent(casper.getElementsAttribute('#download_yakklrc', 'href')),
            regex_yakklrc,
            'Looks like a yakklrc file');
    });
});
...
casper.then(function () {
    casper.waitUntilVisible(button_sel + '[href^="data:application"]', function () {
        casper.test.assertMatch(
            ...
            regex_outgoing_webhook_yakklrc,
            'Looks like an outgoing webhook bot ~/.yakklrc file');
    });
});
