...
const _page_params = {
    realm_bots: [{email: 'bot0@yakkl.com', user_id: 42, full_name: 'Bot 0'},
                 {email: 'outgoingwebhook@yakkl.com', user_id: 314, full_name: "Outgoing webhook",
...
run_test('test_basics', () => {
    ...
    (function test_owner_can_admin() {
        ...
        bot_data.add({owner: 'owner@yakkl.com', ...test_bot});

        bot = bot_data.get(43);
        assert(bot.can_admin);

        bot_data.add({owner: 'notowner@yakkl.com', ...test_bot});
    ...
    (function test_get_editable() {
        ...
        bot_data.add({...test_bot, user_id: 44, owner: 'owner@yakkl.com', is_active: true});
        bot_data.add({...test_bot, user_id: 45, email: 'bot2@yakkl.com', owner: 'owner@yakkl.com', is_active: true});
        bot_data.add({...test_bot, user_id: 46, email: 'bot3@yakkl.com', owner: 'not_owner@yakkl.com', is_active: true});

        can_admin = _.pluck(bot_data.get_editable(), 'email');
        assert.deepEqual(['bot1@yakkl.com', 'bot2@yakkl.com'], can_admin);
    ...
    (function test_get_all_bots_for_current_user() {
        ...
        assert.equal(bots[0].email, 'bot1@yakkl.com');
        assert.equal(bots[1].email, 'bot2@yakkl.com');
    }());

    (function test_get_bot_owner_email() {
        let bot_owner_email = bot_data.get_bot_owner_email(test_embedded_bot.user_id);
        assert.equal('cordelia@yakkl.com', bot_owner_email);
