...
run_test('validate', () => {
    ...
    initialize_pm_pill();
    add_content_to_compose_box();
    compose_state.recipient('foo@yakkl.com');

    assert(!compose.validate());

    assert.equal($('#compose-error-msg').html(), i18n.t('Please specify at least one valid recipient', {}));

    compose_state.recipient('foo@yakkl.com,alice@yakkl.com');
    assert(!compose.validate());
...
run_test('finish', () => {
    ...
    (function test_when_compose_validation_succeed() {
        ...
        $(document).trigger = function (e) {
            assert.equal(e.name, 'compose_finished.yakkl');
...
run_test('on_events', () => {
    (function test_usermention_completed_yakkl_triggered() {
        var handler = $(document).get_on_handler('usermention_completed.yakkl');
    ...
    (function test_stream_name_completed_triggered() {
        var handler = $(document).get_on_handler('streamname_completed.yakkl');
    ...
    (function test_video_link_compose_clicked() {
        ...
        page_params.realm_google_hangouts_domain = 'yakkl';

        handler(ev);

        video_link_regex = /\[Click to join video call\]\(https:\/\/hangouts.google.com\/hangouts\/\_\/yakkl\/\d{15}\)/;
        assert(video_link_regex.test(syntax_to_insert));
