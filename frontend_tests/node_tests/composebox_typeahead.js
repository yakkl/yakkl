...
var deactivated_user = {
    email: 'other@yakkl.com',
    user_id: 103,
    full_name: "Deactivated User",
};
var lear = {
    email: 'lear@yakkl.com',
    user_id: 104,
    full_name: "King Lear",
};
...
run_test('content_typeahead_selected', () => {
    ...
    $('document-stub').trigger = function (event, params) {
        assert.equal(event, 'usermention_completed.yakkl');
    ...
    $('document-stub').trigger = function (event, params) {
        assert.equal(event, 'streamname_completed.yakkl');
...
run_test('initialize', () => {
    ...
    $('#private_message_recipient').typeahead = function (options) {
        ...
        expected_value = '        <img class="typeahead-image" src="https://secure.gravatar.com/avatar/md5-othello@yakkl.com?d&#x3D;identicon&amp;s&#x3D;50" />\n<strong>Othello, the Moor of Venice</strong>';
        assert.equal(actual_value, expected_value);

        options.query = 'Lear';
        actual_value = options.highlighter(cordelia);
        expected_value = '        <img class="typeahead-image" src="https://secure.gravatar.com/avatar/md5-cordelia@yakkl.com?d&#x3D;identicon&amp;s&#x3D;50" />\n<strong>Cordelia Lear</strong>';
        assert.equal(actual_value, expected_value);

        options.query = 'othello@yakkl.com, co';
        actual_value = options.highlighter(cordelia);
        expected_value = '        <img class="typeahead-image" src="https://secure.gravatar.com/avatar/md5-cordelia@yakkl.com?d&#x3D;identicon&amp;s&#x3D;50" />\n<strong>Cordelia Lear</strong>';
        assert.equal(actual_value, expected_value);
        ...
        options.query = 'othello@yakkl.com,     ';
        assert.equal(options.matcher(othello), false);
        assert.equal(options.matcher(cordelia), false);

        // options.query = 'othello@yakkl.com,, , cord';
        options.query = 'cord';
        assert.equal(options.matcher(othello), false);
        assert.equal(options.matcher(cordelia), true);
        ...
        // again.
        options.query = 'cordelia@yakkl.com, cord';
        assert.equal(options.matcher(othello), false);
        assert.equal(options.matcher(cordelia), false);
        ...
        // A literal match at the beginning of an element puts it at the top.
        options.query = 'co';  // Matches everything ("x@yakkl.com")
        ...
        options.query = 'othello@yakkl.com, cor';
    ...
    $('#compose-textarea').typeahead = function (options) {
        ...
        var hamburger = {
            email: 'coolham@yakkl.com',
            user_id: 200,
            full_name: "Hamburger",
        };
        var hammer = {
            email: 'hammer@yakkl.com',
            user_id: 202,
            full_name: "Apollo",
        };
        var zeus = {
    ...
    $('#private_message_recipient').blur = function (handler) {
        if (handler) {  // The blur handler is being set.
            this.val('othello@yakkl.com, ');
            handler.call(this);
            var actual_value = this.val();
            var expected_value = 'othello@yakkl.com';
            assert.equal(actual_value, expected_value);
...
run_test('begins_typeahead', () => {
    ...
    assert_typeahead_equals("~~~ @yakkl", all_mentions);
    assert_typeahead_equals("@yakkl :ta", emoji_list);
