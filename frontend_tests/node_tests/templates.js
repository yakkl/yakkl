...
run_test('admin_invites_list', () => {
    ...
    assert.equal(span.text(), "alice@yakkl.com");
});
...
run_test('admin_tab', () => {
    ...
    assert.equal($(html).find("input.admin-realm-name").val(), 'Yakkl');
});
...
run_test('bot_owner_select', () => {
    ...
    assert.equal(option.val(), "hamlet@yakkl.com");
        ...
        group_pms: [
            {
                ...
                emails: "alice@yakkl.com,bob@yakkl.com",
...
run_test('new_stream_users', () => {
    ...
    assert.equal(label.text().trim(), 'King Lear (lear@yakkl.com)');
});
...
run_test('revoke_invite_modal', () => {
    ...
    html += "</div>";
    assert.equal($(html).find("p strong").text(), "iago@yakkl.com");
});
...
run_test('stream_member_list_entry', () => {
    ...
    var html = render('stream_member_list_entry',
                      {name: "King Hamlet", email: "hamlet@yakkl.com"});
...
run_test('subscription_settings', () => {
    var sub = {
        ...
        email_address: 'xxxxxxxxxxxxxxx@yakkl.com',
        stream_id: 888,
        is_muted: false,
    };
...
run_test('user_info_popover_title', () => {
    var html = render('user_info_popover_title', {user_avatar: 'avatar/hamlet@yakkl.com'});

    html = '<div>' + html + '</div>';
    assert.equal($(html).find('.popover-avatar').css('background-image'), "url(avatar/hamlet@yakkl.com)");
});
...
run_test('user_profile_modal', () => {
    ...
    assert.equal(div.text().trim(), 'iago@yakkl.com');
