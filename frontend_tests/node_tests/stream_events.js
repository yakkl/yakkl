...
run_test('update_property', () => {
    ...
    stream_events.update_property(1, 'email_address', 'zooly@yakkl.com');
    assert.equal(frontend.email_address, 'zooly@yakkl.com');
...
run_test('marked_subscribed', () => {
    ...
    with_overrides(function (override) {
        override('stream_color.update_stream_color', noop);
        global.with_stub(function (stub) {
            $(document).on('subscription_add_done.yakkl', stub.f);
            stream_events.mark_subscribed(frontend, [], '');
...
run_test('mark_unsubscribed', () => {
    ...
    $(document).on('subscription_remove_done.yakkl', function () {
        removed_sub = true;
    });
    ...
    with_overrides(function (override) {
        ...
        $(document).trigger = function (ev) {
            assert.equal(ev.name, 'subscription_remove_done.yakkl');
