...
set_global('page_params', {
    ...
    realm_filters: [
        ...
        [
            "ZBUG_(?P<id>[0-9]{2,8})",
            "https://trac2.yakkl.net/ticket/%(id)s",
        ],
        [
            "ZGROUP_(?P<id>[0-9]{2,8}):(?P<zone>[0-9]{1,8})",
            "https://zone_%(zone)s.yakkl.net/ticket/%(id)s",
        ],
    ],
...
people.add({
    ...
    user_id: 103,
    email: 'bobby@yakkl.com',
});

people.add({
    full_name: 'Mark Twin',
    user_id: 104,
    email: 'twin1@yakkl.com',
});

people.add({
    full_name: 'Mark Twin',
    user_id: 105,
    email: 'twin2@yakkl.com',
});

people.add({
    full_name: 'Brother of Bobby|123',
    user_id: 106,
    email: 'bobby2@yakkl.com',
});
...
run_test('bugdown_detection', () => {
    var no_markup = [
        ...
        "Stream #**Verona**",
        "This contains !gravatar(leo@yakkl.com)",
        "And an avatar !avatar(leo@yakkl.com) is here",
    ];

    var markup = [
        "Contains a https://yakkl.com/image.png file",
        "Contains a https://yakkl.com/image.jpg file",
        "https://yakkl.com/image.jpg",
        "also https://yakkl.com/image.jpg",
        "https://yakkl.com/image.jpg too",
        "Contains a yakkl.com/foo.jpeg file",
        "Contains a https://yakkl.com/image.png file",
...
run_test('marked', () => {
    var test_cases = [
        ...
        {input: 'This is a realm filter #1234 with text after it',
         expected: '<p>This is a realm filter <a href="https://trac.yakkl.net/ticket/1234" target="_blank" title="https://trac.yakkl.net/ticket/1234">#1234</a> with text after it</p>'},
        ...
        {input: 'This is a realm filter with ZGROUP_123:45 groups',
         expected: '<p>This is a realm filter with <a href="https://zone_45.yakkl.net/ticket/123" target="_blank" title="https://zone_45.yakkl.net/ticket/123">ZGROUP_123:45</a> groups</p>'},
        {input: 'This is an !avatar(cordelia@yakkl.com) of Cordelia Lear',
         expected: '<p>This is an <img alt="cordelia@yakkl.com" class="message_body_gravatar" src="/avatar/cordelia@yakkl.com?s=30" title="cordelia@yakkl.com"> of Cordelia Lear</p>'},
        {input: 'This is a !gravatar(cordelia@yakkl.com) of Cordelia Lear',
         expected: '<p>This is a <img alt="cordelia@yakkl.com" class="message_body_gravatar" src="/avatar/cordelia@yakkl.com?s=30" title="cordelia@yakkl.com"> of Cordelia Lear</p>'},
        {input: 'Test *italic*',
        ...
        // Test HTML Escape in Custom Yakkl Rules
...
run_test('topic_links', () => {
    ...
    assert.equal(util.get_topic_links(message)[0], "https://trac.yakkl.net/ticket/123");

    message = {type: 'stream', topic: "Two #123 #456 link here"};
    markdown.add_topic_links(message);
    assert.equal(util.get_topic_links(message).length, 2);
    assert.equal(util.get_topic_links(message)[0], "https://trac.yakkl.net/ticket/123");
    assert.equal(util.get_topic_links(message)[1], "https://trac.yakkl.net/ticket/456");

    message = {type: 'stream', topic: "New ZBUG_123 link here"};
    markdown.add_topic_links(message);
    assert.equal(util.get_topic_links(message).length, 1);
    assert.equal(util.get_topic_links(message)[0], "https://trac2.yakkl.net/ticket/123");

    message = {type: 'stream', topic: "New ZBUG_123 with #456 link here"};
    markdown.add_topic_links(message);
    assert.equal(util.get_topic_links(message).length, 2);
    assert(util.get_topic_links(message).indexOf("https://trac2.yakkl.net/ticket/123") !== -1);
    assert(util.get_topic_links(message).indexOf("https://trac.yakkl.net/ticket/456") !== -1);

    message = {type: 'stream', topic: "One ZGROUP_123:45 link here"};
    markdown.add_topic_links(message);
    assert.equal(util.get_topic_links(message).length, 1);
    assert.equal(util.get_topic_links(message)[0], "https://zone_45.yakkl.net/ticket/123");
...
run_test('python_to_js_filter', () => {
    ...
    var actual_value = marked.InlineLexer.rules.yakkl.realm_filters;
    ...
    actual_value = marked.InlineLexer.rules.yakkl.realm_filters;
