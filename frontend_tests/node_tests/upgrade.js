...
run_test("initialize", () => {
    ...
    const open_func = (config_opts) => {
        assert.equal(config_opts.name, "Yakkl");
        ...
        assert.equal(config_opts.description, "Yakkl Cloud Standard");
        token_func("stripe_add_card_token");
    };

    StripeCheckout.configure = (config_opts) => {
        assert.equal(config_opts.image, '/static/images/logo/yakkl-icon-128x128.png');
