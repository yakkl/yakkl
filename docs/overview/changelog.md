...
All notable changes to the Yakkl server are documented in this file.
...
- Significantly improved the email->Yakkl gateway, and added [nice
  setup documentation](../production/email-gateway.html).  It now
  should be possible to subscribe a Yakkl stream to an email list and
...
- The defaults for Yakkl's now beta inline URL preview setting have changed.
...
websites](https://yakkl.com/help/allow-image-link-previews) will
...
- We rewrote the Google Authentication backend to use the
  ...
  release:
    * In `/etc/yakkl/yakkl-secrets.conf`, `google_oauth2_client_secret`
      is now called with `social_auth_google_secret`.
    * In `/etc/yakkl/settings.py`, `GOOGLE_OAUTH2_CLIENT_ID` should be
      replaced with `SOCIAL_AUTH_GOOGLE_KEY`.
    * In `/etc/yakkl/settings.py`, `GoogleMobileOauth2Backend` should
      be replaced with called `GoogleAuthBackend`.
- The Yakkl web and desktop apps have been converted to directly count
  ...
  happened is the user never read these messages, and the Yakkl webapp
...
- Added proper open graph tags for linking to a Yakkl organization.
...
- Redesigned Yakkl's missed-message emails to use a clean, more
  readable style.
...
- Significantly improved the visual spacing around bulleted lists,
  blockquotes, and code blocks in Yakkl's message feed.
...
- Fixed incorrect caching settings for the Yakkl API, which could result
...
- Fixed HTML styling when copy-pasting content out of Yakkl's night theme.
...
- Rewrote the Google authentication backend to use python-social-auth,
  removing Yakkl's original 2013-era SSO authentication backend.
...
- Renamed the system bot Yakkl realm to "yakklinternal" (was "yakkl").
...
- Made uwsgi buffer size configurable (relevant for sites putting
  Yakkl behind a proxy that adds many HTTP headers).
- Fixed loss of LaTeX syntax inside quote-and-reply.
- Fixed virtualenv-related bug when upgrading Yakkl when the system
...
- Yakkl's "Guest users" feature is no longer experimental.
...
- Deprecated support for Ubuntu Trusty.  Yakkl 2.0.x will continue to
  support Ubuntu Trusty, but Yakkl 2.1.0 will remove support for
  installing on Trusty.

**Upgrade notes:**
- This release adds support for [submitting basic usage statistics] to
help the Yakkl core team. This feature can be enabled only if a server
...
`SUBMIT_USAGE_STATISTICS = False` in `/etc/yakkl/settings.py`.
...
- Added support for copying avatar and other profile data when
  creating a second account on a Yakkl server with a given email address.
...
- Yakkl's Help Center now has nicely generated open graph tags.
...
- Fixed several bugs with importing data into Yakkl's S3 backend.
...
- Countless behind-the-scenes improvements to Yakkl's codebase,
  tooling, automated tests, error handling, and APIs.

### 1.9.2 -- 2019-01-29

This release migrates Yakkl off a deprecated Google+ API (necessary
...
Yakkl installations; it has minimal changes for existing servers.
...
- Support for Ubuntu bionic and Debian stretch (our first non-Ubuntu
  ...
  Yakkl server on Ubuntu Trusty in the coming months, in preparation
  for Trusty’s end-of-life in April 2019.
- New data import tools for HipChat and Gitter.  The Slack importer
  is now out of beta.
- Yakkl Python process startup time is about 30% faster; this effort
...
- You can now configure custom (organization-specific) fields for user
  profiles; Yakkl can now serve as your organization’s employee
  directory.
- Yakkl now supports using Google Hangouts instead of Jitsi as the
  video chat provider.
...
- New [stream settings](https://yakkl.com/help/stream-permissions)
...
- Yakkl now has experimental support for guest users (intended
...
- New native integrations for Ansible Tower, Appveyor, Clubhouse,
  Netlify, and Zabbix; Yakkl now has over 100 native integrations (in
  addition to hundreds more available via Zapier and IFTTT).
- New translations for Ukrainian, Portuguese, Indonesian, Dutch, and
  Finnish.  Yakkl now has complete or nearly-complete translations
...
* Yakkl 1.9 contains a significant database migration that can take
...
- Added the new `SOCIAL_AUTH_SUBDOMAIN` setting, which all servers using
  both GitHub authentication and hosting multiple Yakkl organizations
...
- Optimized the performance of loading Yakkl in an organization with
...
- Fixed several installer issues to make it easier to Dockerize Yakkl.
...
- Yakkl's data export system has full support for all features, and
...
- Improved Yakkl's layout for windows with a width around 1024px.
- Improved Yakkl's generic error handling behavior for webhooks.
...
- Emails and several other onboarding strings are now tagged for
  translation.
- Optimized the performance of importing Yakkl by about 30%.  This
  significantly decreases the load spike when restarting a Yakkl server.
...
- Dramatically simplified the server installation process; it's now possible
  to install Yakkl without first setting up outgoing email.
...
- The security model for private streams has changed.  Now
  ...
  rename private streams they are not subscribed to.  See Yakkl's
...
- Added a user setting to choose the emoji set used in Yakkl: Google,
...
- Improved syntax for permanent links to streams in Yakkl.
...
- Removed the legacy "Yakkl labs" autoscroll_forever setting.  It was
...
- Added a new nagios check for the Yakkl analytics state.
...
since 1.7.1.  All Yakkl server admins are encouraged to upgrade
promptly.
...
This is a security release, with a handful of cherry-picked changes
since 1.7.0.  All Yakkl server admins are encouraged to upgrade
promptly.
...
- We fixed a bug in the script that runs after upgrading Yakkl (so
...
Web
- We’ve completely redesigned our onboarding process to explain Yakkl,
...
- We’ve built a beautiful new emoji picker with categories, a
  ...
  display!
- The emails sent by Yakkl are more consistent, readable, and visually
  interesting.
...
Backend and scaling
- Yakkl now runs exclusively on Python 3.  This is the culmination of
...
- Yakkl's performance at scale has improved significantly. Performance
  now scales primarily with number of active users (not total
  users). As an example, yakkl.com serves 400 monthly active
  users and about 3500 total users, on one VM with just 8GB of RAM and
  a CPU consistently over 90% idle.

**Upgrade notes:**

* Yakkl 1.7 contains some significant database migrations that can
...
* We've removed support for an uncommon legacy deployment model where
  a Yakkl server served multiple organizations on the same domain.
  Installs with multiple organizations now require each organization
  to have its own subdomain.

  This change should have no effect for the vast majority of Yakkl
...
* We simplified the configuration for our password strength checker to
  ...
  [it has been replaced](https://github.com/yakkl/yakkl/commit/a116303604e362796afa54b5d923ea5312b2ea23) by
  the more intuitive `PASSWORD_MIN_GUESSES`.

**Full feature changelog:**

- Simplified the process for installing a new Yakkl server, as well as
...
- Added a new "incoming webhook" bot type, limited to only sending
  messages into Yakkl, for better security.
...
- Added support for installing a Yakkl server from a Git checkout.
...
- Fixed most accessibility errors detected by major accessibility
  checker tools.
- Extracted Yakkl's Python API and bots ecosystem into its own
  repository, yakkl/python-yakkl-api.
...
- Redesigned Yakkl's error pages to feature cute illustrations.
...
- Restructured Yakkl's management commands to use a common system for
...
- Fixed caching of source repository in upgrade-yakkl-from-git.
...
- Worked around Google Compute Engine's default boto configuration,
  which broke Yakkl (and any other app using boto).
- Yakkl now will gracefully handle the Postgres server being restarted.
...
- A complete redesign of the Yakkl settings interfaces to look a lot
...
[electron-app]: https://github.com/yakkl/yakkl-desktop/releases
[ios-app]: https://itunes.apple.com/us/app/yakkl/id1203036395
...
* Added email notifications on new Yakkl logins.
...
* Fixed regressions in Yakkl's browser state preservation on reload logic.
...
* Cleaned up Yakkl's supervisord configuration.  A side effect is the
...
- Added UI for deactivating your own Yakkl account.
- Added support for viewing the raw markdown content of a message.
- Added support for deploying Yakkl with subdomains for each realm.
...
- Normalized the message formatting for all the Yakkl Git integrations.
- Added support for VMWare Fusion Vagrant provider for faster OSX
  development.
- Added a shields.io style badge for joining a Yakkl server.
...
- Added support for removing one's own Yakkl account.
...
- Added new organization type concept.  This will be used to control
  whether Yakkl is optimized around protecting user privacy
...
- Added setting to hide private message content in desktop
  notifications.
- Added buttons to download .yakklrc files.
...
- Upgraded all Python dependencies to modern versions, including
  Django 1.10 (all of Yakkl's patches have been merged into mainline).
...
- Optimized the speed of the Yakkl upgrade process, especially with Git.
- Have peer_add events send user_id, not email.
- Fixed problems with rabbitmq when installing Yakkl.
...
- Fixed several real-time sync bugs, and removed several AJAX calls.
  Yakkl should be more performant than ever before.
- Fixed Yakkl Tornado service not working with http_proxy set in environment.
...
- Fixed duplicate notifications with multiple open Yakkl tabs.
...
- Upgraded Django to version 1.8.15 (with the Yakkl patches applied),
  fixing a CSRF vulnerability in Django (see
  https://www.djangoproject.com/weblog/2016/sep/26/security-releases/),
  and a number of other Django bugs from past Django stable releases
  that largely affects parts of Django that are not used by Yakkl.
...
- Migrated Yakkl's python dependencies to be installed via a virtualenv,
  instead of the via apt.  This is a major change to how Yakkl
...
- Added TERMS_OF_SERVICE setting using markdown formatting to configure
  the terms of service for a Yakkl server.
...
- Added scripts/upgrade-yakkl-from-git for upgrading Yakkl from a Git repo.
- Added preliminary support for Python 3.  All of Yakkl's test suites now
...
- Fixed yakkl-puppet-apply exit code when puppet throws errors.
...
- Fixed escaping of Yakkl extensions to markdown.
...
- Eliminated all remaining settings hardcoded for yakkl.com.
...
- Updated header for Yakkl static assets to reflect Yakkl being
  open source.

### 1.3.13 - 2016-06-21
- Added nearly complete internationalization of the Yakkl UI.
...
- Added mypy static type annotations to about 85% of the Yakkl Python codebase.
- Added automated test of backend templates to test for regressions.
- Added lots of detailed documentation on the Yakkl development environment.
- Added setting allowing only administrators to create new streams.
- Added button to exit the Yakkl tutorial early.
- Added web UI for configuring default streams.
- Added new OPEN_REALM_CREATION setting (default off), providing a UI
  for creating additional realms on a Yakkl server.
...
- Fixed netcat issue causing Yakkl installation to hang on Scaleway machines.
...
- Fixed LDAP auth backend not working with Yakkl mobile apps.
- Fixed erroneous WWW-Authenticate headers with expired sessions.
- Changed "coworkers" to "users" in the Yakkl UI.
- Changed add_default_stream REST API to correctly use PUT rather than PATCH.
- Updated the Yakkl emoji set (the Android Emoji) to a modern version.
- Made numerous small improvements to the Yakkl development experience.
...
### 1.3.11 - 2016-05-02
- Moved email digest support into the default Yakkl production configuration.
...
- Added documentation on using Hubot to integrate with useful services
  not yet integrated with Yakkl directly (e.g. Google Hangouts).
- Added new management command to test sending email from Yakkl.
...
- Refactored the Yakkl puppet modules to be more modular.
...
- Implemented running queue processors multithreaded in development,
  decreasing RAM requirements for a Yakkl development environment from
  ~1GB to ~300MB.
...
- Added support for running Yakkl development environment in Docker.
...
- Fixed support for disabling the "yakkl" notifications stream.
...
- Added partial support for translating the Yakkl UI.
...
- Fixed security issue where recent history on private streams might
  be visible to new users (to the Yakkl team) who were invited with that
  private stream as one of their initial streams
  (https://github.com/yakkl/yakkl/issues/230).
