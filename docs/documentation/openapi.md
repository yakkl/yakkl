...
Yakkl uses the Swagger spec to generate an API reference from the
`yakkl.yaml` file. This page is a basic introduction to the format of
...
for how Yakkl's API documentation will work.

## Working with the `yakkl.yaml` file
...
These objects, at the top of `yakkl.yaml`, identify the API, define
...
`/static/yaml/yakkl.yaml`.

## Yakkl Swagger YAML style:
