...
This document explains the system for documenting [Yakkl's REST
API](https://yakkl.com/api/rest).  This documentation is an
essential resource both for users and the developers of Yakkl's mobile
and terminal apps. We carefully designed a system for both displaying
it and helping ensure it stays up to date as Yakkl's API changes.
...
* The text for the Python examples comes from a test suite for the
  ...
  This structure ensures that Yakkl's API documentation is robust to a
...
[here](https://yakkl.com/api/render-message) or in the development
...
`description` field in `yakkl.yaml`; currently, the `description`
section in `yakkl.yaml` is not used for anything.
...
and structure of [Yakkl's existing JavaScript examples][javascript-examples].

For the Python examples, you'll write the example in
`zerver/lib/api_test_helpers.py`, and it'll be run and verified
automatically in Yakkl's automated test suite.  The code there will
...
`zerver/openapi/yakkl.yaml`.  To register a function correctly:
...
You will still want to manually test the example using Yakkl's Python
...
URL definition in `zerver/openapi/yakkl.yaml`, and that URL definition
...
defined in the OpenAPI `yakkl.yaml` for a given endpoint and status
code:
...
1. Start by adding [OpenAPI format](../documentation/openapi.html)
   data to `zerver/openapi/yakkl.yaml` for the endpoint.  If you
   ...
   are specified in Yakkl backend endpoints, read our [REST API
   ...
   * `tools/check-swagger` will verify the syntax of `zerver/openapi/yakkl.yaml`.
...
1. Make the desired API call inside the function. If our Python
   ...
   function to the [yakkl PyPI
   package](https://github.com/yakkl/python-yakkl-api/tree/master/yakkl).
...
[javascript-examples]: https://github.com/yakkl/yakkl-js/tree/master/examples
...
* It is extremely common for API documentation to become out of date
  ...
  possible for Yakkl to maintain accurate documentation without a lot
  of manual management.
* Every Yakkl server can host correct API documentation for its
  version, with the key variables (like the Yakkl server URL) already
