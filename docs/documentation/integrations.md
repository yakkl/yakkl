...
Every Yakkl integration must be documented in
...
* Add text explaining all of the steps required to setup the
  ...
  Yakkl's pre-defined Markdown macros can be used for some of these steps.
...
[main Yakkl server repository](https://github.com/yakkl/yakkl). If you find
...
Here are a few common macros used to document Yakkl's integrations:
...
* `{!create-stream.md!}` macro - Recommends that users create a dedicated
  ...
  [the docs for Yakkl's GitHub integration][GitHub].
...
* `{!create-bot-construct-url.md!}` macro - Instructs users to create a bot
  ...
  rendering, see **Step 2** of [the docs for Yakkl's GitHub integration][GitHub].
...
* `{!append-stream-name.md!}` macro - Recommends appending `&stream=stream_name`
  ...
  Supplying a stream name is optional for most Yakkl integrations. If you use
...
* `{!append-topic.md!}` macro - Recommends appending `&topic=my_topic` to a URL
  ...
  topic is optional for most Yakkl integrations. If you use
  `{!create-bot-construct-url.md!}`, this macro need not be used.
...
* `{!download-python-bindings.md!}` macro - Links to Yakkl's
  [API page](https://yakkl.com/api/) to download and install Yakkl's
  ...
  [the docs for Yakkl's Codebase integration][codebase].

* `{!change-yakkl-config-file.md!}` macro - Instructs users to create a bot and
...
* `{!yakkl-config.md!}` - Used internally by `{!change-yakkl-config-file.md!}`
...
* `{!webhook-url-with-bot-email.md!}` - Used in certain non-webhook integrations
  to generate URLs of the form:
    ...
    ```
    https://bot_email:bot_api_key@yourYakklDomain.yakkl.com/api/v1/external/beanstalk
    ```

    For an example rendering, see
    [Yakkl's Beanstalk integration](https://yakkl.com/integrations/doc/beanstalk).

[GitHub]: https://yakkl.com/integrations/doc/github
[codebase]: https://yakkl.com/integrations/doc/codebase
[beanstalk]: https://yakkl.com/integrations/doc/beanstalk
[1]: https://github.com/yakkl/yakkl/blob/708f3a4bb19c8e823c9ea1e577d360ac4229b199/zerver/lib/integrations.py#L78
...
[basecamp]: https://yakkl.com/integrations/doc/basecamp
...
integration, for example, "Get `webhook name` notifications in Yakkl!"
