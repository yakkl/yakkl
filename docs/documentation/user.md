...
Our goal is for Yakkl to have complete, high-quality
documentation about Yakkl's features and how to perform certain tasks, such
...
* Feature discovery, for someone browsing the `/help` page, and looking at
  the set of titles.
* Public documentation of our featureset, for someone googling "can yakkl do .."
* Canned responses to support questions; if someone emails a yakkl admin
  asking "how do I change my name", they can reply with a link to the doc.
* Feature explanations for new Yakkl users and admins, especially for
...
The user documentation is available under `/help/` on any Yakkl server;
(e.g. <https://yakkl.com/help/> or `http://localhost:9991/help/` in
the Yakkl development environment). The user documentation is not hosted on
ReadTheDocs, since Yakkl supports running a server completely disconnected
...
[main Yakkl server repository](https://github.com/yakkl/yakkl). The file
...
This means that you can contribute to the Yakkl user documentation by just
adding to or editing the collection of markdown files under
`templates/zerver/help`.  If you have the Yakkl development environment
...
* Fewer words is better than more. Many Yakkl users have English as a second
  language.

* Try to put yourself in the shoes of a new Yakkl user. What would you want
  to know?
...
When you refer to the features in the Yakkl UI, you should **bold** the
...
Do not use `we` to refer to Yakkl or its creators; e.g. "Yakkl also
allows .." rather than "we also allow ..". `You` is ok and used liberally.

## Features

Yakkl's Markdown processor allows you to include several special features in
...
* Code blocks allow you to highlight syntax, similar to Yakkl's own markdown.
* Anchor tags can be used to link to headers in other documents.
* [Images](#images) of Yakkl UI can be added to documentation.
* Inline [icons](#icons) used to refer to features in the Yakkl UI.
...
Yakkl browser window in a screenshot; instead, it should only show
...
You can refer to features in the Yakkl UI by referencing their names
...
[main Yakkl server repository](https://github.com/yakkl/yakkl).
