...
output, which will show any errors your Yakkl development server
encounters.

To manually query the Postgres database, run `psql yakkl` for an
interactive console.
...
Yakkl Django and Tornado servers whenever you save changes to Python
...
restart if it crashes, and `upgrade-yakkl` will take care of running
