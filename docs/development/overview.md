...
The Yakkl development environment can be installed on **macOS,
Windows, and Linux** (Debian or Ubuntu recommended). You'll need at least **2GB
of available RAM**.

Installing the Yakkl development environment requires downloading several hundred
...
Linux container (otherwise) inside which the Yakkl server and all
related services will run. Vagrant adds a bit of overhead to using the
Yakkl development server, but provides an isolated environment that is
easy to install, update, and uninstall. It has been well-tested and
performs well.

## Advanced setup (non-Vagrant)

Yakkl also supports a wide range of ways to install the Yakkl
...
a cloud server and install the Yakkl development environment for
remote use. See the [next section][self-install-remote] for details.

## Installing remotely

The Yakkl development environment works well on remote virtual
...
We recommend giving the Yakkl development environment its **own
...
Once you've installed the Yakkl development environment, you'll want
...
And if you've setup the Yakkl development environment on a remote
