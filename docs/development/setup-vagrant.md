...
Yakkl development environment on Windows, macOS, Ubuntu and Debian.

The recommended method for installing the Yakkl development environment is to use
...
or a Linux container (for Ubuntu) inside which the Yakkl server and
...
* [Step 2: Get Yakkl code](#step-2-get-yakkl-code)
...
**If you encounter errors installing the Yakkl development
...
help](https://yakkl.com/#narrow/stream/21-provision-help) in the
[Yakkl development community
server](../contributing/chat-yakkl-org.html) for real-time help or
[file an issue](https://github.com/yakkl/yakkl/issues).
...
* a copy of Yakkl's `vagrant` provisioning logs, available in
  `/var/log/provision.log` on your virtual machine

### Requirements

Installing the Yakkl development environment with Vagrant requires
...
Now you are ready for [Step 2: Get Yakkl Code.](#step-2-get-yakkl-code).
...
privileges** when working on Yakkl code, as not doing so will cause
...
The Yakkl code includes symbolic links (symlinks). By default, native Windows
symlinks are not enabled in either Git BASH or Cygwin, so you need to do a bit
of configuration. **You must do this before you clone the Yakkl code.**
...
If you see `true`, you are ready for [Step 2: Get Yakkl
Code.](#step-2-get-yakkl-code)
...
if you use it to clone a repository, which will break the Yakkl
...
### Step 2: Get Yakkl Code

1. In your browser, visit <https://github.com/yakkl/yakkl>
...
3. In Terminal/Git BASH,
   [clone your fork of the Yakkl repository](../git/cloning.html#step-1b-clone-to-your-machine) and
   [connect the Yakkl upstream repository](../git/cloning.html#step-1c-connect-your-fork-to-yakkl-upstream):

```
git clone --config pull.rebase git@github.com:YOURUSERNAME/yakkl.git
cd yakkl
git remote add -f upstream https://github.com/yakkl/yakkl.git
```

This will create a 'yakkl' directory and download the Yakkl code into it.
...
Change into the yakkl directory and tell vagrant to start the Yakkl
...
- configures this virtual machine/container for use with Yakkl,
- creates a shared directory mapping your clone of the Yakkl code inside the
  virtual machine/container at `~/yakkl`
- runs the `tools/provision` script inside the virtual machine/container, which
  downloads all required dependencies, sets up the python environment for
  the Yakkl development server, and initializes a default test
...
[#provision help](https://yakkl.com/#narrow/stream/21-provision-help)
...
content is served from /srv/yakkl
...
Now the Yakkl server should be running and accessible. Verify this by
navigating to <http://localhost:9991/> in the browser on your main machine.

You should see something like this:

![Image of Yakkl development environment](../images/yakkl-dev.png)

The Yakkl server will continue to run and send output to the terminal window.
When you navigate to Yakkl in your browser, check your terminal and you
...
cloned Yakkl. Use your favorite editor (Sublime, Atom, Vim, Emacs, Notepad++,
etc.).

When you save changes they will be synced automatically to the Yakkl
development environment on the virtual machine/container.

Each component of the Yakkl development server will automatically
...
Yakkl's whitespace rules are all enforced by linters, so be sure to
...
directory where you cloned Yakkl on your main machine.
...
If after rebasing onto a new version of the Yakkl server, you receive
new errors while starting the Yakkl server or running tests, this is
probably not because Yakkl's master branch is broken.  Instead, this
...
runs `tools/provision` from your Yakkl checkout inside the Vagrant
guest); this should complete in about a minute.

After provisioning, you'll want to
[(re)start the Yakkl development server](#step-3-start-the-development-environment).
...
`tools/custom_provision` in your Yakkl Git checkout; and place any
...
^C
KeyboardInterrupt
(yakkl-py3-venv) vagrant@ubuntu-bionic:/srv/yakkl$ exit
logout
...
When you're ready to work on Yakkl again, run `vagrant up` (no need to
...
`./tools/provision` (from `/srv/yakkl`) inside the Vagrant guest or
...
* Ask in [#provision help](https://yakkl.com/#narrow/stream/21-provision-help)
  in the [Yakkl development community server](../contributing/chat-yakkl-org.html).
* [File an issue](https://github.com/yakkl/yakkl/issues).
...
#### Vagrant guest doesn't show (yakkl-py3-venv) at start of prompt
...
```
ESC[94mYakkl development environment setup succeeded!ESC[0m
```
...
christie@win10 ~/yakkl
$ vboxmanage list vms
"yakkl_default_1462498139595_55484" {5a65199d-8afa-4265-b2f6-6b1f162f157d}

christie@win10 ~/yakkl
$ vboxmanage getextradata yakkl_default_1462498139595_55484 enumerate
Key: VBoxInternal2/SharedFoldersEnableSymlinksCreate/srv_yakkl, Value: 1
...
VBoxInternal2/SharedFoldersEnableSymlinksCreate/srv_yakkl, then enable
symbolic links by running this command in Terminal/Git BASH/Cygwin:

```
vboxmanage setextradata YOURVMNAME VBoxInternal2/SharedFoldersEnableSymlinksCreate/srv_yakkl 1
```
...
prompt.  You can debug interactively by using e.g. `cd yakkl &&
...
The yakkl provisioning system is generally highly reliable; the most common
...
```
Yakkl development environment setup succeeded!
(yakkl-py3-venv) vagrant@vagrant-base-trusty-amd64:~/yakkl$
```

If the `(yakkl-py3-venv)` part is missing, this is because your
installation failed the first time before the Yakkl virtualenv was
created.  You can fix this by just closing the shell and running
`vagrant ssh` again, or using `source /srv/yakkl-py3-venv/bin/activate`.

Finally, if you encounter any issues that weren't caused by your
Internet connection, please report them!  We try hard to keep Yakkl
...
gigabytes of RAM, which is the minimum Yakkl
...
It is okay to proceed and start the Yakkl server.
...
Vagrant assumes that this means the command failed!
 mount -o vers=3,udp 172.28.128.1:<yakkl_path> /srv/yakkl
 Stdout from the command:
 Stderr from the command:
 mount.nfs: mount to NFS server '172.28.128.1:<yakkl_path>' failed: RPC Error: Unable to receive
```
...
```
    ...
    default:   File "/srv/yakkl/scripts/lib/yakkl_tools.py", line 163, in run
    ...
    default: * Logs are here: yakkl/var/log/provision.log
    default:
...
implementation (Yakkl uses Vagrant's NFS feature for syncing files on
...
`~/.yakkl-vagrant-config` and add a line like this, replacing the URL
as appropriate:
...
Then create `~/.yakkl-vagrant-config` and add the following lines to
...
`HTTP_PROXY` and `HTTPS_PROXY` lines in `~/.yakkl-vagrant-config` and
then do a `vagrant reload`.

### Using a different port for Vagrant

You can also change the port on the host machine that Vagrant uses by
adding to your `~/.yakkl-vagrant-config` file.  E.g. if you set:
...
'~/.yakkl-vagrant-config' file as well. For example, if you set:
...
improve performance of the Yakkl development environment by allocating
more resources.

To do so, create a `~/.yakkl-vagrant-config` file containing the
following lines:
