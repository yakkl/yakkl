...
The Yakkl developer environment works well on remote virtual machines. This can
be a good alternative for those with poor network connectivity or who have
limited storage/memory on their local machines.

We recommend giving the Yakkl development environment its own virtual
machine, running Ubuntu 16.04 or
18.04, with at least 2GB of memory. If the Yakkl development
...
the Yakkl development environment.  If you have one already, continue
to the next section.

You can create a new user with sudo privileges by running the
following commands as root:
* You can create a `yakkldev` user by running the command `adduser
yakkldev`. Run through the prompts to assign a password and user
...
`usermod -aG sudo yakkldev`.
* Finally, you can switch to the user by running the command `su -
yakkldev` (or just login to that user using `ssh`).
...
If the Yakkl development environment will be the only thing running on
...
allows you to access Yakkl running in your development environment
...
This will start up the Yakkl server on port 9991. You can then
navigate to `http://<REMOTE_IP>:9991` and you should see something like
this screenshot of the Yakkl development environment:

![Image of Yakkl development
environment](../images/yakkl-dev.png)

The `--interface=''` option makes the Yakkl development environment
...
Now you can access Yakkl by navigating to `http://127.0.0.1:3000` in
...
* [Edit locally](#editing-locally): Clone Yakkl code to your computer and
  then use your favorite editor to make changes. When you want to see changes
  on your remote Yakkl development instance, sync with Git.
* [Edit remotely](#editing-remotely): Edit code directly on your remote
  Yakkl development instance using a [Web-based IDE](#web-based-ide) (recommended for
...
your fork of yakkl on your local computer.
...
2. Change directory to where you cloned Yakkl (e.g. `cd yakkl`).
...
2. Change to the yakkl directory (e.g., `cd yakkl`).
...
To setup Codeanywhere for Yakkl:
...
Facebook), you may need your Yakkl development to have a valid SSL
...
The following instructions assume you have a Yakkl Droplet working and
that the user is `yakkldev`; edit accordingly if the situation is
different.
...
1. First, get an SSL certificate; you can use
    ...
    mkdir -p /var/lib/yakkl/certbot-webroot/
    # if nginx running this will fail and you need to run `service nginx stop`
    /home/yakkldev/yakkl/scripts/setup/setup-certbot \
      hostname.example.com --no-yakkl-conf \
...
1. Start the Yakkl development environment with the following command:
   ```
