...
Yakkl, like many popular applications, is designed with
internationalization (i18n) in mind, which means users can fully use
the the Yakkl UI in their preferred language.

This article aims to teach Yakkl contributors enough about
internationalization and Yakkl's tools for it so that they can make
...
* Our goal is for **all end-user facing strings** in Yakkl to be
  ...
  exceptions: we don't tag strings in Yakkl's landing pages
...
* Translating all the strings in Yakkl for a language and maintaining
  that translation is a lot of work, and that work scales with the
  number of strings tagged for translation in Yakkl.  For this reason,
...
* This is more about string consistency in general, but we have a
  ...
  that check all strings tagged for translation in Yakkl.
...
i18n (e.g. Yakkl has a "Full Name" field rather than "First Name" and
...
most people working on Yakkl.

## Translation process

The end-to-end tooling process for translations in Yakkl is as follows.
...
2. Translation [resource][] files are created using the `./manage.py
   ...
   - It will only delete singular keys in the resource file when they
     are no longer used in Yakkl code.
...
4. Translators translate the strings in the Transifex UI.  (In theory,
   ...
   Yakkl, making changes to translations in Yakkl risks having the
...
Yakkl makes use of the [Jinja2][] templating system for the backend
...
All user-facing text in the Yakkl UI should be generated by an Jinja2 HTML
...
Yakkl expects all the error messages to be translatable as well.  To
...
Yakkl linter (`tools/lint`) checks for correct usage.
...
Handlebars [helpers][] that Yakkl registers.  The syntax for simple strings is:
...
The config file that maps the resources from Yakkl to Transifex is
