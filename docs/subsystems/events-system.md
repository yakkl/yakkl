...
Yakkl's "events system" is the server-to-client push system that
...
Any single-page web application like Yakkl needs a story for how
...
like Yakkl, since the state is constantly changing.  When we talk
about clients, think a browser tab, mobile app, or API bot that needs
to receive updates to the Yakkl data.  The simplest example is a new
message being sent by one client; other clients must be notified in
order to display the message.  But a complete application like Yakkl
...
avatar, settings changes, etc.  In Yakkl, we call these updates that
...
This document discusses how Yakkl solves the generation and delivery
problems in a scalable, correct, and predictable way.

## Generation system

Yakkl's generation system is built around a Python function,
...
Yakkl's event delivery (real-time push) system is based on Tornado,
...
Yakkl's event delivery system is based on "long-polling"; basically
...
[Yakkl Python API bindings][api-bindings-code] for a complete example
...
[api-bindings-code]: https://github.com/yakkl/python-yakkl-api/blob/master/yakkl/yakkl/__init__.py

The queue servers are a very high-traffic system, processing at a
minimum one request for every message delivered to every Yakkl client.
...
that the N clients for Yakkl don't need to worry about a wide range of
potential rare and hard to reproduce race conditions; we just have to
implement things correctly once in the Yakkl server.

This is quite challenging to do technically, because fetching the
initial state for a complex web application like Yakkl might involve
...
every event type that can be generated by Yakkl.  This can be tested
...
In the Yakkl webapp, the data returned by the `register` API is
available via the `page_params` parameter.

### Messages

One exception to the protocol described in the last section is the
actual messages.  Because Yakkl clients usually fetch them in a
