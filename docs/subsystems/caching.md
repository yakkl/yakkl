...
Like any product with good performance characteristics, Yakkl makes
...
On the backend, Yakkl uses `memcached`, a popular key-value store, for
caching.  Our `memcached` caching helps let us optimize Yakkl's
...
amount of Yakkl's core caching code correctly, then the code most developers
...
Yakkl's Django codebase, all one needs to do is call the standard
...
transparently use Yakkl's memcached caching system, and the developer
...
Yakkl.  But the caching is certainly a nice bonus.
...
* Cache entries expire after the timeout; in this case, a week.
  Though in frequently deployed environments like yakkl.com,
...
We use this decorator in more than 30 places in Yakkl, and it saves a
...
project, Django will call the `flush_user_profile` function.  Yakkl is
...
cache-flushing functions correctly, and people writing Yakkl code
...
code in Yakkl just needs to modify Django model objects and call
`.save()`, and the caching system will do the right thing.

### Production deployments and database migrations

When upgrading a Yakkl server, it's important to avoid having one
version of the code interact with cached objects from another version
that has a different data layout.  In Yakkl, we avoid this through
some clever caching strategies.  Each "deployment directory" for Yakkl
...
For Yakkl's `test-backend` unit tests, we use the same strategy.  In
particular, we just edit `KEY_PREFIX` before each unit test; this
means each of the thousands of test cases in Yakkl has its own
...
won't potentially conflict with whatever you're doing in a Yakkl
...
We generally try to avoid in-process backend caching in Yakkl's Django
codebase, because every Yakkl production installation involves
...
* Caches of various data, like the SourceMap object, that are
  ...
  change once a Yakkl server has been deployed in production.

## Browser caching of state

Yakkl makes extensive use of caching of data in the browser and mobile
