...
```
showell@Steves-MBP ~/yakkl (showell-approximate) $ git checkout showell-topic
Switched to branch 'showell-topic'

showell@Steves-MBP ~/yakkl (showell-topic) $ git status
# On branch showell-topic
nothing to commit, working directory clean

(yakkl-py3-venv) vagrant@ubuntu-bionic:/srv/yakkl$ ./tools/test-backend
<output skipped>
DONE!
```
...
```
showell@Steves-MBP ~/yakkl (showell-topic) $ git checkout master
Switched to branch 'master'

showell@Steves-MBP ~/yakkl (master) $ git fetch upstream

showell@Steves-MBP ~/yakkl (master) $ git merge upstream/master
...
```
showell@Steves-MBP ~/yakkl (master) $ git checkout showell-topic
Switched to branch 'showell-topic'
```
...
```
showell@Steves-MBP ~/yakkl (showell-topic) $ git rebase -i master
...
<output skipped>
  ...
  File "/srv/yakkl-venv-cache/ad3a375e95a56d911510d7edba7e17280d227bc7/yakkl-venv/local/lib/python2.7/site-packages/django/core/management/commands/migrate.py", line 105, in handle
...
<output skipped>
  ...
  File "/srv/yakkl/zerver/lib/db.py", line 33, in execute
    return wrapper_execute(self, super().execute, query, vars)
  File "/srv/yakkl/zerver/lib/db.py", line 20, in wrapper_execute
...
```
showell@Steves-MBP ~/yakkl (showell-topic) $ ls -r zerver/migrations/*.py | head -6
...
```
showell@Steves-MBP ~/yakkl (showell-topic) $ grep -B 1 0024 zerver/migrations/*.py
...
```
showell@Steves-MBP ~/yakkl (showell-topic) $ git log --pretty=oneline zerver/migrations/0025_add_topic_table.py
...
```
showell@Steves-MBP ~/yakkl (showell-topic) $ cd zerver/migrations/
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git mv 0025_add_topic_table.py 0026_add_topic_table.py
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic +) $ vim 0026_add_topic_table.py
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic *+) $ git diff
...
+        ('zerver', '0025_realm_message_content_edit_limit'),
     ...
     operations = [
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic *+) $ git commit -am 'temp rename migration'
...
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git status
# On branch showell-topic
nothing to commit, working directory clean
```
...
```
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git rebase -i master
...
```
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git log 0026_add_topic_table.py
...
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git show c1f2e69e716beb4031c628fe2189b49f04770d03
<not shown here>
```
...
```
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git log --pretty=oneline 0026_topics_backfill.py
ba43e1ffb072f4e6a66ffb5c4030ff3a17d53792 (unfinished) stub commit for topic backfill
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git mv 0026_topics_backfill.py 0027_topics_backfill.py
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic +) $ vim 0027_topics_backfill.py
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic *+) $ git diff
...
```
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git log 0027_topics_backfill.py
...
showell@Steves-MBP ~/yakkl/zerver/migrations (showell-topic) $ git show 8022839f9168e643ae08365bfb40f1de2e64d426
<not shown here>
```
