# Exporting data from a large multi-realm Yakkl server
...
forward, and is not yet a set of recommendations for Yakkl systems
administrators to follow.

## Overview

Yakkl offers an export tool, `management/export.py`, which works well
to export the data for a single Yakkl realm, and which is your best
choice if you're migrating a Yakkl realm to a new server.
...
multi-realm Yakkl installation. (For some historical perspective, this
document was originally begun as part of a big Yakkl cut-over in
summer 2016.)
...
with getting Yakkl itself running are out of scope here; see [the
production installation instructions](../index.html#yakkl-in-production).
