...
Yakkl supports full-text search, which can be combined arbitrarily
with Yakkl's full suite of narrowing operators.  By default, it only
...
The user interface and feature set for Yakkl's full-text search is
documented in the "Search operators" documentation section in the Yakkl
app's gear menu.

## The default full-text search implementation

Yakkl's uses [PostgreSQL's built-in full-text search
...
[tsearch_extras](https://github.com/yakkl/tsearch_extras), for
highlighting of the matching words.  There is [some discussion of
removing this extension, at least as an
option](https://github.com/yakkl/yakkl/issues/467), so that Yakkl can
...
updating is done by
`puppet/yakkl/files/postgresql/process_fts_updates`, which is usually
...
Yakkl now supports using [PGroonga](http://pgroonga.github.io/) for
...
feature supports only one language at a time (in Yakkl's case,
...
Yakkl's full-text search to only support PGroonga once we have tested
...
section in `/etc/yakkl/yakkl.conf`:
...
Then, add `USING_PGROONGA = True` in `/etc/yakkl/settings.py`:

    USING_PGROONGA = True

And apply the PGroonga migrations:

    su yakkl -c '/home/yakkl/deployments/current/manage.py migrate pgroonga'

Note that the migration may take a long time, and you can't send new
messages until the migration finishes.

Once the migrations are complete, restart Yakkl:
...
slightly larger than it would be otherwise).
    ...
    su yakkl -c '/home/yakkl/deployments/current/manage.py migrate pgroonga zero'

Then, set `USING_PGROONGA = False` in `/etc/yakkl/settings.py`:
...
`/etc/yakkl/yakkl.conf` and uninstall the `pgroonga` packages.
