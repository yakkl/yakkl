...
When you're using Yakkl and you reload, or narrow to a stream, how
does Yakkl decide where to place you?

Conceptually, Yakkl takes you to the place where you left off
...
counting unread messages. Here's how Yakkl currently decides which
...
* The blue cursor box (the "pointer") is around is called the
  "selected" message.  Yakkl ensures that the currently selected
...
of messages), Yakkl will select the message you clicked on. This
...
typing some terms into the search box, Yakkl will instead select
...
"first unread message", Yakkl ignores unread messages in muted streams
...
interleaved view you get if you visit `/`), Yakkl will select the
...
browser), Yakkl will preserve the state -- what (if any) narrow the
...
How does Yakkl decide whether a message has been read by the user?
...
In a Yakkl development environment, you can use `manage.py
