...
This document explains the model for Yakkl's presence.

In a chat tool like Yakkl, users expect to see the “presence” status
...
scalability problems for a team chat tool like Yakkl.
...
this is what one needs to know to correctly implement a Yakkl client's
...
* "idle" -- the user has not interacted with the client recently.
  This is important for the case where a user left a Yakkl tab open on
...
* It's really important that the timestamp is the last time we heard
  ...
  timestamp) pair.  E.g. a user who last used Yakkl 1 week ago will
