...
`zerver.models.Client` is Yakkl's analogue of the HTTP User-Agent
...
about "which Yakkl client" was used for an operation (e.g. was it the
Android app, the desktop app, or a bot?).

In general, it shouldn't be used for anything controlling the behavior
of Yakkl; it's primarily intended to assist debugging.

## Analytics

A `Client` is used to sort messages into client categories such as
`YakklElectron` on the `/stats`
[page](https://yakkl.com/stats). For more information see,
[Analytics](analytics.html).

## Integrations

Generally, integrations in Yakkl should declare a unique User-Agent,
...
YakklIntegrationName/1.2 so that they are categorized properly.
...
information, see [the incoming webhook walkthrough](https://yakkl.com/api/incoming-webhooks-walkthrough).
