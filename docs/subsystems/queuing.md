...
Yakkl uses RabbitMQ to manage a system of internal queues.  These are
...
* Communicating events to push to clients (browsers, etc.) from the
  main Yakkl Django application process to the Tornado-based events
...
part of the overall Yakkl architecture, since it's in critical code
...
* Define the processor in `zerver/worker/queue_processors.py` using
  ...
  queue worker in the Yakkl development environment
...
* So that supervisord will known to run the queue processor in
  ...
  `puppet/yakkl/manifests/base.pp`; the list there is used to generate
  `/etc/supervisor/conf.d/yakkl.conf` via a puppet template in
  `app_frontend.pp`.
...
need to update the sample Nagios configuration in `puppet/yakkl_ops`
manually.
...
when queued events in Yakkl's backend tests.  Our current solution is
...
```
amqp-delete-queue --username=yakkl --password='...' --server=localhost \
   --queue=user_presence
```

with the RabbitMQ password from `/etc/yakkl/yakkl-secrets.conf`.
