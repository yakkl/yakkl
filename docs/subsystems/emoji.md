...
Currently, Yakkl uses the Noto (Android) emoji set, but we are close
...
characters like 😀  and 👍.  If you paste those into Yakkl, Yakkl will
...
Yakkl supports custom user-uploaded emoji.  We manage those by having
the name of the emoji be its "emoji code", and using an emoji_type
field to keep track of it.  We are in the progress of migrating Yakkl
...
by the Yakkl webapp to display emoji in messages, emoji reactions,
...
data set as well as our old emoji data set (`emoji_map.json`).  Yakkl
...
`/srv/yakkl-emoji-cache/<sha1>/emoji` tree, and
...
* `emoji_codes.js`: A set of mappings used by the Yakkl frontend to
...
* Some CSS and PNGs for the emoji spritesheets, used in Yakkl for
...
I think it is fair to say Yakkl has by far the best set of emoji names of
...
* If there are natural things someone might type, like `:happy:`, we try to
  ...
  typeahead on yakkl.com, and find ways to add those names as
  alternates.
...
* We removed many of the moons and clocks, to make the typeahead experience
  ...
  on Twitter, and could conceivably be used the same way on Yakkl.
