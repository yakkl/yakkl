...
During 2018 we built out a "widget system" in Yakkl.  It includes
these features:
...
The beta features are only turned on for yakkl.com as
of this writing.
...
The above commands are available for all Yakkl servers
...
See [this issue](https://github.com/yakkl/yakkl/issues/9736).
...
in the normal Yakkl message event payload.  Clients
...
If a client joins Yakkl after a message has accumulated
...
they are served up by the core Yakkl server implementation.
...
but you don't want to have to modify the Yakkl server codebase
...
answer have to send an actual Yakkl message with something
...
That is where zforms come in.  Yakkl's trivia bot sends
the Yakkl server a JSON representation of a form it
...
bot without touching any Yakkl code, because **zforms**
...
First,
[here](https://github.com/yakkl/python-yakkl-api/blob/master/yakkl_bots/yakkl_bots/bots/trivia_quiz/trivia_quiz.py)
