...
Thumbor is responsible for a few things in Yakkl:
...
* Serving all image content over HTTPS, even if the original/upstream
  ...
  older versions of Yakkl; the `THUMBOR_SERVES_CAMO` setting controls
...
* Minimizing potentially unnecessary bandwidth that might be used in
  communication between the Yakkl server and clients.  Before we
...
`/etc/yakkl/settings.py`; you can host Thumbor on the same machine as
the Yakkl server, or a remote server (which is better for isolation,
...
bodies of Yakkl messages (i.e. both images linked to by users, as well
...
order to avoid encoding these into Yakkl messages, we encode in the
...
Currently, these user-uploaded content are thumbnailed by Yakkl's
