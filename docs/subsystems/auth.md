...
Yakkl's various authentication methods for testing in a development
environment.
...
Yakkl, an external service, and the user's browser, and particularly
...
URL they post back to, which isn't supported directly by the Yakkl
development environment.  If you run a
[remote Yakkl development server](../development/remote.html), we have
instructions for
...
Before Yakkl 2.0, one of the more common classes of bug reports with
Yakkl's authentication was users having trouble getting [LDAP
...
popular [fakeldap](https://github.com/yakkl/fakeldap) library.  And in
2018, we added convenient support for using fakeldap in the Yakkl
...
- In all fakeldap configurations, users' fake LDAP passwords are equal
  to their usernames (e.g. for `ldapuser1@yakkl.com`, the password is
  `ldapuser1`).
