...
is sent in Yakkl, and why that is correct behavior.  It assumes the
...
This is just a bit of terminology: A "message list" is what Yakkl
...
number of purposes:
   ...
   * Handling certain client configuration options that affect
     ...
     [events API docs](https://yakkl.com/api/register-queue)).
...
* The core `do_send_messages` function (which handles actually sending
   ...
   * Trigger any other deferred work caused by the current message,
     e.g. [outgoing webhooks](https://yakkl.com/api/outgoing-webhooks)
     or embedded bots.
...
Yakkl aims for a near-perfect local echo experience, which requires is
...
The rest of this section details how Yakkl manages locally echoed
messages.
...
* The core function in the frontend codebase
  ...
  causes Yakkl to insert the message into the relevant feed(s).
...
lose the text the user had composed.  Yakkl handles this with a few
approaches:
...
* Additionally, Yakkl renders UI for editing/retransmitting/resending
...
* In the default configuration, Yakkl stores the message edit history
...
Yakkl's inline URL previews feature (`zerver/lib/url_preview/`) uses
...
* For this case, Yakkl's backend markdown processor will render the
...
Yakkl's message editing feature) in order to avoid needing custom code
to implement the notification-and-rerender part of this implementation.

## Soft deactivation

This section details a somewhat subtle issue: How Yakkl uses a
...
For background, Yakkl’s threading model requires tracking which
...
This isn’t a problem for most Yakkl servers, but is a major problem
for communities like yakkl.com, where might be 10,000s of
...
While these delays may not be immediately obvious to users (Yakkl,
...
communities with the rough structure of the Yakkl development
...
users that are soft-deactivated consume less resources from Yakkl in a
way that is designed to be invisible both to other users and to the
user themself.  If a user hasn’t logged into a given Yakkl
...
* If/when the user ever returns to Yakkl, we can at that time
...
Yakkl, this work can be done in a nightly cron job).  We can construct
...
to Yakkl.  Conveniently, those messages are rare, and so we can just
