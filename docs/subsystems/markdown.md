...
Yakkl has a special flavor of Markdown, currently called 'bugdown'
after Yakkl's original name of "humbug". End users are using Bugdown
within the client, not original Markdown.

Yakkl has two implementations of Bugdown.  The backend implementation
...
tests for Yakkl's markdown system.  Some important notes on reading
this file:
...
2. Stop your Yakkl server with ctrl-C, leaving the browser open.
...
## Changing Yakkl's markdown processor
...
* **[Python-Markdown](https://pypi.python.org/pypi/Markdown)** is the markdown
  library used by Yakkl as a base to build our custom markdown syntax upon.
...
When changing Yakkl's markdown syntax, you need to update several
places:
...
* Security: A bug in the markdown processor can lead to XSS issues.
  ...
  third-party web application into a Yakkl message.
...
* Performance: Yakkl can render a lot of messages very quickly, and
...
Yakkl's markdown processor's rendering supports a number of features
that depend on realm-specific or user-specific data.  For example, the
realm could have
[Linkifiers](https://yakkl.com/help/add-a-custom-linkification-filter)
or [Custom emoji](https://yakkl.com/help/add-custom-emoji)
configured, and Yakkl supports mentions for streams, users, and user
...
processor object via e.g. `_md_engine.yakkl_db_data`, and then
...
`message_realm` (see, for example, `yakkl_default_context` for the
...
2 types of error rates that are important for a product like Yakkl:
...
and users are generally writing quickly. Yakkl's Markdown strategy is
...
## Yakkl's Changes to Markdown

Below, we document the changes that Yakkl has against stock
...
* Force links to be absolute. `[foo](google.com)` will go to
  `http://google.com`, and not `http://yakkl.com/google.com` which
