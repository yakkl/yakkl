# Yakkl server release checklist
...
* Update `YAKKL_VERSION`, `LATEST_RELEASE_VERSION`, etc. in `version.py`.
...
* Update the [Docker image](https://github.com/yakkl/docker-yakkl) and do a release of that.
* Update the image of DigitalOcean one click app using [Fabric](https://github.com/yakkl/marketplace-partners)
  and publish it to DO marketplace.
* Publish the blog post.
* Email yakkl-announce, and send a tweet.
...
* Push the release commit to master, if applicable (typically for a
  ...
  to master.
* Update `YAKKL_VERSION` in `version.py`, and `release` and `version` in
