...
Yakkl is a large project, with well over 100 third-party dependencies,
...
Yakkl has, and how we manage them.  Yakkl's dependency management has
some really nice properties:

* **Fast provisioning**.  When switching to a different commit in the
  Yakkl project with the same dependencies, it takes under 10 seconds
  to re-provision a working Yakkl development environment after
...
* **Consistent provisioning**.  Every time a Yakkl development or
...
The purpose of this document is to detail all of Yakkl's third-party
dependencies and how we manage their versions.

## Provisioning

We refer to "provisioning" as the process of installing and
configuring the dependencies of a Yakkl development environment.  It's
...
time, most Yakkl tools will crash early and ask the user to provision.
...
In the Yakkl project, we take a pragmatic approach to third-party
dependencies.  Overall, if a third-party project does something well
that Yakkl needs to do (and has an appropriate license), we'd love to
...
want to keep the Yakkl webapp's JS bundle small, so that Yakkl
...
on which Yakkl is deployed.  Because Yakkl
...
The exact lists of `apt` packages needed by Yakkl are maintained in a
few places:
* For production, in our puppet configuration, `puppet/yakkl/`, using
  the `Package` and `SafePackage` directives.
* For development, in `SYSTEM_DEPENDENCIES` in `tools/lib/provision.py`.
* The packages needed to build a Yakkl virtualenv, in
...
unique to Yakkl (e.g the `tsearch_extras` postgres extension) and
...
about how Yakkl makes this system work well for us that are worth
...
* **virtualenv with pinned versions**.  For a large application like
  Yakkl, it is important to ensure that we're always using consistent,
  ...
  only the packages and versions that Yakkl needs, and we always pin
  ...
  Yakkl won't break if a dependency makes a buggy release.  A side
...
* **Pinning versions of indirect dependencies**.  We "pin" or "lock"
  ...
  `requirements/common.in`, that declare the packages that Yakkl
  depends on directly.  Those packages have their own recursive
  dependencies.  When adding or removing a dependency from Yakkl, one
  ...
  Yakkl's recursive dependencies.  For indirect dependencies (i.e.
...
* **Caching of virtualenvs and packages**.  To make updating the
  dependencies of a Yakkl installation efficient, we maintain a cache
  of virtualenvs named by the hash of the relevant `requirements.txt`
  file (`scripts/lib/hash_reqs.py`).  These caches live under
  `/srv/yakkl-venv-cache/<hash>`.  That way, when re-provisioning a
...
* **Garbage-collecting caches**.  We have a tool,
  ...
  production deployment directory under `/home/yakkl/deployments/`.
  This helps ensure that a Yakkl installation doesn't leak large
  amounts of disk over time.
* **Scripts**.  Often, we want a script running in production to use
  the Yakkl virtualenv.  To make that work without a lot of duplicated
  code, we have a helpful library,
  `scripts/lib/setup_path_on_import.py`, which on import will put the
  currently running Python script into the Yakkl virtualenv.  This is
...
* **Mypy type checker**.  Because we're using mypy in a strict mode,
  ...
  `mypy.ini` in the root of the Yakkl project to configure
...
[requirements-readme]: https://github.com/yakkl/yakkl/blob/master/requirements/README.md
...
* In a fashion very analogous to the Python codebase,
  ...
  directories in `/srv/yakkl-npm-cache`.  Each is named by its hash,
...
* `tools/update-prod-static`.  This process is discussed in detail in
  ...
  directory in a Yakkl production release tarball, which is a good
  thing, because doing so would more than double the size of a Yakkl
  release tarball.
* **Checked-in packages**.  In contrast with Python, we have a few
  JavaScript dependencies that we have copied into the main Yakkl
...
`/srv/yakkl-yarn`.  We don't do anything special to try to manage
...
the Yakkl server repository, and how we provision and otherwise
maintain them.

### Emoji

Yakkl uses the [iamcal emoji data package][iamcal] for its emoji data
...
`tools/update-prod-static` to make Yakkl's emoji work in the various
...
Yakkl's [translations infrastructure](../translating/translating.html) generates
...
Yakkl maintains data on the developers who have contributed the most to
the current version of Yakkl in the /about page.  These data are
...
When making changes to Yakkl's provisioning process or dependencies,
...
* Production.  Our tools for compiling/generating static assets need
  ...
  `tools/build-release-tarball` (for doing Yakkl releases) as well as
  `tools/upgrade-yakkl-from-git` (for deploying a Yakkl server off of
  master).
...
[npm]: https://npmjs.com/
[ppa]: https://launchpad.net/~tabbott/+archive/ubuntu/yakkl
