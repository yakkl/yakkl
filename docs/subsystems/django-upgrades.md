...
This article documents notes on the process for upgrading Yakkl to
...
* Carefully read the Django upstream changelog, and `git grep` to
  ...
  [example](https://github.com/yakkl/yakkl/issues/2564).
...
* Look at the pieces of Django code that we've copied and then
  ...
  * `CursorDebugWrapper`, which we have a modified version of in
    ...
    [the issue for contributing this upstream](https://github.com/yakkl/yakkl/issues/974)
  ...
  * `AsyncDjangoHandlerBase` in `zerver/tornado/handlers.py` is a very
    ...
    YAKKL` for our changes (unfortunately now more different due to
