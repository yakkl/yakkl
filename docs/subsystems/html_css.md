...
## Yakkl CSS organization

The Yakkl application's CSS can be found in the `static/styles/`
directory.  Yakkl uses [Bootstrap](http://getbootstrap.com/) as its
main third-party CSS library.

Yakkl currently does not use any CSS preprocessors, and is organized
...
* `fonts.css` - Fonts for text in the Yakkl app

The CSS for the Yakkl web application UI is primarily here:

* `settings.css` - CSS for the Yakkl settings (including organization settings) pages
* `yakkl.css` - CSS for the rest of the Yakkl logged-in app
* `media.css` - CSS for media queries (particularly related to screen width)

We are in the process of [splitting yakkl.css into several more
files](https://github.com/yakkl/yakkl/issues/731); help with that
...
When changing any part of the Yakkl CSS, it's important to check that
