...
The page documents the Yakkl settings system, and hopefully should
help you decide how to correctly implement new settings you're adding
to Yakkl.

We have two types of administrative settings in Yakkl:
* **Server settings** are set via configuration files, and apply to
  the whole Yakkl installation.
* **Realm settings** (or **organization settings**) are usually
  set via the /#organization page in the Yakkl web application, and
  apply to a single Yakkl realm/organization. (Which, for most Yakkl
  servers, is the only realm on the server).

Philosophically, the goals of the settings system are to make it
convenient for:

* Yakkl server administrators to configure
Yakkl's featureset for their server without needing to patch Yakkl
...
Yakkl uses the [Django settings
...
access these anywhere in the Yakkl Django code using e.g.:
...
Yakkl has separated those settings that we expect a system
administrator to change (with nice documentation) from the ~1000 lines
of settings needed by the Yakkl Django app.  As a result, there are a
few files involved in the Yakkl settings for server administrators.
In a production environment, we have:

* `/etc/yakkl/settings.py` (the template is in the Yakkl repo at
  `zproject/prod_settings_template.py`) is the main system
  administrator-facing settings file for Yakkl.  It contains all the
  ...
  access to a Yakkl server is to put a default in
  `zproject/settings.py`, and then override it here.

* `/etc/yakkl/yakkl-secrets.conf` (generated by
  `scripts/setup/generate_secrets.py` as part of installation)
  contains secrets used by the Yakkl installation.  These are read
  using the standard Python `ConfigParser`, and accessed in
  `zproject/settings.py` by the `get_secret` function.  All
  secrets/API keys/etc. used by the Yakkl Django application should be
  stored here, and read using the `get_secret` function in
  `zproject/settings.py`.

* `zproject/settings.py` is the main Django settings file for Yakkl.
  It contains all the settings that are constant for all Yakkl
  installations (e.g. configuration for logging, static assets,
  middleware, etc.), as well as default values for the settings the
  user would set in `/etc/yakkl/settings.py` (you can look at the
  ...
  `/etc/yakkl/settings.py` (via a symlink).

In a development environment, we have `zproject/settings.py`, and
additionally:

* `zproject/dev_settings.py` has the settings for the Yakkl development
  environment; it mostly just imports prod_settings_template.py.

* `zproject/dev-secrets.conf` replaces `/etc/yakkl/yakkl-secrets.conf`.

* `zproject/test_settings.py` has the (default) settings used for the
  Yakkl tests (both backend and Casper), which are applied on top of
  the development environment settings.

When adding a new server setting to Yakkl, you will typically add it
...
* Possibly also `zproject/dev_settings.py` and/or
  ...
  Yakkl development and/or test environments is different from the
  default for production.

Most settings should be enabled in the development environment, to
maximize convenience of testing all of Yakkl's features; they should
be enabled by default in production if we expect most Yakkl sites to
want those settings.
...
accessed in initialization of Django (or Yakkl) internals
...
process for adding a new realm setting to Yakkl.
