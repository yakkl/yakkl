...
making a large project like Yakkl successful.  Without reliable error
...
to make it easier for very large installations like yakkl.com to
...
* The ability to send emails to the server's administrators with any
  ...
  sending Yakkls to a stream about production errors).
...
Since 500 errors in any Yakkl server are usually a problem the server
...
`/var/log/yakkl/errors.log`.  Lower log levels just appear in the main
...
The main Yakkl server log contains a line for each backend request.
It also contains warnings, errors, and the full tracebacks for any
Python exceptions.  In production, it goes to
`/var/log/yakkl/server.log`; in development, it goes to the terminal
...
2016-05-20 14:50:26.538 INFO [zr] 127.0.0.1       POST    200  12ms (db: 1ms/2q) (+start: 53ms) /api/v1/events/internal [1463769771:0/0] (cordelia@yakkl.com via internal)
2016-05-20 14:50:26.657 INFO [zr] 127.0.0.1       POST    200  10ms (+start: 8ms) /api/v1/events/internal [1463769771:0/0] (cordelia@yakkl.com via internal)
2016-05-20 14:50:26.959 INFO [zr] 127.0.0.1       GET     200 588ms (db: 26ms/21q) / [1463769771:0] (cordelia@yakkl.com via website)
```

The format of this output is:
* Timestamp
* Log level
* Logger name, abbreviated as "zr" for these Yakkl request logs
* IP address
* HTTP method
* HTTP status code
* Time to process
...
blueslip does the following:
  * Wraps every function passed into `$.ready()`, i.e., every
  on-webapp-startup method used by Yakkl.
  * Wraps every jQuery AJAX request handler used by Yakkl.
  * Wraps every function passed into `$.on()`, i.e. all event
  handlers declared in Yakkl.
  * Declares a default browser exception handler.
  * Has methods for being manually triggered by Yakkl JavaScript code
