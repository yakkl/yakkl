...
developing new features for Yakkl that require front-end changes,
...
Most of the existing JS in Yakkl is written in
...
and `tools/upgrade-yakkl-from-git`) is responsible for orchestrating the
...
- Third-party packages from the NPM repository should be added to
  ...
  providers release new versions with incompatible APIs, it can't break Yakkl.
...
* Yakkl installs static assets in production in
`/home/yakkl/prod-static`.  When a new version is deployed, before the
...
* We use the VFL (Versioned File Layout) strategy, where each file in
  ...
  (e.g. `/home/yakkl/deployments/current/staticfiles.json`) that maps
