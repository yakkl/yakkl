...
Yakkl has a number of [Django management commands][django-docs] that
live under `{zerver,zilencer,analytics}/management/commands/`.

If you need some Python code to run with a Yakkl context (access to
...
While Yakkl takes advantage of built-in Django management commands for
...
* Useful parts of provisioning or upgrading a Yakkl development
...
* The actual scripts run by supervisord to run the persistent
  processes in a Yakkl server, e.g. `runtornado` and `process_queue`.
* For a sysadmin to verify a Yakkl server's configuration during
...
* For a sysadmin to easily interact with and script common possible
  changes they might want to make to the database on a Yakkl server.
...
is good, but we have a few pieces advice specific to the Yakkl
project.

* If you need to access a realm or user, use the `YakklBaseCommand`
...
* Avoid writing a lot of code in management commands; management
  ...
  (and ideally, also used in Yakkl's existing code).  Look for code in
