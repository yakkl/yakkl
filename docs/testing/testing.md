...
Yakkl has a full test suite that includes many components.  The most
...
Yakkl tests must be run inside a Yakkl development environment; if
...
Then, to run the full Yakkl test suite, do this:
```
./tools/test-all
```
...
Yakkl also has about a dozen smaller tests suites:
...
- `tools/check-capitalization`: Checks whether translated strings (aka
  user-facing strings) correctly follow Yakkl's capitalization
  conventions.  This requires some maintainance of an exclude list
  (`tools.lib.capitalization.IGNORED_PHRASES`) of proper nouns
  mentioned in the Yakkl project, but helps a lot in avoiding new
...
- `./tools/test-queue-worker-reload`: Verifies that Yakkl's queue
...
- `./tools/setup/optimize-svg`: Checks whether all integration logo SVG
  ...
  third-party logos, this helps keep the Yakkl codebase from getting huge).
...
something valuable to helping keep Yakkl bug-free.
...
- Or, when running the test suite, if you get an error involving Git that looks like this:
  ...
  ... then you may need to connect the Yakkl upstream repository with the following command:
...
```
Running node_modules/.bin/casperjs  test /srv/yakkl/frontend_tests/casper_tests/00-realm-creation.js
...
As a policy matter, the Yakkl test suites should never make outgoing
...
As a result, Yakkl's major test suites should never access the
Internet directly.  Since code in Yakkl does need to access the
Internet (e.g. to access various third-party APIs), this means that
the Yakkl tests use mocking to basically hardcode (for the purposes of
the test) what responses should be used for any outgoing Internet
requests that Yakkl would make in the code path being tested.
...
[our guide on mocking](../testing/testing-with-django.html#yakkl-mocking-practices) to
...
This enforcement code results in the following exception:
  ...
  File "tools/test-backend", line 120, in internet_guard
    raise Exception("Outgoing network requests are not allowed in the Yakkl tests."
  Exception: Outgoing network requests are not allowed in the Yakkl tests.
  ...
  ```
...
If that fails you should try to do:
    ...
    > DROP DATABASE yakkl_test;
    > DROP DATABASE yakkl_test_template;
