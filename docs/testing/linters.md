...
Yakkl does extensive linting of much of its source code, including
...
Obviously, a large reason for linting code is to enforce the [Yakkl
...
Yakkl also uses some home-grown code to perform tasks like validating
indentation in template files, enforcing coding standards that are unique
to Yakkl, allowing certain errors from third party linters to pass through,
...
**Important:** We provide a
  [Git pre-commit hook](http://localhost:9991/docs/git/yakkl-tools.html#set-up-git-repo-script)
...
Once you have read the [Yakkl coding guidelines](../contributing/code-style.html), you can
...
can find other people in the [Yakkl Community](../contributing/chat-yakkl-org.html)
to help you.

Also, bear in mind that 100% of the lint code is open source, so if you
find limitations in either the Yakkl home-grown stuff or our third party
tools, feedback will be highly appreciated.

Finally, one way to clean up your code is to thoroughly exercise it
with tests.  The [Yakkl test documentation](../testing/testing.html)
...
- Check Python code for custom Yakkl rules.
- Check non-Python code for custom Yakkl rules.
...
Yakkl has a script called `lint` that lives in our "tools" directory.
...
You can find the source code [here](https://github.com/yakkl/yakkl/blob/master/tools/lint).
...
types of checks mostly lives [here](https://github.com/yakkl/yakkl/blob/master/tools/linter_lib).
...
output to exclude certain types of errors that Yakkl is comfortable
ignoring.  (One notable class of error that Yakkl currently tolerates is
...
Yakkl also has custom regex-based rules that it applies to Python code.
...
- We run eslint.
- We perform custom Yakkl regex checks on the code.
...
Yakkl uses two HTML templating systems:

- [Django templates](https://docs.djangoproject.com/en/1.10/topics/templates/)
- [handlebars](http://handlebarsjs.com/)

Yakkl has an internal tool that validates both types of templates for
correct indentation and matching tags.  You can find the code here:

- driver: [check-templates](https://github.com/yakkl/yakkl/blob/master/tools/check-templates)
- engine: [lib/template_parser.py](https://github.com/yakkl/yakkl/blob/master/tools/lib/template_parser.py)
...
Yakkl uses [stylelint](https://github.com/stylelint/stylelint) to lint
its CSS; see our
[configuration](https://github.com/yakkl/yakkl/blob/master/.stylelintrc)
for the rules we currently enforce.

#### Shell scripts

Yakkl uses [shellcheck](https://github.com/koalaman/shellcheck) to
...
If you want to help improve Yakkl's system for linting, here are some
considerations.
