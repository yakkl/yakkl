...
JavaScript/TypeScript code in Yakkl.  We prefer it over the [Casper
...
which use the `phantomjs` browser connected to a running Yakkl
...
manipulations (which in Yakkl are almost exclusively done via
`jQuery`) using a custom library
[zjquery](https://github.com/yakkl/yakkl/blob/master/frontend_tests/zjsunit/zjquery.js).

The
[unit test file](https://github.com/yakkl/yakkl/blob/master/frontend_tests/node_tests/zjquery.js)
...
working on or debugging the Yakkl node tests.
...
>     page_params.email = 'alice@yakkl.com';
...
2. In WebStorm, navigate to `Preferences -> Tools -> Vagrant` and
    ...
    * `Instance folder` should be the root of the `yakkl` repository on
      your host (where the Vagrantfile is located).
    * `Provider` should be `virtualbox` on macOS and Docker on Linux
    * In `Boxes`, choose the one used for Yakkl (unless you use
...
3. You'll now need to set up a WebStorm "Debug Configuration".  Open
   the `Run/Debug Configuration` menu and create a new `Node.js` config:
    ...
    1. Under `Working Directory` select the root `yakkl` directory.
    1. Under `JavaScript file`, enter `frontend_tests/zjsunit/index.js`
     -- this is the root script for Yakkl's node unit tests.
...
1. Under `Path Mappings`, set `Project Root` to `/srv/yakkl`
   (i.e. where the `yakkl` Git repository is mounted in the Vagrant guest).
