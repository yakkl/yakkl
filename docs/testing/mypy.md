...
for Python, allowing optional, gradual typing of Python code.  Yakkl
...
mypy is installed by default in the Yakkl development environment.  If
...
## Running mypy on Yakkl's code locally

To run mypy on Yakkl's python code, you can run the command:
...
## Mypy is there to find bugs in Yakkl before they impact users

For the purposes of Yakkl development, you can treat `mypy` like a
much more powerful linter that can catch a wide range of bugs.  If,
after running `tools/run-mypy` on your Yakkl branch, you get mypy
...
* A bug in Yakkl!
* Some Yakkl code is correct but confusingly reuses variables with
  different types.
...
should be solving the mypy warning in a way that makes the Yakkl
...
For other third-party modules that we call from Yakkl, one either
...
have untracked files in your Yakkl checkout safely).  So if you get a
