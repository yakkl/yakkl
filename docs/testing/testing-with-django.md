...
Yakkl uses the Django framework for its Python back end.  We
...
All changes to the Yakkl backend code should be supported by tests.  We
...
will probably find familiar patterns in Yakkl's code.  This document
describes how to write tests for the Yakkl back end, with a particular
emphasis on areas where we have either wrapped Django's test framework
or just done things that are kind of unique in Yakkl.
...
There are many command line options for running Yakkl tests, such
...
Before you write your first tests of Yakkl, it is worthwhile to read
...
[zerver/lib/test_helpers.py](https://github.com/yakkl/yakkl/blob/master/zerver/lib/test_helpers.py),
which contains test helpers.
[zerver/lib/test_classes.py](https://github.com/yakkl/yakkl/blob/master/zerver/lib/test_classes.py),
which contains our `YakklTestCase` and `WebhookTestCase` classes.
...
Shakesepeare characters, and they are part of the "yakkl.com" realm.

Generally, you will also do some explicit data setup of your own. Here
are a couple useful methods in YakklTestCase:
...
## Yakkl Testing Philosophy

If there is one word to describe Yakkl's philosophy for writing tests,
...
These are some of the testing strategies that you will see in the Yakkl
test suite...

### Endpoint tests

We strive to test all of our URL endpoints.  The vast majority of Yakkl
...
- Use a Yakkl test helper to hit the endpoint.
...
test class that is a subclass of `YakklTestCase`, which will provide
...
For certain Yakkl library functions, especially the ones that are
...
Particularly for testing Yakkl's integrations with third party systems,
...
- to make it so that you can [run the Yakkl tests on the airplane without wifi][no-internet]
...
In [zerver/tests/test_templates.py](https://github.com/yakkl/yakkl/blob/master/zerver/tests/test_templates.py)
...
The Yakkl back end has a mechanism where it will fetch initial data
