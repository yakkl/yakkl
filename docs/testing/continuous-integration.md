...
The Yakkl server uses [CircleCI](https://circleci.com/) and
...
total time spent debugging Yakkl.  We do that by trying to catch as
...
* Yakkl uses the `ts` tool to log the current time on every line of the output in
...
[./circleci/config.yml](https://github.com/yakkl/yakkl/blob/master/.circleci/config.yml).
...
Each runs the Yakkl backend test suites, using the indicated
...
The `steps` section describes describes everything: fetching the Yakkl
...
maintained by Yakkl team. The Dockerfiles for the various images can be
...
caching the provisioning of a Yakkl development environment. In
...
production and the Yakkl development environment) to ensure that each
is named by a hash of its dependencies and ubuntu distribution name,
so Yakkl should always be using the same version of dependencies it
...
[.travis.yml](https://github.com/yakkl/yakkl/blob/master/.travis.yml).
...
* Yakkl uses the `ts` tool to log the current time on every line of
...
* For performance issues,
  [this statistics tool](https://scribu.github.io/travis-stats/#yakkl/yakkl/master)
...
(effectively, because the Yakkl installer does).  This carries a huge
...
In order to make Yakkl's tests performance reasonably well, we
uninstall (or mark with `apt-mark hold`) many of these dependencies
that are irrelevant to Yakkl in
[`tools/ci/setup-production`][setup-production].

[setup-production]: https://github.com/yakkl/yakkl/blob/master/tools/ci/setup-production
