...
When in doubt, ask in [yakkl.com](https://yakkl.com).
...
to read secrets from `/etc/yakkl/secrets.conf`.
...
`YAKKL_PATHS` in `zproject/settings.py`, which will do the right thing
...
external CSS files such as `yakkl.css`.
