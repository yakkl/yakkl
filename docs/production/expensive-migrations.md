...
Yakkl 1.7 and 1.9 each contain some significant database migrations
...
1. Log into your Yakkl server as the `yakkl` user (or as `root` and
  then run `su yakkl` to drop privileges), and `cd
  /home/yakkl/deployments/current`
...
(These first migrations are the only new ones in Yakkl 1.9).
...
finish, you can proceed with installing Yakkl 1.7.
...
were run on yakkl.com, it had 75M UserMessage rows; the first 5
