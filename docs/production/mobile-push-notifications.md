...
Yakkl's iOS and Android mobile apps support receiving push
notifications from Yakkl servers to let users know when new messages
have arrived.  This is an important feature to having a great
experience using the Yakkl mobile apps.
...
will need to register your Yakkl server with the Yakkl mobile push
notification service.  This service will forward push notifications
generated by your server to the Yakkl mobile app automatically.

## How to sign up

Starting with Yakkl 1.6 for both Android and iOS, Yakkl servers
support forwarding push notifications to a central push notification
forwarding service.  You can enable this for your Yakkl server as
follows:

1. Uncomment the `PUSH_NOTIFICATION_BOUNCER_URL =
   'https://push.yakkl.com'` line in your `/etc/yakkl/settings.py`
   file (i.e. remove the `#` at the start of the line), and
   [restart your Yakkl server](../production/maintain-secure-upgrade.html#updating-settings).
   If you installed your Yakkl server with a version older than 1.6,
   you'll need to add the line (it won't be there to uncomment).

1. If you're running Yakkl 1.8.1 or newer, you can run the
    registration command:
    ```
    # As root:
    su yakkl -c '/home/yakkl/deployments/current/manage.py register_server'
    # Or as the yakkl user, you can skip the `su yakkl -c`:
    /home/yakkl/deployments/current/manage.py register_server
    ```
   ...
   email support@yakkl.com with the output of this command.

1. If you or your users have already set up the Yakkl mobile app,
...
[Yakkl Android app](https://play.google.com/store/apps/details?id=com.yakklmobile).
...
* Hit the home button, so Yakkl is running in the background, and then
have **another user** send you a **private message** (By default,
Yakkl only sends push notifications for private messages sent by other
users and messages mentioning you).  A push notification should appear
in the Android notification area.

[mobile-notifications-always]: https://yakkl.com/help/test-mobile-notifications

## Updating your server's registration

Your server's registration includes the server's hostname and contact
email address (from `EXTERNAL_HOST` and `YAKKL_ADMINISTRATOR` in
`/etc/yakkl/settings.py`, aka the `--hostname` and `--email` options
...
(`yakkl_org_key`), you need to use `manage.py register_server
--rotate-key` option; it will automatically generate a new
`yakkl_org_key` and store that new key in
...
Yakkl's solution to this problem is to provide a central push
notification forwarding service, which allows registered Yakkl servers
to send push notifications to the Yakkl app indirectly (through the
forwarding service).
...
* The Push Notification Service only stores the necessary metadata for
  delivering the notifications to the appropriate devices, and nothing
    ...
    * The APNS/FCM tokens needed to securely send mobile push
      ...
      registered to be notified by your Yakkl server.
    * User ID numbers generated by your Yakkl server, needed to route
...
* The Push Notification Service receives (but does not store) the
  contents of individual mobile push notifications:
    * The numeric message ID generated by your Yakkl server.
  ...
  We plan to
  [replace that setting with end-to-end encryption](https://github.com/yakkl/yakkl/issues/6954)
  ...
  section.
* All of the network requests (both from Yakkl servers to the Push
...
* The code for the push notification forwarding service is 100% open
  ...
  [Yakkl server project on GitHub](https://github.com/yakkl/yakkl).
...
submit basic usage statistics (e.g. Yakkl version, number of users,
number of messages sent) to the service. These statistics help the
Yakkl open source project understand how many people are using Yakkl,
...
notifications, for Yakkl 1.8 and older.

1. First, contact support@yakkl.com with the `yakkl_org_id` and
   `yakkl_org_key` values from your `/etc/yakkl/yakkl-secrets.conf` file, as
...
is for an individual Yakkl server's administrators to build and
distribute their own copy of the Yakkl mobile apps, hardcoding a key
that they possess.

This solution is possible with Yakkl, but it requires the server
administrators to publish their own copies of
the Yakkl mobile apps (and there's nothing the Yakkl team can do to
eliminate this onerous requirement).

The main work is distributing your own copies of the Yakkl mobile apps
...
store (as would be the case here). The Yakkl mobile app also gets
frequent updates that you will have to either forgo or republish to
the app stores yourself.

If you've done that work, the Yakkl server configuration for sending
...
* Create a
  ...
  `/etc/yakkl/yakkl-secrets.conf` to that key.
* Register for a
