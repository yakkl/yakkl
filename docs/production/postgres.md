...
This is a bit annoying to setup, but you can configure Yakkl to use a
dedicated postgres server by setting the `REMOTE_POSTGRES_HOST`
variable in /etc/yakkl/settings.py, and configuring Postgres
...
called "yakkl" in your database server. You can configure these
options in `/etc/yakkl/settings.py` (the below descriptions are from the
...
Then you should specify the password of the user yakkl for the
database in /etc/yakkl/yakkl-secrets.conf:

```
postgres_password = xxxx
```

Finally, you can stop your database on the Yakkl server via:
...
automatically by using the Yakkl install script with a different set
of puppet manifests than the all-in-one feature; if you're interested
in working on this, post to the Yakkl development mailing list and we
...
#### Stopping the Yakkl postgres database
