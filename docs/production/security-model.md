...
This section attempts to document the Yakkl security model.
It likely does not cover every issue; if
there are details you're curious about, please feel free to ask
questions in [#production help](https://yakkl.com/#narrow/stream/31-production-help)
on the [Yakkl community server](../contributing/chat-yakkl-org.html)
(or if you think
you've found a security bug, please report it to
yakkl-security@googlegroups.com so we can do a responsible security
announcement).

## Secure your Yakkl server like your email server

* It's reasonable to think about security for a Yakkl server like you
  ...
  In particular, anyone with root access to a Yakkl application server
  or Yakkl database server, or with access to the `yakkl` user on a
  Yakkl application server, has complete control over the Yakkl
...
* Traffic between clients (web, desktop and mobile) and the Yakkl is
  encrypted using HTTPS.  By default, all Yakkl services talk to each
  other either via a localhost connection or using an encrypted SSL
  connection.

* Yakkl requires CSRF tokens in all interactions with the web API to
  prevent CSRF attacks.

* The preferred way to login to Yakkl is using an SSO solution like
  Google Auth, LDAP, or similar, but Yakkl also supports password
  authentication.  See
  [the authentication methods documentation](../production/authentication-methods.html)
  for details on Yakkl's available authentication methods.

### Passwords

Yakkl stores user passwords using the standard PBKDF2 algorithm.

When the user is choosing a password, Yakkl checks the password's
...
* `PASSWORD_MIN_GUESSES`: The minimum acceptable strength of the
  ...
  `PASSWORD_MIN_GUESSES`, then Yakkl rejects the password.
...
* Yakkl message content is rendered using a specialized Markdown
  parser which escapes content to protect against cross-site scripting
  attacks.

* Yakkl supports both public streams and private streams.
  ...
  * See [Stream permissions](https://yakkl.com/help/stream-permissions) for more details.

* Yakkl supports editing the content and topics of messages that have
  ...
  * See
    [Configuring message editing and deletion](https://yakkl.com/help/configure-message-editing-and-deletion)
    for more details.

## Users and Bots

* There are four types of users in a Yakkl organization: Organization
...
* Administrators have the ability to deactivate and reactivate other
  ...
  * Administrators may get access to private messages via some types of
    [data export](https://yakkl.com/help/export-your-organization).
...
* Every Yakkl user has an API key, available on the settings page.
...
* To properly remove a user's access to a Yakkl team, it does not
  ...
  should
  [deactivate the user's account](https://yakkl.com/help/deactivate-or-reactivate-a-user)
  via Yakkl's "Organization settings" interface.

* The Yakkl mobile apps authenticate to the server by sending the
  ...
  you should rotate the user's Yakkl API key.

* Guest users are like Members, but they do not have automatic access
  to public streams.

* Yakkl supports several kinds of bots with different capabilities.

  * Incoming webhook bots can only send messages into Yakkl.
  ...
  * API super user bots can send messages that appear to have been sent by
    ...
    API super user bots cannot be created by Yakkl users, including
...
* Yakkl supports user-uploaded files.  Ideally they should be hosted
  from a separate domain from the main Yakkl server to protect against
  various same-domain attacks (e.g. yakkl-user-content.example.com).
  ...
  Yakkl server's filesystem, and the S3 backend, where the files are
  ...
  users accessing files uploaded in Yakkl (an authorized user would
  ...
  In the Yakkl S3 backend, the random URLs to access files that are
  presented to users don't actually host the content.  Instead, the S3
  backend verifies that the user has a valid Yakkl session in the
  relevant organization (and that has access to a Yakkl message linking to
  ...
  possessing a URL to a secret file in Yakkl does not provide
  ...
  browser is logged into a Yakkl account that has received the
  uploaded file in question).

* Yakkl supports using the Camo image proxy to proxy content like
  inline image previews that can be inserted into the Yakkl message
  feed by other users over HTTPS.

* By default, Yakkl will provide image previews inline in the body of
  messages when a message contains a link to an image.  You can
  control this using the `INLINE_IMAGE_PREVIEW` setting.

## Final notes and security response

If you find some aspect of Yakkl that seems inconsistent with this
security model, please report it to yakkl-security@googlegroups.com so
that we can investigate and coordinate an appropriate security release
if needed.

Yakkl security announcements will be sent to
yakkl-announce@googlegroups.com, so you should subscribe if you are
