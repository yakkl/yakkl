...
The vast majority of Yakkl servers host just a single organization (or
"realm", as the Yakkl code calls organizations).  This article
documents what's involved in hosting multiple Yakkl organizations on a
single server.

Throughout this article, we'll assume you're working on a yakkl server
with hostname `yakkl.example.com`.  You may also find the more
[technically focused article on realms](../subsystems/realms.html) to be useful
reading.

## Subdomains

Yakkl's approach for supporting multiple organizations on a single
Yakkl server is for each organization to be hosted on its own
subdomain.  E.g. you'd have `org1.yakkl.example.com` and
`org2.yakkl.example.com`.
...
* If you're using Yakkl older than 1.7, you'll need to set
  `REALMS_HAVE_SUBDOMAINS=True` in your `/etc/yakkl/settings.py`
...
[yakkl.com](https://yakkl.com), one can set
`ROOT_DOMAIN_LANDING_PAGE = True` in `/etc/yakkl/settings.py` so that
the homepage for the server is a copy of the Yakkl homepage.

### SSL Certificates

You'll need to install an SSL certificate valid for all the
(sub)domains you're using your Yakkl server with.  You can get an SSL
certificate covering several domains for free by using
[our Certbot wrapper tool](../production/ssl-certificates.html#after-yakkl-is-already-installed),
...
you can set the `REALM_HOSTS` setting in `/etc/yakkl/settings.py` to a
...
realm whose `subdomain` in the Yakkl database is `mysubdomain`.

In a future version of Yakkl, we expect to move this configuration
into the database.

### The root domain

Most Yakkl servers host a single Yakkl organization on the root domain
(i.e. `yakkl.example.com`).  The way this is implemented internally
...
on subdomains (e.g. `subdivision.yakkl.example.com`), but this only
...
Every Yakkl server comes with 1 realm that isn't created by users: the
`yakklinternal` realm.  By default, this realm only contains the Yakkl "system
