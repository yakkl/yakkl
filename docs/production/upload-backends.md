...
Yakkl in production supports a couple different backends for storing
files uploaded by users of the Yakkl server (messages, profile
pictures, organization icons, custom emoji, etc.).

The default is the `LOCAL_UPLOADS_DIR` backend, which just stores
files on disk in the specified directory on the Yakkl server.
Obviously, this backend doesn't work with multiple Yakkl servers and
doesn't scale, but it's great for getting a Yakkl server up and
...
Here, we document the process for configuring Yakkl's S3 file upload
...
user) for your Yakkl server, and two buckets in S3, one for uploaded
...
1. Set `s3_key` and `s3_secret_key` in /etc/yakkl/yakkl-secrets.conf
to be the S3 access and secret keys for the IAM account.

1. Set the `S3_AUTH_UPLOADS_BUCKET` and `S3_AVATAR_BUCKET` settings in
`/etc/yakkl/settings.py` to be the names of the S3 buckets you
created (e.g. `exampleinc-yakkl-uploads`).

1. Comment out the `LOCAL_UPLOADS_DIR` setting in
`/etc/yakkl/settings.py` (add a `#` at the start of the line).
...
1. In some AWS regions, you need to explicitly
    ...
    by adding an `/etc/yakkl/boto.cfg` containing the following:
    ```
    [s3]
    use-sigv4 = True
...
1. You will need to configure `nginx` to direct requests for uploaded
    files to the Yakkl server (which will then serve a redirect to the
    appropriate place in S3), rather than serving them directly.

    With Yakkl 1.9.0 and newer, you can do this automatically with the
    following commands run as root:

    ```
    crudini --set /etc/yakkl/yakkl.conf application_server no_serve_uploads true
    /home/yakkl/deployments/current/scripts/yakkl-puppet-apply
    ```

    (The first line will update your `/etc/yakkl/yakkl.conf`).

    With older Yakkl, you need to edit
    `/etc/nginx/sites-available/yakkl-enterprise` to comment out the
    `nginx` configuration block for `/user_avatars` and the `include
    /etc/nginx/yakkl-include/uploads.route` line and then reload the
    `nginx` service (`service nginx reload`).

1. Finally, restart the Yakkl server so that your settings changes
   take effect
   (`/home/yakkl/deployments/current/scripts/restart-server`).

It's simplest to just do this configuration when setting up your Yakkl
...
in [#production help on yakkl.com][production-help] for advice on
...
IAM user just for your Yakkl server with limited permissions.  For
...
[documentation on the Yakkl security model](security-model.html) for
