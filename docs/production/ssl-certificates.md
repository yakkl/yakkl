...
To keep your communications secure, Yakkl runs over HTTPS only.
...
* `/etc/ssl/private/yakkl.key` for the private key
* `/etc/ssl/certs/yakkl.combined-chain.crt` for the certificate.
...
work with some browsers, but not others and not the Yakkl Android app.
...
* Alternatively, run a command like `curl -SsI https://yakkl.example.com`
...
entire Web.  Yakkl offers a simple automation for
...
We recommend most Yakkl servers use Certbot.  You'll want something
else if:
...
* your Yakkl server is not on the public Internet. (In this case you
  can [still use Certbot][certbot-manual-mode], but it's less
  convenient; and you'll want to ignore Yakkl's automation.)
...
### At initial Yakkl install

To enable the Certbot automation when first installing Yakkl, just
...
Yakkl server machine to be reachable by that name from the public
Internet.

If you need to configure a multiple domain certificate, you can generate
one as described in the section below after installing Yakkl.

[doc-install-script]: ../production/install.html#step-2-install-yakkl

### After Yakkl is already installed

To enable the Certbot automation on an already-installed Yakkl
server, run the following commands:
```
sudo -s  # If not already root
/home/yakkl/deployments/current/scripts/setup/setup-certbot --email=EMAIL HOSTNAME [HOSTNAME2...]
```
where HOSTNAME is the domain name users see in their browser when
using the server (e.g., `yakkl.example.com`), and EMAIL is a contact
address for the server admins. Additional hostnames can also be
specified to issue a certificate for multiple domains.

### How it works

When the Certbot automation in Yakkl is first enabled, by either
...
Then it records a flag in `/etc/yakkl/yakkl.conf` saying Certbot is in
...
(because it's insecure, and because browsers and the Yakkl apps will
complain that it's insecure), but may be convenient for testing.

To generate a self-signed certificate when first installing Yakkl,
just pass the `--self-signed-cert` flag when
[running the install script][doc-install-script].

To generate a self-signed certificate for an already-installed Yakkl
server, run the following commands:
```
sudo -s  # If not already root
/home/yakkl/deployments/current/scripts/setup/generate-self-signed-cert HOSTNAME
```
