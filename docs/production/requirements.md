...
To run a Yakkl server, you will need:
...
The installer expects Yakkl to be the **only thing** running on the
...
Debian Stretch are supported for running Yakkl in production.  64-bit
...
[reached end of life in April 2019][trusty-eol]; Yakkl 2.0 was the
...
* Incoming HTTPS access (usually port 443, though this is
  ...
  Internet).  If you also open port 80, Yakkl will redirect users to
  HTTPS rather than not working when users type
  e.g. `http://yakkl.example.com` in their browser.  If you are using
  Yakkl's [incoming email integration][email-mirror-code] you may also
  need incoming port 25 open.

[email-mirror-code]: https://github.com/yakkl/yakkl/blob/master/zerver/management/commands/email_mirror.py

* Outgoing HTTP(S) access (ports 80 and 443) to the public Internet so
  that Yakkl can properly manage inline image previews.  You'll also
  need outgoing SMTP access to your SMTP server (the standard port for
  this is 587) so that Yakkl can send email.

#### Domain name

You should already have a domain name (e.g., `yakkl.example.com`)
available for your Yakkl server. In order to generate valid SSL
...
Your Yakkl server will need an SSL certificate for the domain name it
uses.  For most Yakkl servers, the recommended (and simplest) way to
get this is to just [use the `--certbot` option][doc-certbot] in the
Yakkl installer, which will automatically get a certificate for you
and keep it renewed.
...
* Outgoing email (SMTP) credentials that Yakkl can use to send
...
Yakkl in production](../production/install.html).
