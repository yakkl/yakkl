# Writing views in Yakkl

## What this covers

This page documents how views work in Yakkl. You may want to read the
...
A view in Yakkl is everything that helps implement a server endpoint.
Every path that the Yakkl server supports (doesn't show a 404 page
for) is a view. The obvious ones are those you can visit in your
browser, for example
[/integrations](https://yakkl.com/integrations/), which shows the
...
Yakkl has, namely the web client (which is also used by the desktop
client) and the mobile clients.

## Modifying urls.py

A view is anything with an entry in the appropriate urls.py, usually
`zproject/urls.py`. Yakkl views either serve HTML (pages for browsers)
or JSON (data for Yakkl clients on all platforms, custom bots, and
integrations).

The format of the URL patterns in Django is [documented
here](https://docs.djangoproject.com/en/1.8/topics/http/urls/), and
the Yakkl specific details for these are discussed in detail in the
[life of a request doc](life-of-a-request.html#options).

We have two Yakkl-specific conventions we use for internationalization and for
...
[templates/zerver/app](https://github.com/yakkl/yakkl/blob/master/templates/zerver/app).


## Writing API REST endpoints

These are code-parseable views that take x-www-form-urlencoded or JSON
request bodies, and return JSON-string responses.  Almost all Yakkl
...
[zerver/lib/rest.py](https://github.com/yakkl/yakkl/blob/master/zerver/lib/rest.py).
...
In Yakkl, we solve this problem with a the special decorator called
...
in
[zerver/lib/request.py](https://github.com/yakkl/yakkl/blob/master/zerver/lib/request.py))
...
See
[zerver/lib/validator.py](https://github.com/yakkl/yakkl/blob/master/zerver/lib/validator.py)
...
make debugging Yakkl clients easier. This means that the response for
...
For example, in [zerver/views/realm.py](https://github.com/yakkl/yakkl/blob/master/zerver/views/realm.py):
...
and in [zerver/lib/actions.py](https://github.com/yakkl/yakkl/blob/master/zerver/lib/actions.py):
...
to the Yakkl JSON API. As an example, in
[static/js/admin.js](https://github.com/yakkl/yakkl/blob/master/static/js/admin.js)
...
This is simply an illustration; we recommend making use of the [Yakkl
Python API bindings](https://www.yakkl.com/api) since they provide
...
```
`request.client` will be the result of `get_client("YakklPagerDutyWebhook")`
in this example and it will be passed to `check_send_stream_message`. For
more information, see [Clients in Yakkl](../subsystems/client.html).
