...
Yakkl codebase, and dive deep into how each part works.
...
When Yakkl is deployed in production, all requests go through nginx.
...
[served-directly]: https://github.com/yakkl/yakkl/blob/master/puppet/yakkl/files/nginx/yakkl-include-frontend/app
...
real-time. Events are served by Yakkl's `tornado` application.
...
[nginx-config-link]: https://github.com/yakkl/yakkl/blob/master/puppet/yakkl/files/nginx/yakkl-include-frontend/app
...
The main Yakkl Django app is `zerver`. The routes are found in
```
zproject/urls.py
...
If we look in
[zproject/urls.py](https://github.com/yakkl/yakkl/blob/master/zproject/urls.py),
...
[here](https://yakkl.com/features/)) gets translated in Chinese at
`zh-hans/features/` (preview
[here](https://yakkl.com/zh-hans/features/)).
...
format of), legacy endpoints, and logged-out endpoints, Yakkl uses REST
...
POST is not idempotent--if I send a message multiple times, Yakkl will
...
This request:
`OPTIONS https://yakkl.com/api/v1/users`
yields a response with this HTTP header:
`Allow: PUT, GET`

We can see this reflected in [zproject/urls.py](https://github.com/yakkl/yakkl/blob/master/zproject/urls.py):
...
You can see them in
[zproject/legacy_urls.py](https://github.com/yakkl/yakkl/blob/master/zproject/legacy_urls.py).

### Incoming webhook integrations may not be RESTful

Yakkl endpoints that are called by other services for integrations have
...
For requests that correspond to a REST url pattern, Yakkl configures
