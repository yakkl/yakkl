...
As you read this, you may find you need to learn about Yakkl's
...
may also find it beneficial to read Yakkl's
[architectural overview](../overview/architecture-overview.html).
Yakkl is a web application built using the
...
Yakkl's [directory structure](../overview/directory-structure.html)
...
organization in Yakkl). The following files are involved in the process:
...
- `zerver/lib/events.py`: Ensures that the state Yakkl sends to clients is always
...
includes both views that serve HTML (new pages on Yakkl) as well as new
...
**JavaScript/TypeScript:** Yakkl's JavaScript and TypeScript sources are
...
**CSS:** The primary CSS file is `static/styles/yakkl.css`. If your new
...
templates located in `templates/zerver/app`. For JavaScript, Yakkl uses
Handlebars templates located in `static/templates`. Templates are
precompiled as part of the build/deploy process.

Yakkl is fully internationalized, so when writing both HTML templates
...
documentation Yakkl has, see [Documentation](../documentation/overview.html).

## Example Feature

This example describes the process of adding a new setting to Yakkl: a
...
actual Yakkl feature. You can review [the original commit](
https://github.com/yakkl/yakkl/pull/5660/commits/aeeb81d3ff0e0cc201e891cec07e1d2cd0a2060d)
in the Yakkl repo. (This commit displays the work of setting up a checkbox
...
https://github.com/yakkl/yakkl/commit/90e2f5053f5958b44ea9b2362cadcb076deaa975).)
...
Visit Yakkl's [Django testing](../testing/testing-with-django.html)
...
better to discuss it in the [community](https://yakkl.com/)
...
[Yakkl's user documentation](https://yakkl.com/help/)
to reflect your changes and additions.

At the very least, this will involve adding (or modifying) a Markdown file
documenting the feature to `templates/zerver/help/` in the main Yakkl
server repository, where the source for Yakkl's user documentation is
stored. For information on writing user documentation, see
[Yakkl's general user guide documentation](../documentation/user.html).
