...
from scripts.lib.yakkl_tools import run_as_root, ENDC, WARNING, \
...
VAR_DIR_PATH = os.path.join(YAKKL_PATH, 'var')

is_travis = 'TRAVIS' in os.environ
is_circleci = 'CIRCLECI' in os.environ

if not os.path.exists(os.path.join(YAKKL_PATH, ".git")):
    print(FAIL + "Error: No Yakkl git repository present!" + ENDC)
    print("To setup the Yakkl development environment, you should clone the code")
    print("from GitHub, rather than using a Yakkl production release tarball.")
    sys.exit(1)
...
if ram_gb < 1.5:
    print("You have insufficient RAM (%s GB) to run the Yakkl development environment." % (
        round(ram_gb, 2),))
...
try:
    ...
    if os.path.exists(os.path.join(VAR_DIR_PATH, 'yakkl-test-symlink')):
        os.remove(os.path.join(VAR_DIR_PATH, 'yakkl-test-symlink'))
    os.symlink(
        os.path.join(YAKKL_PATH, 'README.md'),
        os.path.join(VAR_DIR_PATH, 'yakkl-test-symlink')
    )
    os.remove(os.path.join(VAR_DIR_PATH, 'yakkl-test-symlink'))
except OSError:
    ...
    print("  https://yakkl.readthedocs.io/en/latest/development/setup-vagrant.html#os-symlink-error")
    sys.exit(1)
...
if platform.architecture()[0] == '64bit':
    ...
    logging.critical("Only x86 is supported;"
                     " ask on yakkl.com if you want another architecture.")
...
if not (vendor in SUPPORTED_PLATFORMS and codename in SUPPORTED_PLATFORMS[vendor]):
    ...
    if codename == 'trusty':
        print()
        print("Ubuntu Trusty reached end-of-life upstream and is no longer a supported platform for Yakkl")
        if os.path.exists('/home/vagrant'):
            print("To upgrade, run `vagrant destroy`, and then recreate the Vagrant guest.\n")
            print("See: https://yakkl.readthedocs.io/en/latest/development/setup-vagrant.html")
    sys.exit(1)
...
def install_yum_deps(deps_to_install):
    ...
    run_as_root(["cp", "-a", "puppet/yakkl/files/postgresql/centos_pg_hba.conf", pg_hba_conf])
...
def main(options):
    ...
    activate_this = "/srv/yakkl-py3-venv/bin/activate_this.py"
    provision_inner = os.path.join(YAKKL_PATH, "tools", "lib", "provision_inner.py")
...
if __name__ == "__main__":
    description = ("Provision script to install Yakkl")
