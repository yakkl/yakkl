# Creates a Droplet on Digital Ocean for remote Yakkl development.
...
# to create a Yakkl developer environment. Requires Python 3.
#
# Requires python-digitalocean library:
# https://github.com/koalalorenzo/python-digitalocean
#
# Also requires Digital Ocean team membership for Yakkl and api token:
...
parser = argparse.ArgumentParser(description='Create a Yakkl devopment VM Digital Ocean droplet.')
parser.add_argument("username", help="Github username for whom you want to create a Yakkl dev droplet")
...
def fork_exists(username):
    # type: (str) -> bool
    print("Checking to see GitHub user has forked yakkl/yakkl...")
    apiurl_fork = "https://api.github.com/repos/{0}/yakkl".format(username)
    try:
        ...
        print(err)
        print("Has user {0} forked yakkl/yakkl?".format(username))
        sys.exit(1)
...
def set_user_data(username, userkeys):
    ...
    setup_repo = """\
cd /home/yakkldev/{1} && git remote add origin https://github.com/{0}/{1}.git && git fetch origin"""

    server_repo_setup = setup_repo.format(username, "yakkl")
    python_api_repo_setup = setup_repo.format(username, "python-yakkl-api")
    ...
    runcmd:
      - su -c '{1}' yakkldev
      - su -c 'git clean -f' yakkldev
      - su -c '{2}' yakkldev
      - su -c 'git clean -f' yakkldev
      - su -c 'git config --global core.editor nano' yakkldev
      - su -c 'git config --global pull.rebase true' yakkldev
    power_state:
     mode: reboot
     condition: True
...
""".format(username))
    ...
    print("See [Developing remotely](https://yakkl.readthedocs.io/en/latest/development/remote.html) "
          ...
          "[Git & GitHub Guide](https://yakkl.readthedocs.io/en/latest/git/index.html) "
          "to learn how to use Git with Yakkl.\n")
    print("Note that this droplet will automatically be deleted after a month of inactivity. "
          "If you are leaving Yakkl for more than a few weeks, we recommend pushing all of your "
...
if __name__ == '__main__':
    ...
    print("Creating Yakkl developer environment for GitHub user {0}...".format(args.username))
    ...
    # now make sure the user has forked yakkl/yakkl
