...
if options.interface is None:
    ...
    if user_name in ["vagrant", "yakkldev"]:
        ...
        # servers using username "yakkldev".
...
from scripts.lib.yakkl_tools import WARNING, ENDC
...
cmds = [['./manage.py', 'runserver'] +
        ...
        ['env', 'PGHOST=127.0.0.1',  # Force password authentication using .pgpass
         './puppet/yakkl/files/postgresql/process_fts_updates'],
        ['./manage.py', 'deliver_scheduled_messages'],
        ['/srv/yakkl-thumbor-venv/bin/thumbor', '-c', './zthumbor/thumbor.conf',
         '-p', '%s' % (thumbor_port,)]]
if options.test:
...
print("Starting Yakkl services on ports: web proxy: {},".format(proxy_port),
