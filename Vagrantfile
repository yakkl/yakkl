...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  # The Yakkl development environment runs on 9991 on the guest.
  host_port = 9991
  ...
  if (/darwin/ =~ RUBY_PLATFORM) != nil
    config.vm.synced_folder ".", "/srv/yakkl", type: "nfs",
        linux__nfs_options: ['rw']
    config.vm.network "private_network", type: "dhcp"
  else
    config.vm.synced_folder ".", "/srv/yakkl"
  end

  vagrant_config_file = ENV['HOME'] + "/.yakkl-vagrant-config"
  ...
  if Vagrant.has_plugin?("vagrant-proxyconf")
    ...
    puts 'You have specified value for proxy in ~/.yakkl-vagrant-config file but did not ' \
...
# Set the MOTD on the system to have Yakkl instructions
sudo ln -nsf /srv/yakkl/tools/setup/dev-motd /etc/update-motd.d/99-yakkl-dev
...
# Check if the yakkl directory is writable
if [ ! -w /srv/yakkl ]; then
    echo "The vagrant user is unable to write to the yakkl directory."
    echo "To fix this, run the following commands on the host machine:"
    # sudo is required since our uid is not 1000
    echo '    vagrant halt -f'
    echo '    rm -rf /PATH/TO/YAKKL/CLONE/.vagrant'
    echo '    sudo chown -R 1000:$(id -g) /PATH/TO/YAKKL/CLONE'
    echo "Replace /PATH/TO/YAKKL/CLONE with the path to where yakkl code is cloned."
    echo "You can resume setting up your vagrant environment by running:"
    echo "    vagrant up"
    exit 1
fi
# Provision the development environment
ln -nsf /srv/yakkl ~/yakkl
/srv/yakkl/tools/provision

# Run any custom provision hooks the user has configured
if [ -f /srv/yakkl/tools/custom_provision ]; then
    chmod +x /srv/yakkl/tools/custom_provision
    /srv/yakkl/tools/custom_provision
fi
SCRIPT
