...
class TestStatsEndpoint(YakklTestCase):
    def test_stats(self) -> None:
        ...
        self.assert_in_response("Yakkl analytics for", result)
    ...
    def test_stats_for_realm(self) -> None:
        ...
        result = self.client_get('/stats/realm/yakkl/')
...
class TestGetChartData(YakklTestCase):
    def setUp(self) -> None:
    ...
    def test_get_chart_data_for_realm(self) -> None:
        ...
        result = self.client_get('/json/analytics/chart_data/realm/yakkl/',
                                 {'chart_name': 'number_of_humans'})
...
class TestSupportEndpoint(YakklTestCase):
    def test_search(self) -> None:
        def check_hamlet_user_query_result(result: HttpResponse) -> None:
            self.assert_in_success_response(['<span class="label">user</span>\n', '<h3>King Hamlet</h3>',
                                             '<b>Email</b>: hamlet@yakkl.com', '<b>Is active</b>: True<br>',
                                             '<b>Admins</b>: iago@yakkl.com\n',
                                             'class="copy-button" data-admin-emails="iago@yakkl.com"'
                                             ], result)

        def check_yakkl_realm_query_result(result: HttpResponse) -> None:
            yakkl_realm = get_realm("yakkl")
            self.assert_in_success_response(['<input type="hidden" name="realm_id" value="%s"' % (yakkl_realm.id,),
                                             'Yakkl Dev</h3>',
        ...
        result = self.client_get("/activity/support", {"q": "hamlet@yakkl.com"})
        check_hamlet_user_query_result(result)
        check_yakkl_realm_query_result(result)
        ...
        with self.settings(REALM_HOSTS={'yakkl': 'localhost'}):
            result = self.client_get("/activity/support", {"q": "http://localhost"})
            check_yakkl_realm_query_result(result)

        result = self.client_get("/activity/support", {"q": "hamlet@yakkl.com, lear"})
        check_hamlet_user_query_result(result)
        check_yakkl_realm_query_result(result)
        check_lear_realm_query_result(result)

        result = self.client_get("/activity/support", {"q": "lear, Hamlet <hamlet@yakkl.com>"})
        check_hamlet_user_query_result(result)
        check_yakkl_realm_query_result(result)
        check_lear_realm_query_result(result)
    ...
    def test_change_plan_type(self) -> None:
        ...
        with mock.patch("analytics.views.do_change_plan_type") as m:
            ...
            m.assert_called_once_with(get_realm("yakkl"), 2)
            self.assert_in_success_response(["Plan type of Yakkl Dev changed from self hosted to limited"], result)
...
class TestGetChartDataHelpers(YakklTestCase):
...
class TestTimeRange(YakklTestCase):
...
class TestMapArrays(YakklTestCase):
    def test_map_arrays(self) -> None:
        a = {'desktop app 1.0': [1, 2, 3],
             ...
             'YakklGitHubWebhook': [7, 7, 9],
             'YakklAndroid': [64, 63, 65]}
