...
from zerver.decorator import require_server_admin, require_server_admin_api, \
    to_non_negative_int, to_utc_datetime, yakkl_login_required, require_non_guest_user
...
def stats(request: HttpRequest) -> HttpResponse:
    ...
    if request.user.is_guest:
        # TODO: Make @yakkl_login_required pass the UserProfile so we
...
def stats_for_remote_realm(request: HttpRequest, remote_server_id: str,
    ...
    server = RemoteYakklServer.objects.get(id=remote_server_id)
...
def get_chart_data(request: HttpRequest, user_profile: UserProfile, chart_name: str=REQ(),
                   ...
                   server: Optional[RemoteYakklServer]=None) -> HttpResponse:
    if for_installation:
        if remote:
    ...
    id_value = {
        InstallationCount: -1,
        RealmCount: realm.id,
        RemoteInstallationCount: cast(RemoteYakklServer, server).id if server is not None else None,
...
def client_label_map(name: str) -> str:
    ...
    if name == "YakklElectron":
        return "Desktop app"
    if name == "YakklAndroid":
        return "Old Android app"
    if name == "YakkliOS":
        return "Old iOS app"
    if name == "YakklMobile":
        return "Mobile app"
    if name in ["YakklPython", "API: Python"]:
        return "Python API"
    if name.startswith("Yakkl") and name.endswith("Webhook"):
        return name[len("Yakkl"):-len("Webhook")] + " webhook"
    return name
...
def get_realm_day_counts() -> Dict[str, Dict[str, str]]:
    query = '''
        ...
        and
            c.name not in ('zephyr_mirror', 'YakklMonitoring')
        group by
            r.string_id,
            age
        order by
...
def ad_hoc_queries() -> List[Dict[str, str]]:
    ...
    for mobile_type in ['Android', 'YakkliOS']:
    ...
    query = '''
        ...
        where
            ...
            and client.name not in ('Android', 'YakkliOS')
            and client.name not like 'test: Yakkl%%'
            )
        or
            query like '%%external%%'
    ...
    query = '''
        ...
        select
            ...
            push_user_count,
            max_end_time
        from zilencer_remoteyakklserver rserver
...
def realm_user_summary_table(all_records: List[QuerySet],
    ...
    for email, user_summary in user_records.items():
        ...
        row_class = ''
        for field in ['use', 'send', 'pointer', 'desktop', 'YakkliOS', 'Android']:
