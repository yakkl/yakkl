...
// Yakkl modification: Remove numbers from this pattern.
block.bullet = /(?:[*+-])/;
block.item = /^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/;
...
Lexer.prototype.token = function(src, top, bq) {
  ...
  while (src) {
    ...
    // def
    // We disable definition style links in Yakkl.
...
inline.yakkl = merge({}, inline.breaks, {
  emoji: /^:([A-Za-z0-9_\-\+]+?):/,
  unicodeemoji: RegExp('^(\ud83c[\udd00-\udfff]|\ud83d[\udc00-\ude4f]|' +
                       '\ud83d[\ude80-\udeff]|\ud83e[\udd00-\uddff]|' +
                       '[\u2000-\u206F]|[\u2300-\u27BF]|[\u2B00-\u2BFF]|' +
...
function InlineLexer(links, options) {
  ...
  if (this.options.yakkl) {
    this.rules = inline.yakkl;
  } else {
    if (this.options.gfm) {
