...
exports.render_bots = function () {
    ...
    _.each(all_bots_for_current_user, function (elem) {
        add_bot_row({
            ...
            yakklrc: 'yakklrc', // Most browsers do not allow filename starting with `.`
        });
...
exports.generate_yakklrc_uri = function (bot_id) {
    var bot = bot_data.get(bot_id);
    var data;
    var token;
    // For outgoing webhooks, include the token in the yakklrc.
    // It's needed for authenticating to the Botserver.
    if (bot.bot_type === 3) {
        token = bot_data.get_services(bot_id)[0].token;
    }
    data = exports.generate_yakklrc_content(bot.email, bot.api_key, token);
    return exports.encode_yakklrc_as_uri(data);
};

exports.encode_yakklrc_as_uri = function (yakklrc) {
    return "data:application/octet-stream;charset=utf-8," + encodeURIComponent(yakklrc);
};

exports.generate_yakklrc_content = function (email, api_key, token) {
    return "[api]" +
           "\nemail=" + email +
           "\nkey=" + api_key +
...
exports.set_up = function () {
    ...
    $("#active_bots_list").on("click", "a.download_bot_yakklrc", function () {
        var bot_info = $(this).closest(".bot-information-box").find(".bot_info");
        var bot_id = bot_info.attr("data-user-id");
        $(this).attr("href", exports.generate_yakklrc_uri(bot_id));
    });

    new ClipboardJS('#copy_yakklrc', {
        text: function (trigger) {
            ...
            var data = exports.generate_yakklrc_content(email.trim(), api_key.trim());
            return data;
        },
    });
