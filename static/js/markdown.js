// This contains Yakkl's frontend markdown implementation; see
...
// Docs: https://yakkl.readthedocs.io/en/latest/subsystems/markdown.html
...
exports.set_realm_filters = function (realm_filters) {
    ...
    marked.InlineLexer.rules.yakkl.realm_filters = marked_rules;
};
...
exports.initialize = function () {
    ...
    marked.InlineLexer.rules.yakkl.strong = /^\*\*([\s\S]+?)\*\*(?!\*)/;

    // Make sure <del> syntax matches the backend processor
    marked.InlineLexer.rules.yakkl.del = /^(?!<\~)\~\~([^~]+)\~\~(?!\~)/;

    // Disable _emphasis_ (keeping *emphasis*)
    // Text inside ** must start and end with a word character
    // to prevent mis-parsing things like "char **x = (char **)y"
    marked.InlineLexer.rules.yakkl.em = /^\*(?!\s+)((?:\*\*|[\s\S])+?)((?:[\S]))\*(?!\*)/;

    // Disable autolink as (a) it is not used in our backend and (b) it interferes with @mentions
    disable_markdown_regex(marked.InlineLexer.rules.yakkl, 'autolink');
