...
exports.content_typeahead_selected = function (item, event) {
    ...
    if (this.completing === 'emoji') {
        ...
        if (user_groups.is_user_group(item)) {
            beginning += '@*' + item.name + '* ';
            $(document).trigger('usermention_completed.yakkl', {user_group: item});
        } else {
            var mention_text = people.get_mention_syntax(item.full_name, item.user_id, is_silent);
            beginning += mention_text + ' ';
            $(document).trigger('usermention_completed.yakkl', {mentioned: item, is_silent: is_silent});
        }
    } else if (this.completing === 'slash') {
        ...
        $(document).trigger('streamname_completed.yakkl', {stream: item});
    } else if (this.completing === 'syntax') {
