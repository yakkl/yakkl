// Read https://yakkl.readthedocs.io/en/latest/subsystems/hashchange-system.html
var reload = (function () {
...
function do_reload_app(send_after_reload, save_pointer, save_narrow, save_compose, message) {
    ...
    // skipping requested reloads), which can leave the Yakkl app in a
...
exports.initiate = function (options) {
    ...
    compose_done_handler = function () {
        ...
        $(document).off('compose_canceled.yakkl compose_finished.yakkl',
                        compose_done_handler);
        $(document).on('compose_started.yakkl', compose_started_handler);
    };
    compose_started_handler = function () {
        idle_control.cancel();
        idle_control = $(document).idle({idle: composing_timeout,
                                         onIdle: reload_from_idle});
        $(document).off('compose_started.yakkl', compose_started_handler);
        $(document).on('compose_canceled.yakkl compose_finished.yakkl',
                       compose_done_handler);
    };
