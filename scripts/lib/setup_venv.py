...
import subprocess
import sys
from scripts.lib.yakkl_tools import run, run_as_root, ENDC, WARNING
...
VENV_CACHE_PATH = "/srv/yakkl-venv-cache"

if 'TRAVIS' in os.environ:
    # In Travis CI, we don't have root access
    VENV_CACHE_PATH = "/home/travis/yakkl-venv-cache"
...
def install_venv_deps(pip, requirements_file):
    # type: (str, str) -> None
    pip_requirements = os.path.join(YAKKL_PATH, "requirements", "pip.txt")
...
def setup_virtualenv(target_venv_path, requirements_file, virtualenv_args=None, patch_activate_script=False):
    ...
    path = os.path.join(YAKKL_PATH, 'scripts', 'lib', 'hash_reqs.py')
