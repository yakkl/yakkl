...
from scripts.lib.yakkl_tools import \
    get_environment, get_recent_deployments, parse_cache_script_args, \
    purge_unused_caches

ENV = get_environment()
NODE_MODULES_CACHE_PATH = "/srv/yakkl-npm-cache"
if ENV == "travis":
    NODE_MODULES_CACHE_PATH = os.path.join(os.environ["HOME"], "yakkl-npm-cache")
    try:
        subprocess.check_output(["/home/travis/yakkl-yarn/bin/yarn", '--version'])
    except OSError:
...
def get_caches_in_use(threshold_days):
    # type: (int) -> Set[str]
    setups_to_check = set([YAKKL_PATH, ])
    caches_in_use = set()
    ...
    if ENV == "dev":
        ...
        CURRENT_CACHE = os.path.dirname(os.path.realpath(os.path.join(YAKKL_PATH, "node_modules")))
...
if __name__ == "__main__":
    args = parse_cache_script_args("This script cleans unused yakkl npm caches.")
    main(args)
