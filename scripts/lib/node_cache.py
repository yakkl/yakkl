...
from scripts.lib.yakkl_tools import subprocess_text_output, run
...
if 'TRAVIS' in os.environ:
    ...
    YAKKL_SRV_PATH = "/home/travis"


NODE_MODULES_CACHE_PATH = os.path.join(YAKKL_SRV_PATH, 'yakkl-npm-cache')
YARN_BIN = os.path.join(YAKKL_SRV_PATH, 'yakkl-yarn/bin/yarn')
YARN_PACKAGE_JSON = os.path.join(YAKKL_SRV_PATH, 'yakkl-yarn/package.json')
...
def do_yarn_install(target_path, yarn_args, success_stamp, stdout=None, stderr=None,
    ...
    if copy_modules:
        ...
        cd_exec = os.path.join(YAKKL_PATH, "scripts/lib/cd_exec")
