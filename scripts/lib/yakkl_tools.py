...
DEPLOYMENTS_DIR = "/home/yakkl/deployments"
...
def get_deployment_version(extract_path):
    ...
    for item in os.listdir(extract_path):
        ...
        if item.startswith('yakkl-server') and os.path.isdir(item_path):
            with open(os.path.join(item_path, 'version.py')) as f:
                result = re.search('YAKKL_VERSION = "(.*)"', f.read())
                if result:
...
def get_yakkl_pwent() -> pwd.struct_passwd:
    ...
    # `yakkl` user as that's the correct value in production.
    return pwd.getpwnam("yakkl")

def su_to_yakkl(save_suid=False):
    # type: (bool) -> None
    """Warning: su_to_yakkl assumes that the yakkl checkout is owned by
    the yakkl user (or whatever normal user is running the Yakkl
    installation).  It should never be run from the installer or other
    production contexts before /home/yakkl/deployments/current is
    created."""
    pwent = get_yakkl_pwent()
...
def get_dev_uuid_var_path(create_if_missing=False):
    # type: (bool) -> str
    yakkl_path = get_deploy_root()
    uuid_path = os.path.join(os.path.realpath(os.path.dirname(yakkl_path)), ".yakkl-dev-uuid")
    if os.path.exists(uuid_path):
        with open(uuid_path) as f:
            yakkl_uuid = f.read().strip()
    else:
        if create_if_missing:
            yakkl_uuid = str(uuid.uuid4())
            # We need root access here, since the path will be under /srv/ in the
            # development environment.
            run_as_root(["sh", "-c", 'echo "$1" > "$2"', "-",
                         yakkl_uuid, uuid_path])
        else:
            raise AssertionError("Missing UUID file; please run tools/provision!")

    result_path = os.path.join(yakkl_path, "var", yakkl_uuid)
...
def log_management_command(cmd, log_path):
    ...
    logger = logging.getLogger("yakkl.management")
...
def get_recent_deployments(threshold_days):
    ...
    # including `/root/yakkl` directory if it exists.
    recent = set()
    ...
    if os.path.exists("/root/yakkl"):
        recent.add("/root/yakkl")
    return recent
...
def generate_sha1sum_emoji(yakkl_path):
    # type: (str) -> str
    YAKKL_EMOJI_DIR = os.path.join(yakkl_path, 'tools', 'setup', 'emoji')
    sha = hashlib.sha1()

    filenames = ['emoji_map.json', 'build_emoji', 'emoji_setup_utils.py', 'emoji_names.py']

    for filename in filenames:
        file_path = os.path.join(YAKKL_EMOJI_DIR, filename)
    ...
    PACKAGE_FILE_PATH = os.path.join(yakkl_path, 'package.json')
...
def assert_not_running_as_root() -> None:
    ...
    if is_root():
        pwent = get_yakkl_pwent()
        msg = ("{shortname} should not be run as root. Use `su {user}` to switch to the 'yakkl'\n"
