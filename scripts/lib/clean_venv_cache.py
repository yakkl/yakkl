...
ENV = get_environment()
VENV_CACHE_DIR = '/srv/yakkl-venv-cache'
if ENV == "travis":
    VENV_CACHE_DIR = os.path.join(os.environ["HOME"], "yakkl-venv-cache")
...
def get_caches_in_use(threshold_days):
    ...
    def add_current_venv_cache(venv_name: str) -> None:
        CACHE_SYMLINK = os.path.join(os.path.dirname(YAKKL_PATH), venv_name)
    ...
    if ENV == "dev":
        add_current_venv_cache("yakkl-py3-venv")
        add_current_venv_cache("yakkl-thumbor-venv")
...
if __name__ == "__main__":
    args = parse_cache_script_args("This script cleans unused yakkl venv caches.")
    main(args)
