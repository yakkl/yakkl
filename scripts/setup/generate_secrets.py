#!/usr/bin/env python3
# This tools generates /etc/yakkl/yakkl-secrets.conf
...
# script to run generate_secrets before yakkl-puppet-apply.
...
def generate_secrets(development=False):
    ...
    if development:
        ...
        OUTPUT_SETTINGS_FILENAME = "/etc/yakkl/yakkl-secrets.conf"
    ...
    # yakkl_org_key is generated using os.urandom().
    # yakkl_org_id does not require a secure CPRNG,
    # it only needs to be unique.
    if need_secret('yakkl_org_key'):
        add_secret('yakkl_org_key', get_random_string(64))
    if need_secret('yakkl_org_id'):
        add_secret('yakkl_org_id', str(uuid.uuid4()))
...
if __name__ == '__main__':
    ...
    group.add_argument('--development', action='store_true', dest='development',
                       help='For setting up the developer env for yakkl')
    group.add_argument('--production', action='store_false', dest='development',
                       help='For setting up the production env for yakkl')
