# Documented in https://yakkl.readthedocs.io/en/latest/subsystems/queuing.html
...
from yakkl_bots.lib import extract_query_without_mention
    ...
    def consume(self, data: Dict[str, Any]) -> None:
        ...
        if settings.MAILCHIMP_API_KEY and settings.PRODUCTION:
            endpoint = "https://%s.api.mailchimp.com/3.0/lists/%s/members" % \
                       (settings.MAILCHIMP_API_KEY.split('-')[1], settings.YAKKL_FRIENDS_LIST_ID)
            params = dict(data)
            del params['user_id']
            params['list_id'] = settings.YAKKL_FRIENDS_LIST_ID
...
class MissedMessageSendingWorker(EmailSendingWorker):  # nocoverage
    ...
    first.
    """
    # TODO: yakkl-1.8: Delete code related to missedmessage_email_senders queue.
    pass
...
class MessageSenderWorker(QueueProcessingWorker):
    ...
    def consume(self, event: Mapping[str, Any]) -> None:
        ...
        environ = {
            ...
            'SERVER_PORT': 9993,
            'SERVER_PROTOCOL': 'YAKKL_SOCKET/1.0',
            'wsgi.version': (1, 0),
            'wsgi.input': StringIO(),
...
class TestWorker(QueueProcessingWorker):
    ...
    def consume(self, event: Mapping[str, Any]) -> None:  # nocoverage
        fn = settings.YAKKL_WORKER_TEST_FILE
