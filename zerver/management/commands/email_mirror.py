...
"""
Forward messages sent to the configured email gateway to Yakkl.

For yakkl.com, messages to that address go to the Inbox of emailgateway@yakkl.com.
Yakkl voyager configurations will differ.

Messages meant for Yakkl have a special recipient form of

    <stream name>+<regenerable stream token>@streams.yakkl.com

This pattern is configurable via the EMAIL_GATEWAY_PATTERN settings.py
variable.

Run this in a cronjob every N minutes if you have configured Yakkl to poll
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: str) -> None:
        ...
        if (not settings.EMAIL_GATEWAY_BOT or not settings.EMAIL_GATEWAY_LOGIN or
            ...
            raise CommandError("Please configure the Email Mirror Gateway in /etc/yakkl/, "
