...
class Command(YakklBaseCommand):
    help = """Exports all data from a Yakkl realm

    This command exports all significant data from a Yakkl realm.  The
    result can be imported using the `./manage.py import` command.

    Things that are exported:
    * All user-accessible data in the Yakkl database (Messages,
    ...
    * RemoteYakklServer (Unlikely to be migrated)
    ...
    * Users will need to logout and re-login to the Yakkl desktop and
      mobile apps.  The apps now all have an option on the login page
      where you can specify which Yakkl server to use; your users
