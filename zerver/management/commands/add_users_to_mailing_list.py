...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Optional[str]) -> None:
        ...
        if options['list_id'] is None:
            try:
                if settings.YAKKL_FRIENDS_LIST_ID is None:
                    raise CommandError('YAKKL_FRIENDS_LIST_ID is None. Check your server settings file.')
                options['list_id'] = settings.YAKKL_FRIENDS_LIST_ID
            except AttributeError:
                raise CommandError('Please supply a MailChimp List ID to --list-id, or add a '
                                   'YAKKL_FRIENDS_LIST_ID to your server settings file.')
