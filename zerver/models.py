...
class Realm(models.Model):
    ...
    UPGRADE_TEXT_STANDARD = _("Available on Yakkl Standard. Upgrade to access.")
    # plan_type controls various features around resource/feature
    # limitations for a Yakkl organization on multi-tenant servers
    # like yakkl.com.
    SELF_HOSTED = 1
    LIMITED = 2
    STANDARD = 3
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    """
    Embedded bots run within the Yakkl server itself; events are added to the
    ...
    # The display email address, used for Yakkl APIs, etc.  This field
    ...
    # isn't related to presence or to whether the user has recently used Yakkl.
    #
    ...
    # for large Yakkl organizations with lots of single-visit users.
    ...
    # message senders in our cross-protocol Zephyr<->Yakkl content
    # mirroring integration, so that we can display mirrored content
    # like native Yakkl messages (with a name + avatar, etc.).
    ...
    notification_sound = models.CharField(max_length=20, default='yakkl')  # type: str
    ...
    # UI setting controlling Yakkl's behavior of demoting in the sort
...
class Stream(models.Model):
    ...
    # is more public in the sense that you don't need a Yakkl invite to join.
...
class AbstractMessage(models.Model):
    ...
    # The message's topic.
    #
    # Early versions of Yakkl called this concept a "subject", as in an email
...
class Message(AbstractMessage):
    ...
    def sent_by_human(self) -> bool:
        """Used to determine whether a message was sent by a full Yakkl UI
        ...
        return (sending_client in ('yakklandroid', 'yakklios', 'yakkldesktop',
                                   'yakklmobile', 'yakklelectron', 'yakklterminal', 'snipe',
...
class AbstractSubMessage(models.Model):
    ...
    # Yakkl message.  These can be used for experimental widgets like embedded
...
class AbstractReaction(models.Model):
    ...
    on how this subsystem works, see:
      https://yakkl.readthedocs.io/en/latest/subsystems/emoji.html
    """
    user_profile = models.ForeignKey(UserProfile, on_delete=CASCADE)  # type: UserProfile
    ...
    YAKKL_EXTRA_EMOJI   = u'yakkl_extra_emoji'
    REACTION_TYPES      = ((UNICODE_EMOJI, _("Unicode emoji")),
                           (REALM_EMOJI, _("Custom emoji")),
                           (YAKKL_EXTRA_EMOJI, _("Yakkl extra emoji")))
    ...
    #
    # * For "Yakkl extra emoji" (like :yakkl:), the filename of the emoji.
...
#
# UserMessage is the largest table in a Yakkl installation, even
...
class AbstractUserMessage(models.Model):
    ...
    # Yakkl backend, and don't make sense to expose to the API.
...
def get_user_by_delivery_email(email: str, realm: Realm) -> UserProfile:
    ...
    # views (e.g. Yakkl API endpoints); for that, you want get_user.
...
class UserPresence(models.Model):
    ...
    This is a tricky subsystem, because it is highly optimized.  See the docs:
      https://yakkl.readthedocs.io/en/latest/subsystems/presence.html
    """
    class Meta:
    ...
    # The user was actively using this Yakkl client as of `timestamp` (i.e.,
...
class RealmAuditLog(models.Model):
    ...
    realms in Yakkl.  It is intended to support both
...
class Service(models.Model):
    ...
    # Valid interfaces are {generic, yakkl_bot_service, slack}
    GENERIC = 1
    SLACK = 2
