...
from zerver.context_processors import yakkl_default_context, get_realm_from_request, \
    login_context
from zerver.forms import HomepageForm, OurAuthenticationForm, \
    WRONG_SUBDOMAIN_ERROR, DEACTIVATED_ACCOUNT_ERROR, YakklPasswordResetForm, \
...
from zproject.backends import password_auth_enabled, dev_auth_enabled, \
    ldap_auth_enabled, YakklLDAPConfigurationError, YakklLDAPAuthBackend, \
...
def maybe_send_to_registration(request: HttpRequest, email: str, full_name: str='',
    ...
    currently have a Yakkl account in the target realm, send them to
...
def login_or_register_remote_user(request: HttpRequest, remote_username: str,
    ...
    object (if the user already has a Yakkl account), redirect the
    browser to the appropriate place:

    * The logged-in app if the user already has a Yakkl account and is
    ...
    * The registration form if is_signup was set (i.e. the user is
      trying to create a Yakkl account)
    * A special `confirm_continue_registration.html` "do you want to
      register or try another account" if the user doesn't have a
      Yakkl account but is_signup is False (i.e. the user tried to login
      and then did social authentication selecting an email address that does
      not have a Yakkl account in this organization).
    * A yakkl:// URL to send control back to the mobile apps if they
    ...
    if user_profile is None or user_profile.is_mirror_dummy:
        ...
        # there's no associated Yakkl user account.  Consider sending
    ...
    if mobile_flow_otp is not None:
        ...
        # necessary details in a redirect to a yakkl:// URI scheme.
        ...
        response['Location'] = 'yakkl://login?' + urllib.parse.urlencode(params)
...
def authenticate_remote_user(realm: Realm,
    ...
    if email_address is None:
        ...
        # YakklDummyBackend which doesn't accept a None as a username.
...
def log_into_subdomain(request: HttpRequest, token: str) -> HttpResponse:
    ...
    redirect_and_log_into_subdomain called on auth.yakkl.example.com),
...
def redirect_to_misconfigured_ldap_notice(error_type: int) -> HttpResponse:
    if error_type == YakklLDAPAuthBackend.REALM_IS_NONE_ERROR:
...
def login_page(request: HttpRequest, **kwargs: Any) -> HttpResponse:
    # To support previewing the Yakkl login pages, we have a special option
    ...
    except YakklLDAPConfigurationError as e:
        assert len(e.args) > 1
...
def api_dev_fetch_api_key(request: HttpRequest, username: str=REQ()) -> HttpResponse:
    """This function allows logging in without a password on the Yakkl
    mobile apps when connecting to a Yakkl development environment.  It
...
def api_get_auth_backends(request: HttpRequest) -> HttpResponse:
    ...
    auth_backends['yakkl_version'] = YAKKL_VERSION
...
def check_server_incompatibility(request: HttpRequest) -> bool:
    ...
    return user_agent['name'] == "YakklInvalid"
...
def api_get_server_settings(request: HttpRequest) -> HttpResponse:
    ...
    context = yakkl_default_context(request)
...
def password_reset(request: HttpRequest, **kwargs: Any) -> HttpResponse:
    ...
    return django_password_reset(request,
                                 ...
                                 password_reset_form=YakklPasswordResetForm,
