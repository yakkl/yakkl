...
class NarrowBuilder:
    ...
    def _by_search_tsearch(self, query: Query, operand: str,
                           maybe_negate: ConditionTransform) -> Query:
        tsquery = func.plainto_tsquery(literal("yakkl.english_us_search"), literal(operand))
        ts_locs_array = func.ts_match_locs_array
        query = query.column(ts_locs_array(literal("yakkl.english_us_search"),
...
def same_realm_zephyr_user(user_profile: UserProfile, email: str) -> bool:
    ...
    # not existing in Yakkl and we may need to make a stub Zephyr
...
def same_realm_jabber_user(user_profile: UserProfile, email: str) -> bool:
    ...
    # Yakkl users, this is where you would do any translation.
    domain = email_to_domain(email)
