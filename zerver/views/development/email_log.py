...
YAKKL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../')
...
def generate_all_emails(request: HttpRequest) -> HttpResponse:
    ...
    registered_email = "hamlet@yakkl.com"
    unregistered_email_1 = "new-person@yakkl.com"
    unregistered_email_2 = "new-person-2@yakkl.com"
    realm = get_realm("yakkl")
    other_realm = Realm.objects.exclude(string_id='yakkl').first()
    ...
    result = client.patch('/json/settings',
                          urllib.parse.urlencode({'email': 'hamlets-new@yakkl.com'}),
                          **host_kwargs)
    assert result.status_code == 200
    ...
    enqueue_welcome_emails(get_user_by_delivery_email("iago@yakkl.com", realm), realm_creation=True)
