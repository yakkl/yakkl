...
from zerver.context_processors import yakkl_default_context

def add_api_uri_context(context: Dict[str, Any], request: HttpRequest) -> None:
    context.update(yakkl_default_context(request))
    ...
    if (subdomain != Realm.SUBDOMAIN_FOR_ROOT_DOMAIN
        ...
        display_subdomain = 'yourYakklDomain'
...
def add_integrations_open_graph_context(context: Dict[str, Any], request: HttpRequest) -> None:
    ...
    description = ('Yakkl comes with over a hundred native integrations out of the box, '
                   'and integrates with Zapier, IFTTT, and Hubot to provide hundreds more. '
                   'Connect the apps you use everyday to Yakkl.')

    if path_name in INTEGRATIONS:
        integration = INTEGRATIONS[path_name]
        context['OPEN_GRAPH_TITLE'] = 'Connect {name} to Yakkl'.format(name=integration.display_name)
        context['OPEN_GRAPH_DESCRIPTION'] = description

    elif path_name in CATEGORIES:
        category = CATEGORIES[path_name]
        context['OPEN_GRAPH_TITLE'] = 'Connect your {category} tools to Yakkl'.format(category=category)
        context['OPEN_GRAPH_DESCRIPTION'] = description

    elif path_name == 'integrations':
        context['OPEN_GRAPH_TITLE'] = 'Connect the tools you use to Yakkl'
        context['OPEN_GRAPH_DESCRIPTION'] = description
