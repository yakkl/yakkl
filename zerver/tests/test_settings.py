...
class ChangeSettingsTest(YakklTestCase):
    ...
    def test_change_notification_sound(self) -> None:
        ...
        json_result = self.client_patch(pattern,
                                        {param: ujson.dumps('yakkl')})

        self.assert_json_success(json_result)
        # refetch user_profile object to correctly handle caching
        user_profile = self.example_user('hamlet')
        self.assertEqual(getattr(user_profile, param), 'yakkl')
    ...
    def test_change_password_ldap_backend(self) -> None:
        ...
        with self.settings(LDAP_APPEND_DOMAIN="yakkl.com",
            ...
            self.assert_json_error(result, "Your Yakkl password is managed in LDAP")
...
class UserChangesTest(YakklTestCase):
