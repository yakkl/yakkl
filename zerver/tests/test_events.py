# -*- coding: utf-8 -*-
# See https://yakkl.readthedocs.io/en/latest/subsystems/events-system.html for
...
class LogEventsTest(YakklTestCase):
...
class EventsEndpointTest(YakklTestCase):
...
class GetEventsTest(YakklTestCase):
...
class EventsRegisterTest(YakklTestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def do_test(self, action: Callable[[], Any], event_types: Optional[List[str]]=None,
        ...
        See also https://yakkl.readthedocs.io/en/latest/subsystems/events-system.html#testing
    ...
    def test_send_message_to_existing_recipient(self) -> None:
        ...
        self.do_test(
            lambda: self.send_stream_message("cordelia@yakkl.com", "Verona", "hello 2"),
            state_change_expected=True,
        )
    ...
    def test_invite_user_event(self) -> None:
        ...
        events = self.do_test(
            lambda: do_invite_users(self.user_profile, ["foo@yakkl.com"], streams, False),
            state_change_expected=False,
        )
        error = schema_checker('events[0]', events[0])
        self.assert_on_error(error)
    ...
    def test_revoke_user_invite_event(self) -> None:
        ...
        for stream_name in ["Denmark", "Verona"]:
            streams.append(get_stream(stream_name, self.user_profile.realm))
        do_invite_users(self.user_profile, ["foo@yakkl.com"], streams, False)
    ...
    def test_invitation_accept_invite_event(self) -> None:
        ...
        do_invite_users(self.user_profile, ["foo@yakkl.com"], streams, False)
        prereg_users = PreregistrationUser.objects.get(email="foo@yakkl.com")

        events = self.do_test(
            lambda: do_create_user('foo@yakkl.com', 'password', self.user_profile.realm,
    ...
    def test_presence_events_multiple_clients(self) -> None:
        schema_checker_android = self.check_events_dict([
            ...
            ('server_timestamp', check_float),
            ('presence', check_dict_only([
                ('YakklAndroid/1.0', check_dict_only([
                    ('status', equals('idle')),
                    ('timestamp', check_int),
                    ('client', check_string),
                    ('pushable', check_bool),
        ...
        self.api_post(self.user_profile.email, "/api/v1/users/me/presence", {'status': 'idle'},
                      HTTP_USER_AGENT="YakklAndroid/1.0")
        self.do_test(lambda: do_update_user_presence(
            self.user_profile, get_client("website"), timezone_now(), UserPresence.ACTIVE))
        events = self.do_test(lambda: do_update_user_presence(
            self.user_profile, get_client("YakklAndroid/1.0"), timezone_now(), UserPresence.IDLE))
    ...
    def test_register_events(self) -> None:
        ...
        events = self.do_test(lambda: self.register("test1@yakkl.com", "test1"))
        self.assert_length(events, 1)
        error = realm_user_add_checker('events[0]', events[0])
        self.assert_on_error(error)
        new_user_profile = get_user_by_delivery_email("test1@yakkl.com", self.user_profile.realm)
        self.assertEqual(new_user_profile.email, "test1@yakkl.com")
    ...
    def test_register_events_email_address_visibility(self) -> None:
        ...
        self.assert_on_error(error)
        new_user_profile = get_user_by_delivery_email("test1@yakkl.com", self.user_profile.realm)
        self.assertEqual(new_user_profile.email, "user%s@yakkl.testserver" % (new_user_profile.id,))
    ...
    def test_change_user_delivery_email_email_address_visibilty_admins(self) -> None:
        ...
        action = lambda: do_change_user_delivery_email(self.user_profile, 'newhamlet@yakkl.com')
    ...
    def test_rename_stream(self) -> None:
        ...
        schema_checker = check_dict([
            ...
            ('message', check_dict([
                ...
                ('content_type', equals('text/html')),
                ('sender_email', equals('notification-bot@yakkl.com')),
                ('sender_id', check_int),
    ...
    def test_do_delete_message_stream(self) -> None:
        ...
        msg_id = self.send_stream_message("hamlet@yakkl.com", "Verona")
    ...
    def test_add_attachment(self) -> None:
        ...
        body = "First message ...[yakkl.txt](http://localhost:9991" + data['uri'] + ")"
        events = self.do_test(
...
class FetchInitialStateDataTest(YakklTestCase):
...
class GetUnreadMsgsTest(YakklTestCase):
...
class ClientDescriptorsTest(YakklTestCase):
...
class FetchQueriesTest(YakklTestCase):
...
class TestEventsRegisterAllPublicStreamsDefaults(YakklTestCase):
    def setUp(self) -> None:
        self.user_profile = self.example_user('hamlet')
...
class TestEventsRegisterNarrowDefaults(YakklTestCase):
    def setUp(self) -> None:
        self.user_profile = self.example_user('hamlet')
        self.email = self.user_profile.email
...
class TestGetRawUserDataSystemBotRealm(YakklTestCase):
    def test_get_raw_user_data_on_system_bot_realm(self) -> None:
        result = get_raw_user_data(get_realm("yakklinternal"), True)
