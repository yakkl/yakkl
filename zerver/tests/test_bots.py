...
from yakkl_bots.custom_exceptions import ConfigValidationError

class BotTest(YakklTestCase, UploadSerializeMixin):
    ...
    def test_bot_domain(self) -> None:
        ...
        self.create_bot()
        self.assertTrue(UserProfile.objects.filter(email='hambot-bot@yakkl.testserver').exists())
        ...
        realm = get_realm('yakkl')
        self.assertEqual(realm.get_bot_domain(), 'yakkl.testserver')

    def deactivate_bot(self) -> None:
        email = 'hambot-bot@yakkl.testserver'
    ...
    def test_add_bot(self) -> None:
        ...
        self.assertEqual(
            dict(
                type='realm_bot',
                op='add',
                bot=dict(email='hambot-bot@yakkl.testserver',
                         user_id=bot.id,
        ...
        bots = [m for m in members if m['email'] == 'hambot-bot@yakkl.testserver']
    ...
    def test_bot_add_subscription(self) -> None:
        ...
        # A bot
        bot_request_data = {
            'principals': '["hambot-bot@yakkl.testserver"]'
        }
    ...
    def test_add_bot_with_default_events_register_stream(self) -> None:
        bot_email = 'hambot-bot@yakkl.testserver'
        bot_realm = get_realm('yakkl')
    ...
    def test_patch_bot_owner_bad_user_id(self) -> None:
        ...
        email = 'hambot-bot@yakkl.testserver'
        profile = get_user('hambot-bot@yakkl.testserver', get_realm('yakkl'))
    ...
    def test_patch_bot_owner_a_bot(self) -> None:
        ...
        bot_info = {
            'bot_owner_id': self.get_bot_user('hamelbot-bot@yakkl.testserver').id,
        }
        email = 'hambot-bot@yakkl.testserver'
        result = self.client_patch("/json/bots/{}".format(self.get_bot_user(email).id), bot_info)
        self.assert_json_error(result, "Failed to change owner, bots can't own other bots")
        profile = get_user(email, get_realm('yakkl'))
        self.assertEqual(profile.bot_owner, self.example_user("hamlet"))
    ...
    @patch('yakkl_bots.bots.giphy.giphy.GiphyHandler.validate_config')
    def test_patch_bot_config_data(self, mock_validate_config: MagicMock) -> None:
        ...
        email = 'test-bot@yakkl.testserver'
        result = self.client_patch("/json/bots/{}".format(self.get_bot_user(email).id), bot_info)
        self.assert_json_success(result)
    ...
    def test_create_outgoing_webhook_bot(self, **extras: Any) -> None:
        ...
        bot_email = "outgoingservicebot-bot@yakkl.testserver"
        bot_realm = get_realm('yakkl')
    ...
    def test_get_bot_handler(self) -> None:
        ...
        self.assertEqual(str(type(test_bot_handler)), "<class 'yakkl_bots.bots.converter.converter.ConverterHandler'>")
    ...
    def test_create_embedded_bot(self, **extras: Any) -> None:
        ...
        self.create_test_bot(short_name='embeddedservicebot',
                             ...
                             **extras)
        bot_email = "embeddedservicebot-bot@yakkl.testserver"
        bot_realm = get_realm('yakkl')
        bot = get_user(bot_email, bot_realm)
        services = get_bot_services(bot.id)
        service = services[0]
    ...
    def test_create_embedded_bot_with_invalid_config_value(self, **extras: Any) -> None:
        ...
        bot_info.update(extras)
        with patch('yakkl_bots.bots.giphy.giphy.GiphyHandler.validate_config', side_effect=ConfigValidationError):
            result = self.client_post("/json/bots", bot_info)
        self.assert_json_error(result, 'Invalid configuration data!')

    def test_is_cross_realm_bot_email(self) -> None:
        self.assertTrue(is_cross_realm_bot_email("notification-bot@yakkl.com"))
        self.assertTrue(is_cross_realm_bot_email("notification-BOT@yakkl.com"))
        self.assertFalse(is_cross_realm_bot_email("random-bot@yakkl.com"))

        with self.settings(CROSS_REALM_BOT_EMAILS={"random-bot@yakkl.com"}):
            self.assertTrue(is_cross_realm_bot_email("random-bot@yakkl.com"))
            self.assertFalse(is_cross_realm_bot_email("notification-bot@yakkl.com"))
