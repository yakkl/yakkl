...
from zerver.lib.management import YakklBaseCommand, CommandError, check_config
...
class TestCheckConfig(YakklTestCase):
    def test_check_config(self) -> None:
        with self.assertRaisesRegex(CommandError, "Error: You must set YAKKL_ADMINISTRATOR in /etc/yakkl/settings.py."):
            check_config()
        with self.settings(REQUIRED_SETTINGS=[('asdf', 'not asdf')]):
            with self.assertRaisesRegex(CommandError, "Error: You must set asdf in /etc/yakkl/settings.py."):
                check_config()
...
class TestYakklBaseCommand(YakklTestCase):
    def setUp(self) -> None:
        self.yakkl_realm = get_realm("yakkl")
        self.command = YakklBaseCommand()

    def test_get_client(self) -> None:
        self.assertEqual(self.command.get_client().name, "YakklServer")

    def test_get_realm(self) -> None:
        self.assertEqual(self.command.get_realm(dict(realm_id='yakkl')), self.yakkl_realm)
        self.assertEqual(self.command.get_realm(dict(realm_id=None)), None)
        self.assertEqual(self.command.get_realm(dict(realm_id=str(self.yakkl_realm.id))),
                         self.yakkl_realm)
    ...
    def test_get_user(self) -> None:
        ...
        self.assertEqual(self.command.get_user(email, self.yakkl_realm), user_profile)
    ...
    def test_get_users(self) -> None:
        ...
        user_profiles = self.get_users_sorted(dict(users=user_emails), self.yakkl_realm)
        ...
        user_profiles = self.get_users_sorted(dict(users=user_emails), None)
        self.assertEqual(user_profiles, expected_user_profiles)
        error_message = "The realm '%s' does not contain a user with email" % (self.yakkl_realm,)
        with self.assertRaisesRegex(CommandError, error_message):
            self.command.get_users(dict(users=user_emails), self.yakkl_realm)

        self.assertEqual(self.command.get_users(dict(users=self.example_email("iago")), self.yakkl_realm),
    ...
    def test_get_users_with_all_users_argument_enabled(self) -> None:
        user_emails = self.example_email("hamlet") + "," + self.example_email("iago")
        expected_user_profiles = [self.example_user("hamlet"), self.example_user("iago")]
        user_profiles = self.get_users_sorted(dict(users=user_emails, all_users=False), self.yakkl_realm)
        self.assertEqual(user_profiles, expected_user_profiles)
        ...
        expected_user_profiles = sorted(UserProfile.objects.filter(realm=self.yakkl_realm),
                                        key = lambda x: x.email)
        user_profiles = self.get_users_sorted(dict(users=None, all_users=True), self.yakkl_realm)
        self.assertEqual(user_profiles, expected_user_profiles)
...
class TestSendWebhookFixtureMessage(TestCase):
    ...
    def test_check_if_command_post_request_to_url_with_fixture(self,
        ...
        client.post.assert_called_once_with(self.url, "{}", content_type="application/json",
                                            HTTP_HOST="yakkl.testserver")

class TestGenerateRealmCreationLink(YakklTestCase):
    ...
    def test_generate_link_and_create_realm(self) -> None:
        ...
        self.assert_in_success_response([u"Create a new Yakkl organization"], result)
...
class TestCalculateFirstVisibleMessageID(YakklTestCase):
    ...
    def test_check_if_command_calls_maybe_update_first_visible_message_id(self) -> None:
        ...
        with patch(func_name) as m:
            call_command(self.COMMAND_NAME, "--realm=yakkl", "--lookback-hours=30")
        m.assert_called_with(get_realm("yakkl"), 30)
...
class TestPasswordRestEmail(YakklTestCase):
...
class TestRealmReactivationEmail(YakklTestCase):
    ...
    def test_if_realm_not_deactivated(self) -> None:
        ...
        with self.assertRaisesRegex(CommandError, "The realm %s is already active." % (realm.name,)):
            call_command(self.COMMAND_NAME, "--realm=yakkl")

class TestSendToEmailMirror(YakklTestCase):
    ...
    def test_stream_option(self) -> None:
        ...
        stream_id = get_stream("Denmark2", get_realm("yakkl")).id
        self.assertEqual(message.recipient.type, Recipient.STREAM)
        self.assertEqual(message.recipient.type_id, stream_id)

class TestConvertMattermostData(YakklTestCase):
...
class TestInvoicePlans(YakklTestCase):
...
class TestExport(YakklTestCase):
    COMMAND_NAME = 'export'
    ...
    def test_command_with_consented_message_id(self) -> None:
        ...
        with patch("zerver.management.commands.export.export_realm_wrapper") as m:
            call_command(self.COMMAND_NAME, "-r=yakkl", "--consent-message-id={}".format(message.id))
        ...
        with self.assertRaisesRegex(CommandError, "Message with given ID does not"):
            call_command(self.COMMAND_NAME, "-r=yakkl", "--consent-message-id=123456")
