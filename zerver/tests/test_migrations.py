# These are tests for Yakkl's database migrations.  System documented at:
#   https://yakkl.readthedocs.io/en/latest/subsystems/schema-migrations.html
#
# You can also read
...
class SubsNotificationSettingsTestCase(MigrationsTestCase):  # nocoverage
    ...
    def test_subs_migrated(self) -> None:
        ...
        iago = UserProfile.objects.get(email='iago@yakkl.com', realm=realm)
        stream_name = 'Denmark'
        ...
        # Yakkl ignores subscription's notification related settings for PMs so don't migrate them.
        iago_recipient = Recipient.objects.get(type=self.RECIPIENT_PERSONAL, type_id=iago.id)
        iago_sub = Subscription.objects.get(user_profile=iago, recipient=iago_recipient)
