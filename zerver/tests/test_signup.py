...
class RedirectAndLogIntoSubdomainTestCase(YakklTestCase):
...
class DeactivationNoticeTestCase(YakklTestCase):
    ...
    def test_deactivation_notice_when_deactivated(self) -> None:
        ...
        result = self.client_get('/accounts/deactivated/')
        self.assertIn("Yakkl Dev, has been deactivated.", result.content.decode())

class AddNewUserHistoryTest(YakklTestCase):
...
class InitialPasswordTest(YakklTestCase):
...
class PasswordResetTest(YakklTestCase):
    """
    ...
    def test_password_reset(self) -> None:
        ...
        self.assertIn("Yakkl Account Security", from_email)
    ...
    @override_settings(AUTHENTICATION_BACKENDS=('zproject.backends.YakklLDAPAuthBackend',
                                                'zproject.backends.YakklDummyBackend'))
...
class LoginTest(YakklTestCase):
    ...
    def test_login_nonexist_user(self) -> None:
        result = self.login_with_return("xxx@yakkl.com", "xxx")
        self.assertEqual(result.status_code, 200)
    ...
    def test_login_wrong_subdomain(self) -> None:
        ...
        self.assertEqual(result.status_code, 200)
        self.assert_in_response("Your Yakkl account is not a member of the "
    ...
    def test_login_invalid_subdomain(self) -> None:
        result = self.login_with_return(self.example_email("hamlet"), "xxx",
                                        subdomain="invalid")
        self.assertEqual(result.status_code, 404)
        self.assert_in_response("There is no Yakkl organization hosted at this subdomain.", result)
        self.assert_logged_in_user_id(None)
    ...
    def test_register_deactivated_partway_through(self) -> None:
        ...
        result = self.submit_reg_form_for_user(email, "abcd1234", subdomain="yakkl")
        self.assertEqual(result.status_code, 302)
        self.assertEqual('/accounts/deactivated/', result.url)
    ...
    def test_login_deactivated_realm(self) -> None:
        ...
        result = self.login_with_return(self.example_email("hamlet"), subdomain="yakkl")
        self.assertEqual(result.status_code, 302)
        self.assertEqual('/accounts/deactivated/', result.url)
    ...
    def test_login_page_redirects_logged_in_user(self) -> None:
        ...
        self.assertEqual(response["Location"], "http://yakkl.testserver")
    ...
    def test_yakkl_default_context_does_not_load_inline_previews(self) -> None:
        realm = get_realm("yakkl")
...
class InviteUserBase(YakklTestCase):
    ...
    def invite(self, invitee_emails: str, stream_names: List[str], body: str='',
        ...
        """
        Invites the specified users to Yakkl with the specified streams.
...
class InviteUserTest(InviteUserBase):
    ...
    def test_invite_user_signup_initial_history(self) -> None:
        ...
        self.assertFalse(invitee_profile.is_realm_admin)
        # Test that exactly 2 new Yakkl messages were sent, both notifications.
        ...
        self.assertEqual(second_msg.sender.email, "notification-bot@yakkl.com")
        self.assertTrue(second_msg.content.startswith("alice_yakkl.com <`alice@yakkl.com`> accepted your"))

        # The second, from welcome-bot to the user who was invited.
        third_msg = last_3_messages[2]
        self.assertEqual(third_msg.sender.email, "welcome-bot@yakkl.com")
        self.assertTrue(third_msg.content.startswith("Hello, and welcome to Yakkl!"))
    ...
    def test_multi_user_invite(self) -> None:
        ...
        self.assert_json_success(self.invite(
            """bob-test@yakkl.com,     carol-test@yakkl.com,
            dave-test@yakkl.com


earl-test@yakkl.com""", ["Denmark"]))
        for user in ("bob", "carol", "dave", "earl"):
            self.assertTrue(find_key_by_email("%s-test@yakkl.com" % (user,)))
        self.check_sent_emails(["bob-test@yakkl.com", "carol-test@yakkl.com",
                                "dave-test@yakkl.com", "earl-test@yakkl.com"])
    ...
    def test_max_invites_model(self) -> None:
        ...
        realm.max_invites = 3
        realm.save()
        self.assertEqual(get_realm("yakkl").max_invites, 3)
    ...
    def test_invite_too_many_users(self) -> None:
        ...
        self.login(self.example_email("iago"))
        invitee_emails = "1@yakkl.com, 2@yakkl.com"
        self.invite(invitee_emails, ["Denmark"])
        invitee_emails = ", ".join([str(i) for i in range(get_realm("yakkl").max_invites - 1)])
        self.assert_json_error(self.invite(invitee_emails, ["Denmark"]),
                               "You do not have enough remaining invites. "
                               "Please contact yakkl-admin@example.com to have your limit raised. "
    ...
    def test_invalid_stream(self) -> None:
        ...
        """
        self.login(self.example_email("hamlet"))
        self.assert_json_error(self.invite("iago-test@yakkl.com", ["NotARealStream"]),
                               "Stream does not exist with id: {}. No invites were sent.".format(self.INVALID_STREAM_ID))
        self.check_sent_emails([])

    def test_invite_existing_user(self) -> None:
        """
        If you invite an address already using Yakkl, no invitation is sent.
        """
        self.login(self.example_email("hamlet"))
    ...
    def test_invite_some_existing_some_new(self) -> None:
        ...
        """
        self.login(self.example_email("hamlet"))
        existing = [self.example_email("hamlet"), u"othello@yakkl.com"]
        new = [u"foo-test@yakkl.com", u"bar-test@yakkl.com"]
        invitee_emails = "\n".join(existing + new)
        self.assert_json_error(self.invite(invitee_emails, ["Denmark"]),
                               "Some of those addresses are already using Yakkl, \
so we didn't send them an invitation. We did send invitations to everyone else!")
        ...
        prereg_user = PreregistrationUser.objects.get(email='foo-test@yakkl.com')
        self.assertEqual(prereg_user.email, 'foo-test@yakkl.com')
    ...
    def test_invite_outside_domain_in_closed_realm(self) -> None:
        ...
        """
        yakkl_realm = get_realm("yakkl")
        yakkl_realm.emails_restricted_to_domains = True
        yakkl_realm.save()
    ...
    def test_invite_using_disposable_email(self) -> None:
        ...
        """
        yakkl_realm = get_realm("yakkl")
        yakkl_realm.emails_restricted_to_domains = False
        yakkl_realm.disallow_disposable_email_addresses = True
        yakkl_realm.save()
    ...
    def test_disposable_emails_before_closing(self) -> None:
        ...
        yakkl_realm = get_realm("yakkl")
        yakkl_realm.emails_restricted_to_domains = False
        yakkl_realm.disallow_disposable_email_addresses = False
        yakkl_realm.save()
    ...
    def test_invite_with_email_containing_plus_before_closing(self) -> None:
        ...
        self.login(self.example_email("hamlet"))
        external_address = "foo+label@yakkl.com"
        ...
        self.assertEqual(result.status_code, 200)
        self.assert_in_response("Yakkl Dev, does not allow signups using emails\n        that contains +", result)

    def test_invalid_email_check_after_confirming_email(self) -> None:
        self.login(self.example_email("hamlet"))
        email = "test@yakkl.com"
    ...
    def test_invitation_reminder_email(self) -> None:
        ...
        # All users belong to yakkl realm
...
class InvitationsTestCase(InviteUserBase):
    def test_successful_get_open_invitations(self) -> None:
        ...
        prereg_user_one = PreregistrationUser(email="TestOne@yakkl.com", referred_by=user_profile)
        prereg_user_one.save()
        expired_datetime = timezone_now() - datetime.timedelta(days=(days_to_activate+1))
        prereg_user_two = PreregistrationUser(email="TestTwo@yakkl.com", referred_by=user_profile)
        prereg_user_two.save()
        PreregistrationUser.objects.filter(id=prereg_user_two.id).update(invited_at=expired_datetime)
        prereg_user_three = PreregistrationUser(email="TestThree@yakkl.com",
        ...
        self.assertEqual(result.status_code, 200)
        self.assert_in_success_response(["TestOne@yakkl.com", "hamlet@yakkl.com"], result)
        self.assert_not_in_success_response(["TestTwo@yakkl.com", "TestThree@yakkl.com", "othello@yakkl.com"], result)
    ...
    def test_delete_multiuse_invite(self) -> None:
        ...
        yakkl_realm = get_realm("yakkl")
        multiuse_invite = MultiuseInvite.objects.create(referred_by=self.example_user("hamlet"), realm=yakkl_realm)
        create_confirmation_link(multiuse_invite, yakkl_realm.host, Confirmation.MULTIUSE_INVITE)
    ...
    def test_successful_resend_invitation(self) -> None:
        ...
        """
        self.login(self.example_email("iago"))
        invitee = "resend_me@yakkl.com"

        self.assert_json_success(self.invite(invitee, ['Denmark']))
        prereg_user = PreregistrationUser.objects.get(email=invitee)

        # Verify and then clear from the outbox the original invite email
        self.check_sent_emails([invitee], custom_from_name="Yakkl")
        from django.core.mail import outbox
        outbox.pop()
    ...
    def test_accessing_invites_in_another_realm(self) -> None:
        invitor = UserProfile.objects.exclude(realm=get_realm('yakkl')).first()
...
class InviteeEmailsParserTests(TestCase):
    def setUp(self) -> None:
        self.email1 = "email1@yakkl.com"
        self.email2 = "email2@yakkl.com"
        self.email3 = "email3@yakkl.com"
...
class MultiuseInviteTest(YakklTestCase):
    def setUp(self) -> None:
        self.realm = get_realm('yakkl')
...
class EmailUnsubscribeTests(YakklTestCase):
...
class RealmCreationTest(YakklTestCase):
    ...
    def check_able_to_create_realm(self, email: str) -> None:
        ...
        self.assertEqual(result.status_code, 302)
        self.assertTrue(result["Location"].startswith('http://yakkltest.testserver/accounts/login/subdomain/'))
    ...
    def test_create_realm_existing_email(self) -> None:
        self.check_able_to_create_realm("hamlet@yakkl.com")

    def test_create_realm_as_system_bot(self) -> None:
        result = self.client_post('/new/', {'email': 'notification-bot@yakkl.com'})
        self.assertEqual(result.status_code, 200)
        self.assert_in_response('notification-bot@yakkl.com is an email address reserved', result)
    ...
    def test_subdomain_check_api(self) -> None:
        result = self.client_get("/json/realm/subdomain/yakkl")
    ...
    def test_subdomain_check_management_command(self) -> None:
        ...
        check_subdomain_available('yakkl', from_management_command=True)
...
class UserSignUpTest(InviteUserBase):
    ...
    def test_signup_system_bot(self) -> None:
        email = "notification-bot@yakkl.com"
    ...
    def test_signup_without_user_settings_from_another_realm(self) -> None:
        ...
        # Make an account in the Yakkl realm, but we're not copying from there.
        hamlet_in_yakkl = get_user(self.example_email("hamlet"), get_realm("yakkl"))
        hamlet_in_yakkl.left_side_userlist = True
        hamlet_in_yakkl.default_language = "de"
        hamlet_in_yakkl.emojiset = "twitter"
        hamlet_in_yakkl.high_contrast_mode = True
        hamlet_in_yakkl.enter_sends = True
        hamlet_in_yakkl.tutorial_status = UserProfile.TUTORIAL_FINISHED
        hamlet_in_yakkl.save()
    ...
    def test_signup_with_user_settings_from_another_realm(self) -> None:
        ...
        hamlet_in_yakkl = get_user(self.example_email("hamlet"), yakkl_realm)
        hamlet_in_yakkl.left_side_userlist = True
        hamlet_in_yakkl.default_language = "de"
        hamlet_in_yakkl.emojiset = "twitter"
        hamlet_in_yakkl.high_contrast_mode = True
        ...
        result = self.client_post(
            ...
            {'password': password,
             ...
             'from_confirmation': '1'},
            subdomain=subdomain)
        self.assert_in_success_response(["Import settings from existing Yakkl account",
                                         "selected >\n                            Yakkl Dev",
                                         "We just need you to do one last thing."], result)

        result = self.submit_reg_form_for_user(email, password, source_realm="yakkl",
                                               HTTP_HOST=subdomain + ".testserver")
        ...
        yakkl_path_id = avatar_disk_path(hamlet_in_yakkl)
        hamlet_path_id = avatar_disk_path(hamlet_in_yakkl)
        self.assertEqual(open(yakkl_path_id, "rb").read(), open(hamlet_path_id, "rb").read())
    ...
    def test_access_signup_page_in_root_domain_without_realm(self) -> None:
        ...
        self.assert_in_success_response(["Find your Yakkl accounts"], result)
    ...
    def test_ldap_registration_from_confirmation(self) -> None:
        ...
        mock_directory = {
            'uid=newuser,ou=users,dc=yakkl,dc=com': {
    ...
    def test_ldap_registration_end_to_end(self) -> None:
        ...
        with self.settings(
            ...
            self.assertEqual(result.status_code, 302)
            self.assertEqual(result.url, "/accounts/login/?email=newuser%40yakkl.com")
    ...
    def test_ldap_registration_multiple_realms(self) -> None:
        ...
        with self.settings(
            ...
            user_profile = UserProfile.objects.get(email=email, realm=get_realm('yakkl'))
    ...
    def test_signup_with_ldap_and_email_enabled_using_email(self) -> None:
        ...
        # successfully create an account with a password in the Yakkl
        # database.
        with self.settings(
                POPULATE_PROFILE_VIA_LDAP=True,
                # Important: This doesn't match the new user
            ...
            with patch('zerver.views.registration.logging.warning') as mock_warning:
                result = self.submit_reg_form_for_user(
                    ...
                    HTTP_HOST=subdomain + ".testserver")
                self.assertEqual(result.status_code, 200)
                mock_warning.assert_called_once_with("New account email newuser@yakkl.com could not be found in LDAP")
...
class DeactivateUserTest(YakklTestCase):
...
class TestLoginPage(YakklTestCase):
...
class TestFindMyTeam(YakklTestCase):
    def test_template(self) -> None:
        result = self.client_get('/accounts/find/')
        self.assertIn("Find your Yakkl accounts", result.content.decode('utf8'))

    def test_result(self) -> None:
        result = self.client_post('/accounts/find/',
                                  dict(emails="iago@yakkl.com,cordelia@yakkl.com"))
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result.url, "/accounts/find/?emails=iago%40yakkl.com%2Ccordelia%40yakkl.com")
        result = self.client_get(result.url)
        ...
        from django.core.mail import outbox
        # 3 = 1 + 2 -- Cordelia gets an email each for the "yakkl" and "lear" realms.
        self.assertEqual(len(outbox), 3)

    def test_find_team_ignore_invalid_email(self) -> None:
        result = self.client_post('/accounts/find/',
                                  dict(emails="iago@yakkl.com,invalid_email@yakkl.com"))
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result.url, "/accounts/find/?emails=iago%40yakkl.com%2Cinvalid_email%40yakkl.com")
        result = self.client_get(result.url)
        content = result.content.decode('utf8')
        self.assertIn("Emails sent! You will only receive emails", content)
        self.assertIn(self.example_email("iago"), content)
    ...
    def test_find_team_one_email(self) -> None:
        ...
        result = self.client_post('/accounts/find/', data)
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result.url, '/accounts/find/?emails=hamlet%40yakkl.com')
        from django.core.mail import outbox
        self.assertEqual(len(outbox), 1)
    ...
    def test_find_team_deactivated_realm(self) -> None:
        do_deactivate_realm(get_realm("yakkl"))
        data = {'emails': self.example_email("hamlet")}
        result = self.client_post('/accounts/find/', data)
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result.url, '/accounts/find/?emails=hamlet%40yakkl.com')
    ...
    def test_find_team_bot_email(self) -> None:
        ...
        result = self.client_post('/accounts/find/', data)
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result.url, '/accounts/find/?emails=webhook-bot%40yakkl.com')
        from django.core.mail import outbox
        self.assertEqual(len(outbox), 0)

    def test_find_team_more_than_ten_emails(self) -> None:
        data = {'emails': ','.join(['hamlet-{}@yakkl.com'.format(i) for i in range(11)])}
        result = self.client_post('/accounts/find/', data)
        self.assertEqual(result.status_code, 200)
...
class ConfirmationKeyTest(YakklTestCase):
...
class MobileAuthOTPTest(YakklTestCase):
...
class FollowupEmailTest(YakklTestCase):
...
class NoReplyEmailTest(YakklTestCase):
...
class TwoFactorAuthTest(YakklTestCase):
...
class RealmRedirectTest(YakklTestCase):
    def test_realm_redirect_without_next_param(self) -> None:
        ...
        self.assert_in_success_response(["We couldn&#39;t find that Yakkl organization."], result)

    def test_realm_redirect_with_next_param(self) -> None:
        result = self.client_get("/accounts/go/?next=billing")
        self.assert_in_success_response(["Enter your organization's Yakkl URL", 'action="/accounts/go/?next=billing"'], result)
