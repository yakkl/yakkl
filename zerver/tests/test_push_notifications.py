...
from zerver.lib.test_classes import (
    TestCase, YakklTestCase,
)

from zilencer.models import RemoteYakklServer, RemotePushDeviceToken, \
    RemoteRealmCount, RemoteInstallationCount
from django.utils.timezone import now

ZERVER_DIR = os.path.dirname(os.path.dirname(__file__))

class BouncerTestCase(YakklTestCase):
    def setUp(self) -> None:
        self.server_uuid = "1234-abcd"
        server = RemoteYakklServer(uuid=self.server_uuid,
    ...
    def tearDown(self) -> None:
        RemoteYakklServer.objects.filter(uuid=self.server_uuid).delete()
        super().tearDown()
...
class PushBouncerNotificationTest(BouncerTestCase):
    ...
    def test_unregister_remote_push_user_params(self) -> None:
        ...
        result = self.api_post(self.example_email("hamlet"), endpoint, {'token': token,
                                                                        ...
                                                                        'token_kind': token_kind},
                               subdomain="")
        self.assert_json_error(result, "Must validate with valid Yakkl server API key")
    ...
    def test_register_remote_push_user_paramas(self) -> None:
        ...
        result = self.api_post(self.server_uuid, endpoint, {'user_id': user_id,
                                                            'token_kind': token_kind,
                                                            'token': token})
        self.assert_json_error(result, "Yakkl server auth failure: key does not match role 1234-abcd",
                               status_code=401)
        ...
        self.assert_json_error(result, "Yakkl server auth failure: 5678-efgh is not registered",
                               status_code=401)
    ...
    def test_push_bouncer_api(self, mock_request: Any) -> None:
        ...
        email = user.email
        self.login(email)
        server = RemoteYakklServer.objects.get(uuid=self.server_uuid)
...
class AnalyticsBouncerTest(BouncerTestCase):
    ...
    def test_analytics_api(self, mock_request: Any) -> None:
        ...
        InstallationCount.objects.create(
            ...
            # https://github.com/yakkl/yakkl/issues/12362
    ...
    def setup_apns_tokens(self) -> None:
        ...
        for token in self.tokens:
            PushDeviceToken.objects.create(
                ...
                ios_app_id=settings.YAKKL_IOS_APP_ID)
...
class HandlePushNotificationTest(PushNotificationTest):
        ...
        with self.settings(PUSH_NOTIFICATION_BOUNCER_URL=True), \
            ...
            mock_send_android.assert_called_with(
                ...
                {},
                {
                    'gcm': True,
                    'event': 'remove',
                    'yakkl_message_ids': str(message.id),
                    'yakkl_message_id': message.id,
                },
                {'priority': 'normal'})
    ...
    def test_user_message_does_not_exist(self) -> None:
        ...
        message_id = self.send_stream_message("iago@yakkl.com", "public_stream", "test")
    ...
    def test_not_configured(self) -> None:
        with mock.patch('zerver.lib.push_notifications.get_apns_client') as mock_get, \
            ...
            mock_logging.warning.assert_called_once_with(
                ...
                "See https://yakkl.readthedocs.io/en/latest/production/mobile-push-notifications.html")
...
class TestSendNotificationsToBouncer(YakklTestCase):
...
class TestSendToPushBouncer(YakklTestCase):
    ...
    def test_400_error_invalid_server_key(self) -> None:
        from zerver.decorator import InvalidYakklServerError
        # This is the exception our decorator uses for an invalid Yakkl server
        error_obj = InvalidYakklServerError("testRole")
        ...
        self.assertEqual(str(exc.exception),
                         ...
                         'Yakkl server auth failure: testRole is not registered')
...
class TestPushApi(YakklTestCase):
...
class TestClearOnRead(YakklTestCase):
...
class TestReceivesNotificationsFunctions(YakklTestCase):
    def setUp(self) -> None:
...
class TestPushNotificationsContent(YakklTestCase):
...
class PushBouncerSignupTest(YakklTestCase):
    def test_push_signup_invalid_host(self) -> None:
        yakkl_org_id = str(uuid.uuid4())
        yakkl_org_key = get_random_string(64)
        request = dict(
            yakkl_org_id=yakkl_org_id,
            yakkl_org_key=yakkl_org_key,
    ...
    def test_push_signup_success(self) -> None:
        ...
        result = self.client_post("/api/v1/remotes/server/register", request)
        self.assert_json_success(result)
        server = RemoteYakklServer.objects.get(uuid=yakkl_org_id)
        ...
        request = dict(
            yakkl_org_id=yakkl_org_id,
            yakkl_org_key=yakkl_org_key,
            hostname="yakkl.example.com",
            contact_email="server-admin@example.com",
        )
        result = self.client_post("/api/v1/remotes/server/register", request)
        self.assert_json_success(result)
        server = RemoteYakklServer.objects.get(uuid=yakkl_org_id)
        self.assertEqual(server.hostname, "yakkl.example.com")
        self.assertEqual(server.contact_email, "server-admin@example.com")
        ...
        self.assertEqual(server.hostname, "example.com")
        self.assertEqual(server.contact_email, "server-admin@example.com")
        yakkl_org_key = request["new_org_key"]
        self.assertEqual(server.api_key, yakkl_org_key)
        ...
        result = self.client_post("/api/v1/remotes/server/register", request)
        self.assert_json_error(result, "Yakkl server auth failure: key does not match role %s" %
                               (yakkl_org_id,))
