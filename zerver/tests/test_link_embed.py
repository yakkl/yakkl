...
class OembedTestCase(YakklTestCase):
...
class OpenGraphParserTestCase(YakklTestCase):
...
class GenericParserTestCase(YakklTestCase):
...
class PreviewTestCase(YakklTestCase):
    open_graph_html = """
          <html>
            <head>
    ...
    def test_get_link_embed_data(self) -> None:
        ...
        msg = self._send_message_with_test_org_url(sender_email='webhook-bot@yakkl.com',
    ...
    def test_inline_relative_url_embed_preview(self) -> None:
        ...
        with mock.patch('zerver.lib.actions.queue_json_publish') as patched:
            self.send_personal_message(
                ...
                self.example_email('cordelia'),
                content="http://yakkl.testserver/api/",
            )
            patched.assert_not_called()
    ...
    def test_youtube_url_title_replaces_url(self) -> None:
        ...
        mocked_data = {'title': 'Clearer Code at Scale - Static Types at Yakkl and Dropbox'}
        mocked_response = mock.Mock(side_effect=self.create_mock_response(url))
        with self.settings(TEST_SUITE=False, CACHES=TEST_CACHES):
            with mock.patch('requests.get', mocked_response):
