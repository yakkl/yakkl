...
class TestDigestEmailMessages(YakklTestCase):
    ...
    def test_no_email_digest_for_bots(self, mock_django_timezone: mock.MagicMock,
        ...
        # A Tuesday
        mock_django_timezone.return_value = datetime.datetime(year=2016, month=1, day=5)
        bot = do_create_user('some_bot@example.com', 'password', get_realm('yakkl'), 'some_bot', '',
    ...
    def test_new_stream_link(self, mock_django_timezone: mock.MagicMock) -> None:
        ...
        expected_html = "<a href='http://yakkl.testserver/#narrow/stream/{stream_id}-New-stream'>New stream</a>".format(stream_id=stream_id)
...
class TestDigestContentInBrowser(YakklTestCase):
    def test_get_digest_content_in_browser(self) -> None:
        self.login(self.example_email('hamlet'))
        result = self.client_get("/digest/")
        self.assert_in_success_response(["Click here to log in to Yakkl and catch up."], result)
