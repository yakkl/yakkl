...
from zerver.lib.test_classes import YakklTestCase, UploadSerializeMixin
...
from zerver.lib.upload import sanitize_name, S3UploadBackend, \
    ...
    YakklUploadBackend, MEDIUM_AVATAR_SIZE, resize_avatar, \
...
from scripts.lib.yakkl_tools import get_dev_uuid_var_path
...
class FileUploadTest(UploadSerializeMixin, YakklTestCase):
    ...
    def test_rest_endpoint(self) -> None:
        ...
        self.assertEqual(b"yakkl!", data)

        # Files uploaded through the API should be accesible via the web client
        self.login(self.example_email("hamlet"))
        self.assert_url_serves_contents_of_file(uri, b"yakkl!")
    ...
    # when yakkl is in DEVELOPMENT mode.
    def test_file_upload_authed(self) -> None:
        ...
        entry = Attachment.objects.get(file_name='yakkl.txt')
        self.assertEqual(entry.is_claimed(), False)

        self.subscribe(self.example_user("hamlet"), "Denmark")
        body = "First message ...[yakkl.txt](http://localhost:9991" + uri + ")"
        self.send_stream_message(self.example_email("hamlet"), "Denmark", body, "test")
        self.assertIn('title="yakkl.txt"', self.get_last_message().rendered_content)
    ...
    def test_delete_old_unclaimed_attachments(self) -> None:
        ...
        self.subscribe(self.example_user("hamlet"), "Denmark")
        body = "Some files here ...[yakkl.txt](http://localhost:9991/user_uploads/" + d1_path_id + ")"
        self.send_stream_message(self.example_email("hamlet"), "Denmark", body, "test")
    ...
    def test_attachment_url_without_upload(self) -> None:
        hamlet = self.example_user("hamlet")
        self.login(hamlet.email)
        body = "Test message ...[yakkl.txt](http://localhost:9991/user_uploads/%s/64/fake_path_id.txt)" % (
            hamlet.realm_id,)
        self.send_stream_message(self.example_email("hamlet"), "Denmark", body, "test")
    ...
    def test_multiple_claim_attachments(self) -> None:
        ...
        self.subscribe(self.example_user("hamlet"), "Denmark")
        body = "First message ...[yakkl.txt](http://localhost:9991/user_uploads/" + d1_path_id + ")"
        self.send_stream_message(self.example_email("hamlet"), "Denmark", body, "test")
        body = "Second message ...[yakkl.txt](http://localhost:9991/user_uploads/" + d1_path_id + ")"
        self.send_stream_message(self.example_email("hamlet"), "Denmark", body, "test")
    ...
    def test_multiple_claim_attachments_different_owners(self) -> None:
        ...
        body = "Illegal message ...[yakkl.txt](http://localhost:9991/user_uploads/" + d1_path_id + ")"
        ...
        body = "Third message ...[yakkl.txt](http://localhost:9991/user_uploads/" + d1_path_id + ")"
    ...
    def test_realm_quota(self) -> None:
        ...
        # The size of StringIO("yakkl!") is 6 bytes. Setting the size of
    ...
    def test_cross_realm_file_access(self) -> None:
        ...
        user2_email = 'test-og-bot@yakkl.com'
        ...
        create_user(user2_email, 'yakkl')
        create_user(user3_email, test_subdomain)

        # Send a message from @yakkl.com -> @uploadtest.example.com
        ...
        body = "First message ...[yakkl.txt](http://localhost:9991/user_uploads/" + fp_path_id + ")"
    ...
    def test_serve_local(self) -> None:
        ...
        check_xsend_links('yakkl.txt', 'yakkl.txt', "filename*=UTF-8''yakkl.txt")
        check_xsend_links('áéБД.txt', '%C3%A1%C3%A9%D0%91%D0%94.txt',
                          "filename*=UTF-8''%C3%A1%C3%A9%D0%91%D0%94.txt")
        check_xsend_links('yakkl.html', 'yakkl.html', "filename*=UTF-8''yakkl.html")
        check_xsend_links('yakkl.sh', 'yakkl.sh', "filename*=UTF-8''yakkl.sh")
        check_xsend_links('yakkl.jpeg', 'yakkl.jpeg')
        check_xsend_links('áéБД.pdf', '%C3%A1%C3%A9%D0%91%D0%94.pdf')
        check_xsend_links('yakkl', 'yakkl', "filename*=UTF-8''yakkl")

    def tearDown(self) -> None:
        destroy_uploads()


class AvatarTest(UploadSerializeMixin, YakklTestCase):
    ...
    def test_avatar_url(self) -> None:
        ...
        backend = LocalUploadBackend()  # type: YakklUploadBackend
    ...
    def test_get_gravatar_avatar(self) -> None:
        ...
        with self.settings(ENABLE_GRAVATAR=True):
            response = self.client_get("/avatar/cordelia@yakkl.com?foo=bar")
    ...
    def test_get_user_avatar(self) -> None:
        ...
        response = self.api_get(hamlet, "/avatar/cordelia@yakkl.com?foo=bar")
        redirect_url = response['Location']
        self.assertTrue(redirect_url.endswith(str(avatar_url(cordelia)) + '&foo=bar'))
        ...
        response = self.api_get(hamlet, "/avatar/welcome-bot@yakkl.com?foo=bar")
        redirect_url = response['Location']
    ...
    def test_get_user_avatar_medium(self) -> None:
        ...
        cordelia.avatar_source = UserProfile.AVATAR_FROM_USER
        cordelia.save()
        response = self.client_get("/avatar/cordelia@yakkl.com/medium?foo=bar")
        redirect_url = response['Location']
        ...
        response = self.api_get(hamlet, "/avatar/cordelia@yakkl.com/medium?foo=bar")
        redirect_url = response['Location']
        self.assertTrue(redirect_url.endswith(str(avatar_url(cordelia, True)) + '&foo=bar'))
    ...
    def test_non_valid_user_avatar(self) -> None:
        ...
        response = self.client_get("/avatar/nonexistent_user@yakkl.com?foo=bar")
        redirect_url = response['Location']
...
class EmojiTest(UploadSerializeMixin, YakklTestCase):
...
class RealmIconTest(UploadSerializeMixin, YakklTestCase):
...
class RealmLogoTest(UploadSerializeMixin, YakklTestCase):
    night = False
...
class LocalStorageTest(UploadSerializeMixin, YakklTestCase):

    def test_file_upload_local(self) -> None:
        user_profile = self.example_user('hamlet')
        uri = upload_message_file(u'dummy.txt', len(b'yakkl!'), u'text/plain', b'yakkl!', user_profile)
        ...
        self.assertEqual(len(b'yakkl!'), uploaded_file.size)
    ...
    def test_tarball_upload_and_deletion_local(self) -> None:
        ...
        expected_url = "http://yakkl.testserver/user_avatars/exports/{realm_id}/{random_name}/tarball.tar.gz".format(
            realm_id=user_profile.realm_id,
...
class S3Test(YakklTestCase):
    ...
    def test_file_upload_s3(self) -> None:
        ...
        self.assertEqual(b"yakkl!", content)
    ...
    def test_file_upload_s3_with_undefined_content_type(self) -> None:
        ...
        user_profile = self.example_user('hamlet')
        uri = upload_message_file(u'dummy.txt', len(b'yakkl!'), None, b'yakkl!', user_profile)

        path_id = re.sub('/user_uploads/', '', uri)
        self.assertEqual(b"yakkl!", bucket.get_key(path_id).get_contents_as_string())
        uploaded_file = Attachment.objects.get(owner=user_profile, path_id=path_id)
        self.assertEqual(len(b"yakkl!"), uploaded_file.size)
    ...
    def test_file_upload_authed(self) -> None:
        ...
        self.assertEqual(b"yakkl!", urllib.request.urlopen(redirect_url).read().strip())
...
class UploadTitleTests(TestCase):
    def test_upload_titles(self) -> None:
        ...
        self.assertEqual(url_filename("http://localhost:9991/user_uploads/%s/LUeQZUG5jxkagzVzp1Ox_amr/dummy.txt" % (
            yakkl_realm.id,)), "dummy.txt")
        self.assertEqual(url_filename("http://localhost:9991/user_uploads/%s/94/SzGYe0RFT-tEcOhQ6n-ZblFZ/yakkl.txt" % (
            yakkl_realm.id,)), "yakkl.txt")
        self.assertEqual(url_filename("https://yakkl.com/user_uploads/4142/LUeQZUG5jxkagzVzp1Ox_amr/pasted_image.png"), "pasted_image.png")
        self.assertEqual(url_filename("https://yakkl.com/integrations"), "https://yakkl.com/integrations")
...
class UploadSpaceTests(UploadSerializeMixin, YakklTestCase):
    def setUp(self) -> None:
...
class DecompressionBombTests(YakklTestCase):
    def setUp(self) -> None:
        self.test_urls = {
