...
class RetentionTestingBase(YakklTestCase):
...
class ArchiveMessagesTestingBase(RetentionTestingBase):
    def setUp(self) -> None:
        ...
        self._set_realm_message_retention_value(self.yakkl_realm, YAKKL_REALM_DAYS)
    ...
    def _send_cross_realm_personal_message(self) -> int:
        ...
        bot_email = 'notification-bot@yakkl.com'
        get_user_profile_by_email(bot_email)
        yakkl_user = self.example_user("hamlet")
        msg_id = internal_send_private_message(
            realm=self.yakkl_realm,
            sender=get_system_bot(bot_email),
            recipient_user=yakkl_user,
            content='test message',
        )
        assert msg_id is not None
        return msg_id

    def _make_expired_yakkl_messages(self, message_quantity: int) -> List[int]:
        msg_ids = list(Message.objects.order_by('id').filter(
                       sender__realm=self.yakkl_realm).values_list('id', flat=True)[3:3 + message_quantity])
        self._change_messages_pub_date(
            msg_ids,
            timezone_now() - timedelta(YAKKL_REALM_DAYS+1)
        )
    ...
    def _send_messages_with_attachments(self) -> Dict[str, int]:
        ...
        body = ("Some files here ...[yakkl.txt](http://localhost:9991/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/yakkl.txt)" +
        ...
        other_message_id = self.send_stream_message("othello@yakkl.com", "Denmark", body)
...
class TestArchiveMessagesGeneral(ArchiveMessagesTestingBase):
    ...
    def test_expired_messages_in_each_realm(self) -> None:
        ...
        # Change some Yakkl messages to be expired:
        expired_yakkl_msg_ids = list(Message.objects.order_by('id').filter(
            sender__realm=self.yakkl_realm).values_list('id', flat=True)[3:10])
        self._change_messages_pub_date(
            expired_yakkl_msg_ids,
            timezone_now() - timedelta(YAKKL_REALM_DAYS+1)
        )

        expired_msg_ids = expired_mit_msg_ids + expired_yakkl_msg_ids
    ...
    def test_expired_messages_in_one_realm(self) -> None:
        ...
        self._set_realm_message_retention_value(self.yakkl_realm, None)
        ...
        # Change some Yakkl messages pub_date, but the realm has no retention policy,
        # so they shouldn't get archived
        yakkl_msg_ids = list(Message.objects.order_by('id').filter(
            sender__realm=self.yakkl_realm).values_list('id', flat=True)[3:10])
        self._change_messages_pub_date(
            yakkl_msg_ids,
            timezone_now() - timedelta(YAKKL_REALM_DAYS+1)
    ...
    def test_different_stream_realm_policies(self) -> None:
        verona = get_stream("Verona", self.yakkl_realm)
        ...
        self._set_realm_message_retention_value(self.yakkl_realm, 1)
    ...
    def test_cross_realm_personal_message_archiving(self) -> None:
        ...
        self._change_messages_pub_date(msg_ids, timezone_now() - timedelta(YAKKL_REALM_DAYS+1))
    ...
    def test_archiving_interrupted(self) -> None:
        ...
        expired_msg_ids = self._make_expired_yakkl_messages(7)
        expired_usermsg_ids = self._get_usermessage_ids(expired_msg_ids)
    ...
    def test_archive_message_tool(self) -> None:
        ...
        # Change some Yakkl messages to be expired:
        expired_yakkl_msg_ids = self._make_expired_yakkl_messages(7)
        ...
        expired_msg_ids = expired_mit_msg_ids + expired_yakkl_msg_ids + [expired_crossrealm_msg_id]
        expired_usermsg_ids = self._get_usermessage_ids(expired_msg_ids)
...
class TestArchivingSubMessages(ArchiveMessagesTestingBase):
    def test_archiving_submessages(self) -> None:
        expired_msg_ids = self._make_expired_yakkl_messages(2)
...
class TestArchivingReactions(ArchiveMessagesTestingBase, EmojiReactionBase):
    def test_archiving_reactions(self) -> None:
        ...
        self.post_yakkl_reaction(expired_msg_ids[0], 'hamlet')
        self.post_yakkl_reaction(expired_msg_ids[0], 'cordelia')

        self.post_yakkl_reaction(expired_msg_ids[1], 'hamlet')
...
class MoveMessageToArchiveBase(RetentionTestingBase):
    def setUp(self) -> None:
        self.sender = 'hamlet@yakkl.com'
        self.recipient = 'cordelia@yakkl.com'
...
class MoveMessageToArchiveGeneral(MoveMessageToArchiveBase):
    ...
    def test_archiving_messages_with_attachment(self) -> None:
        ...
        body1 = """Some files here ...[yakkl.txt](
            http://localhost:9991/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/yakkl.txt)
    ...
    def test_archiving_message_with_shared_attachment(self) -> None:
        ...
        body = """Some files here ...[yakkl.txt](
            http://localhost:9991/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/yakkl.txt)
            http://localhost:9991/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/temp_file.py ....
            Some more.... http://localhost:9991/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/abc.py ...
            http://localhost:9991/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/new.py ....
...
class MoveMessageToArchiveWithReactions(MoveMessageToArchiveBase, EmojiReactionBase):
    def test_archiving_message_with_reactions(self) -> None:
        ...
        self.post_yakkl_reaction(msg_id, 'hamlet')
        self.post_yakkl_reaction(msg_id, 'cordelia')
...
class TestCleaningArchive(ArchiveMessagesTestingBase):
    def test_clean_archived_data(self) -> None:
        self._make_expired_yakkl_messages(7)
