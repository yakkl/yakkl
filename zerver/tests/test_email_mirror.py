...
class TestEncodeDecode(YakklTestCase):
    ...
    # https://github.com/yakkl/yakkl/issues/9840
    ...
    def test_decode_ignores_stream_name(self) -> None:
        stream = get_stream("Denmark", get_realm("yakkl"))
...
class TestGetMissedMessageToken(YakklTestCase):
...
class TestFilterFooter(YakklTestCase):
...
class TestStreamEmailMessagesSuccess(YakklTestCase):
...
class TestEmailMirrorMessagesWithAttachments(YakklTestCase):
...
class TestStreamEmailMessagesEmptyBody(YakklTestCase):
...
class TestMissedMessageEmailMessageTokenMissingData(YakklTestCase):
...
class TestMissedPersonalMessageEmailMessages(YakklTestCase):
...
class TestMissedHuddleMessageEmailMessages(YakklTestCase):
...
class TestMissedStreamMessageEmailMessages(YakklTestCase):
...
class TestEmptyGatewaySetting(YakklTestCase):
...
class TestReplyExtraction(YakklTestCase):
...
class TestScriptMTA(YakklTestCase):
...
class TestEmailMirrorTornadoView(YakklTestCase):
...
class TestStreamEmailMessagesSubjectStripping(YakklTestCase):
...
class TestContentTypeUnspecifiedCharset(YakklTestCase):
...
class TestEmailMirrorProcessMessageNoValidRecipient(YakklTestCase):
    def test_process_message_no_valid_recipient(self) -> None:
        ...
        incoming_valid_message['From'] = self.example_email('hamlet')
        incoming_valid_message['To'] = "address@wrongdomain, address@notyakkl"
        incoming_valid_message['Reply-to'] = self.example_email('othello')
...
class TestEmailMirrorLogAndReport(YakklTestCase):
