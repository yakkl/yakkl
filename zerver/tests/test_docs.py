...
class DocPageTest(YakklTestCase):
    ...
    def _test(self, url: str, expected_content: str, extra_strings: List[str]=[],
        ...
        with self.settings(ROOT_DOMAIN_LANDING_PAGE=True):
            ...
            if not doc_html_str:
                self.assert_in_success_response(['<meta name="description" content="Yakkl combines'], result)
    ...
    def test_doc_endpoints(self) -> None:
        ...
        self._test('/api/create-user', 'yakklrc-admin')
        ...
        self._test('/why-yakkl/', 'Why Yakkl?')
    ...
    def test_portico_pages_open_graph_metadata(self) -> None:
        ...
        # Features
        url = '/features/'
        title = '<meta property="og:title" content="Yakkl Features">'
    ...
    def test_integration_pages_open_graph_metadata(self) -> None:
        ...
        title = '<meta property="og:title" content="Connect GitHub to Yakkl">'
        description = '<meta property="og:description" content="Yakkl comes with over'
        self._test(url, title, doc_html_str=True)
        self._test(url, description, doc_html_str=True)

        # Test category pages
        url = '/integrations/communication'
        title = '<meta property="og:title" content="Connect your Communication tools to Yakkl">'
        description = '<meta property="og:description" content="Yakkl comes with over'
        self._test(url, title, doc_html_str=True)
        self._test(url, description, doc_html_str=True)
        ...
        url = '/integrations/'
        title = '<meta property="og:title" content="Connect the tools you use to Yakkl">'
        description = '<meta property="og:description" content="Yakkl comes with over'
        self._test(url, title, doc_html_str=True)
        self._test(url, description, doc_html_str=True)
    ...
    def test_email_integration(self) -> None:
        ...
        with self.settings(EMAIL_GATEWAY_PATTERN=''):
            result = self.get_doc('integrations/doc-html/email', subdomain='yakkl')
    ...
    def test_electron_detection(self) -> None:
        ...
        result = self.client_get("/accounts/password/reset/",
                                 HTTP_USER_AGENT="YakklElectron/1.0.0")
        self.assertTrue('data-platform="YakklElectron"' in result.content.decode("utf-8"))

class HelpTest(YakklTestCase):
    def test_help_settings_links(self) -> None:
        ...
        self.assertIn('Getting started with Yakkl', str(result.content))
...
class IntegrationTest(TestCase):
    ...
    def test_api_url_view_subdomains_homepage_base(self) -> None:
        context = dict()  # type: Dict[str, Any]
        add_api_uri_context(context, HostRequestMock())
        self.assertEqual(context["api_url_scheme_relative"], "yourYakklDomain.testserver/api")
        self.assertEqual(context["api_url"], "http://yourYakklDomain.testserver/api")
    ...
    def test_html_settings_links(self) -> None:
        ...
        self.assertEqual(
            context['settings_html'],
            '<a href="/#settings">Yakkl settings page</a>')
        self.assertEqual(
            context['subscriptions_html'],
...
class AboutPageTest(YakklTestCase):
    def setUp(self) -> None:
...
class ConfigErrorTest(YakklTestCase):
    ...
    def test_google(self) -> None:
        ...
        self.assert_not_in_success_response(["/etc/yakkl/settings.py"], result)
        self.assert_not_in_success_response(["/etc/yakkl/yakkl-secrets.conf"], result)
    ...
    def test_google_production_error(self) -> None:
        ...
        result = self.client_get(result.url)
        self.assert_in_success_response(["SOCIAL_AUTH_GOOGLE_KEY"], result)
        self.assert_in_success_response(["/etc/yakkl/settings.py"], result)
        self.assert_in_success_response(["social_auth_google_secret"], result)
        self.assert_in_success_response(["/etc/yakkl/yakkl-secrets.conf"], result)
        self.assert_not_in_success_response(["social_auth_google_key"], result)
        self.assert_not_in_success_response(["zproject/dev_settings.py"], result)
...
class PlansPageTest(YakklTestCase):
    def test_plans_auth(self) -> None:
        ...
        result = self.client_get("/plans/", subdomain="yakkl")
        self.assertEqual(result.status_code, 302)
    ...
    def test_CTA_text_by_plan_type(self) -> None:
        ...
        with self.settings(PRODUCTION=True):
            result = self.client_get("/plans/", subdomain="yakkl")
            self.assertEqual(result.status_code, 302)
            self.assertEqual(result["Location"], "https://yakkl.com/plans")
