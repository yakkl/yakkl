...
class TornadoWebTestCase(AsyncHTTPTestCase, YakklTestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def create_queue(self, **kwargs: Any) -> str:
        response = self.client_get('/json/events?dont_block=true', subdomain="yakkl")
...
class WebSocketBaseTestCase(AsyncHTTPTestCase, YakklTestCase):
    def setUp(self) -> None:
    ...
    def test_sending_stream_message_from_electron(self) -> Generator[str, Any, None]:
        ...
        user_message = {
            ...
            "request": {
                ...
                "reply_to": self.example_email('hamlet'),
                "local_id": -1,
                "socket_user_agent": "YakklElectron/1.5.0"
            }
        }
        user_message_str = ujson.dumps(user_message)
        ws.write_message(ujson.dumps([user_message_str]))
