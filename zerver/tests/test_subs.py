...
class TestMiscStuff(YakklTestCase):
...
class TestCreateStreams(YakklTestCase):
    ...
    def test_auto_mark_stream_created_message_as_read_for_stream_creator(self) -> None:
        realm = Realm.objects.get(name='Yakkl Dev')
        ...
        data = {
            ...
            "invite_only": 'false',
            "announce": 'true',
            "principals": '["iago@yakkl.com", "AARON@yakkl.com", "cordelia@yakkl.com", "hamlet@yakkl.com"]',
        ...
        expected_response = {
            ...
            "msg": "",
            "subscribed": {
                "AARON@yakkl.com": ["brand new stream"],
                "cordelia@yakkl.com": ["brand new stream"],
                "hamlet@yakkl.com": ["brand new stream"],
                "iago@yakkl.com": ["brand new stream"]
            },
            "already_subscribed": {}
        }
...
class RecipientTest(YakklTestCase):
...
class StreamAdminTest(YakklTestCase):
    ...
    def test_notify_on_stream_rename(self) -> None:
        ...
        self.assertEqual(message.sender.email, 'notification-bot@yakkl.com')
    ...
    def test_change_stream_description(self) -> None:
        ...
        with self.settings(INLINE_URL_EMBED_PREVIEW=True):
            result = self.client_patch('/json/streams/%d' % (stream_id,),
                                       {'description': ujson.dumps('See https://yakkl.com/team')})
        self.assert_json_success(result)
        stream = get_stream('stream_name1', realm)
        self.assertEqual(stream.rendered_description,
                         '<p>See <a href="https://yakkl.com/team" target="_blank" '
                         'title="https://yakkl.com/team">https://yakkl.com/team</a></p>')
    ...
    def delete_stream(self, stream: Stream) -> None:
        ...
        with self.assertRaises(Stream.DoesNotExist):
            Stream.objects.get(realm=get_realm("yakkl"), name=active_name)
    ...
    def test_remove_invalid_user(self) -> None:
        ...
        result = self.client_delete("/json/users/me/subscriptions",
                                    {"subscriptions": ujson.dumps([stream_name]),
                                     "principals": ujson.dumps(["baduser@yakkl.com"])})
        self.assert_json_error(
            result,
            "User not authorized to execute queries on behalf of 'baduser@yakkl.com'",
            status_code=403)

class DefaultStreamTest(YakklTestCase):
...
class DefaultStreamGroupTest(YakklTestCase):
...
class SubscriptionPropertiesTest(YakklTestCase):
...
class SubscriptionRestApiTest(YakklTestCase):
...
class SubscriptionAPITest(YakklTestCase):
    ...
    def test_subscriptions_add_for_principal_deactivated(self) -> None:
        ...
        result = self.common_subscribe_to_streams(self.test_email, "Denmark",
                                                  {"principals": ujson.dumps([target_profile.email])})
        self.assert_json_error(result, "User not authorized to execute queries on behalf of 'cordelia@yakkl.com'",
                               status_code=403)
    ...
    def test_non_ascii_subscription_for_principal(self) -> None:
        ...
        """
        self.assert_adding_subscriptions_for_principal(self.example_email("iago"), get_realm('yakkl'), [u"hümbüǵ"])
    ...
    def test_subscription_add_invalid_principal(self) -> None:
        ...
        """
        invalid_principal = "rosencrantz-and-guildenstern@yakkl.com"
        invalid_principal_realm = get_realm("yakkl")
...
class GetBotOwnerStreamsTest(YakklTestCase):
...
class GetPublicStreamsTest(YakklTestCase):
...
class StreamIdTest(YakklTestCase):
    def setUp(self) -> None:
        self.user_profile = self.example_user('hamlet')
...
class InviteOnlyStreamTest(YakklTestCase):
...
class GetSubscribersTest(YakklTestCase):
    ...
    def test_gather_subscribed_streams_for_guest_user(self) -> None:
        ...
        self.make_stream(stream_name_sub, realm=get_realm("yakkl"))
        self.subscribe(guest_user, stream_name_sub)

        stream_name_unsub = "public_stream_2"
        self.make_stream(stream_name_unsub, realm=get_realm("yakkl"))
        ...
        self.make_stream(stream_name_never_sub, realm=get_realm("yakkl"))
    ...
    def test_previously_subscribed_private_streams(self) -> None:
        ...
        self.make_stream(stream_name, realm=get_realm("yakkl"), invite_only=True)
        self.subscribe(admin_user, stream_name)
        self.subscribe(non_admin_user, stream_name)
        self.subscribe(self.example_user("othello"), stream_name)
...
class AccessStreamTest(YakklTestCase):
...
class StreamTrafficTest(YakklTestCase):
