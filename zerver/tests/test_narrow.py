...
class NarrowBuilderTest(YakklTestCase):
    def setUp(self) -> None:
    ...
    def test_add_term_using_sender_operator_with_non_existing_user_as_operand(
            self) -> None:  # NEGATED
        term = dict(operator='sender', operand='non-existing@yakkl.com')
        self.assertRaises(BadNarrowOperator, self._build_query, term)
    ...
    def test_add_term_using_pm_with_operator_and_self_and_user_as_operand(self) -> None:
        term = dict(operator='pm-with', operand='hamlet@yakkl.com, othello@yakkl.com')
        self._do_add_term_test(term, 'WHERE sender_id = :sender_id_1 AND recipient_id = :recipient_id_1 OR sender_id = :sender_id_2 AND recipient_id = :recipient_id_2')

    def test_add_term_using_pm_with_operator_more_than_one_user_as_operand(self) -> None:
        term = dict(operator='pm-with', operand='cordelia@yakkl.com, othello@yakkl.com')
        self._do_add_term_test(term, 'WHERE recipient_id = :recipient_id_1')

    def test_add_term_using_pm_with_operator_self_and_user_as_operand_and_negated(
            self) -> None:  # NEGATED
        term = dict(operator='pm-with', operand='hamlet@yakkl.com, othello@yakkl.com', negated=True)
        self._do_add_term_test(term, 'WHERE NOT (sender_id = :sender_id_1 AND recipient_id = :recipient_id_1 OR sender_id = :sender_id_2 AND recipient_id = :recipient_id_2)')

    def test_add_term_using_pm_with_operator_more_than_one_user_as_operand_and_negated(self) -> None:
        term = dict(operator='pm-with', operand='cordelia@yakkl.com, othello@yakkl.com', negated=True)
        self._do_add_term_test(term, 'WHERE recipient_id != :recipient_id_1')
    ...
    def test_add_term_using_pm_with_operator_with_existing_and_non_existing_user_as_operand(self) -> None:
        term = dict(operator='pm-with', operand='othello@yakkl.com,non-existing@yakkl.com')
        self.assertRaises(BadNarrowOperator, self._build_query, term)
    ...
    def test_add_term_using_group_pm_operator_with_non_existing_user_as_operand(self) -> None:
        term = dict(operator='group-pm-with', operand='non-existing@yakkl.com')
        self.assertRaises(BadNarrowOperator, self._build_query, term)
...
class IncludeHistoryTest(YakklTestCase):
...
class PostProcessTest(YakklTestCase):
...
class GetOldMessagesTest(YakklTestCase):
    ...
    def test_bad_narrow_nonexistent_email(self) -> None:
        self.login(self.example_email("hamlet"))
        self.exercise_bad_narrow_operand("pm-with", ['non-existent-user@yakkl.com'],
    ...
    def test_get_messages_with_search_queries(self) -> None:
        ...
        sql_template = "SELECT anon_1.message_id, anon_1.flags, anon_1.subject, anon_1.rendered_content, anon_1.content_matches, anon_1.topic_matches \nFROM (SELECT message_id, flags, subject, rendered_content, ts_match_locs_array('yakkl.english_us_search', rendered_content, plainto_tsquery('yakkl.english_us_search', 'jumping')) AS content_matches, ts_match_locs_array('yakkl.english_us_search', escape_html(subject), plainto_tsquery('yakkl.english_us_search', 'jumping')) AS topic_matches \nFROM zerver_usermessage JOIN zerver_message ON zerver_usermessage.message_id = zerver_message.id \nWHERE user_profile_id = {hamlet_id} AND (search_tsvector @@ plainto_tsquery('yakkl.english_us_search', 'jumping')) ORDER BY message_id ASC \n LIMIT 10) AS anon_1 ORDER BY message_id ASC"  # type: str
        sql = sql_template.format(**query_ids)
        self.common_check_get_messages_query({'anchor': 0, 'num_before': 0, 'num_after': 9,
                                              'narrow': '[["search", "jumping"]]'},
                                             sql)

        sql_template = "SELECT anon_1.message_id, anon_1.subject, anon_1.rendered_content, anon_1.content_matches, anon_1.topic_matches \nFROM (SELECT id AS message_id, subject, rendered_content, ts_match_locs_array('yakkl.english_us_search', rendered_content, plainto_tsquery('yakkl.english_us_search', 'jumping')) AS content_matches, ts_match_locs_array('yakkl.english_us_search', escape_html(subject), plainto_tsquery('yakkl.english_us_search', 'jumping')) AS topic_matches \nFROM zerver_message \nWHERE recipient_id = {scotland_recipient} AND (search_tsvector @@ plainto_tsquery('yakkl.english_us_search', 'jumping')) ORDER BY zerver_message.id ASC \n LIMIT 10) AS anon_1 ORDER BY message_id ASC"
        sql = sql_template.format(**query_ids)
        self.common_check_get_messages_query({'anchor': 0, 'num_before': 0, 'num_after': 9,
                                              'narrow': '[["stream", "Scotland"], ["search", "jumping"]]'},
                                             sql)

        sql_template = 'SELECT anon_1.message_id, anon_1.flags, anon_1.subject, anon_1.rendered_content, anon_1.content_matches, anon_1.topic_matches \nFROM (SELECT message_id, flags, subject, rendered_content, ts_match_locs_array(\'yakkl.english_us_search\', rendered_content, plainto_tsquery(\'yakkl.english_us_search\', \'"jumping" quickly\')) AS content_matches, ts_match_locs_array(\'yakkl.english_us_search\', escape_html(subject), plainto_tsquery(\'yakkl.english_us_search\', \'"jumping" quickly\')) AS topic_matches \nFROM zerver_usermessage JOIN zerver_message ON zerver_usermessage.message_id = zerver_message.id \nWHERE user_profile_id = {hamlet_id} AND (content ILIKE \'%jumping%\' OR subject ILIKE \'%jumping%\') AND (search_tsvector @@ plainto_tsquery(\'yakkl.english_us_search\', \'"jumping" quickly\')) ORDER BY message_id ASC \n LIMIT 10) AS anon_1 ORDER BY message_id ASC'
        sql = sql_template.format(**query_ids)
        self.common_check_get_messages_query({'anchor': 0, 'num_before': 0, 'num_after': 9,
