...
class TestEmbeddedBotMessaging(YakklTestCase):
    def setUp(self) -> None:
    ...
    def test_message_to_embedded_bot_with_initialize(self) -> None:
        assert self.bot_profile is not None
        with patch('yakkl_bots.bots.helloworld.helloworld.HelloWorldHandler.initialize',
    ...
    def test_embedded_bot_quit_exception(self) -> None:
        assert self.bot_profile is not None
        with patch('yakkl_bots.bots.helloworld.helloworld.HelloWorldHandler.handle_message',
...
class TestEmbeddedBotFailures(YakklTestCase):
    def test_message_embedded_bot_with_invalid_service(self) -> None:
        ...
        self.create_test_bot(short_name='embedded', user_profile=user_profile,
                             bot_type=UserProfile.EMBEDDED_BOT,
                             service_name='helloworld')
        bot_profile = get_user("embedded-bot@yakkl.testserver",
                               get_realm('yakkl'))
