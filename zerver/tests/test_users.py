...
class PermissionTest(YakklTestCase):
    ...
    def test_admin_api_hide_emails(self) -> None:
        ...
        members = result.json()['members']
        hamlet = find_dict(members, 'user_id', user.id)
        self.assertEqual(hamlet['email'], "user%s@yakkl.testserver" % (user.id,))
    ...
    def test_admin_user_can_change_profile_data(self) -> None:
        ...
        fields = {
            ...
            'Favorite editor': 'vim',
            'Birthday': '1909-3-5',
            'Favorite website': 'https://yakkl.com',
            'Mentor': [cordelia.id],
            'GitHub': 'timabbott',
        }
        ...
        new_fields = {
            ...
            'Birthday': None,
            'Favorite website': 'https://yakkl.github.io',
            'Mentor': [hamlet.id],
            'GitHub': 'timabbott',
        }
        new_profile_data = []
...
class AdminCreateUserTest(YakklTestCase):
    def test_create_user_backend(self) -> None:
        ...
        self.assert_json_error(result,
                               "Email 'romeo@not-yakkl.com' not allowed in this organization")

        RealmDomain.objects.create(realm=get_realm('yakkl'), domain='yakkl.net')
        valid_params = dict(
            email='romeo@yakkl.net',
            password='xxxx',
            full_name='Romeo Montague',
            short_name='Romeo',
        )
        ...
        new_user = get_user('romeo@yakkl.net', get_realm('yakkl'))
        ...
        result = self.client_post("/json/users", valid_params)
        self.assert_json_error(result,
                               "Email 'romeo@yakkl.net' already in use")
        ...
        valid_params["email"] = "iago+label@yakkl.com"
        result = self.client_post("/json/users", valid_params)
...
class UserProfileTest(YakklTestCase):
    ...
    def test_user_ids_to_users(self) -> None:
        ...
        self.assertEqual(user_ids_to_users([], get_realm("yakkl")), [])
        self.assertEqual(set([user_profile.id for user_profile in user_ids_to_users(real_user_ids, get_realm("yakkl"))]),
                         set(real_user_ids))
        with self.assertRaises(JsonableError):
    ...
    def test_bulk_get_users(self) -> None:
        ...
        result = bulk_get_users([hamlet, cordelia], get_realm("yakkl"))
    ...
    def test_get_accounts_for_email(self) -> None:
        ...
        cordelia_in_yakkl = self.example_user("cordelia")
        cordelia_in_lear = get_user("cordelia@yakkl.com", lear_realm)

        email = "cordelia@yakkl.com"
        accounts = get_accounts_for_email(email)
        self.assert_length(accounts, 2)
        check_account_present_in_accounts(cordelia_in_yakkl, accounts)
    ...
    def test_get_source_profile(self) -> None:
        iago = get_source_profile("iago@yakkl.com", "yakkl")
        assert iago is not None
        self.assertEqual(iago.email, "iago@yakkl.com")
        self.assertEqual(iago.realm, get_realm("yakkl"))

        iago = get_source_profile("IAGO@yakkl.com", "yakkl")
        assert iago is not None
        self.assertEqual(iago.email, "iago@yakkl.com")

        cordelia = get_source_profile("cordelia@yakkl.com", "lear")
        assert cordelia is not None
        self.assertEqual(cordelia.email, "cordelia@yakkl.com")

        self.assertIsNone(get_source_profile("iagod@yakkl.com", "yakkl"))
        self.assertIsNone(get_source_profile("iago@yakkl.com", "YAKKL"))
        self.assertIsNone(get_source_profile("iago@yakkl.com", "lear"))
...
class ActivateTest(YakklTestCase):
...
class RecipientInfoTest(YakklTestCase):
...
class BulkUsersTest(YakklTestCase):
...
class GetProfileTest(YakklTestCase):
    ...
    def common_get_profile(self, user_id: str) -> Dict[str, Any]:
        # Assumes all users are example users in realm 'yakkl'
