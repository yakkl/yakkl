...
class TestFollowupEmails(YakklTestCase):
    def test_day1_email_context(self) -> None:
        ...
        self.assertEqual(email_data["context"]["getting_started_link"], "https://yakkl.com")
        ...
        self.assertEqual(email_data["context"]["getting_started_link"],
                         "http://yakkl.testserver/help/getting-your-organization-started-with-yakkl")
        self.assertNotIn("ldap_username", email_data["context"])

    # See https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html#ldap-including-active-directory
    # for case details.
    ...
    def test_day1_email_ldap_case_a_login_credentials(self) -> None:
        ...
        mock_ldap.directory = {
            "uid=newuser@yakkl.com,ou=users,dc=yakkl,dc=com": {
        ...
        ldap_search = LDAPSearch("ou=users,dc=yakkl,dc=com", ldap.SCOPE_SUBTREE, "(email=%(user)s)")
        with self.settings(
                AUTH_LDAP_USER_ATTR_MAP=ldap_user_attr_map,
                AUTH_LDAP_USER_DN_TEMPLATE='uid=%(user)s,ou=users,dc=yakkl,dc=com',
                AUTH_LDAP_USER_SEARCH=ldap_search):
            self.login_with_return("newuser@yakkl.com", "testing")
            user = UserProfile.objects.get(email="newuser@yakkl.com")
            ...
            self.assertEqual(email_data["context"]["ldap_username"], "newuser@yakkl.com")
    ...
    def test_day1_email_ldap_case_c_login_credentials(self) -> None:
        ...
        with self.settings(
            ...
            user = UserProfile.objects.get(email="newuser_email@yakkl.com")
            scheduled_emails = ScheduledEmail.objects.filter(users=user)
    ...
    def test_followup_emails_count(self) -> None:
        ...
        # Hamlet has account only in Yakkl realm so both day1 and day2 emails should be sent
...
class TestMissedMessages(YakklTestCase):
    ...
    def _test_cases(self, tokens: List[str], msg_id: int, body: str, email_subject: str,
        ...
        if settings.EMAIL_GATEWAY_PATTERN != "":
            ...
            reply_to_emails = [formataddr(("Yakkl", address)) for address in reply_to_addresses]
        else:
            reply_to_emails = ["noreply@testserver"]
        msg = mail.outbox[0]
        from_email = formataddr(("Yakkl missed messages", FromAddress.NOREPLY))
    ...
    def _realm_name_in_missed_message_email_subject(self,
        ...
        if realm_name_in_notifications:
            email_subject = 'PMs with Othello, the Moor of Venice [Yakkl Dev]'
    ...
    def _extra_context_in_missed_stream_messages_mention(self, send_as_user: bool,
        ...
        if show_message_content:
            ...
            body = 'Manage email preferences: http://yakkl.testserver/#settings/notifications'
    ...
    def _extra_context_in_personal_missed_stream_messages(self, send_as_user: bool,
        ...
        if show_message_content:
            ...
            verify_body_does_not_include = ['Othello, the Moor of Venice', 'Extremely personal message!',
                                            'mentioned', 'group', 'Reply to this email directly, or view it in Yakkl']
        self._test_cases(tokens, msg_id, body, email_subject, send_as_user,
                         show_message_content=show_message_content,
                         verify_body_does_not_include=verify_body_does_not_include)
    ...
    def _reply_to_email_in_personal_missed_stream_messages(self, send_as_user: bool,
        ...
        body = 'Reply to this email directly, or view it in Yakkl'
        email_subject = 'PMs with Othello, the Moor of Venice'
        self._test_cases(tokens, msg_id, body, email_subject, send_as_user)
    ...
    def test_realm_emoji_in_missed_message(self, mock_random_token: MagicMock) -> None:
        ...
        realm_emoji_url = "http://yakkl.testserver/user_avatars/%s/emoji/images/%s.png" % (
    ...
    def test_emojiset_in_missed_message(self, mock_random_token: MagicMock) -> None:
        ...
        body = '<img alt=":hamburger:" src="http://yakkl.testserver/static/generated/emoji/images-apple-64/1f354.png" title="hamburger" style="height: 20px;">'
        email_subject = 'PMs with Othello, the Moor of Venice'
        self._test_cases(tokens, msg_id, body, email_subject, send_as_user=False, verify_html_body=True)
    ...
    def test_stream_link_in_missed_message(self, mock_random_token: MagicMock) -> None:
        ...
        stream_id = get_stream('Verona', get_realm('yakkl')).id
        href = "http://yakkl.testserver/#narrow/stream/{stream_id}-Verona".format(stream_id=stream_id)
    ...
    def test_relative_to_full_url(self) -> None:
        ...
        test_data = test_data.format(realm_id=yakkl_realm.id)
        actual_output = relative_to_full_url("http://example.com", test_data)
        ...
        expected_output = expected_output.format(realm_id=yakkl_realm.id)
        self.assertEqual(actual_output, expected_output)
