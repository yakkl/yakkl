...
class DoRestCallTests(YakklTestCase):
    def setUp(self) -> None:
        realm = get_realm("yakkl")
        user_profile = get_user("outgoing-webhook@yakkl.com", realm)
        self.mock_event = {
    ...
    def test_retry_request(self: mock.Mock) -> None:
        ...
        with mock.patch('requests.request', return_value=response):
            ...
            self.assertEqual(bot_owner_notification.content,
                             '''[A message](http://yakkl.testserver/#narrow/stream/999-Verona/topic/Foo/near/) triggered an outgoing webhook.
...
The webhook got a response with status code *400*.''')
    ...
    def test_headers(self) -> None:
        with mock.patch('requests.request') as mock_request:
            ...
            user_agent = 'YakklOutgoingWebhook/' + YAKKL_VERSION
            headers = {
...
class TestOutgoingWebhookMessaging(YakklTestCase):
    def setUp(self) -> None:
