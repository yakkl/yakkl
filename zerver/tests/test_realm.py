...
class RealmTest(YakklTestCase):
    ...
    def test_update_realm_description_events(self) -> None:
        realm = get_realm('yakkl')
        new_description = u'yakkl dev group'
        events = []  # type: List[Mapping[str, Any]]
        with tornado_redirected_to_list(events):
    ...
    def test_do_change_realm_subdomain_clears_user_realm_cache(self) -> None:
        ...
        realm appears to be deactivated.  You can make this test fail
        by disabling cache.flush_realm()."""
        user = get_user_profile_by_email('hamlet@yakkl.com')
        realm = get_realm('yakkl')
        do_change_realm_subdomain(realm, "newyakkl")
        user = get_user_profile_by_email('hamlet@yakkl.com')
        self.assertEqual(user.realm.string_id, "newyakkl")
    ...
    def test_do_send_realm_reactivation_email(self) -> None:
        ...
        self.assertIn('Reactivate your Yakkl organization', outbox[0].subject)
    ...
    def test_change_email_address_visibility(self) -> None:
        ...
        self.assertEqual(edited_user_profile.email, "user%s@yakkl.testserver" % (edited_user_profile.id,))

    def test_change_video_chat_provider(self) -> None:
        self.assertEqual(get_realm('yakkl').video_chat_provider, Realm.VIDEO_CHAT_PROVIDERS['jitsi_meet']['id'])
        email = self.example_email("iago")
        self.login(email)
        ...
        req = {
            "video_chat_provider": ujson.dumps(Realm.VIDEO_CHAT_PROVIDERS['google_hangouts']['id']),
            "google_hangouts_domain": ujson.dumps("yakkl.com"),
        }
        result = self.client_patch('/json/realm', req)
        self.assert_json_success(result)
        self.assertEqual(get_realm('yakkl').video_chat_provider,
...
class RealmAPITest(YakklTestCase):
    ...
    def do_test_realm_update_api(self, name: str) -> None:
        ...
        test_values = dict(
            ...
            video_chat_provider=[
                dict(
                    video_chat_provider=ujson.dumps(Realm.VIDEO_CHAT_PROVIDERS['google_hangouts']['id']),
                    google_hangouts_domain=ujson.dumps(u'yakkl.com')
                ),
                dict(
...
class ScrubRealmTest(YakklTestCase):
    def test_scrub_realm(self) -> None:
        ...
        Attachment.objects.filter(realm=yakkl).delete()
        Attachment.objects.create(realm=yakkl, owner=iago, path_id="a/b/temp1.txt")
        Attachment.objects.create(realm=yakkl, owner=othello, path_id="a/b/temp2.txt")
        ...
        self.assertNotEqual(CustomProfileField.objects.filter(realm=yakkl).count(), 0)

        with mock.patch('logging.warning'):
            do_scrub_realm(yakkl)
        ...
        self.assertEqual(Attachment.objects.filter(realm=yakkl).count(), 0)
        self.assertEqual(Attachment.objects.filter(realm=lear).count(), 2)

        self.assertEqual(CustomProfileField.objects.filter(realm=yakkl).count(), 0)
        self.assertNotEqual(CustomProfileField.objects.filter(realm=lear).count(), 0)

        yakkl_users = UserProfile.objects.filter(realm=yakkl)
        for user in yakkl_users:
            self.assertTrue(re.search("Scrubbed [a-z0-9]{15}", user.full_name))
            self.assertTrue(re.search("scrubbed-[a-z0-9]{15}@" + yakkl.host, user.email))
            self.assertTrue(re.search("scrubbed-[a-z0-9]{15}@" + yakkl.host, user.delivery_email))
        ...
        for user in lear_users:
            ...
            self.assertIsNone(re.search("scrubbed-[a-z0-9]{15}@" + yakkl.host, user.email))
            self.assertIsNone(re.search("scrubbed-[a-z0-9]{15}@" + yakkl.host, user.delivery_email))
