...
class EmailChangeTestCase(YakklTestCase):
    ...
    def test_confirm_email_change_when_time_exceeded(self) -> None:
        ...
        new_email = 'hamlet-new@yakkl.com'
        self.login(self.example_email("hamlet"))
    ...
    def test_confirm_email_change(self) -> None:
        ...
        self.assert_in_success_response(["This confirms that the email address for your Yakkl"],
                                        response)
    ...
    def test_start_email_change_process(self) -> None:
        user_profile = self.example_user('hamlet')
        do_start_email_change_process(user_profile, 'hamlet-new@yakkl.com')
        self.assertEqual(EmailChangeStatus.objects.count(), 1)

    def test_end_to_end_flow(self) -> None:
        data = {'email': 'hamlet-new@yakkl.com'}
        ...
        result = self.client_patch(url, {"email": "hamlet@yakkl.com"})
        self.assert_in_success_response(['Check your email for a confirmation link.'], result)
    ...
    def test_change_delivery_email_end_to_end_with_admins_visibility(self) -> None:
        ...
        self.assertEqual(user_profile.email, "user4@yakkl.testserver")
        obj.refresh_from_db()
        self.assertEqual(obj.status, 1)
