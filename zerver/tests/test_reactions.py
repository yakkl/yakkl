...
class ReactionEmojiTest(YakklTestCase):
    ...
    def test_yakkl_emoji(self) -> None:
        """
        Reacting with yakkl emoji succeeds
        """
        sender = self.example_email("hamlet")
        result = self.api_put(sender, '/api/v1/messages/1/emoji_reactions/yakkl')
        self.assert_json_success(result)
        self.assertEqual(200, result.status_code)
    ...
    def test_emoji_name_to_emoji_code(self) -> None:
        ...
        # Test ':yakkl:' emoji.
        emoji_code, reaction_type = emoji_name_to_emoji_code(realm, 'yakkl')
        self.assertEqual(emoji_code, 'yakkl')
        self.assertEqual(reaction_type, 'yakkl_extra_emoji')
        ...
        # Test override `:yakkl:` emoji.
        overriding_emoji = RealmEmoji.objects.create(
            name='yakkl', realm=realm, file_name='yakkl')
        emoji_code, reaction_type = emoji_name_to_emoji_code(realm, 'yakkl')
        self.assertEqual(emoji_code, str(overriding_emoji.id))
        self.assertEqual(reaction_type, 'realm_emoji')
...
class ReactionMessageIDTest(YakklTestCase):
...
class ReactionTest(YakklTestCase):
...
class ReactionEventTest(YakklTestCase):
...
class EmojiReactionBase(YakklTestCase):
    ...
    def post_yakkl_reaction(self, message_id: int=1, sender: str='hamlet') -> HttpResponse:
        reaction_info = {
            'emoji_name': 'yakkl',
            'emoji_code': 'yakkl',
            'reaction_type': 'yakkl_extra_emoji',
        }
    ...
    def delete_yakkl_reaction(self, message_id: int=1, sender: str='hamlet') -> HttpResponse:
        reaction_info = {
            'emoji_name': 'yakkl',
            'emoji_code': 'yakkl',
            'reaction_type': 'yakkl_extra_emoji',
...
class YakklExtraEmojiReactionTest(EmojiReactionBase):
    def test_add_yakkl_emoji_reaction(self) -> None:
        result = self.post_yakkl_reaction()
        self.assert_json_success(result)

    def test_add_duplicate_yakkl_reaction(self) -> None:
        result = self.post_yakkl_reaction()
        self.assert_json_success(result)
    ...
    def test_delete_yakkl_emoji(self) -> None:
        result = self.post_yakkl_reaction()
        self.assert_json_success(result)

        result = self.delete_yakkl_reaction()
        self.assert_json_success(result)

    def test_delete_non_existent_yakkl_reaction(self) -> None:
        result = self.delete_yakkl_reaction()
        self.assert_json_error(result, "Reaction doesn't exist.")
