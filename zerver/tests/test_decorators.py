...
class DecoratorTestCase(TestCase):
    def test_get_client_name(self) -> None:
        ...
        req = Request(
            GET=dict(),
            POST=dict(),
            META=dict(HTTP_USER_AGENT='YakklDesktop/bla bla bla'),
        )

        self.assertEqual(get_client_name(req, is_browser_view=True), 'YakklDesktop')
        self.assertEqual(get_client_name(req, is_browser_view=False), 'YakklDesktop')

        req = Request(
            GET=dict(),
            POST=dict(),
            META=dict(HTTP_USER_AGENT='YakklMobile/bla bla bla'),
        )

        self.assertEqual(get_client_name(req, is_browser_view=True), 'YakklMobile')
        self.assertEqual(get_client_name(req, is_browser_view=False), 'YakklMobile')
    ...
    def test_api_key_only_webhook_view(self) -> None:
        ...
        webhook_bot_email = 'webhook-bot@yakkl.com'
        webhook_bot_realm = get_realm('yakkl')
        webhook_bot = get_user(webhook_bot_email, webhook_bot_realm)
        webhook_bot_api_key = get_api_key(webhook_bot)
        webhook_client_name = "YakklClientNameWebhook"
        ...
        with mock.patch('logging.warning') as mock_warning:
            ...
            mock_warning.assert_called_with(
                ...
                "subdomain ({})".format(webhook_bot_email, 'yakkl', ''))
        ...
        with mock.patch('logging.warning') as mock_warning:
            ...
            mock_warning.assert_called_with(
                "User {} ({}) attempted to access API on wrong "
                "subdomain ({})".format(webhook_bot_email, 'yakkl', 'acme'))

        request.host = "yakkl.testserver"
...
class SkipRateLimitingTest(YakklTestCase):
    def test_authenticated_rest_api_view(self) -> None:
        ...
        request = HostRequestMock(host="yakkl.testserver")
...
class DecoratorLoggingTestCase(YakklTestCase):
...
{body}
            ...
            mock_exception.assert_called_with(message.format(
                email=webhook_bot_email,
                realm=webhook_bot_realm.string_id,
                client_name='YakklClientNameWebhook',
                path_info=request.META.get('PATH_INFO'),
                content_type=request.content_type,
                custom_headers=None,
                body=request.body,
...
{body}
    ...
    def test_authenticated_rest_api_view_with_non_webhook_view(self) -> None:
        ...
        request = HostRequestMock()
        request.META['HTTP_AUTHORIZATION'] = self.encode_credentials("aaron@yakkl.com")
        request.method = 'POST'
        request.host = "yakkl.testserver"
...
class ValidatorTestCase(TestCase):
    ...
    def test_check_url(self) -> None:
        ...
        url = "http://yakkl-bots.example.com/"
        self.assertEqual(check_url('url', url), None)
...
class DeactivatedRealmTest(YakklTestCase):
...
class LoginRequiredTest(YakklTestCase):
    def test_login_required(self) -> None:
        """
        Verifies the yakkl_login_required decorator blocks deactivated users.
        """
        user_profile = self.example_user('hamlet')
        email = user_profile.email
...
class InactiveUserTest(YakklTestCase):
    ...
    def test_login_deactivated_mirror_dummy(self) -> None:
        ...
        request.get_host.return_value = 'yakkl.testserver'
...
class TestIncomingWebhookBot(YakklTestCase):
    def setUp(self) -> None:
        yakkl_realm = get_realm('yakkl')
        self.webhook_bot = get_user('webhook-bot@yakkl.com', yakkl_realm)

    def test_webhook_bot_permissions(self) -> None:
        result = self.api_post("webhook-bot@yakkl.com",
                               "/api/v1/messages", {"type": "private",
                                                    "content": "Test message",
                                                    "client": "test suite",
                                                    "to": self.example_email("othello")})
        ...
        result = self.api_get("webhook-bot@yakkl.com", "/api/v1/messages", dict(post_params))
        self.assert_json_error(result, 'This API is not available to incoming webhook bots.',
                               status_code=401)

class TestValidateApiKey(YakklTestCase):
    def setUp(self) -> None:
        yakkl_realm = get_realm('yakkl')
        self.webhook_bot = get_user('webhook-bot@yakkl.com', yakkl_realm)
        self.default_bot = get_user('default-bot@yakkl.com', yakkl_realm)
    ...
    def test_validate_api_key_if_profile_is_incoming_webhook_and_is_webhook_is_set(self) -> None:
        api_key = get_api_key(self.webhook_bot)
        profile = validate_api_key(HostRequestMock(host="yakkl.testserver"),
    ...
    def test_validate_api_key_if_email_is_case_insensitive(self) -> None:
        api_key = get_api_key(self.default_bot)
        profile = validate_api_key(HostRequestMock(host="yakkl.testserver"), self.default_bot.email.upper(), api_key)
    ...
    def test_valid_api_key_if_user_is_on_wrong_subdomain(self) -> None:
        with self.settings(RUNNING_INSIDE_TORNADO=False):
            ...
            with mock.patch('logging.warning') as mock_warning:
                ...
                mock_warning.assert_called_with(
                    "User {} ({}) attempted to access API on wrong "
                    "subdomain ({})".format(self.default_bot.email, 'yakkl', ''))
            ...
            with mock.patch('logging.warning') as mock_warning:
                ...
                mock_warning.assert_called_with(
                    "User {} ({}) attempted to access API on wrong "
                    "subdomain ({})".format(self.default_bot.email, 'yakkl', 'acme'))
...
class TestHumanUsersOnlyDecorator(YakklTestCase):
    def test_human_only_endpoints(self) -> None:
        ...
        for endpoint in post_endpoints:
            result = self.api_post('default-bot@yakkl.com', endpoint)
        ...
        for endpoint in patch_endpoints:
            result = self.api_patch('default-bot@yakkl.com', endpoint)
            self.assert_json_error(result, "This endpoint does not accept bot requests.")
        ...
        for endpoint in delete_endpoints:
            result = self.api_delete('default-bot@yakkl.com', endpoint)
            self.assert_json_error(result, "This endpoint does not accept bot requests.")

class TestAuthenticatedJsonPostViewDecorator(YakklTestCase):
    def test_authenticated_json_post_view_if_everything_is_correct(self) -> None:
        user_email = self.example_email('hamlet')
        user_realm = get_realm('yakkl')
    ...
    def test_authenticated_json_post_view_if_subdomain_is_invalid(self) -> None:
        ...
        with mock.patch('logging.warning') as mock_warning, \
            ...
            mock_warning.assert_called_with(
                "User {} ({}) attempted to access API on wrong "
                "subdomain ({})".format(user_email, 'yakkl', ''))
...
class TestAuthenticatedJsonViewDecorator(YakklTestCase):
...
class TestYakklLoginRequiredDecorator(YakklTestCase):
    def test_yakkl_login_required_if_subdomain_is_invalid(self) -> None:
        user_email = self.example_email("hamlet")
        self.login(user_email)

        with mock.patch('zerver.decorator.get_subdomain', return_value='yakkl'):
            result = self.client_get('/accounts/accept_terms/')
    ...
    def test_2fa_failure(self) -> None:
        ...
        request.get_host = lambda: 'yakkl.testserver'
...
class TestRequireDecorators(YakklTestCase):
    ...
    def test_require_member_or_admin_decorator(self) -> None:
        result = self.api_get("outgoing-webhook@yakkl.com", '/api/v1/bots')
        self.assert_json_error(result, "This endpoint does not accept bot requests.")
...
class ReturnSuccessOnHeadRequestDecorator(YakklTestCase):
...
class RestAPITest(YakklTestCase):
...
class CacheTestCase(YakklTestCase):
...
class TestUserAgentParsing(YakklTestCase):
...
class TestIgnoreUnhashableLRUCache(YakklTestCase):
