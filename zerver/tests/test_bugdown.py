...
def bugdown_convert(content: str) -> str:
    ...
    return bugdown.convert(
        content=content,
        message_realm=get_realm('yakkl'),
        message=message
    )

class BugdownMiscTest(YakklTestCase):
...
class BugdownTest(YakklTestCase):
    def setUp(self) -> None:
    ...
    def test_inline_dropbox_bad(self) -> None:
        ...
        msg = "https://yakkl-test.dropbox.com/photos/cl/ROmr9K1XYtmpneM"
        with mock.patch('zerver.lib.bugdown.fetch_open_graph_image', return_value=None):
            converted = bugdown_convert(msg)
        self.assertEqual(converted, '<p><a href="https://yakkl-test.dropbox.com/photos/cl/ROmr9K1XYtmpneM" target="_blank" title="https://yakkl-test.dropbox.com/photos/cl/ROmr9K1XYtmpneM">https://yakkl-test.dropbox.com/photos/cl/ROmr9K1XYtmpneM</a></p>')

    def test_inline_github_preview(self) -> None:
        # Test photo album previews
        msg = 'Test: https://github.com/yakkl/yakkl/blob/master/static/images/logo/yakkl-icon-128x128.png'
        converted = bugdown_convert(msg)

        self.assertEqual(converted, '<p>Test: <a href="https://github.com/yakkl/yakkl/blob/master/static/images/logo/yakkl-icon-128x128.png" target="_blank" title="https://github.com/yakkl/yakkl/blob/master/static/images/logo/yakkl-icon-128x128.png">https://github.com/yakkl/yakkl/blob/master/static/images/logo/yakkl-icon-128x128.png</a></p>\n<div class="message_inline_image"><a href="https://github.com/yakkl/yakkl/blob/master/static/images/logo/yakkl-icon-128x128.png" target="_blank" title="https://github.com/yakkl/yakkl/blob/master/static/images/logo/yakkl-icon-128x128.png"><img data-src-fullsize="/thumbnail?url=https%3A%2F%2Fraw.githubusercontent.com%2Fyakkl%2Fyakkl%2Fmaster%2Fstatic%2Fimages%2Flogo%2Fyakkl-icon-128x128.png&amp;size=full" src="/thumbnail?url=https%3A%2F%2Fraw.githubusercontent.com%2Fyakkl%2Fyakkl%2Fmaster%2Fstatic%2Fimages%2Flogo%2Fyakkl-icon-128x128.png&amp;size=thumbnail"></a></div>')
    ...
    def test_inline_interesting_links(self) -> None:
        ...
        emoji_in_tweet_html = """Yakkl is <span aria-label=\"100\" class="emoji emoji-1f4af" role=\"img\" title="100">:100:</span>% open-source!"""
        ...
        def make_inline_twitter_preview(url: str, tweet_html: str, image_html: str='') -> str:
            ...
            return ('<div class="inline-preview-twitter">'
                    ...
                    ' src="https://external-content.yakklcdn.net/external_content/1f7cd2436976d410eab8189ebceda87ae0b34ead/687474703a2f2f7062732e7477696d672e63'
    ...
    def test_realm_patterns(self) -> None:
        realm = get_realm('yakkl')
        url_format_string = r"https://trac.yakkl.net/ticket/%(id)s"
        ...
        self.assertEqual(
            ...
            '<RealmFilter(yakkl): #(?P<id>[0-9]{2,8})'
            ' https://trac.yakkl.net/ticket/%(id)s>')
        ...
        content = "We should fix #224 and #115, but not issue#124 or #1124z or [trac #15](https://trac.yakkl.net/ticket/16) today."
        converted = bugdown.convert(content, message_realm=realm, message=msg)
        converted_topic = bugdown.topic_links(realm.id, msg.topic_name())

        self.assertEqual(converted, '<p>We should fix <a href="https://trac.yakkl.net/ticket/224" target="_blank" title="https://trac.yakkl.net/ticket/224">#224</a> and <a href="https://trac.yakkl.net/ticket/115" target="_blank" title="https://trac.yakkl.net/ticket/115">#115</a>, but not issue#124 or #1124z or <a href="https://trac.yakkl.net/ticket/16" target="_blank" title="https://trac.yakkl.net/ticket/16">trac #15</a> today.</p>')
        self.assertEqual(converted_topic, [u'https://trac.yakkl.net/ticket/444'])

        msg.set_topic_name("#444 https://google.com")
        converted_topic = bugdown.topic_links(realm.id, msg.topic_name())
        self.assertEqual(converted_topic, [u'https://trac.yakkl.net/ticket/444', u'https://google.com'])

        RealmFilter(realm=realm, pattern=r'#(?P<id>[a-zA-Z]+-[0-9]+)',
                    url_format_string=r'https://trac.yakkl.net/ticket/%(id)s').save()
        msg = Message(sender=self.example_user('hamlet'))

        content = '#ZUL-123 was fixed and code was deployed to production, also #zul-321 was deployed to staging'
        converted = bugdown.convert(content, message_realm=realm, message=msg)

        self.assertEqual(converted, '<p><a href="https://trac.yakkl.net/ticket/ZUL-123" target="_blank" title="https://trac.yakkl.net/ticket/ZUL-123">#ZUL-123</a> was fixed and code was deployed to production, also <a href="https://trac.yakkl.net/ticket/zul-321" target="_blank" title="https://trac.yakkl.net/ticket/zul-321">#zul-321</a> was deployed to staging</p>')

        def assert_conversion(content: str, convert: bool=True) -> None:
            converted = bugdown.convert(content, message_realm=realm, message=msg)
            converted_topic = bugdown.topic_links(realm.id, content)
            if convert:
                self.assertTrue('trac.yakkl.net' in converted)
                self.assertEqual(len(converted_topic), 1)
                self.assertTrue('trac.yakkl.net' in converted_topic[0])
            else:
                self.assertTrue('trac.yakkl.net' not in converted)
        ...
        RealmFilter(realm=realm, pattern=r'hello#(?P<id>[0-9]+)',
                    url_format_string=r'https://trac.yakkl.net/hello/%(id)s').save()
        converted_topic = bugdown.topic_links(realm.id, 'hello#123 #234')
        self.assertEqual(converted_topic, ['https://trac.yakkl.net/ticket/234', 'https://trac.yakkl.net/hello/123'])
    ...
    def test_maybe_update_markdown_engines(self) -> None:
        ...
        yakkl_filters = all_filters[realm.id]
        self.assertEqual(len(yakkl_filters), 1)
        self.assertEqual(yakkl_filters[0],
                         (u'#(?P<id>[0-9]{2,8})', u'https://trac.yakkl.net/ticket/%(id)s', realm_filter.id))
    ...
    def create_user_group_for_test(self, user_group_name: str) -> UserGroup:
        othello = self.example_user('othello')
        return create_user_group(user_group_name, [othello], get_realm('yakkl'))
    ...
    def test_stream_single(self) -> None:
        denmark = get_stream('Denmark', get_realm('yakkl'))
        sender_user_profile = self.example_user('othello')
        msg = Message(sender=sender_user_profile, sending_client=get_client("test"))
        content = "#**Denmark**"
        self.assertEqual(
    ...
    def test_topic_multiple(self) -> None:
        denmark = get_stream('Denmark', get_realm('yakkl'))
        scotland = get_stream('Scotland', get_realm('yakkl'))
        sender_user_profile = self.example_user('othello')
        msg = Message(sender=sender_user_profile, sending_client=get_client("test"))
    ...
    def test_relative_link(self) -> None:
        ...
        sender_user_profile = self.example_user('othello')
        message = Message(sender=sender_user_profile, sending_client=get_client("test"))
        msg = "http://yakkl.testserver/#narrow/stream/999-hello"

        self.assertEqual(
            bugdown.convert(msg, message_realm=realm, message=message),
            '<p><a href="#narrow/stream/999-hello" title="#narrow/stream/999-hello">http://yakkl.testserver/#narrow/stream/999-hello</a></p>'
        )
    ...
    def test_relative_link_streams_page(self) -> None:
        ...
        sender_user_profile = self.example_user('othello')
        message = Message(sender=sender_user_profile, sending_client=get_client("test"))
        msg = "http://yakkl.testserver/#streams/all"

        self.assertEqual(
            bugdown.convert(msg, message_realm=realm, message=message),
            '<p><a href="#streams/all" target="_blank" title="#streams/all">http://yakkl.testserver/#streams/all</a></p>'
        )
    ...
    def test_md_relative_link(self) -> None:
        ...
        sender_user_profile = self.example_user('othello')
        message = Message(sender=sender_user_profile, sending_client=get_client("test"))
        msg = "[hello](http://yakkl.testserver/#narrow/stream/999-hello)"
...
class BugdownApiTests(YakklTestCase):
    ...
    def test_render_mention_stream_api(self) -> None:
        ...
        stream_id = get_stream('Denmark', get_realm('yakkl')).id
        self.assertEqual(result.json()['rendered'],
...
class BugdownErrorTests(YakklTestCase):
...
class BugdownAvatarTestCase(YakklTestCase):
