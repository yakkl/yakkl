...
class HomeTest(YakklTestCase):
    ...
    def test_bad_pointer(self) -> None:
        ...
        with patch('logging.warning') as mock:
            result = self._get_home_page()
        mock.assert_called_once_with('hamlet@yakkl.com has invalid pointer 999999')
        self._sanity_check(result)
    ...
    def test_people(self) -> None:
        ...
        self.assertIn('defunct-1@yakkl.com', non_active_emails)

        # Bots can show up in multiple buckets.
        self.assertIn('bot-2@yakkl.com', bot_emails)
        self.assertIn('bot-2@yakkl.com', active_emails)

        # Make sure nobody got mis-bucketed.
        self.assertNotIn(hamlet.email, non_active_emails)
        self.assertNotIn('defunct-1@yakkl.com', active_emails)
        ...
        self.assertEqual(sorted(cross_bots, key=by_email), sorted([
            dict(
                user_id=get_system_bot('emailgateway@yakkl.com').id,
                is_admin=False,
                email='emailgateway@yakkl.com',
                full_name='Email Gateway',
                is_bot=True
            ),
            dict(
                user_id=get_system_bot('feedback@yakkl.com').id,
                is_admin=False,
                email='feedback@yakkl.com',
                full_name='Yakkl Feedback Bot',
                is_bot=True
            ),
            dict(
            ...
            dict(
                user_id=get_system_bot('welcome-bot@yakkl.com').id,
                is_admin=False,
                email='welcome-bot@yakkl.com',
    ...
    def test_show_invites_for_guest_users(self) -> None:
        ...
        self.login(email)
        self.assertFalse(user_profile.is_realm_admin)
        self.assertFalse(get_realm('yakkl').invite_by_admins_only)
        result = self._get_home_page()
        html = result.content.decode('utf-8')
        self.assertNotIn('Invite more users', html)

    def test_show_billing(self) -> None:
        customer = Customer.objects.create(realm=get_realm("yakkl"), stripe_customer_id="cus_id")
    ...
    def test_apps_view(self) -> None:
        ...
        with self.settings(ZILENCER_ENABLED=False):
            result = self.client_get('/apps/')
        self.assertEqual(result.status_code, 301)
        self.assertTrue(result['Location'] == 'https://yakkl.com/apps/')
    ...
    def test_compute_navbar_logo_url(self) -> None:
        ...
        self.assertEqual(compute_navbar_logo_url(page_params),
                         "/static/images/logo/yakkl-org-logo.png?version=0")
