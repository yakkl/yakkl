...
class EmailTranslationTestCase(YakklTestCase):
    def test_email_translation(self) -> None:
        ...
        # check_translation("Viele Grüße", "patch", "/json/settings", {"email": "hamlets-new@yakkl.com"})
        check_translation("Incrível!", "post", "/accounts/home/", {"email": "new-email@yakkl.com"}, HTTP_ACCEPT_LANGUAGE="pt")
        check_translation("Danke, dass Du", "post", '/accounts/find/', {'emails': hamlet.email})
        check_translation("Hallo", "post", "/json/invites",  {"invitee_emails": "new-email@yakkl.com",
...
class TranslationTestCase(YakklTestCase):
    """
...
class JsonTranslationTestCase(YakklTestCase):
    def tearDown(self) -> None:
        translation.activate(settings.LANGUAGE_CODE)
