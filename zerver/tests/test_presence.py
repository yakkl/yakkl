...
class ActivityTest(YakklTestCase):
    ...
    def test_activity(self, unused_mock: mock.Mock) -> None:
        ...
        flush_per_request_caches()
        with queries_captured() as queries:
            result = self.client_get('/realm_activity/yakkl/')
            self.assertEqual(result.status_code, 200)
        ...
        flush_per_request_caches()
        with queries_captured() as queries:
            result = self.client_get('/user_activity/iago@yakkl.com/')
            self.assertEqual(result.status_code, 200)

        self.assert_length(queries, 4)

class TestClientModel(YakklTestCase):
...
class UserPresenceModelTests(YakklTestCase):
...
class UserPresenceTests(YakklTestCase):
    ...
    def test_same_realm(self) -> None:
        ...
        # Ensure we don't see hamlet@yakkl.com information leakage
        self.login(self.example_email("hamlet"))
        result = self.client_post("/json/users/me/presence", {'status': 'idle'})
        self.assert_json_success(result)
        json = result.json()
        self.assertEqual(json['presences'][self.example_email("hamlet")]["website"]['status'], 'idle')
        # We only want @yakkl.com emails
        for email in json['presences'].keys():
            self.assertEqual(email_to_domain(email), 'yakkl.com')

class SingleUserPresenceTests(YakklTestCase):
    def test_single_user_get(self) -> None:
        ...
        result = self.client_post("/json/users/me/presence", {'status': 'active'})
        result = self.client_post("/json/users/me/presence", {'status': 'active'},
                                  HTTP_USER_AGENT="YakklDesktop/1.0")
        result = self.api_post(email, "/api/v1/users/me/presence", {'status': 'idle'},
                               HTTP_USER_AGENT="YakklAndroid/1.0")
        self.assert_json_success(result)

        # Check some error conditions
        result = self.client_get("/json/users/nonexistence@yakkl.com/presence")
        self.assert_json_error(result, "No such user")

        result = self.client_get("/json/users/cordelia@yakkl.com/presence")
        self.assert_json_error(result, "No presence data for cordelia@yakkl.com")

        do_deactivate_user(self.example_user('cordelia'))
        result = self.client_get("/json/users/cordelia@yakkl.com/presence")
        self.assert_json_error(result, "No such user")

        result = self.client_get("/json/users/default-bot@yakkl.com/presence")
        ...
        result = self.client_get("/json/users/othello@yakkl.com/presence",
                                 subdomain="zephyr")
        self.assert_json_error(result, "No such user")
        ...
        self.assertEqual(
            ...
            {"YakklAndroid", "website", "aggregated"})
...
class UserPresenceAggregationTests(YakklTestCase):
    ...
    def test_aggregated_info(self) -> None:
        ...
        with mock.patch('zerver.views.presence.timezone_now',
            ...
            result = self.api_post(email, "/api/v1/users/me/presence", {'status': 'active'},
                                   HTTP_USER_AGENT="YakklTestDev/1.0")
        result_dict = result.json()
        self.assertDictEqual(
...
class GetRealmStatusesTest(YakklTestCase):
