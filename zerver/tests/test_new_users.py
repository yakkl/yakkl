...
class SendLoginEmailTest(YakklTestCase):
    """
    ...
    def test_dont_send_login_emails_for_new_user_registration_logins(self) -> None:
        with self.settings(SEND_LOGIN_EMAILS=True):
            self.register("test@yakkl.com", "test")

            # Verify that there's just 1 email for new user registration.
            self.assertEqual(mail.outbox[0].subject, "Activate your Yakkl account")
            self.assertEqual(len(mail.outbox), 1)

    def test_without_path_info_dont_send_login_emails_for_new_user_registration_logins(self) -> None:
        with self.settings(SEND_LOGIN_EMAILS=True):
            self.client_post('/accounts/home/', {'email': "orange@yakkl.com"})
            self.submit_reg_form_for_user("orange@yakkl.com", "orange", PATH_INFO='')
...
class TestBrowserAndOsUserAgentStrings(YakklTestCase):
    ...
    def setUp(self) -> None:
        self.user_agents = [
            ...
            ('YakklAndroid/1.0', 'Yakkl', 'Android'),
            ('YakklMobile/1.0.12 (Android 7.1.1)', 'Yakkl', 'Android'),
            ('YakklMobile/0.7.1.1 (iOS 10.3.1)', 'Yakkl', 'iOS'),
            ('YakklElectron/1.1.0-beta Mozilla/5.0 (Windows NT 10.0; Win64; x64) ' +
             'AppleWebKit/537.36 (KHTML, like Gecko) Yakkl/1.1.0-beta ' +
             'Chrome/56.0.2924.87 Electron/1.6.8 Safari/537.36', 'Yakkl', 'Windows'),
...
class TestNotifyNewUser(YakklTestCase):
    ...
    def test_notify_realm_of_new_user(self) -> None:
        ...
        self.assertIn('@_**Cordelia Lear|%d** just signed up for Yakkl.' % (new_user.id,), message.content)
