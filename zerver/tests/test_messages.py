...
class MiscMessageTest(YakklTestCase):
...
class TopicHistoryTest(YakklTestCase):
...
class TopicDeleteTest(YakklTestCase):
...
class TestCrossRealmPMs(YakklTestCase):
    ...
    @override_settings(CROSS_REALM_BOT_EMAILS=['feedback@yakkl.com',
    ...
    def test_realm_scenarios(self) -> None:
        ...
        feedback_email = 'feedback@yakkl.com'
        support_email = 'support@3.example.com'  # note: not yakkl.com
        ...
        with self.settings(CROSS_REALM_BOT_EMAILS=['feedback@yakkl.com', 'welcome-bot@yakkl.com']):
        ...
        # Cross-realm bots in the yakkl.com realm can PM any realm
        ...
        # All users can PM cross-realm bots in the yakkl.com realm
        ...
        # Users can PM cross-realm bots on non-yakkl realms.
        # (The support bot represents some theoretical bot that we may
        # create in the future that does not have yakkl.com as its realm.)
        ...
        # Users on non-yakkl realms can't PM "ordinary" Yakkl users
        with assert_invalid_email():
            self.send_personal_message(user1_email, "hamlet@yakkl.com", sender_realm="1.example.com")
...
class TestAddressee(YakklTestCase):
    ...
    def test_addressee_for_user_ids_nonexistent_id(self) -> None:
        ...
        with assert_invalid_user_id():
            Addressee.for_user_ids(user_ids=[779], realm=get_realm('yakkl'))
...
class InternalPrepTest(YakklTestCase):
    ...
    def test_error_handling(self) -> None:
        ...
        prefix = "Error queueing internal message by cordelia@yakkl.com: You can't send private messages outside of your organization."
...
class ExtractedRecipientsTest(TestCase):
    def test_extract_recipients_emails(self) -> None:
        ...
        s = ujson.dumps([' alice@yakkl.com ', ' bob@yakkl.com ', '   ', 'bob@yakkl.com'])
        ...
        self.assertEqual(result, ['alice@yakkl.com', 'bob@yakkl.com'])

        # simple string with one name
        s = 'alice@yakkl.com    '
        self.assertEqual(extract_recipients(s), ['alice@yakkl.com'])
        ...
        s = 'bob@yakkl.com, alice@yakkl.com'
        result = sorted(extract_recipients(s))  # type: ignore
        self.assertEqual(result, ['alice@yakkl.com', 'bob@yakkl.com'])
...
class PersonalMessagesTest(YakklTestCase):
    ...
    def test_near_pm_message_url(self) -> None:
        ...
        self.assertEqual(url, 'http://yakkl.testserver/#narrow/pm-with/77,80-pm/near/555')
    ...
    def test_auto_subbed_to_personals(self) -> None:
        ...
        with mock.patch('zerver.models.get_display_recipient', return_value='recip'):
            self.assertEqual(str(message),
                             '<Message: recip /  / '
                             '<UserProfile: test@yakkl.com %s>>' % (user_profile.realm,))

            user_message = most_recent_usermessage(user_profile)
            self.assertEqual(str(user_message),
                             '<UserMessage: recip / test@yakkl.com ([])>'
                             )
    ...
    def assert_personal(self, sender_email: str, receiver_email: str, content: str="testcontent") -> None:
        ...
        """
        realm = get_realm('yakkl')  # Assume realm is always 'yakkl'
...
class StreamMessagesTest(YakklTestCase):
    ...
    def test_stream_message_unicode(self) -> None:
        ...
        self.assertEqual(str(message),
                         ...
                         '<UserProfile: hamlet@yakkl.com %s>>' % (sending_user_profile.realm,))
...
class MessageDictTest(YakklTestCase):
    ...
    def test_applying_markdown_invalid_format(self, convert_mock: Any) -> None:
        ...
        row = MessageDict.get_raw_db_rows([message.id])[0]
        dct = MessageDict.build_dict_from_raw_db_row(row)
        error_content = '<p>[Yakkl note: Sorry, we could not understand the formatting of your message]</p>'
...
class SewMessageAndReactionTest(YakklTestCase):
...
class MessagePOSTTest(YakklTestCase):
    ...
    def test_message_to_stream_with_nonexistent_id(self) -> None:
        ...
        expected = ("Your bot `whatever-bot@yakkl.testserver` tried to send a message to "
    ...
    def test_message_to_announce(self) -> None:
        ...
        notification_bot = get_system_bot("notification-bot@yakkl.com")
    ...
    def test_message_fail_to_announce(self) -> None:
        ...
        bot_without_owner = do_create_user(
            email='free-bot@yakkl.testserver',
            password='',
    ...
    def test_personal_message_to_deactivated_user(self) -> None:
        ...
        result = self.client_post("/json/messages", {
            ...
            "client": "test suite",
            "to": self.example_email("othello")})
        self.assert_json_error(result, "'othello@yakkl.com' is no longer using Yakkl.")
    ...
    def test_send_message_irc_mirror(self) -> None:
        ...
        email = "irc-bot@yakkl.testserver"
        user = get_user(email, get_realm('yakkl'))
        user.is_api_super_user = True
        user.save()
        user = get_user(email, get_realm('yakkl'))
    ...
    def test_unsubscribed_api_super_user(self) -> None:
        ...
        test_with(
            sender_email='irc_person@yakkl.com',
            client='irc_mirror',
            forged=True,
        )
    ...
    def test_cross_realm_bots_can_use_api_on_own_subdomain(self) -> None:
        ...
        stream = self.make_stream("notify_channel", get_realm("yakklinternal"))
...
class ScheduledMessageTest(YakklTestCase):
    ...
    def do_schedule_message(self, msg_type: str, to: str, msg: str,
                            ...
                            realm_str: str='yakkl') -> HttpResponse:
        self.login(self.example_email("hamlet"))
...
class EditMessageTest(YakklTestCase):
    ...
    def test_edit_cases(self) -> None:
        """This test verifies the accuracy of construction of Yakkl's edit
...
class MirroredMessageUsersTest(YakklTestCase):
    ...
    def test_irc_mirror(self) -> None:
        ...
        recipients = [self.nonreg_email('alice'), 'bob@irc.yakkl.com', self.nonreg_email('cordelia')]
        ...
        self.assertIn('bob@irc.yakkl.com', realm_emails)

        bob = get_user('bob@irc.yakkl.com', sender.realm)
        self.assertTrue(bob.is_mirror_dummy)
...
class MessageAccessTests(YakklTestCase):
...
class AttachmentTest(YakklTestCase):
    def test_basics(self) -> None:
        ...
        for ext in [".bmp", ".gif", ".jpg", "jpeg", ".png", ".webp", ".JPG"]:
            content = 'yo\n /user_uploads/%s/wEAnI-PEmVmCjo15xxNaQbnj/photo-10.%s foo' % (yakkl_realm.id, ext)
    ...
    def test_claim_attachment(self) -> None:
        ...
        dummy_files = [
            ('yakkl.txt', '%s/31/4CBjtTLYZhk66pZrF8hnYGwc/yakkl.txt' % (realm_id,), sample_size),
        ...
        body = ("Some files here ...[yakkl.txt](http://localhost:9991/user_uploads/{realm_id}/31/4CBjtTLYZhk66pZrF8hnYGwc/yakkl.txt)" +
...
class MissedMessageTest(YakklTestCase):
...
class LogDictTest(YakklTestCase):
    def test_to_log_dict(self) -> None:
        ...
        self.assertEqual(dct['sender_realm_str'], 'yakkl')
...
class CheckMessageTest(YakklTestCase):
    ...
    def test_bot_pm_feature(self) -> None:
        ...
        self.assertEqual(ret['message'].sender.email, 'othello-bot@yakkl.com')
...
class DeleteMessageTest(YakklTestCase):
    ...
    def test_delete_message_by_user(self) -> None:
        ...
        self.login("hamlet@yakkl.com")
        msg_id = self.send_stream_message("hamlet@yakkl.com", "Scotland")
        ...
        msg_id_1 = self.send_stream_message("hamlet@yakkl.com", "Scotland")
        message = Message.objects.get(id=msg_id_1)
        message.pub_date = message.pub_date - datetime.timedelta(seconds=120)
        message.save()

        msg_id_2 = self.send_stream_message("hamlet@yakkl.com", "Scotland")
...
class SoftDeactivationMessageTest(YakklTestCase):
...
class MessageHydrationTest(YakklTestCase):
...
class MessageVisibilityTest(YakklTestCase):
