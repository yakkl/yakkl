...
if TYPE_CHECKING:
    ...
    from zerver.lib.test_classes import YakklTestCase, MigrationsTestCase
...
class HostRequestMock:
    ...
    routes that use Yakkl's subdomains feature"""
...
def instrument_url(f: UrlFuncT) -> UrlFuncT:
    if not INSTRUMENTING:  # nocoverage -- option is always enabled; should we remove?
        ...
        def wrapper(self: 'YakklTestCase', url: str, info: Dict[str, Any]={},
...
def write_instrumentation_reports(full_suite: bool, include_webhooks: bool) -> None:
    if INSTRUMENTING:
        ...
        def cleanup_url(url: str) -> str:
            ...
            if url.startswith('http://yakkl.testserver/'):
                url = url[len('http://yakkl.testserver/'):]
