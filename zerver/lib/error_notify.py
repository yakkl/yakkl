...
def deployment_repr(report: Dict[str, Any]) -> str:
    ...
    for (label, field) in [('git', 'git_described'),
                           ('YAKKL_VERSION', 'yakkl_version_const'),
                           ('version', 'yakkl_version_file'),
                           ]:
...
def yakkl_browser_error(report: Dict[str, Any]) -> None:
...
def notify_server_error(report: Dict[str, Any], skip_error_yakkl: Optional[bool]=False) -> None:
    report = defaultdict(lambda: None, report)
    email_server_error(report)
    if settings.ERROR_BOT and not skip_error_yakkl:
        yakkl_server_error(report)

def yakkl_server_error(report: Dict[str, Any]) -> None:
