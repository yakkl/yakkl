...
def check_config() -> None:
    for (setting_name, default) in settings.REQUIRED_SETTINGS:
        ...
        raise CommandError("Error: You must set %s in /etc/yakkl/settings.py." % (setting_name,))

def sleep_forever() -> None:
    while True:  # nocoverage
        time.sleep(10**9)

class YakklBaseCommand(BaseCommand):
    ...
    def add_realm_args(self, parser: ArgumentParser, required: bool=False,
        ...
        if help is None:
            help = """The numeric or string ID (subdomain) of the Yakkl organization to modify.
You can use the command list_realms to find ID of the realms in this server."""
    ...
    def get_user(self, email: str, realm: Optional[Realm]) -> UserProfile:
        ...
        try:
            ...
            raise CommandError("This Yakkl server contains multiple users with that email " +
                               "(in different realms); please pass `--realm` "
                               "to specify which one to modify.")
        except UserProfile.DoesNotExist:
            raise CommandError("This Yakkl server does not contain a user with email '%s'" % (email,))

    def get_client(self) -> Client:
        """Returns a Yakkl Client object to be used for things done in management commands"""
        return get_client("YakklServer")
