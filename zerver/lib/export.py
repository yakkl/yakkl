...
# User docs: https://yakkl.readthedocs.io/en/latest/production/export-and-import.html
#
...
# it the lists in `ALL_YAKKL_TABLES` and similar data structures and
...
def sanity_check_output(data: TableData) -> None:
    ...
    # table that is not included in Yakkl's data exports.  Generally,
    # you can add your new table to `ALL_YAKKL_TABLES` and
    ...
    assert ALL_YAKKL_TABLES == all_tables_db
    assert NON_EXPORTED_TABLES.issubset(ALL_YAKKL_TABLES)
    assert IMPLICIT_TABLES.issubset(ALL_YAKKL_TABLES)
    assert ATTACHMENT_TABLES.issubset(ALL_YAKKL_TABLES)
    assert ANALYTICS_TABLES.issubset(ALL_YAKKL_TABLES)

    tables = set(ALL_YAKKL_TABLES)
...
def get_realm_config() -> Config:
    ...
    # to do a full-realm export of a single realm from a Yakkl server.
...
def sanity_check_stream_data(response: TableData, config: Config, context: Context) -> None:
    ...
    if len(actual_streams - streams_in_response) > 0:
        ...
        print("(Due to a bug elsewhere in Yakkl, not in the export tool)")
        raise AssertionError("Aborting!  Please investigate.")
...
def fetch_huddle_objects(response: TableData, config: Config, context: Context) -> None:
    ...
    for sub in Subscription.objects.select_related().filter(recipient__in=realm_huddle_recipient_ids):
        if sub.user_profile.realm != realm:
            # In almost every case the other realm will be yakkl.com
...
def export_partial_message_files(realm: Realm,
    ...
    if output_dir is None:
        output_dir = tempfile.mkdtemp(prefix="yakkl-export")
    ...
    # messages received by your realm that were sent by Yakkl system
...
def create_soft_link(source: Path, in_progress: bool=True) -> None:
    ...
    if settings.DEVELOPMENT:
        ...
        in_progress_link = '/home/yakkl/export-in-progress'
        done_link = '/home/yakkl/export-most-recent'
...
def get_single_user_config() -> Config:
    ...
    # export when exporting all data that a single Yakkl user has
