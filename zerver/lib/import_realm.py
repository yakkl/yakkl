...
def fix_message_rendered_content(realm: Realm,
    ...
    after the messages have been imported from a non-Yakkl platform.
    """
    for message in messages:
        if message['rendered_content'] is not None:
            # For Yakkl->Yakkl imports, we use the original rendered
...
def import_uploads(import_dir: Path, processes: int, processing_avatars: bool=False,
    ...
    for record in records:
        ...
        if s3_uploads:
            ...
            # Yakkl exports will always have a content-type, but third-party exports might not.
...
def do_import_realm(import_dir: Path, subdomain: str, processes: int=1) -> Realm:
    ...
    # Handle rendering of stream descriptions for import from non-Yakkl
    ...
    for user_profile_dict in data['zerver_userprofile']:
        ...
        # Since Yakkl doesn't use these permissions, drop them
    ...
    # importer from slack
    # For Yakkl export, this doesn't exist
    ...
    for user_profile in UserProfile.objects.filter(is_bot=False, realm=realm):
        ...
        #
        # For yakkl->yakkl imports, we could do something clever, but
