...
def get_apns_client() -> Any:
    # We lazily do this import as part of optimizing Yakkl's base
    # import time.
...
def modernize_apns_payload(data: Dict[str, Any]) -> Dict[str, Any]:
    '''Take a payload in an unknown Yakkl version's format, and return in current format.'''
    ...
    if 'message_ids' in data:
        ...
        # item in `custom.yakkl` is `message_ids`.
        return data
...
def send_apple_push_notification(user_id: int, devices: List[DeviceToken],
    ...
    # We lazily do the APNS imports as part of optimizing Yakkl's base
    ...
    for device in devices:
        ...
        def attempt_send() -> Optional[str]:
            try:
                stream_id = client.send_notification_async(
                    device.token, payload, topic='com.yakkl',
        ...
        if result == 'Success':
            ...
            # could be multiple for different Yakkl servers).
...
def parse_gcm_options(options: Dict[str, Any], data: Dict[str, Any]) -> str:
    ...
    The options permitted here form part of the Yakkl notification
    bouncer's API.  They are:

    `priority`: Passed through to GCM; see upstream doc linked below.
        Yakkl servers should always set this; when unset, we guess a value
    ...
    if options:
        # We're strict about the API; there is no use case for a newer Yakkl
...
def add_push_device_token(user_profile: UserProfile,
    ...
    if uses_notification_bouncer():
        post_data = {
            'server_uuid': settings.YAKKL_ORG_ID,
...
def push_notifications_enabled() -> bool:
    ...
    if (uses_notification_bouncer()
            and settings.YAKKL_ORG_KEY is not None
            and settings.YAKKL_ORG_ID is not None):  # nocoverage
...
def initialize_push_notifications() -> None:
    if not push_notifications_enabled():
        ...
        logger.warning("Mobile push notifications are not configured.\n  "
                       "See https://yakkl.readthedocs.io/en/latest/"
...
def get_message_payload_apns(user_profile: UserProfile, message: Message) -> Dict[str, Any]:
    ...
    yakkl_data = get_message_payload(user_profile, message)
    yakkl_data.update({
...
def get_message_payload_gcm(
    ...
    data.update({
        ...
        'yakkl_message_id': message.id,  # message_id is reserved for CCS
...
def get_remove_payload_gcm(
    ...
    gcm_payload.update({
        'event': 'remove',
        'yakkl_message_ids': ','.join(str(id) for id in message_ids),
        # Older clients (all clients older than 2019-02-13) look only at
        # `yakkl_message_id` and ignore `yakkl_message_ids`.  Do our best.
        'yakkl_message_id': message_ids[0],
    })
