...
from zerver.lib.email_mirror_helpers import decode_email_address, \
    get_email_gateway_message_string_from_address, YakklEmailForwardError
...
def report_to_yakkl(error_message: str) -> None:
    if settings.ERROR_BOT is None:
        return
    error_bot = get_system_bot(settings.ERROR_BOT)
    error_stream = Stream.objects.get(name="errors", realm=error_bot.realm)
    send_yakkl(settings.ERROR_BOT, error_stream, "email mirror error",
               """~~~\n%s\n~~~""" % (error_message,))
...
def get_missed_message_token_from_address(address: str) -> str:
    ...
    if not is_mm_32_format(msg_string):
        raise YakklEmailForwardError('Could not parse missed message address')
...
def mark_missed_message_address_as_used(address: str) -> None:
    ...
    if new_value < 0:
        ...
        raise YakklEmailForwardError('Missed message address has already been used')

def construct_yakkl_body(message: message.Message, realm: Realm, show_sender: bool=False,
                         include_quotes: bool=False, include_footer: bool=False) -> str:
    body = extract_body(message, include_quotes)
    # Remove null characters, since Yakkl will reject
...
def send_to_missed_message_address(address: str, message: message.Message) -> None:
    ...
    if not all(val is not None for val in result):
        raise YakklEmailForwardError('Missing missed message address data')
    user_profile_id, recipient_id, subject_b = result  # type: (bytes, bytes, bytes)

    user_profile = get_user_profile_by_id(user_profile_id)
    recipient = Recipient.objects.get(id=recipient_id)

    body = construct_yakkl_body(message, user_profile.realm)
...
class YakklEmailForwardUserError(YakklEmailForwardError):
    pass

def send_yakkl(sender: str, stream: Stream, topic: str, content: str) -> None:
...
def extract_body(message: message.Message, include_quotes: bool=False) -> str:
    ...
    if plaintext_content is not None or html_content is not None:
        raise YakklEmailForwardUserError("Email has no nonempty body sections; ignoring.")

    logging.warning("Content types: %s" % ([part.get_content_type() for part in message.walk()],))
    raise YakklEmailForwardUserError("Unable to find plaintext or HTML message body")
...
def extract_and_validate(email: str) -> Tuple[Stream, Dict[str, bool]]:
    ...
    try:
        ...
        raise YakklEmailForwardError("Bad stream token from email recipient " + email)
...
def find_emailgateway_recipient(message: message.Message) -> str:
    ...
    raise YakklEmailForwardError("Missing recipient in mirror email")
...
def process_stream_message(to: str, message: message.Message) -> None:
    ...
    body = construct_yakkl_body(message, stream.realm, **options)
    send_yakkl(settings.EMAIL_GATEWAY_BOT, stream, subject, body)
...
def process_message(message: message.Message, rcpt_to: Optional[str]=None, pre_checked: bool=False) -> None:
    ...
    except YakklEmailForwardError as e:
        if isinstance(e, YakklEmailForwardUserError):
