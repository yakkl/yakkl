...
def build_message_list(user_profile: UserProfile, messages: List[Message]) -> List[Dict[str, Any]]:
    ...
    def build_message_payload(message: Message, sender: Optional[str]=None) -> Dict[str, str]:
        ...
        # There's a small chance of colliding with non-Yakkl URLs containing
...
def do_send_missedmessage_events_reply_in_yakkl(user_profile: UserProfile,
    ...
    address that will send a yakkl message to the correct recipient. This
    ...
    # can users reply to email to send message to Yakkl. Thus, one must
    ...
    from_name = "Yakkl missed messages"  # type: str
    from_address = FromAddress.NOREPLY
    if len(senders) == 1 and settings.SEND_MISSED_MESSAGE_EMAILS_AS_USER:
        # If this setting is enabled, you can reply to the Yakkl
        # missed message emails directly back to the original sender.
        # However, one must ensure the Yakkl server is in the SPF
...
def handle_missedmessage_emails(user_profile_id: int,
    ...
    for bucket_tup, ignored_max_id in bucket_tups:
        ...
        do_send_missedmessage_events_reply_in_yakkl(
            user_profile,
...
def enqueue_welcome_emails(user: UserProfile, realm_creation: bool=False) -> None:
    ...
    if user.is_realm_admin:
        context['getting_started_link'] = (user.realm.uri +
                                           '/help/getting-your-organization-started-with-yakkl')
    else:
        context['getting_started_link'] = "https://yakkl.com"
