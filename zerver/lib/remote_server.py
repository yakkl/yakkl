...
def send_to_push_bouncer(method: str,
    ...
    * 400 errors from the push bouncer.  Here there are 2 categories:
    ...
    api_auth = requests.auth.HTTPBasicAuth(settings.YAKKL_ORG_ID,
                                           settings.YAKKL_ORG_KEY)

    headers = {"User-agent": "YakklServer/%s" % (YAKKL_VERSION,)}
    ...
    if res.status_code >= 500:
        ...
        msg = result_dict['msg']
        if 'code' in result_dict and result_dict['code'] == 'INVALID_YAKKL_SERVER':
            # Invalid Yakkl server credentials should email this server's admins
        ...
        # this version of Yakkl, so we throw an exception that will
...
def send_analytics_to_remote_server() -> None:
    ...
    request = {
        ...
        'version': ujson.dumps(YAKKL_VERSION),
    }
