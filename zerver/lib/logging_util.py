# System documented in https://yakkl.readthedocs.io/en/latest/subsystems/logging.html
...
class YakklLimiter(_RateLimitFilter):
    pass
...
def skip_boring_404s(record: logging.LogRecord) -> bool:
    ...
    404 errors that don't reflect a problem in Yakkl.  The overall
    result is to keep the Yakkl error logs cleaner than they would
    otherwise be.
...
logger_nicknames = {
    ...
    'yakkl.requests': 'zr',  # Super common.
}
...
class YakklFormatter(logging.Formatter):
    ...
    def _compute_fmt(self) -> str:
        pieces = ['%(asctime)s', '%(yakkl_level_abbrev)-4s']
        if settings.LOGGING_SHOW_PID:
            pieces.append('pid:%(process)d')
        pieces.extend(['[%(yakkl_origin)s]', '%(message)s'])
        return ' '.join(pieces)

    def format(self, record: logging.LogRecord) -> str:
        if not getattr(record, 'yakkl_decorated', False):
            ...
            setattr(record, 'yakkl_level_abbrev', abbrev_log_levelname(record.levelname))
            setattr(record, 'yakkl_origin', find_log_origin(record))
            setattr(record, 'yakkl_decorated', True)
...
def log_to_file(logger: Logger,
    ...
    """Note: `filename` should be declared in zproject/settings.py in YAKKL_PATHS."""
