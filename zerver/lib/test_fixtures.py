...
from scripts.lib.yakkl_tools import get_dev_uuid_var_path, run, \
...
def run_db_migrations(platform: str) -> None:
    if platform == 'dev':
        ...
        db_name = 'YAKKL_DB_NAME=yakkl'
    elif platform == 'test':
        migration_status_file = 'migration_status_test'
        settings = 'zproject.test_settings'
        db_name = 'YAKKL_DB_NAME=yakkl_test_template'
...
def update_test_databases_if_required(use_force: bool=False,
    ...
    """Checks whether the yakkl_test_template database template, is
    ...
    * Otherwise, we rebuild the test template database from scratch.
    ...
    us to drop and re-cloning the yakkl_test database from the
...
def template_database_status(
        database_name: str='yakkl_test_template',
...
def destroy_leaked_test_databases(expiry_time: int = 60 * 60) -> int:
    ...
    * Delete every database with a name like yakkl_test_template_*
    ...
    * And that file is less expiry_time old.
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for row in rows:
                if 'yakkl_test_template_' in row[0]:
    ...
    for file in files:
        if round(time.time()) - os.path.getmtime(file) < expiry_time:
            with open(file, "r") as f:
                for line in f:
                    databases_in_use.add('yakkl_test_template_{}'.format(line).rstrip())
        else:
