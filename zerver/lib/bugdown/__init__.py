# Yakkl's main markdown implementation.  See docs/subsystems/markdown.md for
    ...
    if settings.TEST_SUITE:
        ...
        try:
            ...
            bugdown_logger.error('Unable to load twitter api, you may have the wrong '
                                 'library installed, see https://github.com/yakkl/yakkl/issues/86')
            return None
...
class InlineInterestingLinkProcessor(markdown.treeprocessors.Treeprocessor):
    ...
    def get_actual_image_url(self, url: str) -> str:
        ...
        # See https://github.com/yakkl/yakkl/issues/4658 for more information
        parsed_url = urllib.parse.urlparse(url)
        if (parsed_url.netloc == 'github.com' or parsed_url.netloc.endswith('.github.com')):
            # https://github.com/yakkl/yakkl/blob/master/static/images/logo/yakkl-icon-128x128.png ->
            # https://raw.githubusercontent.com/yakkl/yakkl/master/static/images/logo/yakkl-icon-128x128.png
    ...
    def twitter_text(self, text: str,
        ...
        db_data = self.markdown.yakkl_db_data
        current_index = 0
    ...
    def run(self, root: Element) -> None:
        ...
        for found_url in found_urls:
            ...
            try:
                ...
                self.markdown.yakkl_message.links_for_preview.add(url)
                continue
...
class Emoji(markdown.inlinepatterns.Pattern):
    def handleMatch(self, match: Match[str]) -> Optional[Element]:
        ...
        if self.markdown.yakkl_message and name in active_realm_emoji:
            return make_realm_emoji(active_realm_emoji[name]['source_url'], orig_syntax)
        elif name == 'yakkl':
            return make_realm_emoji('/static/generated/emoji/images/emoji/unicode/yakkl.png', orig_syntax)
...
def sanitize_url(url: str) -> Optional[str]:
    ...
    # Yakkl modification: If scheme is not specified, assume http://
...
class UserMentionPattern(markdown.inlinepatterns.Pattern):
    def handleMatch(self, m: Match[str]) -> Optional[Element]:
        ...
        db_data = self.markdown.yakkl_db_data
        if self.markdown.yakkl_message and db_data is not None:
            ...
            if wildcard:
                self.markdown.yakkl_message.mentions_wildcard = True
                user_id = "*"
            elif user:
                if not silent:
                    self.markdown.yakkl_message.mentions_user_ids.add(user['id'])
                name = user['full_name']
                user_id = str(user['id'])
            else:
...
class UserGroupMentionPattern(markdown.inlinepatterns.Pattern):
    def handleMatch(self, m: Match[str]) -> Optional[Element]:
        ...
        if self.markdown.yakkl_message and db_data is not None:
            ...
            if user_group:
                self.markdown.yakkl_message.mentions_user_group_ids.add(user_group.id)
...
class StreamPattern(CompiledPattern):
    ...
    def handleMatch(self, m: Match[str]) -> Optional[Element]:
        ...
        if self.markdown.yakkl_message:
...
class AlertWordsNotificationProcessor(markdown.preprocessors.Preprocessor):
    ...
    def run(self, lines: Iterable[str]) -> Iterable[str]:
        ...
        if self.markdown.yakkl_message and db_data is not None:
            ...
            # we find to the set self.markdown.yakkl_message.alert_words.
                ...
                for end_index, (original_value, user_ids) in realm_alert_words_automaton.iter(content):
                    if self.check_valid_start_position(content, end_index - len(original_value)) and \
                       self.check_valid_end_position(content, end_index + 1):
                        self.markdown.yakkl_message.user_ids_with_alert_words.update(user_ids)
        return lines
...
def log_bugdown_error(msg: str) -> None:
    ...
    original markdown formatting into another Yakkl message, which
...
def do_convert(content: str,
    ...
    """Convert Markdown to HTML, with Yakkl-specific settings and hacks."""
    ...
    _md_engine.yakkl_message = message
    _md_engine.yakkl_realm = message_realm
    _md_engine.yakkl_db_data = None  # for now
    ...
    try:
        ...
        _md_engine.yakkl_message = None
        _md_engine.yakkl_realm = None
        _md_engine.yakkl_db_data = None
