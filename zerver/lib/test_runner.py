...
from scripts.lib.yakkl_tools import get_dev_uuid_var_path, TEMPLATE_DATABASE_DIR, \
...
def destroy_test_databases(worker_id: Optional[int]=None) -> None:
    for alias in connections:
        ...
        try:
            ...
            # template database name (yakkl_test_template).  So to
            # delete the database yakkl_test_template_<number>, we
            ...
            # updated to have `yakkl_test_template_<number>` as its
...
class Runner(DiscoverRunner):
    ...
    def teardown_test_environment(self, *args: Any, **kwargs: Any) -> Any:
        # The test environment setup clones the yakkl_test_template
        # database, creating databases with names:
        #     'yakkl_test_template_<N, N + self.parallel - 1>',
