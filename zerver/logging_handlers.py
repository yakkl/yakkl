...
def add_deployment_metadata(report: Dict[str, Any]) -> None:
    ...
    report['yakkl_version_const'] = YAKKL_VERSION

    version_path = os.path.join(os.path.dirname(__file__), '../version')
    if os.path.exists(version_path):
        with open(version_path, 'r') as f:  # nocoverage
            report['yakkl_version_file'] = f.read().strip()
...
class AdminNotifyHandler(logging.Handler):
    """An logging handler that sends the log/exception to the queue to be
       turned into an email and/or a Yakkl message for the server admins.
    """
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        # This parameter determines whether Yakkl should attempt to
        # send Yakkl messages containing the error report.  If there's
        # syntax that makes the markdown processor throw an exception,
        # we really don't want to send that syntax into a new Yakkl
        ...
        # prevent the infinite loop of yakkl messages by ERROR_BOT if
        ...
        try:
            ...
            report['stack_trace'] = "See /var/log/yakkl/errors.log"
