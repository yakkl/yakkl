...
# This is a hack to ensure that RemoteYakklServer always exists even
# if Zilencer isn't enabled.
if settings.ZILENCER_ENABLED:
    from zilencer.models import get_remote_server_by_uuid, RemoteYakklServer
else:  # nocoverage # Hack here basically to make impossible code paths compile
    from mock import Mock
    get_remote_server_by_uuid = Mock()
    RemoteYakklServer = Mock()  # type: ignore # https://github.com/JukkaL/mypy/issues/1188

ReturnT = TypeVar('ReturnT')

webhook_logger = logging.getLogger("yakkl.zerver.webhooks")
log_to_file(webhook_logger, settings.API_KEY_ONLY_WEBHOOK_LOG_PATH)

webhook_unexpected_events_logger = logging.getLogger("yakkl.zerver.lib.webhooks.common")
...
# response will be provided later by calling handler.yakkl_finish(),
    ...
    def wrapper(request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        if (request.method != "POST" and
            not (request.method == "SOCKET" and
                 request.META['yakkl.emulated_method'] == "POST")):
            if request.method == "SOCKET":  # nocoverage # yakkl.emulated_method is always POST
                err_method = "SOCKET/%s" % (request.META['yakkl.emulated_method'],)
            else:
...
def get_client_name(request: HttpRequest, is_browser_view: bool) -> str:
    ...
    if user_agent is not None:
        ...
        if is_browser_view and not user_agent["name"].startswith("Yakkl"):
            # Avoid changing the client string for browsers, but let
            # the Yakkl desktop and mobile apps be themselves.
            return "website"
        else:
...
class InvalidYakklServerError(JsonableError):
    code = ErrorCode.INVALID_YAKKL_SERVER
    data_fields = ['role']
    ...
    @staticmethod
    def msg_format() -> str:
        return "Yakkl server auth failure: {role} is not registered"

class InvalidYakklServerKeyError(InvalidYakklServerError):
    @staticmethod
    def msg_format() -> str:
        return "Yakkl server auth failure: key does not match role {role}"

def validate_api_key(request: HttpRequest, role: Optional[str],
                     api_key: str, is_webhook: bool=False,
                     client_name: Optional[str]=None) -> Union[UserProfile, RemoteYakklServer]:
    ...
    if settings.ZILENCER_ENABLED and role is not None and is_remote_server(role):
        ...
        except RemoteYakklServer.DoesNotExist:
            raise InvalidYakklServerError(role)
        if api_key != remote_server.api_key:
            raise InvalidYakklServerKeyError(role)

        if get_subdomain(request) != Realm.SUBDOMAIN_FOR_ROOT_DOMAIN:
            raise JsonableError(_("Invalid subdomain for push notifications bouncer"))
        request.user = remote_server
        request._email = "yakkl-server:" + role
...
def full_webhook_client_name(raw_client_name: Optional[str]=None) -> Optional[str]:
    if raw_client_name is None:
        return None
    return "Yakkl{}Webhook".format(raw_client_name)
...
def yakkl_login_required(
    ...
    otp_required_decorator = yakkl_otp_required(
        redirect_field_name=redirect_field_name,
        login_url=login_url
    )
    ...
    if function:
        ...
        return actual_decorator(yakkl_otp_required(add_logging_data(function)))
...
def authenticated_rest_api_view(*, webhook_client_name: Optional[str]=None,
    ...
    def _wrapped_view_func(view_func: ViewFuncT) -> ViewFuncT:
        ...
        def _wrapped_func_arguments(request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
            ...
            try:
                # profile is a Union[UserProfile, RemoteYakklServer]
...
def internal_notify_view(is_tornado_view: bool) -> Callable[[ViewFuncT], ViewFuncT]:
    ...
    # https://mypy.readthedocs.io/en/latest/kinds_of_types.html#extended-callable-types
    """Used for situations where something running on the Yakkl server
...
def yakkl_otp_required(view: Any=None,
