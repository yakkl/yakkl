...
def guess_yakkl_user_from_jira(jira_username: str, realm: Realm) -> Optional[UserProfile]:
    try:
...
def convert_jira_markup(content: str, realm: Realm) -> str:
    ...
    # formatting to Markdown, for consumption in Yakkl
    ...
    # Yakkl user mention. We don't know the email, just the JIRA username,
    # so we naively guess at their Yakkl account using this
    if realm:
        mention_re = re.compile(u'\\[~(.*?)\\]')
        for username in mention_re.findall(content):
            # Try to look up username
            user_profile = guess_yakkl_user_from_jira(username, realm)
            if user_profile:
