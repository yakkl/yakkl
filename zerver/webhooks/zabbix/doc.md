Receive Zabbix notifications in Yakkl!

1. {!create-stream.md!}

1. {!create-bot-construct-url-indented.md!}

1.  On your Zabbix Server, create a script called `yakkl.sh` in the
    `alertscripts` folder with the following contents:
        ...
        # Build our JSON payload and send it as a POST request to the Yakkl incoming web-hook URL
        payload="$3"
...
1. Set **name** to a name of your choice, such as `Yakkl`. Set **type** to **Script**,
   and set **Script name** to `yakkl.sh`. Add the following **Parameters**:
...
1. Set **Type** to **Yakkl**, and set **Send To** to the URL constructed.
...
1. Set **Name** to a name of your choice, such as `Yakkl`. Under
...
1. Set **Default Subject** to `{TRIGGER.STATUS}-{TRIGGER.SEVERITY}-{TRIGGER.NAME}`.
   Set **Default Message** to the following:
    ...
    Under **Send only to**, select **Yakkl**, and click **Add**.
