...
format that Yakkl doesn't recognize. This usually indicates a
...
def guess_yakkl_user_from_teamcity(teamcity_username: str, realm: Realm) -> Optional[UserProfile]:
    try:
        # Try to find a matching user in Yakkl
...
""".strip()
    ...
    if get_teamcity_property_value(message['teamcityProperties'], 'env.BUILD_IS_PERSONAL') == 'true':
        ...
        teamcity_user = guess_yakkl_user_from_teamcity(teamcity_fullname, user_profile.realm)
        ...
        if teamcity_user is None:
            ...
            if teamcity_shortname is not None:
                teamcity_user = guess_yakkl_user_from_teamcity(teamcity_shortname, user_profile.realm)

        if teamcity_user is None:
            # We can't figure out who started this build - there's nothing we can do here.
            logging.info("Teamcity webhook couldn't find a matching Yakkl user for "
