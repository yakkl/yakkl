...
def convert_room_data(raw_data: List[ZerverFieldsT],
    ...
    for in_dict in flat_data:
        ...
        if invite_only:
            ...
            if api_token is not None:
                ...
                for item in room_data['items']:
                    ...
                    yakkl_user_id = user_id_mapper.get(hipchat_user_id)
                    users.add(yakkl_user_id)
        ...
        # unmapped fields:
        #    guest_access_url: no Yakkl equivalent
        #    created: we just use "now"
...
def write_emoticon_data(realm_id: int,
    ...
    We move all the relevant files to Yakkl's more nested
    directory structure.
    '''
...
def process_raw_message_batch(realm_id: int,
    ...
    def fix_mentions(content: str,
        ...
        for user_id in mention_user_ids:
            ...
            yakkl_mention = '@**{full_name}**'.format(**user)
            content = content.replace(hipchat_mention, yakkl_mention)
