...
from zerver.data_import.slack_message_conversion import convert_to_yakkl_markdown, \
    get_user_full_name
...
def slack_workspace_to_realm(domain_name: str, realm_id: int, user_list: List[ZerverFieldsT],
    ...
    2. added_users, which is a dictionary to map from slack user id to yakkl user id
    3. added_recipient, which is a dictionary to map from channel name to yakkl recipient_id
    4. added_channels, which is a dictionary to map from channel name to channel id, yakkl stream_id
    5. added_mpims, which is a dictionary to map from MPIM name to MPIM id, yakkl huddle_id
    6. dm_members, which is a dictionary to map from DM id to tuple of DM participants.
    7. avatars, which is list to map avatars to yakkl avatar records.json
    ...
    # See https://yakkl.com/help/set-default-streams-for-new-users
...
def users_to_zerver_userprofile(slack_data_dir: str, users: List[ZerverFieldsT], realm_id: int,
    ...
    2. avatar_list, which is list to map avatars to yakkl avatard records.json
    3. added_users, which is a dictionary to map from slack user id to yakkl
       user id
    ...
    for user in users:
        ...
        # avatar
        # ref: https://yakkl.com/help/set-your-profile-picture
...
def channels_to_zerver_stream(slack_data_dir: str, realm_id: int,
    ...
    Returns:
    1. realm, Converted Realm data
    2. added_channels, which is a dictionary to map from channel name to channel id, yakkl stream_id
    3. added_mpims, which is a dictionary to map from MPIM(multiparty IM) name to MPIM id, yakkl huddle_id
    4. dm_members, which is a dictionary to map from DM id to tuple of DM participants.
    5. added_recipient, which is a dictionary to map from channel name to yakkl recipient_id
    """
    ...
    def process_channels(channels: List[Dict[str, Any]], invite_only: bool=False) -> None:
        ...
        for channel in channels:
            ...
            # map Slack's topic and purpose content into Yakkl's stream description.
            ...
            # TODO map Slack's pins to Yakkl's stars
    ...
    for slack_user_id, yakkl_user_id in added_users.items():
        recipient = build_recipient(yakkl_user_id, recipient_id_count, Recipient.PERSONAL)
        added_recipient[slack_user_id] = recipient_id_count
        sub = build_subscription(recipient_id_count, yakkl_user_id, subscription_id_count)
        realm["zerver_recipient"].append(recipient)
...
def process_long_term_idle_users(slack_data_dir: str, users: List[ZerverFieldsT],
    ...
    for slack_user in users:
        if slack_user["id"] in recent_senders:
            continue
        yakkl_user_id = added_users[slack_user['id']]
        long_term_idle.add(yakkl_user_id)
...
def channel_message_to_zerver_message(realm_id: int,
    ...
    for message in all_messages:
        ...
        if subtype in [
                # Yakkl doesn't have a pinned_item concept
                "pinned_item",
                "unpinned_item",
        ...
        try:
            content, mentioned_user_ids, has_link = convert_to_yakkl_markdown(
        ...
        yakkl_message = build_message(topic_name, float(message['ts']), message_id, content,
                                      rendered_content, added_users[user], recipient_id,
                                      has_image, has_link, has_attachment)
        zerver_message.append(yakkl_message)
...
def do_convert_data(slack_zip_file: str, output_dir: str, token: str, threads: int=6) -> None:
    ...
    logging.info("Yakkl data dump created at %s" % (output_dir,))
