...
SLACK_USERMENTION_REGEX = r"""
                           ...
                           (>)                   # ends with '>'
                           """
# Slack doesn't have mid-word message-formatting like Yakkl.
# Hence, ~stri~ke doesn't format the word in slack, but ~~stri~~ke
# formats the word in Yakkl
...
# Markdown mapping
def convert_to_yakkl_markdown(text: str, users: List[ZerverFieldsT],
...
def convert_markdown_syntax(text: str, regex: str, yakkl_keyword: str) -> str:
    """
    Returns:
    1. For strikethrough formatting: This maps Slack's '~strike~' to Yakkl's '~~strike~~'
    2. For bold formatting: This maps Slack's '*bold*' to Yakkl's '**bold**'
    3. For italic formatting: This maps Slack's '_italic_' to Yakkl's '*italic*'
    """
    for match in re.finditer(regex, text, re.VERBOSE):
        converted_token = (match.group(1) + yakkl_keyword + match.group(3)
                           + match.group(4) + yakkl_keyword + match.group(6))
