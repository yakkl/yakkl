...
def gitter_workspace_to_realm(domain_name: str, gitter_data: GitterDataT,
    ...
    2. avatars, which is list to map avatars to yakkl avatar records.json
    3. user_map, which is a dictionary to map from gitter user id to yakkl user id
    """
...
def build_userprofile(timestamp: Any, domain_name: str,
    ...
    2. avatar_list, which is list to map avatars to yakkl avatars records.json
    3. added_users, which is a dictionary to map from gitter user id to yakkl id
    """
...
def convert_gitter_workspace_messages(gitter_data: GitterDataT, output_dir: str,
    ...
    while True:
        ...
        for message in message_data:
            ...
            yakkl_message = build_message(topic_name, float(message_time), message_id, message['text'],
    ...
    if 'mentions' in message:
        for mention in message['mentions']:
            if mention.get('userId') in user_map:
                ...
                yakkl_mention = ('@**%s**' % (full_name,))
                message['text'] = message['text'].replace(gitter_mention, yakkl_mention)
