...
def build_message(topic_name: str, pub_date: float, message_id: int, content: str,
    ...
    yakkl_message = Message(
        rendered_content_version=1,  # this is Yakkl specific
        pub_date=pub_date,
        id=message_id,
        content=content,
        ...
        has_link=has_link)
    yakkl_message.set_topic_name(topic_name)
    yakkl_message_dict = model_to_dict(yakkl_message,
                                       exclude=['recipient', 'sender', 'sending_client'])
    yakkl_message_dict['sender'] = user_id
    yakkl_message_dict['sending_client'] = 1
    yakkl_message_dict['recipient'] = recipient_id

    return yakkl_message_dict
