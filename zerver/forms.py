...
MIT_VALIDATION_ERROR = u'That user does not exist at MIT or is a ' + \
                       ...
                       u'If you want to sign up an alias for Yakkl, ' + \
                       u'<a href="mailto:support@yakkl.com">contact us</a>.'
WRONG_SUBDOMAIN_ERROR = "Your Yakkl account is not a member of the " + \
...
def email_is_not_mit_mailing_list(email: str) -> None:
    """Prevent MIT mailing lists from signing up for Yakkl"""
    if "@mit.edu" in email:
...
class YakklPasswordResetForm(PasswordResetForm):
    def save(self,
        ...
        if email_belongs_to_ldap(realm, email):
            ...
            # password in the Yakkl server and should change it in LDAP.
        ...
        if user is not None:
            ...
            send_email('zerver/emails/password_reset', to_user_ids=[user.id],
                       from_name="Yakkl Account Security",
...
class OurAuthenticationForm(AuthenticationForm):
    ...
    def add_prefix(self, field_name: str) -> str:
        """Disable prefix, since Yakkl doesn't use this Django forms feature
...
class RealmRedirectForm(forms.Form):
    ...
    def clean_subdomain(self) -> str:
        ...
        try:
            ...
            raise ValidationError(_("We couldn't find that Yakkl organization."))
        return subdomain
