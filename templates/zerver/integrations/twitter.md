Fetch tweets from Twitter in Yakkl! This is great for seeing and
...
Download your new bot's `yakklrc` configuration file.
...
1.  Create a `~/.yakkl_twitterrc` with the following contents:
...
2. Place your bot's `yakklrc` in a directory of your choice (for the next step,
   `~/yakklrc` is used).

3.  Test the script by running it manually:

        /usr/local/share/yakkl/integrations/twitter/twitter-bot --search="@nprnews,quantum
        physics" --config-file=~/yakklrc

        /usr/local/share/yakkl/integrations/twitter/twitter-bot --twitter-name="<@your-
        twitter-handle>" --config-file=~/yakklrc
...
4.  Configure a crontab entry for this script. A sample crontab entry
    that will process tweets every minute is:
        ...
        * * * * * /usr/local/share/yakkl/integrations/twitter/twitter-bot --search="@nprnews,
        quantum physics" --config-file=~/yakklrc

When someone tweets a message containing one of your search terms,
you’ll get a Yakkl on your specified stream, with the search term as
the topic.
...
`/etc/yakkl/settings.py` on the Yakkl server.
