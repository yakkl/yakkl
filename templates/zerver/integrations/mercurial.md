Get Yakkl notifications when you `hg push`!
...
hook if it installs in a different location on this system:
        ...
        [hooks]
        changegroup = python:yakkl_changegroup.hook
...
3.  Add the directory where the `yakkl_changegroup.py` script was
installed to the environment variable `PYTHONPATH`.  For example, if
you installed the Yakkl Python bindings at the system level, it'd be:

        export PYTHONPATH=/usr/local/share/yakkl/integrations/hg:$PYTHONPATH

That’s all it takes for the basic setup! On the next `hg push`, you’ll
get a Yakkl update for the changeset.
...
add a `web_url` configuration option to the `yakkl` section of your
master `.hg/hgrc` to get changelog and revision links in your Yakkl
notifications:
...
By default, this integration will send Yakkl notifications for
changegroup events for all branches. If you’d prefer to only receive
Yakkl notifications for specified branches, add a `branches`
configuration option to the `yakkl` section of your master `.hg/hgrc`,
...
to the `yakkl` section of your master `.hg/hgrc`, containing a
