Get Codebase notifications in Yakkl!
...
1. {!change-yakkl-config-file-indented.md!}

    You may also need to update the value of `YAKKL_TICKETS_STREAM_NAME` and
    `YAKKL_COMMITS_STREAM_NAME`.

1.  Go to your Codebase settings, and click on **My Profile**. Under
    **API Credentials**, you will find your API key and username.
    Edit the following lines in `yakkl_codebase_config.py` to add your Codebase
    credentials:

    ```
    CODEBASE_API_USERNAME = "yakkl-inc/leo-franchi-15"
...
1. Run the `/usr/local/share/yakkl/integrations/codebase/yakkl_codebase_mirror`
