...
As Yakkl is using a StartCOM SSL certificate that is not recognized by
...
2. Edit the constants at the top of
   `org/yakkl/jira/YakklListener.groovy` and fill them with the
   appropriate values:

``` Python
String yakklEmail = "jira-notifications-bot@example.com"
String yakklAPIKey = "0123456789abcdef0123456789abcdef"
String yakklStream = "{{ recommended_stream_name }}"
...
4. In the **Add Listener** section, click on the **Custom Listener**
   option. Select the events you wish the Yakkl integration to fire for,
   and the projects you wish Yakkl to be notified for.

5. In the **Name of groovy class** field, enter
   `org.yakkl.jira.YakklListener`.

6. Click **Add Listener**, and JIRA will now notify your Yakkl of
