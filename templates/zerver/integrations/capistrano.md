Get Yakkl notifications for your Capistrano deploys!

1. {!create-a-bot-indented.md!}

1. {!download-python-bindings.md!}

1. You can now send Yakkl messages by calling the `yakkl-send`
   utility from your `deploy.rb` config file. Here's some example code for
   sending a Yakkl notification when a deployment has completed:

``` bash
after 'deploy', 'notify:humbug'

namespace :notify do
  desc "Post a message to Yakkl after deploy"
  task :humbug do
    run_locally "echo 'I just deployed to #{stage}! :tada:' | yakkl-send \
...
The `--user` and `--api-key` should be the email and API key of the Yakkl
bot created above. You can also put these values in a `~/.yakklrc` file on
