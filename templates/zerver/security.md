Yakkl’s security strategy covers all aspects of our product and
business. Making sure your information stays protected is our highest
priority.

## Security basics

- All Yakkl clients (web, mobile, desktop, terminal, and integrations)
  ...
  Encryption-at-rest is available with Yakkl on-premise, via hardware and
  software disk encryption of the database and other data storage media.
- Yakkl’s on-premise offerings can be hosted entirely behind your firewall,
  or even on an air-gapped network (disconnected from the Internet).
- Every Yakkl authenticated API endpoint has built in rate limiting to
  prevent DoS attacks.
- Connections from the Yakkl servers to Active Directory/LDAP can be secured
  with TLS.  If Yakkl is
  [deployed on multiple servers](https://yakkl.readthedocs.io/en/latest/production/deployment.html),
  all connections between parts of the Yakkl infrastructure can be secured
  with TLS or SSH.
...
- Yakkl operates a HackerOne bug bounty program to reward hackers for
  finding and responsibly reporting security vulnerabilities in Yakkl.  Our
  [completely open source codebase](https://github.com/yakkl/yakkl) means
  that HackerOne’s white-hat hackers can audit Yakkl for potential security
  issues with full access to the source code.

[redact-content]: https://yakkl.readthedocs.io/en/latest/production/mobile-push-notifications.html#security-and-privacy

## Configurable access control policies

- Yakkl supports private messages (to one or more individuals), private
...
- Yakkl provides many options for
...
- Yakkl can limit the features that new users have access to until their
  accounts are older than a [configurable waiting period][waiting_period].
- Yakkl also supports customizing whether non-admins can
...
[ldap-name]: https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html#ldap-including-active-directory

## Authentication

- Yakkl supports integrated single sign-on with Google, GitHub, and Active
  Directory/LDAP. SAML and Okta authentication are coming soon. With Yakkl
...
- Yakkl uses the zxcvbn password strength checker by default, and supports
  customizing users’ password strength requirements. See our documentation
  on
  [password strength](https://yakkl.readthedocs.io/en/latest/production/security-model.html#passwords)
  for more detail.
- Users can rotate their accounts’ credentials, blocking further access from
  any compromised Yakkl credentials.  With Yakkl on-premise, server
...
- With Yakkl on-premise,
  [session length](https://github.com/yakkl/yakkl/blob/master/zproject/prod_settings_template.py#L206)
  and idle timeouts can be configured to match your organization’s security
  policies.

[apache-sso]: https://yakkl.readthedocs.io/en/latest/production/authentication-methods.html#apache-based-sso-with-remote-user

## Integrity and auditing

- Yakkl administrators can configure users’
  ...
  permanently. Yakkl by default stores the complete history of all message
  content on the platform, including edits and deletions, and all uploaded
  files.
- Yakkl’s server logging has configurable log rotation policies and can be
  used for an end-to-end history of system usage.
- Yakkl stores in its database a permanent long-term audit log containing
  the history of important actions (e.g. changes to passwords, email
  addresses, and stream subscriptions).
- Yakkl’s powerful data exports
  ([on-premise](https://yakkl.readthedocs.io/en/latest/production/export-and-import.html),
  [cloud](/help/export-your-organization)) can be imported into third-party
  tools for legal discovery and other compliance purposes.  Yakkl’s
  enterprise offerings include support for integrating these with your
  compliance tools.
- Yakkl supports GDPR and HIPAA compliance.
...
pride. Yakkl founder Tim Abbott was previously the CTO of Ksplice, which
...
- The Yakkl server’s automated test suite has over 98% test coverage,
  including 100% of Yakkl’s API layer (responsible for parsing user input).
...
- Our error handling systems have been designed from the beginning to
  ...
  codebase).
- Yakkl has a carefully designed API surface area of only about 100 API
...
- Detailed
  [security model documentation](https://yakkl.readthedocs.io/en/latest/production/security-model.html)
