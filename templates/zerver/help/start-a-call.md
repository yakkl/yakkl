...
By default, Yakkl integrates with
...
!!! warn ""
    **Note**: Zoom support is experimental. In particular, Yakkl currently
    supports having only one active Zoom meeting at a time.

{tab|jitsi-on-premise}

If you're running both Yakkl and Jitsi Meet on-premise, just set
`JITSI_SERVER_URL` in `/etc/yakkl/settings.py`.
