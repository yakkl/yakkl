...
Yakkl has a powerful search engine under its hood. Search for messages using
...
`https://your-yakkl-url/#narrow/stream/123-design/has/link/is/starred/search/new.20logo`.

## List of operators

As you start typing, Yakkl will suggest possible operator completions.
...
* `pm-with:ada@yakkl.com`: Search 1-on-1 messages with Ada.
* `group-pm-with:ada@yakkl.com`: Search group private messages that
  include Ada.
* `sender:ada@yakkl.com`: Search messages sent by Ada.
...
Yakkl does some basic stemming, so `wave` will match `waves` and
...
Note that Yakkl ignores common words like `a`, `the`, and about 100
others. A quirk in Yakkl's current implementation means that if all of your
