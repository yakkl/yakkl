...
When you send a message, it first goes to a Yakkl server, and then the Yakkl
server sends it out to all the recipients.

Sometimes there can be delays if your device is on a poor network
connection. Yakkl lets you know when your message successfully reaches the
server.

## Verify that a message reached the Yakkl server
...
By default, Yakkl will try to resend the message when it is re-connected to
the internet.

If Yakkl gives up (or if the Yakkl server returns an error), it will
