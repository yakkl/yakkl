...
Starting with Yakkl 1.8, Yakkl supports importing data from Slack,
...
us at support@yakkl.com for help.

**Note:** You can only import a Slack workspace as a new Yakkl
...
Email support@yakkl.com with `slack_data.zip`, the Slack API token
generated above, and your desired subdomain. Your imported organization will
be hosted at `<subdomain>.yakkl.com`.
...
- Permission hierarchy:
    Slack's `Primary owner`, `owner`, and `admin` are mapped to Yakkl's `Organization admin`.
    Slack's `Member`, `restricted`, and `ultra restricted` are mapped to regular Yakkl users.
    `Channel creators` have no special permissions in Yakkl.
