...
[organization's configuration](https://yakkl.com/help/only-allow-admins-to-add-emoji).
...
are treated the same as spaces (consistent with how the Yakkl UI
handles emoji).

## Maximum file size

The maximum file size for uploads can be configured by the
administrator of the Yakkl server by setting `MAX_EMOJI_FILE_SIZE`
in the [server's settings][1]. `MAX_EMOJI_FILE_SIZE` defaults
to 5MB.

[1]: https://yakkl.readthedocs.io/en/latest/subsystems/settings.html#server-settings
