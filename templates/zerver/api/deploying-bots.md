...
* The simplest is running `yakkl-run-bot` inside a `screen` session on
...
* Using `supervisord` or a similar tool for managing a production
  process with `yakkl-run-bot`.  This consumes a bit of resources
  (since you need a persistent process running), but otherwise works
  great.
* Using the Yakkl Botserver, which is a simple Flask server for
  running a bot in production, and connecting that to Yakkl's outgoing
  webhooks feature.  This can be deployed in environments like
  Heroko's free tier without running a persistent process.

## Yakkl Botserver

The Yakkl Botserver is for people who want to

* run bots in production.
* run multiple bots at once.

The Yakkl Botserver is a Python (Flask) server that implements Yakkl's
...
### Installing the Yakkl Botserver

Install the `yakkl_botserver` PyPI package using `pip`:

```
pip install yakkl_botserver
```

### Running a bot using the Yakkl Botserver
...
1. Register new bot users on the Yakkl server's web interface.

    * Log in to the Yakkl server.
...
1. Download the `yakklrc` file for your bot from the *Active Bots*
   panel, using the download button.

1. Run the Botserver, where `helloworld` is the name of the bot you
   want to run:

    `yakkl-botserver --config-file <path_to_yakklrc> --bot-name=helloworld`

    You can specify the port number and various other options; run
    `yakkl-botserver --help` to see how to do this.

1.  Congrats, everything is set up! Test your Botserver like you would
    test a normal bot.

### Running multiple bots using the Yakkl Botserver

The Yakkl Botserver also supports running multiple bots from a single
...
1. Download the `botserverrc` from the `your-bots` settings page, using
   the "Download config of all active outgoing webhook bots in Yakkl
...
1.  Run the Yakkl Botserver by passing the `botserverrc` to it. The
    command format is:
     ...
     ```
     yakkl-botserver  --config-file <path_to_botserverrc>
     ```

     If omitted, `hostname` defaults to `127.0.0.1` and `port` to `5002`.

### Running Yakkl Botserver with supervisord
...
section documents how to run the Yakkl Botserver using *supervisord*.

Running the Yakkl Botserver with *supervisord* works almost like
running it manually.
...
1.  Configure *supervisord*.  *supervisord* stores its configuration in
    ...
    * Do **one** of the following:
      * Download the [sample config file][supervisord-config-file]
        and store it in `/etc/supervisor/conf.d/yakkl-botserver.conf`.
      * Copy the following section into your existing supervisord config file.

            [program:yakkl-botserver]
            command=yakkl-botserver --config-file=<path/to/your/botserverrc>
            --hostname <address> --port <port>
            startsecs=3
            stdout_logfile=/var/log/yakkl-botserver.log ; all output of your Botserver will be logged here
            redirect_stderr=true

    * Edit the `<>` sections according to your preferences.

[supervisord-config-file]: https://raw.githubusercontent.com/yakkl/python-yakkl-api/master/yakkl_botserver/yakkl-botserver-supervisord.conf
...
1. Test if your setup is successful:
    ...
    > yakkl-botserver                 RUNNING   pid 28154, uptime 0:00:27
