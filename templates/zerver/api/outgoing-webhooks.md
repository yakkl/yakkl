...
Outgoing Webhooks allow you to build or set up Yakkl integrations
which are notified when certain types of messages are sent in
Yakkl. When one of those events is triggered, we'll send a HTTP POST
payload to the webhook's configured URL.  Webhooks can be used to
power a wide range of Yakkl integrations.  For example, the
[Yakkl Botserver][yakkl-botserver] is built on top of this API.

Yakkl supports outgoing webhooks both in a clean native Yakkl format,
as well as a format that's compatible with
[Slack's outgoing webhook API][slack-outgoing-webhook], which can help
with porting an existing Slack integration to work with Yakkl.

[yakkl-botserver]: https://yakkl.com/api/deploying-bots#yakkl-botserver
...
* Navigate to *Settings (<i class="fa fa-cog"></i>)* -> *Your bots* ->
  *Add a new bot*.  Select *Outgoing webhook* for bot type, the URL
  you'd like Yakkl to post to as the **Endpoint URL**, the format you
...
## Yakkl message format

The Yakkl-format webhook messages post the following data, encoded as JSON:
...
* It will calculate a response that we call the "content" of
  the response.
* It will encode the content in Yakkl's flavor of markdown (or
  just plain text).
...
This is an example of the JSON payload that the Yakkl server will `POST`
to your server:

{generate_code_example|yakkl-outgoing-webhook-payload|fixture}
...
```
{
    "content": "Hey, we just received something from Yakkl!"
}
```

## Slack-format webhook format

This interface translates Yakkl's outgoing webhook's request into the
...
<table class="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>String ID of the Yakkl organization</td>
        </tr>
        <tr>
            <td><code>team_domain</code></td>
            <td>Domain of the Yakkl organization</td>
        </tr>
        <tr>
