...
Fetch message history from a Yakkl server.

`GET {{ api_url }}/v1/messages`

This `GET /api/v1/messages` endpoint is the primary way to fetch
message history from a Yakkl server.  It is useful both for Yakkl
...
supported by Yakkl's powerful full-text search backend.
...
{generate_api_arguments_table|yakkl.yaml|/messages:get}
...
present in all Yakkl API responses).
...
* `messages`: an array of `message` objects, each containing the following
    ...
    * `client`: A Yakkl "client" string, describing what Yakkl client
      sent the message.
    ...
    * `submessages`: Data used for certain experimental Yakkl integrations.
