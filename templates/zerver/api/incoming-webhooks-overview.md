...
An incoming webhook allows a third-party service to push data to Yakkl when
...
* Use our [REST API](/api/rest) endpoint for [sending
  ...
  of the messages in Yakkl.
* Adding an incoming webhook integration (detailed on this page),
  where all the logic for formatting the Yakkl messages lives in the
  Yakkl server.  This is how most of [Yakkl's official
  integrations](/integrations) work, because they enable Yakkl to
...
it has something for you, and then the Yakkl "incoming webhook"
integration handles that incoming data to format and send a message in
Yakkl.

New official Yakkl webhook integrations can take just a few hours to
...
* Set up the
  [Yakkl development environment](https://yakkl.readthedocs.io/en/latest/development/overview.html).
...
* Document the integration (required for getting it merged into Yakkl). You
...
* Consider using our Yakkl markup to make the output from your
...
* Integrations that don't match a team's workflow can often be
  ...
  triggering Yakkl messages only for certain message types, certain
...
* A helpful tool for testing your integration is
  ...
  calls via your local Yakkl development environment. This enables you to do end-to-end
