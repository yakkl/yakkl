...
* A step-by-step
  [guide](#installing-a-development-version-of-the-yakkl-bots-package)
...
* A [guide](#adding-a-bot-to-yakkl) on adding a bot to Yakkl.
...
## Installing a development version of the Yakkl bots package

1. `git clone https://github.com/yakkl/python-yakkl-api.git` - clone the [python-yakkl-api](
  https://github.com/yakkl/python-yakkl-api) repository.

2. `cd python-yakkl-api` - navigate into your cloned repository.
...
5. *Finished*. You should now see the name of your venv preceding your prompt,
   e.g. `(yakkl-api-py3-venv)`.

*Hint: `provision` installs the `yakkl`, `yakkl_bots`, and
 `yakkl_botserver` packages in developer mode. This enables you to
...
## Adding a bot to Yakkl

Yakkl's bot system resides in the [python-yakkl-api](
https://github.com/yakkl/python-yakkl-api) repository.
...
we have a little tool to help you out: `yakkl-terminal`

* [Install all requirements](#installing-a-development-version-of-the-yakkl-bots-package).

* Run `yakkl-terminal` to test one of the bots in
  [`yakkl_bots/bots`](https://github.com/yakkl/python-yakkl-api/tree/master/yakkl_bots/yakkl_bots/bots).

Example invocations are below:

```
> yakkl-terminal converter

Enter your message: "12 meter yard"
Response: 12.0 meter = 13.12336 yard

> yakkl-terminal -b ~/followup.conf followup

Enter your message: "Task Completed"
Response: stream: followup topic: foo_sender@yakkl.com
          from foo_sender@yakkl.com: Task Completed
...
* intercept, view, and process messages sent by users on Yakkl.
...
* message - a dictionary describing a Yakkl message
...
From `yakkl_bots/bots/incrementor/incrementor.py`:
...
The data is stored in the Yakkl Server's database.  Each bot user has
...
* key - the API key you created for the bot; this is how Yakkl knows
  the request is from an authorized user.

* email - the email address of the bot, e.g. `some-bot@yakkl.com`
...
[`python-yakkl-api`](https://github.com/yakkl/python-yakkl-api/tree/master/yakkl_bots/yakkl_bots/bots)
...
*Unit tests for bots make heavy use of mocking. If you want to get comfortable with mocking,
 ...
 https://yakkl.readthedocs.io/en/latest/testing/testing-with-django.html#testing-with-mocks).*

### A simple example

 Let's have a look at a simple test suite for the [`helloworld`](
 https://github.com/yakkl/python-yakkl-api/tree/master/yakkl_bots/yakkl_bots/bots/helloworld)
 bot.

```
from yakkl_bots.test_lib import StubBotTestCase
...
* To test a bot in [Yakkl's bot directory](
...
*The code for the bot testing library can be found [here](
 https://github.com/yakkl/python-yakkl-api/blob/master/yakkl_bots/yakkl_bots/test_lib.py).*


#### Testing bots with config files

Some bots, such as [Giphy](
https://github.com/yakkl/python-yakkl-api/tree/master/yakkl_bots/yakkl_bots/bots/giphy),
...
Check out our [bots](https://github.com/yakkl/python-yakkl-api/tree/master/yakkl_bots/yakkl_bots/bots)
to see examples of bot tests.

## Common problems

* I modified my bot's code, yet the changes don't seem to have an effect.
    * Ensure that you restarted the `yakkl-run-bot` script.

* My bot won't start
    * Ensure that your API config file is correct (download the config file from the server).
    * Ensure that you bot script is located in `yakkl_bots/bots/<my-bot>/`
...
* Run directly using the Yakkl `call_on_each_message` API, which is
...
* Run in a simple Python webserver server, processing messages
  received from Yakkl's outgoing webhooks integration.
* For bots merged into the mainline Yakkl codebase, enabled via a
  button in the Yakkl web UI, with no code deployment effort required.
