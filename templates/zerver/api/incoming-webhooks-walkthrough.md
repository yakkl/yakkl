...
service you want to integrate will be sending to Yakkl.
...
[Testing](https://yakkl.readthedocs.io/en/latest/testing/testing.html) for further details.
...
test Yakkl's handling of that integration, you will need to record
which HTTP headers are used with each fixture you capture.

Since this is integration-dependent, Yakkl offers a simple API for
...
https://yakkl.readthedocs.io/en/latest/tutorials/writing-views.html#request-variables).

You must pass the name of your integration to the
`api_key_only_webhook_view` decorator; that name will be used to
describe your integration in Yakkl's analytics (e.g. the `/stats`
...
object), and `user_profile` (Yakkl's user object). You may also want to
...
(available by default in the Yakkl development environment), and
...
page](https://yakkl.readthedocs.io/en/latest/testing/mypy.html) for details about
...
This tells the Yakkl api to call the `api_helloworld_webhook` function in
`zerver/webhooks/helloworld/view.py` when it receives a request at
`/api/v1/external/helloworld`.

This line also tells Yakkl to generate an entry for Hello World on the Yakkl
...
from the **Your bots** section of your Yakkl user's Settings page. To test the webhook,
you'll need to [create a bot](https://yakkl.com/help/add-a-bot-or-integration) with
the **Incoming Webhook** type. Replace `<api_key>` with your bot's API key in the examples
presented below! This is how Yakkl knows that the request was made by an authorized user.
...
Using `manage.py` from within the Yakkl development environment:
...
```
2016-07-07 15:06:59,187 INFO     127.0.0.1       POST    200 143ms (mem: 6ms/13) (md: 43ms/1) (db: 20ms/9q) (+start: 147ms) /api/v1/external/helloworld (helloworld-bot@yakkl.com via YakklHelloWorldWebhook)
```
...
By opening Yakkl in one tab and then this tool in another, you can quickly tweak
...
the Yakkl development environment with this command:
...
The standard location in a Vagrant environment is `/srv/yakkl`. If you are not
...
can see the existing examples at <https://yakkl.com/integrations>
or by accessing `/integrations` in your Yakkl development environment.
...
`{!congrats.md!}` is an example of a Markdown macro. Yakkl has a macro-based
Markdown/Jinja2 framework that includes macros for common instructions in
Yakkl's webhooks/integrations documentation.
...
[this one](https://raw.githubusercontent.com/yakkl/yakkl/master/zerver/webhooks/github/doc.md).

[integration-docs-guide]: https://yakkl.readthedocs.io/en/latest/documentation/integrations.html

## Step 7: Preparing a pull request to yakkl/yakkl

When you have finished your webhook integration and are ready for it to be
available in the Yakkl product, follow these steps to prepare your pull
request:

1. Run tests including linters and ensure you have addressed any issues they
   report. See [Testing](https://yakkl.readthedocs.io/en/latest/testing/testing.html)
   and [Linters](https://yakkl.readthedocs.io/en/latest/testing/linters.html) for details.
2. Read through [Code styles and conventions](
   https://yakkl.readthedocs.io/en/latest/contributing/code-style.html) and take a look
   through your code to double-check that you've followed Yakkl's guidelines.
3. Take a look at your git history to ensure your commits have been clear and
   logical (see [Version Control](
   https://yakkl.readthedocs.io/en/latest/contributing/version-control.html) for tips). If not,
...
5. Submit a pull request to yakkl/yakkl.

If you would like feedback on your integration as you go, feel free to post a
message on the [public Yakkl instance](https://yakkl.com/#narrow/stream/bots).
You can also create a [`[WIP]` pull request](
https://yakkl.readthedocs.io/en/latest/overview/contributing.html#working-on-an-issue) while you
are still working on your integration. See the
[Git guide](https://yakkl.readthedocs.io/en/latest/git/pull-requests.html#create-a-pull-request)
for more on Yakkl's pull request process.
...
In actual use, you might configure the 3rd party service to call your Yakkl
