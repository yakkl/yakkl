class yakkl::postgres_common {
  ...
  case $::osfamily {
    'debian': {
      ...
      $postgres_packages = [
        ...
        'ptop',
        # Needed just to support adding postgres user to 'yakkl' group
        'ssl-cert',
        # our dictionary
        'hunspell-en-us',
  ...
  yakkl::safepackage { $postgres_packages: ensure => 'installed' }
  ...
  file { "${yakkl::common::nagios_plugins_dir}/yakkl_postgres_common":
    ...
    group   => 'root',
    mode    => '0755',
    source  => 'puppet:///modules/yakkl/nagios_plugins/yakkl_postgres_common',
  }
  ...
  file { '/usr/local/bin/env-wal-e':
    ...
    group   => 'postgres',
    mode    => '0750',
    source  => 'puppet:///modules/yakkl/postgresql/env-wal-e',
  ...
  file { '/usr/local/bin/pg_backup_and_purge':
    ...
    group   => 'postgres',
    mode    => '0754',
    source  => 'puppet:///modules/yakkl/postgresql/pg_backup_and_purge',
    require => File['/usr/local/bin/env-wal-e'],
  }

  # Use arcane puppet virtual resources to add postgres user to yakkl group
  @user { 'postgres':
    groups     => ['ssl-cert'],
    membership => minimum,
    require    => $postgres_user_reqs,
  }
  User <| title == postgres |> { groups +> 'yakkl' }
}
