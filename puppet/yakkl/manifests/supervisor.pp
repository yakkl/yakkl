class yakkl::supervisor {
  ...
  if $::osfamily == 'redhat' {
    file { $yakkl::common::supervisor_conf_dir:
      ensure => 'directory',
      owner  => 'root',
      group  => 'root',
    }
  ...
  $supervisor_service = $yakkl::common::supervisor_service
  ...
  $puppet_classes = yakklconf('machine', 'puppet_classes', undef)
  if $puppet_classes == 'yakkl::dockervoyager' {
  ...
  file { $supervisord_conf:
    ...
    group   => 'root',
    mode    => '0644',
    source  => 'puppet:///modules/yakkl/supervisor/supervisord.conf',
    notify  => Service[$supervisor_service],
  }

  if $yakkl::base::release_name == 'xenial' {
