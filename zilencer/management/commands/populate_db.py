...
from scripts.lib.yakkl_tools import get_or_create_dev_uuid_var_path
...
class Command(BaseCommand):
    ...
    def handle(self, **options: Any) -> None:
        ...
        if options["delete"]:
            ...
            yakkl_realm = Realm.objects.create(
                string_id="yakkl", name="Yakkl Dev", emails_restricted_to_domains=True,
                description="The Yakkl development environment default organization."
                            "  It's great for testing!",
                invite_required=False, org_type=Realm.CORPORATE)
            RealmDomain.objects.create(realm=yakkl_realm, domain="yakkl.com")
            if options["test_suite"]:
            ...
            names = [
                ("Zoe", "ZOE@yakkl.com"),
                ("Othello, the Moor of Venice", "othello@yakkl.com"),
                ("Iago", "iago@yakkl.com"),
                ("Prospero from The Tempest", "prospero@yakkl.com"),
                ("Cordelia Lear", "cordelia@yakkl.com"),
                ("King Hamlet", "hamlet@yakkl.com"),
                ("aaron", "AARON@yakkl.com"),
                ("Polonius", "polonius@yakkl.com"),
            ]
            for i in range(options["extra_users"]):
                names.append(('Extra User %d' % (i,), 'extrauser%d@yakkl.com' % (i,)))
            create_users(yakkl_realm, names)

            iago = get_user("iago@yakkl.com", yakkl_realm)
            do_change_is_admin(iago, True)
            iago.is_staff = True
            iago.save(update_fields=['is_staff'])

            guest_user = get_user("polonius@yakkl.com", yakkl_realm)
            ...
            internal_realm_bots += [
                ("Yakkl Feedback Bot", "feedback@yakkl.com"),
            ]
            yakkl_realm_bots = [
                ("Yakkl Error Bot", "error-bot@yakkl.com"),
                ("Yakkl Default Bot", "default-bot@yakkl.com"),
            ]
            for i in range(options["extra_bots"]):
                yakkl_realm_bots.append(('Extra Bot %d' % (i,), 'extrabot%d@yakkl.com' % (i,)))

            create_users(internal_realm, internal_realm_bots, bot_type=UserProfile.DEFAULT_BOT)
            create_users(yakkl_realm, yakkl_realm_bots, bot_type=UserProfile.DEFAULT_BOT)

            # Initialize the email gateway bot as an API Super User
            email_gateway_bot = get_system_bot(settings.EMAIL_GATEWAY_BOT)
            do_change_is_admin(email_gateway_bot, True, permission="api_super_user")

            zoe = get_user("zoe@yakkl.com", yakkl_realm)
            yakkl_webhook_bots = [
                ("Yakkl Webhook Bot", "webhook-bot@yakkl.com"),
            ]
            ...
            create_users(yakkl_realm, yakkl_webhook_bots,
                         bot_type=UserProfile.INCOMING_WEBHOOK_BOT, bot_owner=zoe)
            aaron = get_user("AARON@yakkl.com", yakkl_realm)

            yakkl_outgoing_bots = [
                ("Outgoing Webhook", "outgoing-webhook@yakkl.com")
            ]
            create_users(yakkl_realm, yakkl_outgoing_bots,
                         bot_type=UserProfile.OUTGOING_WEBHOOK_BOT, bot_owner=aaron)
            outgoing_webhook = get_user("outgoing-webhook@yakkl.com", yakkl_realm)
            ...
            bulk_create_streams(yakkl_realm, stream_dict)
            recipient_streams = [Stream.objects.get(name=name, realm=yakkl_realm).id
            ...
            if options["test_suite"]:
                subscriptions_map = {
                    'AARON@yakkl.com': ['Verona'],
                    'cordelia@yakkl.com': ['Verona'],
                    'hamlet@yakkl.com': ['Verona', 'Denmark'],
                    'iago@yakkl.com': ['Verona', 'Denmark', 'Scotland'],
                    'othello@yakkl.com': ['Verona', 'Denmark', 'Scotland'],
                    'prospero@yakkl.com': ['Verona', 'Denmark', 'Scotland', 'Venice'],
                    'ZOE@yakkl.com': ['Verona', 'Denmark', 'Scotland', 'Venice', 'Rome'],
                    'polonius@yakkl.com': ['Verona'],
                }
            ...
            phone_number = try_add_realm_custom_profile_field(yakkl_realm, "Phone number",
                                                              CustomProfileField.SHORT_TEXT,
                                                              hint='')
            biography = try_add_realm_custom_profile_field(yakkl_realm, "Biography",
                                                           CustomProfileField.LONG_TEXT,
                                                           hint='What are you known for?')
            favorite_food = try_add_realm_custom_profile_field(yakkl_realm, "Favorite food",
                                                               CustomProfileField.SHORT_TEXT,
            ...
            favorite_editor = try_add_realm_custom_profile_field(yakkl_realm,
                                                                 "Favorite editor",
                                                                 CustomProfileField.CHOICE,
                                                                 field_data=field_data)
            birthday = try_add_realm_custom_profile_field(yakkl_realm, "Birthday",
                                                          CustomProfileField.DATE)
            favorite_website = try_add_realm_custom_profile_field(yakkl_realm, "Favorite website",
                                                                  CustomProfileField.URL,
                                                                  hint="Or your personal blog's URL")
            mentor = try_add_realm_custom_profile_field(yakkl_realm, "Mentor",
                                                        CustomProfileField.USER)
            external_account_field_data = {
                'subtype': 'github'
            }  # type: ProfileFieldData
            github_profile = try_add_realm_custom_profile_field(yakkl_realm, "GitHub",
                                                                CustomProfileField.EXTERNAL_ACCOUNT,
                                                                field_data=external_account_field_data)

            # Fill in values for Iago and Hamlet
            hamlet = get_user("hamlet@yakkl.com", yakkl_realm)
            do_update_user_custom_profile_data(iago, [
                ...
                {"id": favorite_website.id, "value": "https://yakkl.readthedocs.io/en/latest/"},
                {"id": mentor.id, "value": [hamlet.id]},
                {"id": github_profile.id, "value": 'yakkl'},
            ])
            do_update_user_custom_profile_data(hamlet, [
                ...
                {"id": favorite_website.id, "value": "https://blog.yakkl.com"},
                {"id": mentor.id, "value": [iago.id]},
                {"id": github_profile.id, "value": 'yakklbot'},
            ])
        else:
        ...
        with open(IMAGE_FILE_PATH, 'rb') as fp:
            check_add_realm_emoji(yakkl_realm, 'green_tick', iago, fp)
        ...
        if not options["test_suite"]:
            ...
            for user in user_profiles:
                ...
                if user.full_name[0] <= 'H':
                    client = get_client("YakklAndroid")
        ...
        if options["delete"]:
            ...
            if not options["test_suite"]:
                ...
                for i in range(options['extra_streams']):
                    ...
                    yakkl_stream_dict[extra_stream_name] = {
                        "description": "Auto-generated extra stream.",
                    }

                bulk_create_streams(yakkl_realm, yakkl_stream_dict)
                # Now that we've created the notifications stream, configure it properly.
                yakkl_realm.notifications_stream = get_stream("announce", yakkl_realm)
                yakkl_realm.save(update_fields=['notifications_stream'])

                # Add a few default streams
                for default_stream_name in ["design", "devel", "social", "support"]:
                    DefaultStream.objects.create(realm=yakkl_realm,
                                                 stream=get_stream(default_stream_name, yakkl_realm))

                # Now subscribe everyone to these streams
                subscribe_users_to_streams(yakkl_realm, yakkl_stream_dict)

                # These bots are not needed by the test suite
                internal_yakkl_users_nosubs = [
                    ("Yakkl Commit Bot", "commit-bot@yakkl.com"),
                    ("Yakkl Trac Bot", "trac-bot@yakkl.com"),
                    ("Yakkl Nagios Bot", "nagios-bot@yakkl.com"),
                ]
                create_users(yakkl_realm, internal_yakkl_users_nosubs, bot_type=UserProfile.DEFAULT_BOT)
...
def create_user_groups() -> None:
    yakkl = get_realm('yakkl')
    members = [get_user('cordelia@yakkl.com', yakkl),
               get_user('hamlet@yakkl.com', yakkl)]
    create_user_group("hamletcharacters", members, yakkl,
