...
from zerver.decorator import require_post, InvalidYakklServerKeyError
...
from zilencer.models import RemotePushDeviceToken, RemoteYakklServer, \
    RemoteRealmCount, RemoteInstallationCount

def validate_entity(entity: Union[UserProfile, RemoteYakklServer]) -> None:
    if not isinstance(entity, RemoteYakklServer):
        raise JsonableError(err_("Must validate with valid Yakkl server API key"))

def validate_bouncer_token_request(entity: Union[UserProfile, RemoteYakklServer],
...
def register_remote_server(
        request: HttpRequest,
        yakkl_org_id: str=REQ(str_validator=check_string_fixed_length(RemoteYakklServer.UUID_LENGTH)),
        yakkl_org_key: str=REQ(str_validator=check_string_fixed_length(RemoteYakklServer.API_KEY_LENGTH)),
        hostname: str=REQ(str_validator=check_capped_string(RemoteYakklServer.HOSTNAME_MAX_LENGTH)),
        contact_email: str=REQ(str_validator=check_string),
        new_org_key: Optional[str]=REQ(str_validator=check_string_fixed_length(
            RemoteYakklServer.API_KEY_LENGTH), default=None),
) -> HttpResponse:
    ...
    remote_server, created = RemoteYakklServer.objects.get_or_create(
        uuid=yakkl_org_id,
        defaults={'hostname': hostname, 'contact_email': contact_email,
                  'api_key': yakkl_org_key})

    if not created:
        if remote_server.api_key != yakkl_org_key:
            raise InvalidYakklServerKeyError(yakkl_org_id)
        else:
...
@has_request_variables
def register_remote_push_device(request: HttpRequest, entity: Union[UserProfile, RemoteYakklServer],
    ...
    server = cast(RemoteYakklServer, entity)
...
@has_request_variables
def unregister_remote_push_device(request: HttpRequest, entity: Union[UserProfile, RemoteYakklServer],
                                  token: bytes=REQ(),
                                  token_kind: int=REQ(validator=check_int),
                                  user_id: int=REQ(),
                                  ios_app_id: Optional[str]=None) -> HttpResponse:
...
@has_request_variables
def remote_server_notify_push(request: HttpRequest, entity: Union[UserProfile, RemoteYakklServer],
...
def validate_count_stats(server: RemoteYakklServer, model: Any,
...
def remote_server_post_analytics(request: HttpRequest,
                                 entity: Union[UserProfile, RemoteYakklServer],
...
def get_last_id_from_server(server: RemoteYakklServer, model: Any) -> int:
...
def remote_server_check_analytics(request: HttpRequest,
                                  entity: Union[UserProfile, RemoteYakklServer]) -> HttpResponse:
    validate_entity(entity)
    server = cast(RemoteYakklServer, entity)
