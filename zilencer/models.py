...
def get_remote_server_by_uuid(uuid: str) -> 'RemoteYakklServer':
    return RemoteYakklServer.objects.get(uuid=uuid)

class RemoteYakklServer(models.Model):
    UUID_LENGTH = 36
    API_KEY_LENGTH = 64
    ...
    def __str__(self) -> str:
        return "<RemoteYakklServer %s %s>" % (self.hostname, self.uuid[0:12])

# Variant of PushDeviceToken for a remote server.
class RemotePushDeviceToken(AbstractPushDeviceToken):
    server = models.ForeignKey(RemoteYakklServer, on_delete=models.CASCADE)  # type: RemoteYakklServer
